...
предоставляется по лицензии AROS Public License (APL), рекомендуется публиковать
весь новый код написанный для проекта именно под ней.
...
1. Лицензия должна позволять нам:
   ...
   AROS, то дополнительно лицензия должна позволять вносить изменения в
...
Например (из файла <filename>AROS/exec/addhead.c</filename>)::
...
NAME
    ...
    для использования функции и имя функции в макросе `AROS_LH#?()` (Library
    ...
    того нужна ли для работы функции основные библиотеки. Функции `AddHead()` не
    ...
    `AROS_LH#?()`. К имени макроса надо добавить количество аргументов. Т.к.
    ...
    (Помните что регистры не нуждаются в особой последовательности)
...
SYNOPSIS
    Это поле содержит попорядку все аргументы функции в макросе `AROS_LHA()`
...
+ Расставляйте фигурные ``{}``, квадратные ``[]``и круглые ``()``скобки друг под
  другом (см. d), если между ними содержится много кода. Квадратные и круглые
...
Код програмных модулей AROS должен быть написан в таком силе, чтобы его можно
было записать в ПЗУ, флэш-ПЗУ или другие разновидности ПЗУ. Это можно сделать
посредством следующих правил по стилю программирования. И конечно же они
...
+ ПЗУ модули не должны иметь секций .data и .bss. Изначально надо избавиться от
  всех непостоянных глобальных данных. Подлинный Amiga Kickstart показывает,
  что оба эти условия возможны и их легко достичь.
...
+ Вышенаписанного достаточно для основных библиотек. Если вы пишете библиотеку,
  поместите основы других библиотек в базовую структуру вашей собственной
  библиотеки. Классы BOOPSI могут хранить основные библиотеки в своих приватных
  данных.
...
+ Попытайтесь установить аттрибуты `static` и `const` для всех ваших глобальных 
  ...
  и для их прототипа. Помните, что с версии Amiga OS 3.5 Олаф Бартел
...
+ **НИКОГДА НЕ** трогайте буферы передаваемые пользователем в качестве
  "входного" параметра. Общее представление входных параметров зачастую явно не
  указывается в описании функций. Например, очевидно что имя файла передаваемое
  функции `Open()` является входной переменной и `Open()` не должна смешиваться
  с ней, даже если это позже будет исправлено. Помните что буфер может
  располагаться в ПЗУ или использоваться совместно несколькими копиями
...
+ Пытайтесь избегать системных вызовов основной ОС, таких как `malloc()` и
  ...
  указатели, был уверен, что нашёл  указатель внутри блоков памяти Exec с
  `TypeOfMem()`.
...
11. Перейдите в директорию bin/$(ARCH)/AROS и запустите "arosshell". Теперь вы
    ...
    "Exit". Для останова "arosshell" вам надо нажать ^C (Ctrl-C), в то время как
    в реальной ОС нет способа завершить его более правильно.
