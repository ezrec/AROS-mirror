...
Для большинства разработчиков простейшим путём для написания и отладки
...
Если значение ``DEBUG`` равно 0 или неопеределёно, то результатом раскрытия
...
отдельно . Использование такое же как и функции ``printf()``. В hosted-версии,
вывод будет произвдиться на ту консоль, в которой была запущена AROS.
...
ulimit оболочки bash. После этого запустите AROS и сгенерируйте дам памяти::
...
(или даже аббревиатуре) команды.

Прямо в режиме отладки вы получите пригодную для чтения справку по всем
...
+ Для продолжения выполнения (или перезапуска сначала) программы до того момента
  как вы её прервёте, или при достижении точки останова, или при сбое программы, 
...
+ Для пошагового выполнения программы используйте команды ``s`` (step) или ``n``
  (next) (последняя продолжит выполняет один шаг);
...
Специфические команды GDB для AROS
==================================

Специфические команды GDB для AROS находятся в файле ``/AROS/_gdbinit``, который
...
команда ``backtrace`` выдаст вам один или более неотслеженный адрес::
...
по полученному адресу начала текста::
...
Таким образом похоже вы нашли ошибку::

    (gdb) f 1
    #1  0x402bd919 in main () at list1.c:107
    107             set(3243243, MUIA_Window_Open, TRUE);

Повторите для оставшихся адресов, которые вы хотите отследить.
...
пока не доступна для AROS, поэтому вам надо самостоятельно внимательно следить
...
включен. Это создает небольшую зону до и после вашего местоположения для
проверки того, что вы не записываете вне ваших границ. Эта проверка проходит во
время процесса выделения памяти, или в любое время вызовом ``AvailMem(MEMF_CLEAR)``.
...
результат на заданное для отладчика устройство вывода (последовательный порт для
 ...
 выделенной памяти не обнаружен, то будет выведено текущее количество выделений
 памяти и их полный размер::
...
объекты: библиотеки, устройства, шрифты, ресурсы, порты и семафоры. Он запускает
 очистку памяти от объектов, которые не используются, для вывода реального
 количества памяти после того как некоторые ресурсы будут закрыты.

Запустите ``LeakWatch`` в его собственной оболочке, использую следующие
сочетания клавиш:
...
``Ctrl-F`` является наиболее полезным сочетанием, если нажать его перед запуском
 программы и после.
Она покажет, между нажатиями ресурсы не были заняты. В противном случае:

+ Проверьте чтобы никакая другая программа не занимала ресурсы в течение этого
  времени.
+ Повторите запуск. Смотрите, нет ли утечек ресурсов.
+ Ограничьте место, где берет начало утечка путём усечения возможностей, которые
  вы используете, а потом закомментировав эту часть исходного кода.

Если вы думаете, что ваша программа инициирует утечку в библиотеке AROS, то
найдите существующую тестовую или напишите небольшую программу, приводящую к
возникновению утечки, для гарантии что утечка действительно возникает в
библиотеке.
...
Показывает список открытых в данный момент библиотек и некоторую информацию
вроде версии и счётчика открытий.
...
когда вы видите странное повреждение памяти, но не можете отследить его другими
способами. Проверка памяти будет производиться после каждой задачи, имеющей свой
квант времени. Это происходит медленно, но в этом случае подозреваемому не уйти.

Утечки памяти
-------------

+ Установите как много памяти утекает и как много распределено: для получения
...
+ Попытайтесь определить размещение утечки в памяти вставкой в код
  ...
  ``rom/exec/allocmem.c``. Конечно вам понадобится запустить вашу программу в
  GDB, чтобы это помогло. Используйте ``bt`` и другие команды GDB для
  определения причины каждого подозрительного размещения.

+ Когда вы нашли возможное место утечки, измените его размер выделения
