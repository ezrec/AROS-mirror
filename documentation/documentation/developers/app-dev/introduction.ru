...
В этой главе рассказывается как разрабатывать программы, которые будут работать
в AROS. Также говориться о том, как скомпилировать их на аппаратном обеспечении
...
+ i386-linux-system
  ...
  директорию и от имени суперпользователя root запустите нахоядщийся там скрипт
...
Вы можете cкомпилировать программe при помощи следующей команды оболочки
Linux::
...
Распакуйте его в тоже место (например, sys:ADE). Скопируйте вложения и
...
AROS распротсраняется с разнообразными файлами включений (include). Они
...
работает. Каждая библиотека представляет из себя набор функций, который
удовлетворяет неким целям. Обычно функции со сходными целями помещаются в одну
...
запоминать всё чтонаписано о них здесь, так как они подробно рассматриваются
далее.

+ `exec.library` наиболее важная библиотека. Она несёт ответственность за 
   работу таких самых основных механизмов операционной системы, как управление
   `задачами` (т.е. программами), `памятью`, `библиотеками` и многие другие
   вещи.

+ `utility.library` воплощает очень важные механизмы для обращения к
   библиотекам: `taglist-ы`, которые рассматриваются далее в этой глава и
   `hook-и`. Отдельно от этого, данная библиотека содержит разнообразные
   небольшие служебные функции.
   
+ `dos.library` отвечает за обращение с файлами, операции с дисками и некторые 
...
После того как библиотека успешно открыта указатель так называемой
`библиотечной базы` возвращается. Библиотечная база - это область памяти,
...
.. [#] Передача библиотечной базы может быть явной либо неявной, в зависимости
       ...
       быть неявно переданы базе: макрос препроцессора C, встраиваемые функции,
       глобальные переменные и т.д.

Как воспользоваться совместно используемыми библиотеками AROS
'''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''

Как уже объяснялось в предыдущем разделе, библиотеки надо открыть до того как 
их функции можно будет использовать. Единственная библиотека, которую нет
необходимости сначала открыть, это `exec.library`. Exec является всегда
открытой и ваш компилятор знает как получить к ней доступ. Дополнительно вам
необходимо включить заголовок, чтобы сделать прототип функций известным коду.
...
библиотеки, следвательно, вам нет необходимоси открывать их вручную. Прочтите
...
Автооткрытие библиотек системой сборки очень похоже не использование
...
Для открытия библиотеки вам надо испольовать функцию из `exec.library`::
...
1. Во-первых, имя библиотеки ищется в списке уже загруженных библиотек. Если
   ...
   всё ещё находится в ней, то всё замечательно и `OpenLibrary()` будет
...
будем показывать его в программе, которая его запросила. Функцией, показывающей
запращивающую программу, является `EasyRequestArgs()` из библиотеки
*intuition.library*. Мы не будем здесь обсуждать её использование. Более
подробную информацию смотрите в разделе о `Requesters`.
...
Пример использования библиотек::
    ...
    /* Эти переменные будут хранить указатель на intuition.library */
...
программу запрашивающую сообщение "Hello, world!".
                    
Присвоение библиотекам номеров версий
'''''''''''''''''''''''''''''''''''''

Совместно используемые библиотеки могут со временем развиваться и предоставлять
новые возможности. Теперь когда программа испольщует возможности более свежей
...
соответствующим образом ограничить свою функциональность. В AROS и 
...
младшего только указывает на некоторую оптимизацию и/или исправление ошибок,
...
следующим утверждением где-нибудь в вашем программном коде::

  const LONG DOSBase_version = 36;

Последствием для библиотек будет то, что они всегда должны быть обратно
...
Совместно используемые библиотеки AROS имеют уникальную архитектуру с их
преимуществами и недостатками. Некоторые аспекты будут далее рассмотрены в этой
главе. В основном будут браться ссылки на Windows и UNIX(-подобные) системы.
...
по начальному адресу, в который она была загружена. AROS и Amiga-подобные
системы разделяют одну большую область памяти между всем кодом запущенным в
системе. Это означает, что программы могут использовать библиотеки также
...
любой позиции в памяти без необходимости перемещать код в памяти. В зависимости
...
функциям совместно используемых библиотк, будут использовать то имя функции,
...
Это создаст глобальную переменную, которая будет доступна из всех частей
библиотеки. Когда совместно используемая библиотека загружаена в память, ваша
...
глобальной переменной будет всегда загружаться в одно и тоже место в памяти не
важно сколько раз библиотека будет открываться или какая программа вызовет
...
Библиотека может открыть другую библиотеку. Псоле того, как библиотека открыла 
другую библиотеку, она получает libbase для этой библиотеки. Это означает, что
библиотека, имеющая per-opener base, вернёт другой библиотеке уникальный
libbase. Когда программа открывает библиотеку с per-opener base, обратно она
получит libbase. Теперь, когда программа открывает вторую библиотеку, она опять
также открывает первую библиотеку, после чего другая libbase будет использована
во второй библиотеке, и затем в программе. Программирующие библиотеки с
...
.. [#] В реальности, на разный вкус, эта таблица может содержать больше, чем
       ...
       пользователь не программирует переход к адресу в векторе, они переходят 
       к вектору сами, и затем инструкция JMP перенаправляет поток данных
       программы в правильную функцию.
...
аргументу `newwin` значение `NULL`. Единственные теги наблюдаемые сейчас::
...
Другая функция, необходимая нам в этом небольшом примере, это::
...
"Hello, world!" не является Музеем програмистского таланта, поэтому вы можете
удивиться если есть больше в AROS чем это. Ещё бы, дорогие, конечно есть. Но
...
будущем, а пока лучшими Руководствами программиста под AROS, которые вы сможете
найти, являются книги написанные для Amiga, а лучшим справочником по AROS
являются `autodocs <./../autodocs/index>`__ (autodocs описывают
функции библиотек AROS, которые создаются путем синтаксического анализа
исходного кода AROS).

Но они в основном полезны опытным программистам для Amiga: они предлагают 
