...
вашей ОС. Все распространенные дистрибутивы Linux уже содержат его.

Также существует графический кроссплатформенный клиент для SVN под называнием
`RapidSVN <http://rapidsvn.org/>`__, который доступен под Unix (дистрибутивы
GNU/Linux, FreeBSD, Sun Solaris и другими), Mac OS X и Windows.
...
Работая в AmigaOS, вам понадобится установить TCP/IP стек и любой порт SVN.
...
При работе в MS Windows (TM) можно рекомендовать SVN-клиент TortoiseSVN__, 
который особенно понравится тем, кто привык работать с Проводником. 
Эта свободная программа с открытым исходным кодом, богатыми возможностями и
хорошей поддержкой. Программа переведена на русский и содержит довольно
подробную документацию (в том числе и на русском). Пожалуйста, отправляя ваши
файлы, проверяйте, установлено ли в SVN свойство eol-style: native. Если нет, то
все ваши файлы должны иметь концы строк в стандарте UNIX (только LF), если да -
это не имеет значения. Концы строк UNIX также должны быть во всех ваших новых
файлах. Примерами редакторов, умеющих указывать подобный EOL, являются следующие
бесплатные программы: TigerPAD__ (сохранять как текст UNIX), DOS Navigator (в
свойствах редактора проставить LF), утилиту dos2unix (конвертор), NotePad++__
...
При работе в Mac OS X вы можете использовать один из нескольких портированных
версий SVN, например, порт `SVN Мартина Отта`__. После его установки вы также 
можете установить графический интерфейс для SVN, например, `svnX`__. svnX это
наиболее современный SVN-клиент для Mac OS X, который к тому же, помимо прочего,
поддерживает интерграцию с свободно распространяемым текстовым редактором
`TextWrangler`__ (ранее назывался BBedit Lite). Помните что при использовании
TextWrangler'а или другого текстового редактора для Mac'а, вам следует изменить
кодировку символов в ваших доукментах с 'Macos roman' на 'ISO latin 1' (ISO8859)
. Это можно сделать в настройках. В TextWrangler'е все остальные настройки по
умолчанию правильные. Все вышеукзанные приложения свободно распространяются и
доступны в виде запускаемых файлов (Universal Binary) для Mac'ов как на основе
PowerPC, так и на основе Intel. Также доступен SVN-клиент RapidSVN.
...
.. Note:: 
    ...
    Репозиторий AROS находится на защищенном паролем сервере SVN, что означает,
    что необходима `регистрация для доступа к серверу`__ для возможности участия
...
сверять, чтобы собрать любую разновидность AROS::
...
и т.п., и раздел "documentation", содержащий исходники для построения
содержимого сайта.
...
конфликте и сохранит **обе** версии файла разделенные символами ``<<<<`` Вам
придется отредактировать файл и разрешать конфликт вручную.
...
Перед посылкой файлов на сервер SVN запросит у вас комментарий для данной
посылки. Обычно эти комментарии содержат краткое описание изменений, а иногда,
и их объяснение. Хорошо написанные комментарии очень важны, поскольку они 
помогают остальным разработчикам видеть ваши изменения, и, возможно, понять,
...
Перед внесением ваших изменений в базу, сначала следует сделать обновление вашей 
...
все конфликты. Также, убедитесь, что вы проверили ваши изменения перед
внесением, по крайней мере, на предмет того, не нарушат ли они сборку системы.


Добавление новых файлов и директорий
====================================

Для добавления новых файлов и директорий в репозиторий, используйте команду
"add"::
...
их содержимое (в отличие, например, от TortoiseSVN), это следует сделать
вручную. После добавления файла, следует использовать команду "commit" для их 
действительного добавления.

.. Note::

    Не добавляйте в репозиторий сгенерированные файлы (обычно это *mmakefile*,
    *strings.h*). Иначе эти файлы не будут обновляться при изменении исходного
    файла.

Установка свойств
=================

Помимо прочего операционные системы различаются тем, какой код используется для
обозначения конца строк. Для того чтобы удбедиться, что текстовый файл созданный
или измененный не в Linux имеет правильные коды конца строк необходимо сделать
следующее::

    svn propset svn:eol-style native <source.c>
    
Также Subversion может вписывать определенные ключевые слова в исходные файлы.
...
.. Note::
    ...
    Вы можете настроить Subversion таким образом, что она автоматически
    установит свойства для определенных типов файлов. Пподробнее читайте в
    документации по Subversion.
    
Импорт
======

Если необходимо добавить большое количество файлов, например таких, как исходный
код уже имеющейся программы, команды "svn add" быстро становятся надоедают. В 
...
1. Добавьте файлы и директории, подлежащие импорту в любое удобное место, но 
   только **не в пределах** вашей рабочей копии. Запуск команды "import" для 
