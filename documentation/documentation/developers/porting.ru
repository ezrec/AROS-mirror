...
ПО с открытым исходным кодом из мира Linux. Данное руководство основано на
`документе`__ написанном Джоаном Самуэллсоном для AmigaOS64.
...
* Следите за тем какие дополнительные пакеты требут программы, которые вы портируете.
...
Наиболее просто портировать программы под AROS из Linux'a, т.к. для этого обычно
...
Первая вещь, которую вы делаете, это проверка того как собрался ваш проект.
Если в корневой директории архива есть файл под названием *configure*, то вы
сможете его настроить под вашу платформу. Как это сделать описано в следующей
главе.

После того как проект правильно настроен наступает время для запуска компилятора.
Запустите сборку вашего проекта введением команды ``make``.

Однако, если нет файла *configure*, то ищите файл под названием *Makefile*.
...
версии вашей программы. Очень частой встречается проблема, когда программа не может
...
настройка неудаётся мы можем попытаться отключить некоторые возможности.
...
.. Подсказка::
	...
	мы повторно запустим скрипт нам надо выполнить команду ``rm -f config.cache``,
...
.. Предупреждение::
    ...
    Не выполняйте команду ``sudo make install``, так как это установит ваше
	приложение в Linux по адресу */usr/local*.
...
``LDLAGS="-nix"``
	...
	вроде */usr/local/app* значат для AROS: подняться на один уровень выше, затем
...
*/PROGDIR*. Так как это не очень хорошее место, то мы скопируем её в место, где
...
Решение:
  Мы используем связывание со статическими библиотеками, и должны точно указывать
  с чем связываться. Поместите эту строку в скрипт настройки::
...
Подобное также часто случается, когда настройка ищет SDL_ttf и теперь вы знаете
почему. Нам надо указать немного большее количество библиотек, чем те, от которых
зависит SDL_ttf. Ему необходимы -lfreetype и -lz. Добавьте их самостоятельно в
качестве тренировки.
...
Если он всё ещё не работает и вы уверены, что установили библиотеку, то попробуйте
удалить в скрипте настройки целиком секцию, в которой производится проверка
недостающей библиотеки. Это не рекомендуется и применимо только когда исчерпаны
...
немного упростить, изменить в соостветствии с вашими нуждами. Обычно вам надо
...
Не забудьте, что вам надо использовать символы табуляции перед командами.


Использование системы сборки
============================

Система сборки содержит скрипты для настройки пакетов. Большими преимуществом при
использовании системы сборки является то, что вы можете легко портировать
приложения на другие разновидность AROS.
...
вы уже можете найти большое количество пакетов GNU.
...
Некоторые пакеты с открытым исходным кодом уже адоптированы под операционные
...
(Ошибка: Нет оператора возврата в функции с непустым возвращаемым значением)
  Функция, которая должна возвращать значение, не возвращает его.

Error: Control reaches end of non-void function
(Ошибка: Проверка достишла конца функции с непустым возвращаемым значением)
  Достигут конец функции, которая должна возвращать значения, но не возвращает его.

Error: May be used uninitialized in this function
(Ошибка: Возможно используется неинициализированная переменная в этой функции)
  Переменная не инициализирована.

Warning: implicit declaration of function 'blah blah'
(Предупреждение: неявное объявление функции 'blah blah')
