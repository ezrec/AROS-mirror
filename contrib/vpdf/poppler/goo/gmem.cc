  ...
  if (p) {
	...
	gMemTail = hdr->prev;
      ...
      if (*trl != gMemDeadVal) {
		...
		kprintf("Overwrite past end of block %d at address %p\n", hdr->index, p);
	...
	*clr = gMemDeadVal;
	  ...
	  fprintf(stderr, "Attempted to free bad address %p, %d\n", p, hdr->size);
	  kprintf("Attempted to free bad address %p:%d\n", p, hdr->size);
	  DumpTaskState(FindTask(NULL));
    }
  }
#else
...
void gvalidate(void *p) {
  ...
  if (p) {
	...
	((hdr->next == NULL) == (hdr == gMemTail))) {
	  ...
	  fprintf(stderr, "Attempted to free bad address %p, %d, header:0x%x\n", p, hdr->size, hdr);
	  kprintf("Attempted to free bad address %p:%d, hdr:0x%x\n", p, hdr->size, hdr);
	  DumpTaskState(FindTask(NULL));
	  Wait(0);
    }
  }
...
void gMemValidate(void) {
  GMemHdr *p;

  //kprintf("%d memory allocations in all\n", gMemIndex);
  if (gMemAlloc > 0) {
	//kprintf("%d memory blocks left allocated:\n", gMemAlloc);
	//kprintf(" index     size\n");
	//kprintf("-------- --------\n");
    for (p = gMemHead; p; p = p->next) {
	  void *memPtr = (char *)p + gMemHdrSize;
	  gvalidate(memPtr);
    }
	//kprintf("-------- --------\n");
  }
