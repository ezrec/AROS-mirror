  ...
  : popplerDataDir(customPopplerDataDir)
{
  ...
  winFontList = NULL;
#endif
#ifdef WITH_FONTCONFIGURATION_MORPHOS
  fontcache = NULL;
#endif
...
static GBool findModifier(const char *name, const char *modifier, const char **start)
{
  ...
  //printf("    matching with modifier:%s\n", modifier);
...
static FcPattern *buildFcPattern(GfxFont *font)
{
  ...
  if (font->getFamily()) {
	...
	//printf("family from descriptor:%s\n", family);
  }
  ...
  if (font->isSerif())
  {
	...
	v.type = FcTypeString;
	v.u.s = (FcChar8*)"serif";
	FcPatternAddWeak(p, FC_FAMILY, v, true);
  }
...
DisplayFontParam *GlobalParams::getDisplayFont(GfxFont *font) {
  ...
  if (!dfp)
  {
    ...
    {
      res = FcPatternGetString(set->fonts[i], FC_FILE, 0, &s);
	  //printf("  match %d = %s. result:%d(match=%d)\n", i, s, res, FcResultMatch);
...
#endif
#if WITH_FONTCONFIGURATION_MORPHOS
#include "GlobalParamsMorphOS.cc"
#endif
