...
/* Following extensions are used in MorphOS build only */

typedef void (*FcExtProgressCallback)(int current, int total, char *currentfile);

void FcExtInsertProgressCallback(FcExtProgressCallback call);
void FcExtRemoveProgressCallback(FcExtProgressCallback call);

struct Library * IXEMUL_InitializeFontconfig();
#endif
