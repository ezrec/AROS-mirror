...
/****************** Do only call from the player-task.  *************/
...
//	PutMidi(midilink,msg);
	...
	Pdebug("sending: %x, maxbuff: %d, skip: %d, midilink: %x\n",msg,maxbuff,skip,midilink);
//	return;

	if((msg>>24)<0x80 || (msg>>16&0xff)>0x7f || (msg>>8&0xff)>0x7f){
		RError("Error. Faulty midi-message. Msg:%x, status: %x, data1: %x, data2: %x\n",msg,msg>>24,msg>>16&0xff,msg>>8&0xff);
		return;
	}

	driverdata=GoodPutMidi(midilink,msg,(ULONG)maxbuff);
// /*
	if(driverdata!=NULL){
		while(Midi2Driver(driverdata,msg,(ULONG)maxbuff)==FALSE){
			if(skip==1) return;
...
void CAMDStopPlaying(struct Instruments *instrument){
	...
	while(patch!=NULL){
		...
		R_PutMidi3(patchdata->mymidilink->midilink,0xb0|patchdata->channel,0x7f,0x00);
