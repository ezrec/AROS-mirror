...
func_shell (char *o, char **argv, const char *funcname)
{
  /* Amiga can't fork nor spawn, but I can start a program with
     ...
     stdout descriptor.  The buffer reading is the same as above,
            ...
            if(!(child_stdout = Open (tmp_output, MODE_NEWFILE)))
               fatal (reading_file,"Cannot open temporary file for calling a command with $(shell ...)");
            ...
            if(FileLock = Lock(tmp_output, SHARED_LOCK))
            {
            ...
            if(len < 0)
               fatal (reading_file,"Cannot examine temporary file created for $(shell ...)");

            buffer = xmalloc (len + 1);

            if(!(child_stdout = Open (tmp_output, MODE_OLDFILE)))
               fatal (reading_file,"Cannot open temporary file created for $(shell ...)");

            if(len)
               len = Read(child_stdout,buffer,len);

            Close (child_stdout);
            DeleteFile (tmp_output);

            if(len < 0)
               fatal (reading_file,"Cannot read from temporary file created for $(shell ...)");
            ...
            o = variable_buffer_output (o, buffer, len);
