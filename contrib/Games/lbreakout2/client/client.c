...
extern void client_recv( GuiWidget *widget, GuiEvent *event );
...
#define HINT_CONNECT "Use the numerical IP address of the game server. (xxx.xxx.xxx.xxx:xxxxx)#"\
"DO NOT use 'localhost' if you run the server on your computer as you won't be able to "\
"challenge others then. Use the IP queried by /sbin/ifconfig instead."
    ...
    int x, int y, SDL_Surface *buffer )
{
    ...
    int score_x = 410;
    StkFont *font;
    if ( y >= client_games->count ) return 0;
    if ( ( game = list_get( client_games, y ) ) == 0 ) return 0;
    /* levelset */
    chat_font_normal->align = STK_FONT_ALIGN_LEFT;
    stk_font_write( chat_font_normal, buffer, 
        2,0, STK_OPAQUE, game->levelset );
    /* : */
    stk_font_write( chat_font_normal, buffer, 
        name_x,0, STK_OPAQUE, ":" );
    stk_font_write( chat_font_normal, buffer, 
        score_x,0, STK_OPAQUE, ":" );
    /* host */
    if ( game->host_wins > game->guest_wins )
        font = chat_font_name;
    else
        font = chat_font_normal;
    font->align = STK_FONT_ALIGN_RIGHT;
    stk_font_write( font, buffer, 
        name_x,0, STK_OPAQUE, game->host );
    sprintf( aux, "%i", game->host_wins );
    stk_font_write( font, buffer, 
        score_x,0, STK_OPAQUE, aux );
    /* guest */
    if ( game->host_wins < game->guest_wins )
        font = chat_font_name;
    else
    ...
    font->align = STK_FONT_ALIGN_LEFT;
    stk_font_write( font, buffer, 
        name_x+font->width,0, STK_OPAQUE, game->guest );
    sprintf( aux, "%i", game->guest_wins );
    stk_font_write( font, buffer, 
        score_x+font->width,0, STK_OPAQUE, aux );
    return 1;
}
/*
====================================================================
    ...
    int x, int y, SDL_Surface *buffer )
{
    ...
    stk_font_write( chat_font_normal, 
        buffer, 2,0, STK_OPAQUE, 
        ((ClientChannel*)list_get( client_channels, y ))->name );
    return 1;
}
#endif    
...
void client_create( void )
{
    ...
    net_init();
    /* initiate net messages */
    net_init_msgs();
    /* users, channels, games */
    client_data_create();
    /* CONNECT WINDOW */
    /* wallpaper with a box. */
    dlg_connect = gui_box_create( 0, 0, 0, 640, 480, 0, 0 );
    surface = gui_widget_get_surface( dlg_connect );
    stk_surface_apply_wallpaper( 
        surface, 0,0,-1,-1, bkgnds[0], STK_OPAQUE );
    parent = gui_box_create( dlg_connect, -1,-1, 400, 214, 0, 0 );
    /* hint label */
    gui_label_create( 
    ...
    gui_label_create( 
        ...
        0, 2, 0, STK_FONT_ALIGN_RIGHT, "Server:" );
    snprintf( server, 24, "%s:%i", config.host, config.port );
    ...
    gui_label_create( 
        ...
        0, 2, 0, STK_FONT_ALIGN_RIGHT, "Username:" );
    ...
    /* chat window */
    list_chatter = gui_list_create( dlg_chatroom, 0,0,470,300, NULL,
        0, 1, 14, 20, client_render_chatter, GUI_LIST_NO_SELECT );
    /* edit + send/whisper buttons */
    gui_button_create( dlg_chatroom, 330,303,70,24, 
        client_send_chatter, 0,0,0,0,0, "Send" );
    gui_button_create( dlg_chatroom, 400,303,70,24, 
        client_whisper_chatter, 0,0,0,0,0, "Whisper" );
    edit_chatter = gui_edit_create( dlg_chatroom, 0,303,327,24,
        client_send_chatter, 2, 0, CHAT_MSG_LIMIT, "" );
    gui_widget_enable_event( edit_chatter, GUI_KEY_RELEASED );
    gui_widget_set_default_key_widget( dlg_chatroom, edit_chatter );
    /* label + running games list */
    gui_label_create( dlg_chatroom, 0,330,470,20,1,
        NULL, 2, 0, STK_FONT_ALIGN_CENTER_X, "Matches" );
    list_games = gui_list_create( dlg_chatroom, 0,353,470,120, NULL,
        1, 1, 14, 20, client_render_game, GUI_LIST_NO_SELECT );
    /* channel button + users in channel */
    label_channel = gui_label_create( dlg_chatroom, 
        474,0,142,18,1, NULL,
        2, 0, STK_FONT_ALIGN_CENTER_X, "MAIN" );
    ...
    gui_label_create( dlg_chatroom, 474,164,124,18,1, NULL,
        2, 0, STK_FONT_ALIGN_CENTER_X, "Levelsets" );
    gui_button_create( dlg_chatroom, 
        598,164,18,18, client_listen,
        0,0,0,0,0, "L" );
    gui_button_create( dlg_chatroom, 
        616,164,18,18, client_transfer,
        0,0,0,0,0, "T" );
    ...
    parent = gui_box_create( dlg_chatroom,
        474,306,160,116, NULL, 0 );
    gui_label_create( parent, 2,2,102,20,0, NULL,
        2, NULL, STK_FONT_ALIGN_LEFT, "Difficulty:" );
    ...
    gui_label_create( parent, 2,2+22,102,20,0, NULL,
        2, NULL, STK_FONT_ALIGN_LEFT, "Rounds:" );
    ...
    gui_label_create( parent, 2,2+44,102,20,0, NULL,
        2, NULL, STK_FONT_ALIGN_LEFT, "Frags:" );
    gui_spinbutton_create( parent, 102,2+44,50,20, 
        client_update_frags,
        5, 30, 5, config.mp_frags );
    gui_label_create( parent, 2,2+66,102,20,0, NULL,
        2, NULL, STK_FONT_ALIGN_LEFT, "Balls:" );
    gui_spinbutton_create( parent, 102,2+66,50,20,
        client_update_balls,
        1, 6, 1, config.mp_balls );
    gui_label_create( parent, 2,2+88,102,20,0, NULL,
        2, NULL, STK_FONT_ALIGN_LEFT, "Game Port:" );
    sprintf( aux, "%i", config.client_game_port );
    gui_edit_create( parent, 102,2+88,50,20,
        client_update_port,
        2, 0, 6, aux );
    /* challenge & disconnect */
    gui_button_create( dlg_chatroom, 
        474,424,160,20, client_challenge,
        0,0,0,0,0, "Challenge" );
    gui_button_create( dlg_chatroom, 
        534,dlg_chatroom->height-20,100,20, 
        client_disconnect,
        0,0,0,0,0, "Disconnect" );
    gui_button_create( dlg_chatroom, 
