...
void config_reset()
{
    ...
    /* multiplayer */
    strcpy( config.host, "192.168.0.1" );
    config.port = 2002;
    strcpy( config.username, "player" );
    config.mp_diff = 1;
    config.mp_rounds = 3;
    config.mp_balls = 5;
    config.mp_frags = 10;
    config.client_game_port = 2003;
}
...
void config_load( )
{
    ...
    if ( parser_get_value( pd, "host", &str, 0 ) )
        strcpy_lt( config.host, str, 15 );
    if ( parser_get_value( pd, "port", &str, 0 ) )
    ...
    if ( parser_get_value( pd, "client_game_port", &str, 0 ) )
        config.client_game_port = atoi( str );
    parser_free( &pd );
}
...
void config_save( )
{
    ...
    if ( ( file = fopen( file_name, "w" ) ) == 0 )
        ...
        fprintf( file, "client_game_port»%i\n", config.client_game_port );
    }
}
