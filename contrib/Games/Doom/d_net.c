...
{	
	...
	*netbuffer = reboundstore;
	...
	if (debugfile)
        ...
        fprintf (stderr, "Net Packet Checksum error, %08x != %08x\n",
                 NetbufferChecksum(),
...
void D_ArbitrateNetStart (void)
{
	...
	do
	{
	    ...
	    //printf("network start info loop: sending packets \n");
	    for (i=0 ; i<doomcom->numnodes ; i++)
	    {
	    ...
	    //printf("network start info loop: getting packets \n");
		...
		{
	           //printf("Got Info from Player %d\n",netbuffer->player&0x7f);		
		   gotinfo[netbuffer->player&0x7f] = true;
		} else {;
		   //printf("Bad Info from Player %d\n",netbuffer->player&0x7f);
		}
	    }
#else
	    while (HGetPacket ())
	...
	//printf("network start info okay for all nodes\n");
