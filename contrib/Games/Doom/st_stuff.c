  ...
  else if (ev->type == ev_keydown)
  {
    if (!netgame)
    {
      ...
      if (cht_CheckCheat(&cheat_god, ev->data1))
      {
        ...
        {
          if (plyr->mo)
            plyr->mo->health = god_health; // Dehacked
          
          plyr->health = god_health; // Dehacked
      ...
      {
        plyr->armorpoints = idfa_armor; // Dehacked
        plyr->armortype = idfa_armor_class; // Dehacked
    ...
    if (cht_CheckCheat(&cheat_clev, ev->data1))
    {
      ...
      if (gamemode == commercial)
      {
	epsd = 1;  /* changed from 0 by Peter McGavin, 28 Feb 1998 */
