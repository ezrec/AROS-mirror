  ...
  byte*		palette )
{
    ...
    pcx->version = 5;			// PaintBrush v3.0
    pcx->encoding = 1;			// uncompressed
    pcx->bits_per_pixel = 8;		// 256 color
    pcx->xmin = 0;
    pcx->ymin = 0;
    pcx->xmax = SWAPSHORT(width-1);
    pcx->ymax = SWAPSHORT(height-1);
    pcx->hres = SWAPSHORT(width);
    pcx->vres = SWAPSHORT(height);
    memset (pcx->palette,0,sizeof(pcx->palette));
    pcx->color_planes = 1;		// chunky image
    pcx->bytes_per_line = SWAPSHORT(width);
    pcx->palette_type = SWAPSHORT(1);	// color information
