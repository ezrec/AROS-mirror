...
void WI_slamBackground(void)
{
    ...
    I_MarkRect (0, 0, SCREENWIDTH, SCREENHEIGHT);
}
...
void WI_drawLF(void)
{
    ...
    // draw <LevelName> 
    V_DrawPatchInDirect((320 - SWAPSHORT(lnames[wbs->last]->width))/2,
		y, FB, lnames[wbs->last]);

    // draw "Finished!"
    y += (5*SWAPSHORT(lnames[wbs->last]->height))/4;
    
    V_DrawPatchInDirect((320 - SWAPSHORT(finished->width))/2,
		y, FB, finished);
}
...
void WI_drawEL(void)
{
    ...
    // draw "Entering"
    V_DrawPatchInDirect((320 - SWAPSHORT(entering->width))/2,
		y, FB, entering);

    // draw level
    y += (5*SWAPSHORT(lnames[wbs->next]->height))/4;

    V_DrawPatchInDirect((320 - SWAPSHORT(lnames[wbs->next]->width))/2,
  ...
  int		digits )
{
    ...
    int		fontwidth = SWAPSHORT(num[0]->width);
    int		neg;
    int		temp;
  ...
  int		t )
{
    ...
    {
	// "sucks"
	V_DrawPatchInDirect(x - SWAPSHORT(sucks->width), y, FB, sucks); 
    }
}
...
void WI_drawDeathmatchStats(void)
{
    ...
    V_DrawPatchInDirect(DM_TOTALSX-SWAPSHORT(total->width)/2,
	...
	if (playeringame[i])
	{
	    V_DrawPatchInDirect(x-SWAPSHORT(p[i]->width)/2,
			DM_MATRIXY - WI_SPACINGY,
			FB,
			p[i]);
	    
	    V_DrawPatchInDirect(DM_MATRIXX-SWAPSHORT(p[i]->width)/2,
			y,
			FB,
			p[i]);
	...
	{
	    // V_DrawPatchInDirect(x-SWAPSHORT(bp[i]->width)/2,
	    //   DM_MATRIXY - WI_SPACINGY, FB, bp[i]);
	    // V_DrawPatchInDirect(DM_MATRIXX-SWAPSHORT(bp[i]->width)/2,
	    //   y, FB, bp[i]);
	}
	x += DM_SPACINGX;
	y += WI_SPACINGY;
...
void WI_drawNetgameStats(void)
{
    ...
    int		x;
    int		y;
    int		pwidth = SWAPSHORT(percent->width);
    ...
    // draw stat titles (top line)
    V_DrawPatchInDirect(NG_STATSX+NG_SPACINGX-SWAPSHORT(kills->width),
		NG_STATSY, FB, kills);

    V_DrawPatchInDirect(NG_STATSX+2*NG_SPACINGX-SWAPSHORT(items->width),
		NG_STATSY, FB, items);

    V_DrawPatchInDirect(NG_STATSX+3*NG_SPACINGX-SWAPSHORT(secret->width),
		NG_STATSY, FB, secret);
    
    if (dofrags)
	V_DrawPatchInDirect(NG_STATSX+4*NG_SPACINGX-SWAPSHORT(frags->width),
		    NG_STATSY, FB, frags);
	...
	x = NG_STATSX;
	V_DrawPatchInDirect(x-SWAPSHORT(p[i]->width), y, FB, p[i]);

	if (i == me)
	    V_DrawPatchInDirect(x-SWAPSHORT(p[i]->width), y, FB, star);
...
void WI_loadData(void)
{
    ...
    if ((i = W_CheckNumForName("WIMINUS")) != -1)
        wiminus = (patch_t *) W_CacheLumpNum(i, PU_STATIC);
