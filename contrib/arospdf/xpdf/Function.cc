...
Function *Function::parse(xObject *funcObj) {
  Function *func;
  Dict *dict;
  int funcType;
  xObject obj1;
...
SampledFunction::SampledFunction(xObject *funcObj, Dict *dict) {
  Stream *str;
  int sampleBits;
  double sampleMul;
  xObject obj1, obj2;
...
ExponentialFunction::ExponentialFunction(xObject *funcObj, Dict *dict) {
  xObject obj1, obj2;
  int i;
...
StitchingFunction::StitchingFunction(xObject *funcObj, Dict *dict) {
  xObject obj1, obj2;
  int i;
...
PostScriptFunction::PostScriptFunction(xObject *funcObj, Dict *dict) {
  Stream *str;
  int codePtr;
  GString *tok;
...
void PostScriptFunction::exec(PSStack *stack, int codePtr) {
  ...
  while (1) {
	...
	return;
      ...
      break;
    default:
      error(-1, "Internal: bad xObject in PostScript function code");
      break;
    }
  }
}
