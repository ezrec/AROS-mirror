...
public:
  ...
  // Create an xObject stream, using xObject number <objStrNum>,
  // generation 0.
  ObjectStream(XRef *xref, int objStrNumA);

  ~ObjectStream();

  // Return the xObject number of this xObject stream.
  int getObjStrNum() { return objStrNum; }

  // Get the <objIdx>th xObject from this stream, which should be
  // xObject number <objNum>, generation 0.
  xObject *getObject(int objIdx, int objNum, xObject *obj);

private:

  int objStrNum;		// xObject number of the xObject stream
  int nObjects;			// number of xObjects in the stream
  xObject *objs;			// the xObjects (length = nObjects)
  int *objNums;			// the xObject numbers (length = nObjects)
};
...
ObjectStream::ObjectStream(XRef *xref, int objStrNumA) {
  ...
  objs = new xObject[nObjects];
  objNums = (int *)gmallocn(nObjects, sizeof(int));
  offsets = (int *)gmallocn(nObjects, sizeof(int));

  // parse the header: xObject numbers and offsets
  objStr.streamReset();
  obj1.initNull();
  ...
  // skip to the first xObject - this shouldn't be necessary because
...
xObject *ObjectStream::getObject(int objIdx, int objNum, xObject *obj) {
...
XRef::XRef(BaseStream *strA) {
  ...
  // get the root dictionary (catalog) xObject
...
xObject *XRef::fetch(int num, int gen, xObject *obj) {
  XRefEntry *e;
  Parser *parser;
...
xObject *XRef::getDocInfo(xObject *obj) {
  return trailerDict.dictLookup("Info", obj);
}

// Added for the pdftex project.
xObject *XRef::getDocInfoNF(xObject *obj) {
