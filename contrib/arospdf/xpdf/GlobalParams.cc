...
Plugin *Plugin::load(char *type, char *name) {
  ...
  if (!(libA = LoadLibrary(path->getCString()))) {
    error(-1, (char*)"Failed to load plugin '%s'",
	  path->getCString());
    goto err1;
  }
  if (!(vt = (XpdfPluginVecTable *)
	         GetProcAddress(libA, "xpdfPluginVecTable"))) {
    error(-1, (char*)"Failed to find xpdfPluginVecTable in plugin '%s'",
	  path->getCString());
    goto err2;
  }
#else
  ...
  if (!(dlA = dlopen(path->getCString(), RTLD_NOW))) {
    error(-1, (char*)"Failed to load plugin '%s': %s",
  ...
  if (vt->version != xpdfPluginVecTable.version) {
    error(-1, (char*)"Plugin '%s' is wrong version", path->getCString());
    goto err2;
  }
  ...
  if (!(xpdfInitPlugin = (XpdfBool (*)(void))
    ...
    error(-1, (char*)"Failed to find xpdfInitPlugin in plugin '%s'",
	  path->getCString());
    goto err2;
  }
#else
  ...
  if (!(*xpdfInitPlugin)()) {
    error(-1, (char*)"Initialization of plugin '%s' failed",
	  path->getCString());
    goto err2;
  }
...
GlobalParams::GlobalParams(char *cfgFileName) {
  ...
  if ((paperName = systempapername())) {
    ...
    error(-1, (char*)"No paper information available - using defaults");
...
void GlobalParams::parseLine(char *buf, GString *fileName, int line) {
  ...
  if (tokens->getLength() > 0 &&
	...
	if ((f2 = fopen(incFile->getCString(), "r"))) {
	  ...
	  error(-1, (char*)"Couldn't find included config file: '%s' (%s:%d)",
		incFile->getCString(), fileName->getCString(), line);
	}
      } else {
	error(-1, (char*)"Bad 'include' config file command (%s:%d)",
      ...
      error(-1, (char*)"Unknown config file command '%s' (%s:%d)",
	    cmd->getCString(), fileName->getCString(), line);
      if (!cmd->cmp("displayFontX") ||
	  !cmd->cmp("displayNamedCIDFontX") ||
	  !cmd->cmp("displayCIDFontX")) {
	error(-1, (char*)"-- Xpdf no longer supports X fonts");
      } else if (!cmd->cmp("t1libControl") || !cmd->cmp("freetypeControl")) {
	error(-1, (char*)"-- The t1libControl and freetypeControl options have been replaced");
	error(-1, (char*)"   by the enableT1lib, enableFreeType, and antialias options");
      } else if (!cmd->cmp("fontpath") || !cmd->cmp("fontmap")) {
	error(-1, (char*)"-- the config file format has changed since Xpdf 0.9x");
      }
    }
  }
...
void GlobalParams::parseNameToUnicode(GList *tokens, GString *fileName,
  ...
  if (tokens->getLength() != 2) {
    error(-1, (char*)"Bad 'nameToUnicode' config file command (%s:%d)",
	  fileName->getCString(), line);
    return;
  }
  name = (GString *)tokens->get(1);
  if (!(f = fopen(name->getCString(), "r"))) {
    error(-1, (char*)"Couldn't open 'nameToUnicode' file '%s'",
	  name->getCString());
    return;
  }
  line2 = 1;
  while (getLine(buf, sizeof(buf), f)) {
    ...
    if (tok1 && tok2) {
      ...
      error(-1, (char*)"Bad line in 'nameToUnicode' file (%s:%d)", name, line2);
    }
    ++line2;
  }
  fclose(f);
...
void GlobalParams::parseCIDToUnicode(GList *tokens, GString *fileName,
  ...
  if (tokens->getLength() != 3) {
    error(-1, (char*)"Bad 'cidToUnicode' config file command (%s:%d)",
	  fileName->getCString(), line);
    return;
  }
...
void GlobalParams::parseUnicodeToUnicode(GList *tokens, GString *fileName,
  ...
  if (tokens->getLength() != 3) {
    error(-1, (char*)"Bad 'unicodeToUnicode' config file command (%s:%d)",
	  fileName->getCString(), line);
    return;
  }
...
void GlobalParams::parseUnicodeMap(GList *tokens, GString *fileName,
  ...
  if (tokens->getLength() != 3) {
    error(-1, (char*)"Bad 'unicodeMap' config file command (%s:%d)",
	  fileName->getCString(), line);
    return;
  }
...
void GlobalParams::parseCMapDir(GList *tokens, GString *fileName, int line) {
  ...
  if (tokens->getLength() != 3) {
    error(-1, (char*)"Bad 'cMapDir' config file command (%s:%d)",
	  fileName->getCString(), line);
    return;
  }
  collection = (GString *)tokens->get(1);
...
void GlobalParams::parseToUnicodeDir(GList *tokens, GString *fileName,
				     int line) {
  if (tokens->getLength() != 2) {
    error(-1, (char*)"Bad 'toUnicodeDir' config file command (%s:%d)",
	  fileName->getCString(), line);
    return;
  }
...
void GlobalParams::parseDisplayFont(GList *tokens, GHash *fontHash,
 ...
 err2:
  delete param;
 err1:
  error(-1, (char*)"Bad 'display*Font*' config file command (%s:%d)",
	fileName->getCString(), line);
}
...
void GlobalParams::parsePSPaperSize(GList *tokens, GString *fileName,
  ...
  if (tokens->getLength() == 2) {
    ...
    if (!setPSPaperSize(tok->getCString())) {
      error(-1, (char*)"Bad 'psPaperSize' config file command (%s:%d)",
	    fileName->getCString(), line);
    }
...
void GlobalParams::parsePSImageableArea(GList *tokens, GString *fileName,
  ...
  if (tokens->getLength() != 5) {
    error(-1, (char*)"Bad 'psImageableArea' config file command (%s:%d)",
	  fileName->getCString(), line);
    return;
  }
...
void GlobalParams::parsePSLevel(GList *tokens, GString *fileName, int line) {
  ...
  if (tokens->getLength() != 2) {
    error(-1, (char*)"Bad 'psLevel' config file command (%s:%d)",
	  fileName->getCString(), line);
    return;
  }
  tok = (GString *)tokens->get(1);
...
void GlobalParams::parsePSFile(GList *tokens, GString *fileName, int line) {
  if (tokens->getLength() != 2) {
    error(-1, (char*)"Bad 'psFile' config file command (%s:%d)",
	  fileName->getCString(), line);
    return;
  }
  if (psFile) {
...
void GlobalParams::parsePSFont(GList *tokens, GString *fileName, int line) {
  ...
  if (tokens->getLength() != 3) {
    error(-1, (char*)"Bad 'psFont' config file command (%s:%d)",
	  fileName->getCString(), line);
    return;
  }
...
void GlobalParams::parsePSFont16(const char *cmdName, GList *fontList,
  ...
  if (tokens->getLength() != 5) {
    error(-1, (char*)"Bad '%s' config file command (%s:%d)",
...
void GlobalParams::parseTextEncoding(GList *tokens, GString *fileName,
				     int line) {
  if (tokens->getLength() != 2) {
    error(-1, (char*)"Bad 'textEncoding' config file command (%s:%d)",
	  fileName->getCString(), line);
    return;
  }
  delete textEncoding;
...
void GlobalParams::parseTextEOL(GList *tokens, GString *fileName, int line) {
  ...
  if (tokens->getLength() != 2) {
    error(-1, (char*)"Bad 'textEOL' config file command (%s:%d)",
	  fileName->getCString(), line);
    return;
  }
  tok = (GString *)tokens->get(1);
...
void GlobalParams::parseFontDir(GList *tokens, GString *fileName, int line) {
  if (tokens->getLength() != 2) {
    error(-1, (char*)"Bad 'fontDir' config file command (%s:%d)",
	  fileName->getCString(), line);
    return;
  }
...
void GlobalParams::parseInitialZoom(GList *tokens,
				    GString *fileName, int line) {
  if (tokens->getLength() != 2) {
    error(-1, (char*)"Bad 'initialZoom' config file command (%s:%d)",
	  fileName->getCString(), line);
    return;
  }
  delete initialZoom;
...
void GlobalParams::parseScreenType(GList *tokens, GString *fileName,
  ...
  if (tokens->getLength() != 2) {
    error(-1, (char*)"Bad 'screenType' config file command (%s:%d)",
	  fileName->getCString(), line);
    return;
  }
  tok = (GString *)tokens->get(1);
...
void GlobalParams::parseBind(GList *tokens, GString *fileName, int line) {
  ...
  if (tokens->getLength() < 4) {
    error(-1, (char*)"Bad 'bind' config file command (%s:%d)",
	  fileName->getCString(), line);
    return;
  }
...
void GlobalParams::parseUnbind(GList *tokens, GString *fileName, int line) {
  ...
  if (tokens->getLength() != 3) {
    error(-1, (char*)"Bad 'unbind' config file command (%s:%d)",
	  fileName->getCString(), line);
    return;
  }
  if (!parseKey((GString *)tokens->get(1), (GString *)tokens->get(2),
  ...
  *mods = xpdfKeyModNone;
  ...
  if (!strcmp(p0, "space")) {
    ...
    *code = (int)*p0;
    ...
    error(-1, (char*)"Bad key/modifier in '%s' config file command (%s:%d)",
	  cmdName, fileName->getCString(), line);
    return gFalse;
  }
  ...
  if (!strcmp(p0, "any")) {
	...
	*context |= xpdfKeyContextScrLockOff;
	...
	error(-1, (char*)"Bad context in '%s' config file command (%s:%d)",
	      cmdName, fileName->getCString(), line);
	return gFalse;
      }
      if (!*p0) {
...
#endif
    if (!fileName) {
      error(-1, (char*)"No display font for '%s'", displayFontTab[i].name);
      delete fontName;
      continue;
    }
