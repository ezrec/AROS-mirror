...
typedef struct {
   ...
   * and construct an authorization data xObject.  Returns non-zero if
   * successful.
   */
...
/*
 * Get a document's info dictionary.  (The returned xObject must be
 * freed with xpdfFreeObj.)
 */
XpdfObject (*_xpdfGetInfoDict)(XpdfDoc doc);

/*
 * Get a document's catalog ("root") dictionary.  (The returned xObject
...
/*
 * Value access.
 * (xObjects returned by xpdfArrayGet and xpdfDictGet must be freed
 * with xpdfFreeObj.)
 */
...
/*
 * xObject destruction.  NB: *all* xObjects must be freed after use.
 */
