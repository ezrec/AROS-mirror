...
Catalog::Catalog(XRef *xrefA) {
  xObject catDict, pagesDict, pagesDictRef;
  xObject obj, obj2;
  char *alreadyRead;
  int numPages0;
  int i;
  ...
  if (!catDict.isDict()) {
    error(-1, "Catalog xObject is wrong type (%s)", catDict.getTypeName());
    goto err1;
  }
  ...
  if (!pagesDict.isDict()) {
    error(-1, "Top-level pages xObject is wrong type (%s)",
  ...
  if (!obj.isNum()) {
    error(-1, "Page count in top-level pages xObject is wrong type (%s)",
	  obj.getTypeName());
    goto err3;
  }
  ...
  if (numPages != numPages0) {
    error(-1, "Page count in top-level pages xObject is incorrect");
  }
  pagesDict.free();
...
int Catalog::readPageTree(Dict *pagesDict, PageAttrs *attrs, int start,
  ...
  if (!kids.isArray()) {
    error(-1, "Kids xObject (page %d) is wrong type (%s)",
  ...
  for (i = 0; i < kids.arrayGetLength(); ++i) {
	...
	goto err2;
      ...
      error(-1, "Kid xObject (page %d) is wrong type (%s)",
...
xObject *Catalog::findDestInTree(xObject *tree, GString *name, xObject *obj) {
  xObject names, name1;
  xObject kids, kid, limits, low, high;
  GBool done, found;
  int cmp, i;
