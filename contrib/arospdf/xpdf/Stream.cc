...
Stream *Stream::addFilters(xObject *dict) {
  xObject obj, obj2;
  xObject params, params2;
  Stream *str;
  int i;
...
Stream *Stream::makeFilter(char *name, Stream *str, xObject *params) {
  int pred;			// parameters
  int colors;
  int bits;
  int early;
...
BaseStream::BaseStream(xObject *dictA) {
  dict = *dictA;
}
...
MemStream::MemStream(char *bufA, Guint startA, Guint lengthA, xObject *dictA):
    BaseStream(dictA) {
  buf = bufA;
  start = startA;
  length = lengthA;
...
EmbedStream::EmbedStream(Stream *strA, xObject *dictA,
