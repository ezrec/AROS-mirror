...
xObject *Parser::getObj(xObject *obj, Guchar *fileKey,
  ...
  // dictionary or stream
    ...
    while (!buf1.isCmd(">>") && !buf1.isEOF()) {
      if (!buf1.isName()) {
	error(getPos(), "Dictionary key must be a name xObject");
	shift();
      } else {
    ...
    // stream xObjects are not allowed inside content streams or
    // xObject streams
...
Stream *Parser::makeStream(xObject *dict, Guchar *fileKey,
			   CryptAlgorithm encAlgorithm, int keyLength,
			   int objNum, int objGen) {
  xObject obj;
  BaseStream *baseStr;
