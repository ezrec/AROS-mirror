...
public:
  ...
  Stream *addFilters(xObject *dict);

private:

  Stream *makeFilter(char *name, Stream *str, xObject *params);
...
public:
  ...
  BaseStream(xObject *dictA);
  virtual ~BaseStream();
  virtual Stream *makeSubStream(Guint start, GBool limited,
				Guint length, xObject *dict) = 0;
  virtual void setPos(Guint pos, int dir = 0) = 0;
...
public:
  ...
  // Create an image stream xObject for an image with the specified
...
public:
  ...
  // Create a predictor xObject.  Note that the parameters are for the
...
public:
  ...
  MemStream(char *bufA, Guint startA, Guint lengthA, xObject *dictA);
  virtual ~MemStream();
  virtual Stream *makeSubStream(Guint start, GBool limited,
				Guint lengthA, xObject *dictA);
...
public:
  ...
  EmbedStream(Stream *strA, xObject *dictA, GBool limitedA, Guint lengthA);
  virtual ~EmbedStream();
