...
public:
  ...
  // Construct a new PageAttrs xObject by merging a dictionary
  // (of type Pages or Page) into another PageAttrs xObject.  If
...
private:
  ...
  PDFRectangle artBox;
  int rotate;
  xObject lastModified;
  xObject boxColorInfo;
  xObject group;
  xObject metadata;
  xObject pieceInfo;
  xObject separationInfo;
  xObject resources;
};
...
public:
  ...
  xObject *getAnnots(xObject *obj) { return annots.fetch(xref, obj); }

  // Return a list of links.
  Links *getLinks(Catalog *catalog);

  // Get contents.
  xObject *getContents(xObject *obj) { return contents.fetch(xref, obj); }
...
private:
  ...
  xObject annots;		// annotations array
  xObject contents;		// page contents
