...
int *prot;
{
	...
	for (a=0;a<b;a++) {
		...
		if (ch=='-') notflag=1;
			...
			for (c=0;c<8;c++) {
				if (ch==globstring[STR_PROTBIT_ALLBITS][c]) {
					prot[notflag]|=(1<<(7-c));
					break;
				}
			}
...
int win;
{
	if (!dopus_curwin[win]->firstentry || dopus_curwin[win]->firstentry->type!=ENTRY_CUSTOM) {
		lsprintf(str_select_info,globstring[STR_DIRS_FILES_BYTES_COUNT],
			...
			dopus_curwin[win]->filetot,dopus_curwin[win]->bytessel,dopus_curwin[win]->bytestot);
	}
	else {
