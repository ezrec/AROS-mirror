...
/* AROS: Because of some now private structs below, we make some changes */
#if 0
...
int *dis;
{
	...
	for (a=0;a<b;a++) if (!(isspace(dbuffer[a]))) break;
	if (a==b) goto getout;
	if (dbuffer[a]=='>') {
		++a; *dis=2;
		goto getout;
	...
	if (a>=b) goto getout;
	for (++a;a<b;a++) if (!(isspace(dbuffer[a]))) break;
	if (a>=b) goto getout;
