...
void give_version_info()
{
    ...
    lsprintf(lib_ver_buf,"v%ld.%ld",
        (long)DOpusBase->LibNode.lib_Version,(long)DOpusBase->LibNode.lib_Revision);
    ...
    if (/*DOSBase->dl_lib.lib_Version<36 ||*/ (GetVar("Workbench",&wb_ver_buf[1],19,GVF_GLOBAL_ONLY))<1)
    {
        ...
        lsprintf(wb_ver_buf,"v%ld.%ld",(long)ver,(long)rev);
    }
    else wb_ver_buf[0]='v';
    ...
    lsprintf(proc_buf,"68%03ld",(long)proc);
    ...
    lsprintf(coproc_buf,fpu?"68%03ld":globstring[STR_PROTECT_NONE],(long)fpu);
