...
static struct NewWindow
	viewwin={
		0,0,0,0,
		255,255,
		IDCMP_RAWKEY|IDCMP_MOUSEBUTTONS|IDCMP_GADGETUP|
...
{
	BPTR in;
	int a,b,c,err,size,old_file_size,old_buffer_size,fsize,retcode=100;
	short
		scroll_speed,
		scroll_pos_1,
		scroll_pos_2,
	...
	if (vdata->view_window) {
		...
		if (config->viewbits&VIEWBITS_TEXTBORDERS) {
			vdata->view_lines_per_screen=(vdata->view_status_bar_ypos-6)/vdata->view_font->tf_YSize;

			vdata->view_display_left=4;
			vdata->view_display_top=((vdata->view_status_bar_ypos-6)-
				(vdata->view_lines_per_screen*vdata->view_font->tf_YSize))/2;
			if (vdata->view_display_top<2) vdata->view_display_top=2;

			vdata->view_display_right=vdata->view_window->Width-20;
			vdata->view_display_bottom=vdata->view_status_bar_ypos-5;

			Do3DBox(vdata->view_rastport,vdata->view_display_left-2,1,
				(vdata->view_display_right-vdata->view_display_left)+4,
				vdata->view_display_bottom,
				vdata->view_colour_table[PEN_SHINE],
				vdata->view_colour_table[PEN_SHADOW]);

			vdata->view_gadgets[VIEW_SCROLLGADGET].LeftEdge=vdata->view_display_right+8;
			vdata->view_gadgets[VIEW_SCROLLGADGET].TopEdge=2;
			vdata->view_gadgets[VIEW_SCROLLGADGET].Height=vdata->view_display_bottom-2;
			...
			vdata->view_lines_per_screen=(vdata->view_status_bar_ypos-1)/vdata->view_font->tf_YSize;
		...
		vdata->view_gadgets[VIEW_JUMPTOLINE].LeftEdge=
			vdata->view_window->Width-((vdata->view_font->tf_XSize*49)+14);
		vdata->view_gadgets[VIEW_JUMPTOPERCENT].LeftEdge=
			vdata->view_window->Width-((vdata->view_font->tf_XSize*22)+14);
...
readnormal:
	...
	if (function==FUNC_ANSIREAD && !vdata->view_display_as_hex) {
		...
		ansiread_win.Height=vdata->view_font->tf_YSize*3;
	...
	SetWindowTitles(vdata->view_window,(char *)-1,titlebuf);

	vdata->view_window->WLayer->Flags|=LAYERBACKDROP;
				...
				case IDCMP_RAWKEY:
					...
					switch (code) {
						...
						case 0x1e:
							...
							break;
						case 0x19: view_checkprint(vdata,0); break;
						case 0x33: view_checkprint(vdata,1); break;
					}
					break;
			}
		}
...
int y,len;
{
	...
	if (vdata->view_display_as_ansi) {
		if (len>0) {
			...
			if (y>-1) {
				ClipBlit(vdata->view_ansiread_window->RPort,0,0,
					vdata->view_rastport,
					vdata->view_display_left,
					vdata->view_display_top+(y*vdata->view_font->tf_YSize),
					vdata->view_ansiread_window->Width,
...
char *str;
{
	...
	RectFill(vdata->view_rastport,
		...
		vdata->view_window->Width-((vdata->view_font->tf_XSize*18)+3),
		vdata->view_window->Height-2);

	view_pensrp(vdata);

	if (str) {
		Move(vdata->view_rastport,2,vdata->view_status_bar_ypos+vdata->view_font->tf_Baseline);
...
struct ViewData *vdata;
{
	...
	Do3DBox(vdata->view_rastport,
		2,
		vdata->view_status_bar_ypos,
		vdata->view_window->Width-((vdata->view_font->tf_XSize*18)+4),
	...
	for (a=0;a<9;a++) {
		...
		vdata->view_gadgets[a].LeftEdge=
			(vdata->view_window->Width-(vdata->view_font->tf_XSize*18))+
			(a*vdata->view_font->tf_XSize*2);
		vdata->view_gadgets[a].TopEdge=vdata->view_window->Height-(vdata->view_font->tf_YSize+2);
	...
	for (a=0;a<2;a++) {
		DoArrow(vdata->view_rastport,
			vdata->view_gadgets[a].LeftEdge+2,
			vdata->view_window->Height-vdata->view_font->tf_YSize,
