...
int win,read;
{
	...
	if (!(findbuffer(dirbuf,win,0,0))) {
		...
		for (a=0;a<data_buffer_count[otherwin];a++,dir=dir->next) {
			if ((LStrCmpI(dirbuf,dir->directory))==0) {
...
int win;
{
	...
	for (a=0;a<data_buffer_count[win];a++) {
		if (LStrCmpI(dir->directory,str_pathbuffer[win])==0 ||
			LStrCmpI(dir->directory,fullbuf)==0) {
			ok=1;
			break;
		}
		dir=dir->next;
	...
	for (try=0;try<2;try++) {
		...
		for (a=0;a<data_buffer_count[win];a++,dir=dir->last) {
			if (dir->directory[0]) {
				...
				if (founddir || (LStrCmpI(tempbuf,dir->directory))==0) {
					if (!(lockandexamine(tempbuf,fblock)) ||
						(CompareDate(&fblock->fib_Date,&dir->dirstamp)!=0)) continue;
...
char *string,*old,*new;
{
	...
	oldlen=strlen(old);
	if (LStrnCmpI(string,old,oldlen)) return(0);
...
int destwin;
{
	...
	busy(); b=scrdata_dispwin_nchars[destwin]; name[0]=0;
	DateStamp(&ds);
	for (win=0;win<2;win++) {
		dir=dopus_firstwin[win];
		for (a=0;a<data_buffer_count[win];a++) {
...
#warning Rexx stuff not implemented
kprintf("userentrmessage() (ARexx stuff) NOT IMPLEMENTED BECAUSE OF MISSING FUNCS\n");
...
int win;
{
	...
	if (config->dirflags&DIRFLAGS_REREADOLD && dopus_curwin[win]->directory[0]) {
		if (dopus_curwin[win]->firstentry &&
			...
			dopus_curwin[win]->firstentry->type==ENTRY_DEVICE)) return;
		else {
			/* AROS: Use AllocDosObject() instead
			struct FileInfoBlock __aligned testinfo;
			*/
			
			struct FileInfoBlock *testinfo;
			
			testinfo = AllocDosObject(DOS_FIB, NULL);
			assert(NULL != testinfo);
			...
			FreeDosObject(DOS_FIB, testinfo);
		}
		if (reread) {
