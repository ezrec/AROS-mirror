...
/* AROS: We use ELF on Linux so this check does not work 
...
int allabort,all;
{
	...
	*/
	...
	struct FileInfoBlock *s_fib;
	
	d_fib = AllocDosObject(DOS_FIB, NULL);
	assert(NULL != d_fib);

	s_fib = AllocDosObject(DOS_FIB, NULL);
	assert(NULL != s_fib);
	
	if (!(lockandexamine(sourcename,s_fib))) { ret = 1; goto exit; }
	...
	if (config->existflags&REPLACE_ALWAYS) { ret = 1; goto exit; }
	...
	if (config->existflags&REPLACE_OLDER) {
		...
		if (suc_dfib) {
			if (CompareDate(date,&(d_fib->fib_Date))>0) { ret = 1; goto exit; }
			doerror(ERROR_OBJECT_EXISTS);
		}
		ret = 0;
		goto exit;
...
exit:
	FreeDosObject(DOS_FIB, d_fib);
	FreeDosObject(DOS_FIB, s_fib);
...
char *title;
{
	req->rb_fg=screen_pens[(int)config->requestfg].pen;
	req->rb_bg=screen_pens[(int)config->requestbg].pen;
	req->rb_shine=screen_pens[(int)config->gadgettopcol].pen;
	req->rb_shadow=screen_pens[(int)config->gadgetbotcol].pen;
...
struct StringExtend *extend;
{
	...
	extend->Pens[0]=screen_pens[(int)config->stringfgcol].pen;
	extend->Pens[1]=screen_pens[(int)config->stringbgcol].pen;
	extend->ActivePens[0]=screen_pens[(int)config->stringselfgcol].pen;
	extend->ActivePens[1]=screen_pens[(int)config->stringselbgcol].pen;
}
