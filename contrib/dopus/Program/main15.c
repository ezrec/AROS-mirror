...
int state;
{
	...
	if ((type=checkfiletype(buf,FTFUNC_DOUBLECLICK,0))) {
		...
		if ((strcmp(type->type,"Default")==0) ||
			(strcmp(type->type,globstring[STR_FTYPE_DEFAULT])==0)) dodef=1;
		else {
			char title[256];
...
#warning We lack inovamusic library
#if 0			
			...
			default:
				...
				switch (function) {
					...
					case FUNC_ADDICON:
						...
						break;
					case FUNC_DELETE:
						dofilefunction(FUNC_DELETE,0,spath,NULL,actwin,-1,rexx);
						break;
					case FUNC_CLONE:
					...
					case FUNC_BYTE:
						...
						dofilefunction(FUNC_BYTE,flag,spath,dpath,a,1-a,rexx);
						break;
					case FUNC_UNBYTE:
						if (!rexx || rexx_argcount<1 ||
							(a=atoi(rexx_args[0]))<0 || a>1) a=data_active_window;
...
*/
	...
	if (function!=FUNC_READ && function!=FUNC_HEXREAD &&
