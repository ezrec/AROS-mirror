...
/* AROS: We don't have __chip keyword */
USHORT /* __chip */
	uniconifydata[2][20]={
		{0x3f80,0xa0a0,0x2080,0xa0a0,0x3f80,0x8020,0x0000,0x8020,
			0x0000,0xaaa0,
		0x0000,0x1f00,0x1f00,0x1f00,0x0000,0x0000,0x0000,0x0000,
			0x0000,0x0000},
		{0xffe0,0x8020,0xffe0,0x8020,0x8020,0x8020,0x8020,0x8020,
			0x8020,0xffe0,
		0x0000,0x7fc0,0x0000,0x7fc0,0x7fc0,0x7fc0,0x7fc0,0x7fc0,
			0x7fc0,0x0000}};
...
struct NewWindow
	icon_win={
		...
		ICON_IDCMP,
		WFLG_CLOSEGADGET|WFLG_RMBTRAP|WFLG_DRAGBAR|WFLG_DEPTHGADGET,
...
static int icon_tx,icon_ty,icon_len,icon_fpen,icon_bpen,icon_maxlen;
...
static struct DiskObject iconify_appicon={
	0,0,
#warning I cannot find a GADGBACKFILL define anywhere
		{NULL,0,0,80,41, 0 /*GADGBACKFILL */,0,0,
...
kprintf("iconify() NOT IMPLEMENTED BECAUSE AOF MISSING APPICON FUNCS\n");
	...
	if (system_version2 && (sptr=LockPubScreen("Workbench"))) {
	...
	if (buttons) {
		...
		if (icon_type&ICON_APPICON) {
			if (!system_version2 || !WorkbenchBase ||
				!(appicon=AddAppIconA(0,0,(UBYTE *)"Directory Opus",
				appmsg_port,NULL,&iconify_appicon,NULL)))
	...
	FOREVER {
		...
		if (icon_gotclock) {
			...
			if (icon_type&ICON_CPU) {
				if (!(usage=(getusage()/10))) usage=oldusage;
