...
static char maxlengths[6]={30,80,40,32,32,-1};
...
char *command;
{
	int a,b,c,d,f,val,function,win,statval,retval,x,y,w,h;
	char buf[520],*buf1,pad[8],buf2[10],*buf3,*ptr,ch,*retbuf;
	BPTR lock;
	struct DateStamp ds;
	...
	if (rexx_argcount==0 && func_global_function!=FUNC_PRINT) rexx_global_flag=0;
		...
		switch (func_global_function) {
			...
			case FUNC_SELECT:
				rexx_arg_value[0]=1;
				for (a=0;a<rexx_argcount;a++) {
					if ((LStrCmpI(rexx_args[a],"onlyfiles"))==0) rexx_arg_value[0]=2;
					else if ((LStrCmpI(rexx_args[a],"onlydirs"))==0) rexx_arg_value[0]=3;
					else if ((LStrCmpI(rexx_args[a],"name"))==0) rexx_arg_value[1]=0;
					else if ((LStrCmpI(rexx_args[a],"date"))==0) rexx_arg_value[1]=1;
					else if ((LStrCmpI(rexx_args[a],"protection"))==0) rexx_arg_value[1]=2;
				}
				return(0);
			case FUNC_SEARCH:
				if (rexx_argcount>2) {
					...
					for (a=2;a<rexx_argcount;a++) {
						if ((LStrCmpI(rexx_args[a],"ucnlc"))==0) search_flags&=~SEARCH_NOCASE;
						else if ((LStrCmpI(rexx_args[a],"wild"))==0) search_flags|=SEARCH_WILDCARD;
						else if ((LStrCmpI(rexx_args[a],"byword"))==0) search_flags|=SEARCH_ONLYWORDS;
					}
				}
				return(0);
		}
	...
	switch (function) {
		...
		case FUNC_STATUS:
			...
			d=0; buf1=buf3=NULL;
			if ((LStrCmpI(rexx_args[1],"set"))==0) {
				f=1; c=0;
				val=atoi(rexx_args[2]); buf3=rexx_args[2];
			}
			else if ((LStrCmpI(rexx_args[2],"set"))==0) {
				c=atoi(rexx_args[1]);
				f=1;
			...
			switch (statval) {
				...
				case RXSTATUS_OKAY_STRING:
					if (f) strcpy(str_okaystring,buf3);
		...
		case FUNC_FILEINFO:
			...
			if ((entry=findfile(dopus_curwin[win],rexx_args[0],NULL))) {
				...
				lsprintf(buf,"%s%lc%ld%lc%ld%lc%ld%lc%ld%lc%ld%lc%ld%lc%s%lc%s%lc",
					entry->name,pad[0],entry->size,pad[0],entry->subtype,pad[0],
		...
		case FUNC_BUSY:
			...
			if ((LStrnCmpI(rexx_args[0],"on",2))==0) busy();
			else if ((LStrnCmpI(rexx_args[0],"off",3))==0) unbusy();
			break;
...
/* AddCustEntry "text" <userdata> <fgpen> <bkpen> <selectable> <show> <before> */
		...
		case FUNC_QUERY:
			...
			for (val=0;;val++) {
				...
				if (LStrCmpI(modifynames[val],rexx_args[0])==0) break;
			}
			b=atoi(rexx_args[1]);
			...
			if (function==FUNC_QUERY) {
				...
				switch (val) {
					...
					case MOD_DISPLAYLENGTH:
						...
						if (b<0 || b>1) b=data_active_window;
						for (a=0;a<5;a++) {
							lsprintf(buf2,"%ld ",config->displaylength[b][a]);
							strcat(buf,buf2);
						}
						retbuf=buf;
						break;
			...
			switch (val) {
				...
				case MOD_DISPLAYLENGTH:
					if (b<0 || b>1) b=data_active_window;
					for (a=0;a<8;a++) {
						if (rexx_argcount<=a+2 || maxlengths[a]==-1) break;
						config->displaylength[b][a]=atoi(rexx_args[a+2]);
						if (config->displaylength[b][a]<5) config->displaylength[b][a]=5;
						else if (config->displaylength[b][a]>maxlengths[a])
							config->displaylength[b][a]=maxlengths[a];
					}
					break;
...
char *buf;
{
	...
	len=strlen(buf);
	for (a=len-1;a>=0;a--) if (!(isspace(buf[a]))) break;
	len=a;
	...
	if (argno<16 && argpos<256) rexx_args[argno][argpos]=0;
...
int num,rem;
{
	...
	for (a=0;;a++) {
		...
		if (LStrCmpI(rexx_args[num],keywords[a])==0) {
			rexx_arg_value[a]=1;
