...
int rexx;
{
	...
	char dirname[32],new_directory[256];
	
	/* AROS: We have no __aligned keyword so we use AllocDosObject 
	...
	*/
	...
	if (config->iconflags&ICONFLAG_MAKEDIRICON) len=25;
	else len=30;
	...
	FOREVER {
		if (rexx) {
			...
			if (!rexx_args[0][a]) {
				if (!(isvalidwindow(data_active_window))) goto exit;
			...
			if (!(status_flags&STATUS_ISINBUTTONS) ||
				(!(getdummypath(new_directory,STR_ENTER_DIRECTORY_NAME)))) goto exit;
			win=-1;
		}
		else {
			if (!(isvalidwindow(data_active_window))) goto exit;
...
void blackout(screen,backupmap)
...
char *name;
{
	...
	if (LStrCmpI(&name[a-5],".info")==0) return(&name[a-5]);
	return(NULL);
}
...
int rexx,win,num,argnum;
{
	...
	/* AROS: We have no __aligned keyword so we must use AllocDosObject 
	...
	*/
	...
	fblock = AllocDosObject(DOS_FIB, NULL);
	if (NULL == fblock)
		return NULL;

	if (!rexx || !rexx_argcount) { ret = str_pathbuffer[win]; goto exit; }
...
int type;
{
#warning AROS has no audio support yet
#if 0
...
int dir;
{
	...
	for (a=0;a<2;a++) {
		...
		DoArrow(main_rp,
			...
			gad->Width-4,
			gad->Height-2,
			screen_pens[(int)config->arrowfg].pen,
			screen_pens[(int)config->arrowbg].pen,dir);
		gad=gad->NextGadget;
	}
}
...
int num;
{
	...
	for (a=0;a<num;a++) {
		...
		SetAPen(main_rp,screen_pens[(int)config->littlegadbg].pen);
		rectfill(main_rp,gad->LeftEdge+2,gad->TopEdge+1,gad->Width-4,gad->Height-2);
		SetAPen(main_rp,screen_pens[(int)config->littlegadfg].pen);
		SetBPen(main_rp,screen_pens[(int)config->littlegadbg].pen);
