...
int win;
{
	...
	while (dir) {
		if (dir->selected) {
			CopyMem(dir->name,&winpar->reselection_list[a],32);
			a+=32;
		}
...
int moveold;
{
	...
	if (winpar->reselection_list &&
		...
		while (dir) {
			...
			if (top==-1 &&
				winpar->top_name[0] &&
				(LStrCmpI(dir->name,winpar->top_name))==0) top=num;
			++num;
			dir=dir->next;
		}
		for (a=0;a<winpar->reselection_size;a+=32) {
			if (winpar->reselection_list[a]) {
				dir=dopus_curwin[winpar->reselection_win]->firstentry;
				while (dir) {
					if (LStrCmpI(&winpar->reselection_list[a],dir->name)==0) {
						dir->selected=1;
						updateselectinfo(dir,winpar->reselection_win,0);
						break;
					}
...
struct Window *wind;
{
	...
	SetAPen(wind->RPort,screen_pens[(int)config->requestbg].pen);
	RectFill(wind->RPort,
		wind->BorderLeft+2,
		wind->BorderTop+1,
...
struct Window *window;
{
	...
	if (window) {
		...
		if (win) {
			request->hi=screen_pens[(int)config->gadgettopcol].pen;
			request->lo=screen_pens[(int)config->gadgetbotcol].pen;
			request->fg=screen_pens[(int)config->requestfg].pen;
			request->bg=screen_pens[(int)config->requestbg].pen;
		}
		else {
			request->hi=-1;
			request->lo=-1;
...
int ftype,funconly;
{
	...
	*/
	...
	if (!(lockandexamine(fullname,info))) { ret = NULL; goto exit; }
	if (!(file=Open(fullname,MODE_OLDFILE))) { ret = NULL; goto exit; }
	...
	while (type) {
		...
		if (ftype==-2) {
			...
			if (!funconly || (type->function[ftype] && type->function[ftype][0])) {
				if (type->recognition && (dochecktype(type,fullname,(IPTR)file,info)) &&
...
struct FileInfoBlock *info;
{
	...
	for (a=0;a<len;a++) {
		if (!operation) {
			...
			switch (operation) {
				case FTYC_MATCH:
					if (!(checktypechars((IPTR)file,buf))) fail=1;
					break;
				case FTYC_MATCHNAME:
...
int checktypechars(fileparam,match)
int fileparam;
unsigned char *match;
{
	unsigned char matchbuf[258];
	...
	switch (match[0]) {
		...
		default:
			for (a=first;a<len;a++,m++) {
				if (match[a]!='?' && match[a]!=matchbuf[m]) return(0);
			}
			break;
	}
	return(1);
	...
	if (buffer) return(searchbuffer(buffer,bufsize,matchbuf,matchsize,flags));
		...
		FOREVER {
			...
			if ((searchbuffer(findbuf,size,matchbuf,matchsize,flags))==1) {
				oldpos+=(((IPTR)search_found_position) - ((IPTR)findbuf));
