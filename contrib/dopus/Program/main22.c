...
int act,inact,rexx;
{
	int askeach=0,a,b,special=0,candoicon=1,old,specflags,noshow=0,err;
	int sourcewild=0,destwild=0,firstset=0,breakout,rexarg,protstuff[2];
	int pt=1,okayflag,show,lastfile,flag,exist,byte,count,data,mask=0,temp;
	int globflag,noremove,doicons=0,total,value=0,progtype,blocksize,retval=0;
	ULONG class;
	USHORT code;
	struct Directory *file,*tempfile,*nextfile,filebuf,dummyfile;
...
#warning We do not have inovamusc library
	...
	if (!file) { ret = 0; goto exit; }            /* No files selected, return */

	if (!(database=LAllocRemember(&funckey,3000,MEMF_CLEAR))) { ret = 0; goto exit; }
...
kprintf("NO FUNC_BYTE AT LINE 366 BECAUSE OF MISSING Info()\n");
			...
			if ((filelock=Lock(destdir,ACCESS_READ))) {
	...
	if (config->dynamicflags&UPDATE_PROGRESSINDICATOR &&
		...
		if (title) StrCombine(titlebuf,title,"...",32);
	...
	while (file) {
		...
		if (prog_indicator) {
			if (progtype==1 && file->type>=ENTRY_DEVICE) ++value;
			else if (progtype==0 && file->type<=ENTRY_FILE) ++value;
			else if (progtype==2) ++value;
			...
			case FUNC_BYTE:
				...
				if (doicons && !(isicon(file->name))) {
					...
					if ((file=findfile(swindow,oldiconname,NULL))) {
						if (!file->selected) b=file->size;
					}
					else {
						StrCombine(tbuf,sourcedir,oldiconname,256);
						if (lockandexamine(tbuf,fileinfo)) b=fileinfo->fib_Size;
					}
					if (b>-1) {
						a=(b+(blocksize-1))/blocksize;
						data+=a+(a/72)+1;
					}
					oldiconname[0]=0;
				}
...
kprintf("file->type <= ENTRY_FILE\n");
					if (checkexec(sourcename)) {
kprintf("REAL EXECUTABLE FILE\n");
...
*/
				...
				if (config->deleteflags&DELETE_FILES && askeach && !lastfile) {
					lsprintf(buf2,globstring[STR_WISH_TO_DELETE],file->name);
					if (!(a=simplerequest(buf2,globstring[STR_DELETE],globstring[STR_LEAVE],
						globstring[STR_ALL],globstring[STR_ABORT],NULL))) {
						okayflag=1;
						break;
					}
					if (a==2) askeach=0;
			...
			case FUNC_RENAME:
				if (firstset || lastfile) a=1;
					...
					if (!(a=whatsit(globstring[STR_ENTER_NEW_NAME],30,namebuf,
						globstring[STR_SKIP]))) {
						myabort();
						break;
					}
				...
				if (a) {
					if (lastfile) StrCombine(destname,sourcedir,newiconname,256);
						...
						if (doicons && oldiconname[0]) namebuf[25]=0;
						else namebuf[30]=0;
					...
					if (!(Rename(sourcename,destname))) {
						if ((a=IoErr())==ERROR_OBJECT_EXISTS) {
							if (askeach) {
								if (!lastfile) {
									if (!(a=checkexistreplace(sourcename,destname,&file->date,destwild,1)))
										break;
									if (a==2) {
										if (!destwild) goto functionloop;
										askeach=0;
									}
									else if (a==3) {
										myabort();
			...
			case FUNC_MOVEAS:
				if (!lastfile && !flag) {
					if (rexx && rexx_argcount>1) strcpy(namebuf,rexx_args[1]);
						...
						if (!(a=whatsit(globstring[STR_ENTER_NEW_NAME_MOVE],30,
			...
			case FUNC_MOVE:
				...
				if ((exist=CheckExist(destname,NULL))) {
					if (askeach) {
						if ((a=checkexistreplace(sourcename,destname,&file->date,(function==FUNC_MOVE),1))==3) {
							myabort();
							break;
						}
						if (!a) break;
						if (a==2) {
							if (function==FUNC_MOVEAS) goto functionloop;
							askeach=0;
						}
					}
			...
			case FUNC_COPYAS:  /* If flag is set then won't ask for new name */
				if (!lastfile && !flag) {
					if (rexx && rexx_argcount>1) strcpy(namebuf,rexx_args[1]);
						...
						if (LStrCmpI(sourcedir,destdir)==0 && LStrCmpI(namebuf,file->name)==0) {
			...
			case FUNC_COPY:
				...
				if ((exist=CheckExist(destname,NULL)) && askeach) {
					if ((a=checkexistreplace(sourcename,destname,&file->date,
						(function==FUNC_COPY),1))==3) {
						myabort();
						break;
					}
					if (!a) break;
					if (a==2) {
						if (function==FUNC_COPYAS) goto functionloop;
						askeach=0;
					}
				}
				if (exist<0) {
			...
			case FUNC_ICONINFO:
				if (LStrCmpI(file->name,".info")==0) okayflag=1;
				else {
...
#warning We do not have inovamusic library
			...
			case FUNC_ADDICON:
				if (strlen(file->name)>25 || isicon(file->name)) {
					okayflag=1;
					file=NULL;
					break;
				}
				StrCombine(buf2,file->name,".info",31);
				...
				if (CheckExist(destname,NULL) && askeach) {
					doerror(ERROR_OBJECT_EXISTS);
					if (!(a=checkexistreplace(destname,destname,NULL,1,1))) {
						file=NULL;
						break;
					}
			...
			if (!status_justabort) {
				...
				if (!(filelock=Lock(destdir,ACCESS_READ))) value=0;
					...
					if (ramdisk_lock && CompareLock(filelock,ramdisk_lock)!=LOCK_DIFFERENT)
						value=AvailMem(0);
					else value=(infodata.id_NumBlocks-infodata.id_NumBlocksUsed)*blocksize;
					UnLock(filelock);
				}
				if (config->errorflags&ERROR_ENABLE_DOS) main_proc->pr_WindowPtr=(APTR)Window;
				if (specflags&FUNCFLAGS_BYTEISCHECKFIT) {
					...
					getfloatstr(percent,tbuf);
					lsprintf(buf,globstring[STR_CHECKFIT_STRING],data*blocksize,value,tbuf);
					dostatustext(buf);
				}
				else {
					doselinfo(act);
