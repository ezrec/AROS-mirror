...
void doconfig()
{
	...
	lsprintf(replyname,"dopus4_config_reply%ld",system_dopus_runcount);
...
struct Screen *scr;
{
	if (scr && (!Window || Window->WScreen!=scr)) {
		...
		vis->vi_fg=screen_pens[(int)config->requestfg].pen;
		vis->vi_bg=screen_pens[(int)config->requestbg].pen;
		vis->vi_shine=screen_pens[(int)config->gadgettopcol].pen;
		vis->vi_shadow=screen_pens[(int)config->gadgetbotcol].pen;
		vis->vi_stringcol[0]=screen_pens[(int)config->stringfgcol].pen;
		vis->vi_stringcol[1]=screen_pens[(int)config->stringbgcol].pen;
		vis->vi_activestringcol[0]=screen_pens[(int)config->stringselfgcol].pen;
		vis->vi_activestringcol[1]=screen_pens[(int)config->stringselbgcol].pen;
	}
