...
void __regargs savepens(struct DOpusListView *view)
{
...
void __regargs restorepens(struct DOpusListView *view)
{
...
void __regargs DisplayView(struct DOpusListView *view)
{
  struct RastPort *rp;
...
int __regargs scroll_view(struct DOpusListView *view,int offset,int *histate,int oldoffset)
{
  int draw=0;
  ...
  if (draw) {
    ...
    DoFixSliderPot(view->window,&view->listgads[0],view->topitem,view->count,view->lines,1);
    DisplayView(view);
  }
  return(offset);
}

int __regargs view_valid(struct DOpusListView *view,int itemnum)
{
...
int __saveds DoAddListView(register struct DOpusListView *view __asm("a0"), register int count __asm("d0"))
{
  struct RastPort *rp;
  int a,addcount=0;
  ...
  while (view && (addcount<count)) {
    ...
    if (!(view->listgads=(struct Gadget *)DoAllocRemember(&view->key,sizeof(struct Gadget)*3,MEMF_CLEAR)) ||
      !(view->listimage=(struct Image *)DoAllocRemember(&view->key,sizeof(struct Image),MEMF_CLEAR)) ||
      !(view->listprop=(struct PropInfo *)DoAllocRemember(&view->key,sizeof(struct PropInfo),MEMF_CLEAR)) ||
      !(view->selected=(char *)DoAllocRemember(&view->key,view->count,MEMF_CLEAR))) {
      DoFreeRemember(&view->key);
      break;
    }
    ...
    DoDoArrow(rp,view->x+view->w+4,view->y+view->h-14,view->sliderwidth+4,6,view->arrowfg,view->arrowbg,0);
    Do3DBox(rp,view->x+view->w+4,view->y+view->h-14,view->sliderwidth+4,6,view->boxhi,view->boxlo);
    DoDoArrow(rp,view->x+view->w+4,view->y+view->h-6,view->sliderwidth+4,6,view->arrowfg,view->arrowbg,1);
    ...
    if (view->flags&DLVF_HIREC)
      DoAddGadgetBorders(&view->key,&view->listgads[1],2,view->boxhi,view->boxlo);
    ...
    view->oldoffset=-1;
    restorepens(view);
    DoFixSliderBody(view->window,&view->listgads[0],view->count,view->lines,0);
    DoFixSliderPot(view->window,&view->listgads[0],view->topitem,view->count,view->lines,2);
    DisplayView(view);
    view=view->next;
    ++addcount;
  }
...
void __saveds DoFixSliderBody(register struct Window *win __asm("a0"), register struct Gadget *gad __asm("a1"), register int count __asm("d0"), register int lines __asm("d1"), register int show __asm("d2"))
{
  UWORD body,gh,ih,min;
  struct Image *image;
  ...
  if (pinfo->Flags&AUTOKNOB) {
    ...
    if (show && win && (oh!=ih || show==2)) DoShowSlider(win,gad);
  }
}

void __saveds DoFixSliderPot(register struct Window *win __asm("a0"), register struct Gadget *gad __asm("a1"), register int off __asm("d0"), register int count __asm("d1"), register int lines __asm("d2"), register int show __asm("d3"))
{
  UWORD vert,vh,gh,ih,te,oh;
  float div;
  struct Image *image;
  struct PropInfo *pinfo;
...
//kprintf("FixSliderPot: off=%ld\tcount=%ld\tlines=%ld\n",off,count,lines);
  if (count<=lines) vert=0;
  else vert=((float)off*0xffff)/(count-lines);
  if (vert==0) te=0;
  else {
  ...
  if (pinfo->Flags&AUTOKNOB) {
    ...
    if (show && win && (oh!=te || show==2)) DoShowSlider(win,gad);
  }
}

void __saveds DoShowSlider(register struct Window *win __asm("a0"), register struct Gadget *gad __asm("a1"))
{
...
int __saveds DoGetSliderPos(register struct Gadget *gad __asm("a0"), register int count __asm("d0"), register int lines __asm("d1"))
{
  int i;
  UWORD vertpot;
  struct PropInfo *pinfo;
...
struct DOpusListView * __saveds DoListViewIDCMP( register struct DOpusListView *view __asm("a0"), register struct IntuiMessage *imsg __asm("a1"))
{
  ...
  if (class==GADGETUP || class==GADGETDOWN) {
    ...
    switch (gadgetid) {
      case GAD_SLIDER:
        view->topitem=DoGetSliderPos(&view->listgads[0],view->count,view->lines);
        DisplayView(view);
        if (view->sliderbgcol) DoShowSlider(view->window,&view->listgads[0]);
...
int __saveds DoRefreshListView(register struct DOpusListView *view __asm("a0"), register int count __asm("d0"))
{
  int realcount=0;
  ...
  while (view && realcount<count) {
    ...
    if (view->selectarray) {
      if (view->selected=(char *)DoAllocRemember(&view->key,view->count,MEMF_CLEAR))
...
int __saveds DoRemoveListView(register struct DOpusListView *view __asm("a0"), register int count __asm("d0"))
{
  int realcount=0;
