...
*****************************************************************************/
{
	...
	reqbase->rb_privatedata=
		LAllocRemember(&reqbase->rb_memory,sizeof(struct PrivateData),MEMF_CLEAR);
...
*****************************************************************************/
{
	...
	/* Object type */
			...
			case RO_Type:
				if (object) break;
				if (!(object=LAllocRemember(&reqbase->rb_memory,
				...
				if (!(object->ro_data=LAllocRemember(&reqbase->rb_memory,
				...
				switch (data) {
					...
					case GADGET_STRING:
						...
						if (!(stringinfo=(struct StringInfo *)
							LAllocRemember(&reqbase->rb_memory,sizeof(struct StringInfo),MEMF_CLEAR)))
							return(NULL);
					...
					case GADGET_PROP:
						...
						if (!(propinfo=(struct PropInfo *)
							LAllocRemember(&reqbase->rb_memory,sizeof(struct PropInfo),MEMF_CLEAR)))
							return(NULL);
						gadget->SpecialInfo=(APTR)propinfo;
						if (!(propimage=(struct Image *)
							LAllocRemember(&reqbase->rb_memory,sizeof(struct Image),MEMF_CLEAR)))
							return(NULL);
						...
						if (private) {
							if (!private->check_off_image)
								private->check_off_image=
									GetCheckImage(reqbase->rb_shadow,reqbase->rb_bg,0,&reqbase->rb_memory);
							if (!private->check_on_image)
								private->check_on_image=
									GetCheckImage(reqbase->rb_shadow,reqbase->rb_bg,1,&reqbase->rb_memory);
						...
						if (private) {
							if (!private->button_off_image)
								private->button_off_image=
									GetButtonImage(15,9,reqbase->rb_shine,reqbase->rb_shadow,
	...
	/* Object text */
			...
			case RO_Text:
				...
				if (string &&
					(*string=LAllocRemember(&reqbase->rb_memory,strlen((char *)data)+1,0)))
			...
			case RO_HighRecess:
				if (data && object) {
					if (object->ro_type==OBJECT_GADGET &&
						...
						AddGadgetBorders(&reqbase->rb_memory,
							gadget,1,
							reqbase->rb_shine,reqbase->rb_shadow);
					}
	...
	if (stringinfo && !stringinfo->Buffer && stringinfo->MaxChars>0) {
		if (!(stringinfo->Buffer=
			LAllocRemember(&reqbase->rb_memory,stringinfo->MaxChars+1,MEMF_CLEAR)))
			return(NULL);
	}
		...
		switch (object->ro_type) {
			...
			case OBJECT_GADGET:
				...
				if (((Object_Gadget *)object->ro_data)->og_type==GADGET_TINYGLASS) {
					DoGlassImage(reqbase->rb_window->RPort,
...
*****************************************************************************/
{
	...
	UScoreText(rp,textbuf,x,y+rp->Font->tf_Baseline,got_uscore);
