...
extern int DoMatchPattern(unsigned char *,unsigned char *,int);
extern void DoParsePattern(unsigned char *,unsigned char *,int);
...
int  DoMatchPattern(pat,str,cas)
unsigned char *pat,*str;
int cas;
{
	if (DOSBase->dl_lib.lib_Version>35) {
		...
		if (cas) suc=(int)MatchPatternNoCase(pat,str);
		else suc=(int)MatchPattern(pat,str);
...
void DoParsePattern(pat,patbuf,cas)
unsigned char *pat,*patbuf;
int cas;
{
	if (DOSBase->dl_lib.lib_Version>35) {
		int len,old;
