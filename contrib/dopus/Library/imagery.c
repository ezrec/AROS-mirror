...
//D(bug("Do3DCycleBox(%lx,%lx)\n",pdb_cycletop,pdb_cyclebot));
    ...
    if (/*DOpusBase->*/pdb_cycletop)
     {
      BltTemplate((PLANEPTR)/*DOpusBase->*/pdb_cycletop,0,2,rp,x+4,y+1,11,6);
     }
    if (/*DOpusBase->*/pdb_cyclebot)
     {
      BltTemplate((PLANEPTR)/*DOpusBase->*/pdb_cyclebot,0,2,rp,x+4,y+1+h-12+7,11,2);
     }
...
D(bug("DrawCheckMark(%lx)\n",pdb_check));
    ...
    if (/*DOpusBase->*/pdb_check)
     {
      BltTemplate((PLANEPTR)/*DOpusBase->*/pdb_check,0,2,rp,x,y,13,7);
     }
    if (clear==0)
     {
      SetAPen(rp,old_apen);
...
D(bug("DoGetCheckImage: fg = %ld, bg = %ld\n",fg,bg));
    ...
    if (pen) {
        SetAPen(&trp,fg);
        BltTemplate((PLANEPTR)/*DOpusBase->*/pdb_check,0,2,&trp,0,0,13,7);
    }
