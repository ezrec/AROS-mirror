			...
			switch (class) {
				...
				case IDCMP_RAWKEY:
					...
					if (editdrivegadgets[1].Flags&SELECTED) {
						...
						seldrive->key=code; seldrive->qual=qual;
					...
					if (code==SELECTDOWN) {
						...
						if (fixrmbimage(&sampleclearrmb,NULL,0,x,y)) {
							...
							if (a==0) {
								seldrive->key=0xff;
								seldrive->qual=0;
								showkey(0,0,x_off+150,y_off+129,331,10);
							}
							editdrivegadgets[1].Flags&=~SELECTED;
							help_ok=1;
						}
					...
					switch (gadgetid) {
						...
						case DRIVE_EDITREQ:
							...
							if (funcrequester(FREQ_PATHREQ,buf,NULL)) {
								LStrnCpy(edit_pathbuf,buf,32); edit_pathbuf[32]=0;
...
struct Gadget *gadget;
{
	...
	x=gadget->LeftEdge;
	y=gadget->TopEdge;
	if (drive) SetAPen(rp,screen_pens[(int)drive->bpen].pen);
	else SetAPen(rp,screen_pens[0].pen);
	RectFill(rp,x+2,y+1,x+77,y+8);
	do3dbox(x+2,y+1,76,8);
	if (drive) {
		SetAPen(rp,screen_pens[(int)drive->fpen].pen);
		SetBPen(rp,screen_pens[(int)drive->bpen].pen);
