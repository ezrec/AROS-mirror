							...
							case 1:
								switch (ITEMNUM(code)) {
									...
									case 7:
										...
										if (!(FileRequest(&filereq)) || !filebuf[0]) break;
...
int savefileclasses()
{
	...
	if (!classname[0]) strcpy(classname,"s:DirectoryOpus.CLA");
	...
	while (class) {
		if (class->type[0]) {
			if ((Write(out,class->type,(a=strlen(class->type))))<a) break;
			if (class->typeid[0]) {
				Write(out,&g,1);
				if ((Write(out,class->typeid,(a=strlen(class->typeid))))<a) break;
			}
		}
		Write(out,&f,1);
		if (class->recognition) {
			if ((Write(out,class->recognition,(a=(strlen(class->recognition)+1))))<a)
				break;
		}
		else Write(out,&f,1);
		class=class->next;
...
{
	struct fileclass *class,*newclass,*last;

	class=firstclass;
	while (class) {
		if (LStrCmpI(class->type,type)>=0) break;
		if (!class->next) last=class;
		class=class->next;
	}
	if (!(newclass=AllocMem(sizeof(struct fileclass),MEMF_CLEAR))) return(0);
	if (class) {
		if (class->last) {
			class->last->next=newclass;
			newclass->last=class->last;
		}
		else if (class==firstclass) firstclass=newclass;
		newclass->next=class;
		class->last=newclass;
	}
	else if (firstclass) {
...
{
	struct fileclass *class,*newclass;

	class=firstclass;
	while (class) {
		newclass=class->next;
		freestring(class->recognition);
		FreeMem(class,sizeof(struct fileclass));
		class=newclass;
	}
	firstclass=NULL;
}

void removefileclass(class)
struct fileclass *class;
{
	if (class->last) class->last->next=class->next;
	if (class->next) class->next->last=class->last;
	if (class==firstclass) firstclass=class->next;
	freestring(class->recognition);
	FreeMem(class,sizeof(struct fileclass));
}
...
struct DOpusRemember **key;
{
	...
	class=firstclass;
	for (count=0;class;count++,class=class->next);
	if (!(list=LAllocRemember(key,(count+1)*4,MEMF_CLEAR))) return(NULL);
	class=firstclass;
	for (a=0;a<count;a++) {
		if ((list[a]=LAllocRemember(key,40,MEMF_CLEAR))) strcpy(list[a],class->type);
		class=class->next;
	}
	return(list);
}
