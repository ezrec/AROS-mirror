...
struct Gadget *gadget;
{
	...
	x=gadget->LeftEdge;
	y=gadget->TopEdge;
	if (gad) SetAPen(rp,screen_pens[(int)gad->bpen].pen);
	else SetAPen(rp,screen_pens[0].pen);
	RectFill(rp,x+2,y+1,x+77,y+8);
	if (gad) {
		SetAPen(rp,screen_pens[(int)gad->fpen].pen);
		SetBPen(rp,screen_pens[(int)gad->bpen].pen);
					...
					switch (MENUNUM(code)) {
						case 0:
							...
							switch ((a=ITEMNUM(code))) {
								case 0:
								case 1:
									if (doload(CFG_MENU,a)) {
										showmenus(5);
										if (a) dogadgetinfo(cfg_string[STR_NEW_MENUS_LOADED]);
										else dogadgetinfo(cfg_string[STR_DEFAULT_MENUS_LOADED]);
									}
									break;
							}
							break;
...
int disp;
{
	...
	for (x=0;x<5;x++) {
		...
		if (disp&4) {
			showmenutext(config->menutit[x],(x*120)+x_off+12,y_off+8,0,1);
