...
void doradiobuttons()
{
	...
	stringex.Pens[0]=screen_pens[(int)config->stringfgcol].pen;
	stringex.Pens[1]=screen_pens[(int)config->stringbgcol].pen;
	stringex.ActivePens[0]=screen_pens[(int)config->stringselfgcol].pen;
	stringex.ActivePens[1]=screen_pens[(int)config->stringselbgcol].pen;
...
void fix_gadget_positions()
{
	...
	for (a=0;a<10;a++) fix_gad_pos(&formatgads[a],formatgads_xy[a]);
