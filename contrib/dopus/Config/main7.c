...
trysave:
		...
		if (!(strstri(configname,".CFG"))) StrConcat(configname,".CFG",256);
	}
	fixcstuff(&cstuff);
		...
		switch (type) {
			...
			case CFG_OPERATION:
				...
				for (a=0;a<2;a++) {
					...
					CopyMem((char *)newcstuff->config->displaylength[a],(char *)config->displaylength[a],16);
				}
				freecon=3;
				break;
			case CFG_GADGET:
			...
			case CFG_HOTKEYS:
				...
				break;
			case CFG_SCREEN:
				CopyMem((char *)&newcstuff->config->gadgettopcol,
		...
		while ((IMsg=(struct IntuiMessage *)GetMsg(wind->UserPort))) {
			if ((view=ListViewIDCMP(&listlist,IMsg))==(struct DOpusListView *)-1) {
				...
				if (class==IDCMP_VANILLAKEY) {
					a=-1;
					if (code=='\r' || code=='o' || code=='y') a=0;
					else if (code=='' || code=='c' || code=='n') a=1;
					if (a!=-1) {
