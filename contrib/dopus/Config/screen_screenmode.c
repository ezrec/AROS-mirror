...
struct ScreenMode *showdisplaydesc()
{
    ...
    lsprintf(buf,"%-16s: %ld %s %ld",cfg_string[STR_MINIMUM_SIZE],(long)mode->minw,cfg_string[STR_BY],(long)mode->minh);
    UScoreText(rp,buf,x_off+240,y_off+147,-1);
    lsprintf(buf,"%-16s: %ld %s %ld",cfg_string[STR_MAXIMUM_SIZE],(long)mode->maxw,cfg_string[STR_BY],(long)mode->maxh);
    UScoreText(rp,buf,x_off+240,y_off+155,-1);
    lsprintf(buf,"%-16s: %ld %s %ld",cfg_string[STR_DEFAULT_SIZE],(long)mode->defw,cfg_string[STR_BY],(long)mode->defh);
    UScoreText(rp,buf,x_off+240,y_off+163,-1);
    lsprintf(buf,"%-16s: %ld",cfg_string[STR_MAXIMUM_COLORS],(long)(1<<mode->maxdepth));
...
struct ScreenMode *mode;
{
    lsprintf(screenwidth_buf,"%ld",(long)config->scrw);
    lsprintf(screenheight_buf,"%ld",(long)config->scrh);
    if (config->scrdepth<2) config->scrdepth+=2;
    if (config->scrdepth>((mode)?mode->maxdepth:8))
        config->scrdepth=((mode)?mode->maxdepth:8);
    lsprintf(screendepth_buf,"%ld",(long)(1<<config->scrdepth));
