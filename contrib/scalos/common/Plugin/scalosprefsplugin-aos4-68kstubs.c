...
static ULONG stub_SCAGetPrefsInfoPPC(uint32 *regarray)
{
	struct Library *Base = (struct Library *) regarray[REG68K_A6/4];
	struct ExtendedLibrary *ExtLib = (struct ExtendedLibrary *) ((uint32)Base + Base->lib_PosSize);
	struct ScalosPrefsPluginIFace *Self = (struct ScalosPrefsPluginIFace *) ExtLib->MainIFace;

	return Self->SCAGetPrefsInfo(
		(LONG)regarray[0]
	);
}
STATIC CONST struct EmuTrap stub_SCAGetPrefsInfo = { TRAPINST, TRAPTYPE, (uint32 (*)(uint32 *))stub_SCAGetPrefsInfoPPC };
