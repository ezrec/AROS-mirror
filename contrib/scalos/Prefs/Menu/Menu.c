...
// limits for various menu entries
...
static struct Catalog *MenuCatalog;

static struct Hook AboutMUIHook = { { NULL, NULL }, HOOKFUNC_DEF(OpenAboutMUIFunc), NULL };

static struct MUI_CustomClass *MenuPluginClass;
...
static APTR MenuOpen, MenuSaveAs, MenuAbout, MenuAboutMUI, MenuQuit;
static APTR MenuResetToDefaults, MenuLastSaved, MenuRestore;
static APTR MenuMerge, MenuImport, MenuImportTD, MenuImportP;
static APTR MenuCreateIcons;
static APTR MenuCollapseAll, MenuExpandAll;
...
int main(int argc, char *argv[])
{
	...
	struct DiskObject *MenuDiskObject = NULL;
	...
	d1(kprintf(__FUNC__ "/%ld:  MenuPluginClass=%08lx\n", __LINE__, MenuPluginClass));

	if (NULL == MenuPluginClass)
		fail(APP_Main, "Failed to open Menu Plugin.");

	MenuDiskObject = GetDiskObject(ProgramName);

	Group_Plugin = NewObject(MenuPluginClass->mcc_Class, 0,
		MUIA_ScalosPrefs_CreateIcons, fCreateIcons,
		MUIA_ScalosPrefs_ProgramName, (ULONG) ProgramName,
		TAG_END);

	d1(kprintf(__FUNC__ "/%ld:  Group_Plugin=%08lx\n", __LINE__, Group_Plugin));
	...
	APP_Main = ApplicationObject,
		...
		MUIA_Application_Version,	"$VER: Scalos Menu V40.20 (" __DATE__ ")" COMPILER_STRING,
		MUIA_Application_Copyright,	"The Scalos Team, 2000" CURRENTYEAR,
		MUIA_Application_Author,	"The Scalos Team",
		MUIA_Application_Description,	"Scalos Menu preferences editor",
		MUIA_Application_Base,		"SCALOS_MENU",
		MenuDiskObject ? MUIA_Application_DiskObject : TAG_IGNORE,	MenuDiskObject,
		...
		MUIA_Application_Menustrip, MenustripObject,
			Child, MenuObjectT(GetLocString(MSGID_MENU_PROJECT)),
				...
				End,
				Child, MenuMerge = MenuitemObject,
					MUIA_Menuitem_Title, GetLocString(MSGID_MENU_PROJECT_MERGE),
					MUIA_Menuitem_Shortcut, GetLocString(MSGID_MENU_PROJECT_MERGE_SHORT),
				End,
				...
				End,
				Child, MenuImport = MenuitemObject,
					MUIA_Menuitem_Title, GetLocString(MSGID_MENU_PROJECT_IMPORT),
					Child, MenuImportTD = MenuitemObject,
						MUIA_Menuitem_Title, GetLocString(MSGID_MENU_PROJECT_IMPORT_TD),
						MUIA_Menuitem_Shortcut, GetLocString(MSGID_MENU_PROJECT_IMPORT_TD_SHORT),
					End,
					Child, MenuImportP = MenuitemObject,
						MUIA_Menuitem_Title, GetLocString(MSGID_MENU_PROJECT_IMPORT_P),
						MUIA_Menuitem_Shortcut, GetLocString(MSGID_MENU_PROJECT_IMPORT_P_SHORT),
					End,
				End,
				Child, MenuitemObject,
					MUIA_Menuitem_Title, -1,
			...
			Child, MenuObjectT(GetLocString(MSGID_MENU_EDIT)),
				...
				Child, MenuitemObject,
					MUIA_Menuitem_Title, -1,
				End,
				Child, MenuCollapseAll = MenuitemObject,
					MUIA_Menuitem_Title, GetLocString(MSGID_MENU_EDIT_COLLAPSEALL),
				End,
				Child, MenuExpandAll = MenuitemObject,
					MUIA_Menuitem_Title, GetLocString(MSGID_MENU_EDIT_EXPANDALL),
				End,
			End, //MenuObjectT
	...
	d1(kprintf(__FUNC__ "/%ld:  APP_Main=%08lx\n", __LINE__, APP_Main));
	...
	DoMethod(MenuImportTD, MUIM_Notify, MUIA_Menuitem_Trigger, MUIV_EveryTime,
		Group_Plugin, 1, MUIM_ScalosPrefs_MenuPrefs_ImportTD);

	DoMethod(MenuImportP, MUIM_Notify, MUIA_Menuitem_Trigger, MUIV_EveryTime,
		Group_Plugin, 1, MUIM_ScalosPrefs_MenuPrefs_ImportP);

	DoMethod(MenuMerge, MUIM_Notify, MUIA_Menuitem_Trigger, MUIV_EveryTime,
		Group_Plugin, 1, MUIM_ScalosPrefs_MenuPrefs_Merge);

	DoMethod(MenuCollapseAll, MUIM_Notify, MUIA_Menuitem_Trigger, MUIV_EveryTime,
		Group_Plugin, 1, MUIM_ScalosPrefs_MenuPrefs_CollapseAll);

	DoMethod(MenuExpandAll, MUIM_Notify, MUIA_Menuitem_Trigger, MUIV_EveryTime,
		Group_Plugin, 1, MUIM_ScalosPrefs_MenuPrefs_ExpandAll);
...
static void init(void)
{
	...
	if (LocaleBase)
		MenuCatalog = OpenCatalogA(NULL, "Scalos/ScalosMenu.catalog", NULL);

	MenuPluginClass = OpenPrefsModule("Menu.prefsplugin");
}
