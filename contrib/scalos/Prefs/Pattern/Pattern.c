...
static struct Catalog *PatternCatalog;

static struct Hook OpenAboutMUIHook = { { NULL, NULL }, HOOKFUNC_DEF(OpenAboutMUIFunc), NULL };

static struct MUI_CustomClass *PatternPluginClass;
...
int main(int argc, char *argv[])
{
	...
	struct DiskObject *PatternDiskObject = NULL;
	STRPTR ProgramName;
	STRPTR UsedClasses[32];
	BOOL fAutoPreview = FALSE;
	BOOL fPreview = TRUE;
	BOOL fThumbnails = FALSE;
	ULONG PreviewWeight = 20;
	...
	if (WBenchMsg && WBenchMsg->sm_ArgList)
		...
		if (icon)
			...
			tt = FindToolType(icon->do_ToolTypes, "THUMBNAILS");
			if (tt)
				fThumbnails = TRUE;

			tt = FindToolType(icon->do_ToolTypes, "AUTOPREVIEW");
			if (tt)
				fAutoPreview = TRUE;

			tt = FindToolType(icon->do_ToolTypes, "PREVIEWWEIGHT");
			if (tt)
				StrToLong(tt, (LONG *) &PreviewWeight);

			tt = FindToolType(icon->do_ToolTypes, "NOPREVIEW");
			if (tt)
				fPreview = FALSE;
		...
		rdArgs = ReadArgs("FROM,EDIT/S,USE/S,SAVE/S,NOPREVIEW/S,AUTOPREVIEW/S,PW=PREVIEWWEIGHT/N/K", ArgArray, NULL);
		...
		if (ArgArray[5])
			fAutoPreview = TRUE;
		if (ArgArray[6])
			PreviewWeight = *((ULONG *) ArgArray[6]);
		}

	if (NULL == PatternPluginClass)
		fail(APP_Main, "Failed to open Pattern Plugin.");

	PatternDiskObject = GetDiskObject(ProgramName);
	...
	Group_Plugin = NewObject(PatternPluginClass->mcc_Class, 0,
		...
		MUIA_ScalosPrefs_PatternPrefs_Thumbnails, fThumbnails,
		TAG_END);
	...
	APP_Main = ApplicationObject,
		...
		MUIA_Application_Version,	"$VER: Scalos Pattern 40.21 (" __DATE__ ")" COMPILER_STRING,
		MUIA_Application_Copyright,	"The Scalos Team, 2000" CURRENTYEAR,
		MUIA_Application_Author,	"The Scalos Team",
		MUIA_Application_Description,	"Scalos Pattern preferences editor",
		MUIA_Application_Base,		"SCALOS_PATTERN",
		PatternDiskObject ? MUIA_Application_DiskObject : TAG_IGNORE,	PatternDiskObject,
	...
	if (PatternDiskObject)
		FreeDiskObject(PatternDiskObject);
	if (oldDir)
		CurrentDir(oldDir);
	if (rdArgs)
		FreeArgs(rdArgs);
...
static void init(void)
{
	...
	if (LocaleBase)
		PatternCatalog = OpenCatalogA(NULL, "Scalos/ScalosPattern.catalog", NULL);

	PatternPluginClass = OpenPrefsModule("Pattern.prefsplugin");
}
