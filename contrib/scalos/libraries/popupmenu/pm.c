...
static APTR pm_OpenPopupMenuA(struct Window *prevwnd, struct TagItem *tags, struct PopupMenuBase *PopupMenuBase);
...
void PM_HandleShadow(struct PM_Window *a)
{
	...
	d1(KPrintF("%s/%s/%ld: Window LeftEdge=%ld  TopEdge=%ld\n", __FILE__, __FUNC__, __LINE__, a->Wnd->LeftEdge, a->Wnd->TopEdge));
	...
	for (worknode = (PMSR *)(delta->mlh_Head);
		nextnode = (PMSR *)PM_NextNode(worknode);
		worknode=nextnode)
		{
		PM_DrawShadow(a,    
			worknode->Left-a->Wnd->LeftEdge,
...
//
BOOL PM_MBHit(struct PM_Window *c, struct PM_InpMsg *msg, BOOL mdis, struct PopupMenuBase *PopupMenuBase)
{
    UBYTE   whattodo=0;
...
LIBFUNC_P1(APTR, LIBPM_OBSOLETEFilterIMsgA,
...
//
LIBFUNC_P5(APTR, LIBPM_FilterIMsgA,
	A0, struct Window *, w,
...
LIBFUNC_P3(APTR, LIBPM_OpenPopupMenuA,
	A1, struct Window *, prevwnd,
	A2, struct TagItem *, tags,
	A6, struct PopupMenuBase *, PopupMenuBase)
{
	return pm_OpenPopupMenuA(prevwnd, tags, PopupMenuBase);
}
LIBFUNC_END
...
LIBFUNC_P3(LONG, LIBPM_InsertMenuItemA,
        ...
        switch(tag->ti_Tag) {
            ...
            case PM_Insert_Item:
			    ...
			    pm=pointer->SubGroup.Sub;
                            pointer->SubGroup.Sub=(struct PopupMenu *)tag->ti_Data;
                            pointer->SubGroup.Sub->Next=pm;
                            break;
                        case PM_InsertSub_Sorted:
...
LIBFUNC_P3(APTR, LIBPM_RemoveMenuItem,
...
/// PM_AbortHook
LIBFUNC_P2(BOOL, LIBPM_AbortHook,
	A0, APTR, handle,
	A6, struct PopupMenuBase *, l)
{
...
LIBFUNC_P1(STRPTR, LIBPM_GetVersion,
	A6, struct PopupMenuBase *, l)
{
...
LIBFUNC_P4(LONG, LIBPM_LayoutMenuA,
	A0, struct Window *, w,
	A1, struct PopupMenu *, pm,
	A2, struct TagItem *, tags,
	A6, struct PopupMenuBase *, PopupMenuBase)
...
LIBFUNC_P1(LONG, LIBPM_RESERVED1,
	A6, struct PopupMenuBase *, PopupMenuBase)
{
	(void) PopupMenuBase;
	return 0;
