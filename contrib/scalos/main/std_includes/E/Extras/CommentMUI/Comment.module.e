...
MODULE  'workbench/startup','workbench/workbench','dos/datetime','dos/dos','utility/hooks','utility',
	...
	'dos/datetime','intuition/classusr','intuition/classes','tools/boopsi','*getpath','jmc/GeticonObject',
	'scalos/scalos40.34','scalos'
...
ENUM ER_MUILIB=1,ER_LIB,ER_ICONLIB,ER_APP,MN_DIR, OP_DIR, ID_REM,ID_COMALL,ID_COM, ID_ASL, ID_ASLD, ID_SAVEOBJS,ID_ABOUT
...
DEF	bufnom[256]:STRING,buftaille[20]:STRING,bufstamp[20]:STRING,bufheure[20]:STRING,
	...
	txt_path,win3,lastcom=0,always=0,gr_path,txt_abort,txt_Num, visible=0
...
PROC main() HANDLE 
 DEF np=0,ncible:PTR TO oListData,nl=0,menu,jmc[600]:STRING,bufsetting[3]:STRING,stack
...
->> *****************************
    ...
    SubWindow, win:= WindowObject,
      ...
      MUIA_Window_Title, 'Scalos Comment.module',
      MUIA_Window_AppWindow, MUI_TRUE,
      WindowContents, VGroup,
	Child,HGroup,
...
->
  ...
  domethod(gr_list, 	[MUIM_Notify, MUIA_ContextMenuTrigger,MUIV_EveryTime,app,3,MUIM_CallHook,contextmenu_hook,MUIV_TriggerValue])
  domethod(str_menu, 	[MUIM_Notify, MUIA_ContextMenuTrigger,MUIV_EveryTime,app,3,MUIM_CallHook,contextmenu_hook,MUIV_TriggerValue])

  doMethodA(strcomment,	[MUIM_Notify, MUIA_String_Contents, MUIV_EveryTime, strcomment, 3, MUIM_CallHook, stringMsgHook, MUIV_TriggerValue])

  domethod(vlist,	[MUIM_Notify,MUIA_NList_Active,MUIV_EveryTime,app,2,MUIM_Application_ReturnID,"disp"])
  domethod(bt_Arret,  	[MUIM_Notify,MUIA_Pressed,0,app,3,MUIM_WriteLong,1,{arret}])
      ...
      result:= doMethodA(app,[MUIM_Application_Input,{sigs}])
		...
		CASE MUIV_Application_ReturnID_Quit
			...
			running:=FALSE
                ...
                CASE ID_ABOUT
			StringF(jmc,'\eb\ecApplication for Scalos\en\n\euWritten in E\en\n\e3\s\e1\en\n\ei\s \en\nCompiled with: \s',VERSTRING, COPYRIGHT,COMPILER_STRING)
		...
		CASE ID_COMALL
			...
			IF numlist > 1
				...
				domethod(vlist,[MUIM_NList_Select,MUIV_NList_Select_All,MUIV_NList_Select_On,TRUE])
			...
			ELSEIF numlist = 1
				...
				domethod(vlist,[MUIM_NList_GetEntry,np,{ncible}])
				proc_Analyse(ncible)
			ENDIF

			doMethodA(aslobj,[MUIM_Set,MUIA_Disabled,FALSE])
			IF (etatendgui=1)
				domethod(vlist,[MUIM_NList_Clear])
				pref()
				running:=FALSE
			ENDIF
...
PROC removesel(obj,tag:PTR TO LONG)
	domethod(obj,[MUIM_NList_Remove,tag])
ENDPROC

PROC retidp(btobj,rid)
	domethod(btobj,[MUIM_Notify, MUIA_Pressed, FALSE, app, 2, MUIM_Application_ReturnID, rid])
ENDPROC
...
DEF	npc=0,ent:PTR TO oListData
	...
	IF (numlist=1)
		...
		domethod(obj,[MUIM_NList_GetEntry,0,{ent}])
		proc_Analyse(ent)
		init_Selected(0,ent)
		set(obj,MUIA_NList_Active,0)
	...
	ELSE
		...
		REPEAT
			refresh()
			domethod(obj,[MUIM_NList_NextSelected,{id}])
			domethod(obj,[MUIM_NList_GetEntry,id,{ent}])
			IF (id<>-1) AND (arret=0)
				npc++
				proc_Analyse(ent)
				init_Selected(id,ent)
				lastcom:=npc
				domethod(obj,[MUIM_NList_Select,id,MUIV_NList_Select_Off,NIL])
			ENDIF
...
DEF tempnom
		...
		ELSE
			...
			set(tir,MUIA_String_Contents,str)
		ENDIF
		domethod(vlist,	[MUIM_NList_Remove, num])
		getpathdir(str)
		refresh()
		intypes(str)
		refresh()
...
PROC intypes(ftypesdir)
  ...
  IF lock:=Lock(ftypesdir,-2)
	...
	domethod(vlist,[MUIM_NList_InsertSingle,new,MUIV_NList_Insert_Sorted])
...
DEF identry:PTR TO oListData,idnum=0,testbuf[500]:STRING,testnom
	...
	FOR idnum:= 0 TO numlist-1
		domethod(vlist,[MUIM_NList_GetEntry,idnum,{identry}])
...
PROC stringMsgFunc()
DEF	string,long=0,buf_Com[4]:STRING
	get(strcomment, MUIA_String_Contents,{string})
	IF string
		long:=StrLen(string)
		StringF(buf_Com,'\d',long)
		set(num_comment, MUIA_Text_Contents, buf_Com)
	ENDIF
ENDPROC
...
PROC hooklvdisplay(array:PTR TO LONG, entry:PTR TO oListData)
...
DEF fent:PTR TO oListData
	...
	domethod(vlist,[MUIM_NList_GetEntry,MUIV_NList_GetEntry_Active,{fent}])
...
PROC hooklvconstruct(pool, entry:PTR TO oListData)
...
PROC hooklvdestruct(pool, entry:PTR TO oListData)
...
PROC hooklvcompare(o1:PTR TO oListData, o2:PTR TO oListData)
DEF mode
...
PROC hooklvcompare2(msg:PTR TO nlist_comparemessage)
...
PROC contextmenu_dispatcher(cl:PTR TO iclass,obj:PTR TO object,msg:PTR TO msg)
DEF state,member
...
PROC contextmenu_func(params:PTR TO LONG)
DEF 	o:PTR TO object, userdata,buffertemp[256]:STRING,buffertemp2[500]:STRING,dirscan,opened=0,defbytes,
	...
	IF (result = RETURN_OK)
		...
		ENDIF
	ENDIF
		domethod(app,	[MUIM_MultiSet, MUIA_Disabled, FALSE,
			vlist, bt_ComAll, bt_Com, bt_Rem, log, endgui, NIL])
