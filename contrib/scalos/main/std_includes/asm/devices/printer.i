...
aRAW     EQU   76 ; ESC[Pn"r Next 'Pn' chars are raw  +++
 ...
 STRUCTURE  IODRPTagsReq,IO_SIZE
    APTR    iodrprt_RastPort    ; raster port
    APTR    iodrprt_ColorMap    ; color map
    ULONG   iodrprt_Modes	   ; graphics viewport modes
    UWORD   iodrprt_SrcX	   ; source x origin
    UWORD   iodrprt_SrcY	   ; source y origin
    UWORD   iodrprt_SrcWidth    ; source x width
    UWORD   iodrprt_SrcHeight   ; source x height
    LONG    iodrprt_DestCols    ; destination x width
    LONG    iodrprt_DestRows    ; destination y height
    UWORD   iodrprt_Special	   ; option flags
    APTR    iodrprt_TagList	   ; tag list
    LABEL   iodrprt_SIZEOF
