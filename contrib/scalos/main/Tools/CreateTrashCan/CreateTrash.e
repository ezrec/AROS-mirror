...
MODULE  'workbench/startup','workbench/workbench','dos/dos','*mod',
	...
	'amigalib/boopsi','libraries/gadtools','utility/tagitem','icon','*VERSTAG'
...
DEF	str[500]:STRING,file:file,debug=0,myname,ent=0,app=0,win=0,running=TRUE,
	...
	name[600]:STRING,corb,var,bt_Quit,c=0,gr_Virtual,bufferstring[256]:STRING
...
PROC main() HANDLE 
  ...
  StringF(bufferstring,'\ebCreateTrash.module \s \e31.1a\en\e2 \ei(c)2003-2008 JMC - The Scalos Team\en\nModule for \ebSCALOS\en', __TARGET__)

  app := ApplicationObject,
    MUIA_Application_Title      , 'CreateTrash Module',
    MUIA_Application_Version    , '$VER: CreateTrash.module 1.1a (22.12.08) © Jean-Marie COAT - The Scalos Team',
    MUIA_Application_Copyright  , 'COAT J_Marie, ®2003-2009',
    MUIA_Application_Author     , 'COAT J_Marie, ALIAS JMC ',
    MUIA_Application_Description, 'Create trashcans under SCALOS.',
    MUIA_Application_Base       , 'CREATETRASH',
    MUIA_Application_DiskObject, appicon:= GetDiskObject('PROGDIR:CreateTrash.module'),
	...
	Child, TextObject,
              MUIA_Text_Contents, bufferstring,
...
IF wb.numargs>1
	IF ent
  	...
  	ELSE
		Mui_RequestA(app,NIL,0,'CreateTrash.module : Error !',' \ebOkay ','\eb\s\en\n is wrong object type !',{str})
		running:=FALSE
  	ENDIF
ELSE
	Mui_RequestA(app,NIL,0,'CreateTrash.module : Error !',' \ebOkay ','\ecNo argument loaded !\n\ebScalos Menu configuration\en :\nLoad \ebCreateTrash\en then, press \ebARGS WB\en\nand select \ebWorkbench\en.\n\nYou can insert \ebCreateTrash.module\en in :\n - PopupMenu: Disks\n - PopupMenu: Windows\n - PopupMenu: Drawers.\nor in:\n"disk" and "drawer" \ebFileTypes\en.',NIL)
	senv()
	running:=FALSE
ENDIF
...
PROC trash()
DEF root_lock,old_lock,disk_obj:PTR TO LONG,trash_lock,bcomp[500]:STRING,lockicon

StrCopy(path,str,ALL)
get(corb,MUIA_String_Contents,{var})
IF StrLen(var)<>0
	StrCopy(name,var,ALL)
	StringF(bcomp,'\s.info',var)
	   root_lock := Lock(path, SHARED_LOCK )
		...
		IF (lockicon := Lock(bcomp,SHARED_LOCK))
			UnLock(lockicon)
			Mui_RequestA(app,win,0,'CreateTrash.module : Error !',' \ebOkay','Icon : \eb\s\en already exists !',bcomp)
		ELSE
  	...
  	ENDIF
ELSE
	Mui_RequestA(app,win,0,'CreateTrash.module : Error !',' \ebOkay ','\ecTextfield is empty !\nEnter a name for the new trashcan to create in:\n\eb\s\en.', {path})
ENDIF
...
PROC faute()
	...
	Mui_RequestA(app, win, 0, ' CreateTrash.module : Error !', ' \ebOkay ','ErrorCode : \d\nError : \s', [IoErr(),textbuffer])
ENDPROC
