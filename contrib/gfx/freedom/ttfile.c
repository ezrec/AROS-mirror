 ...
 ******************************************************************/
  ...
  static  TT_Error  Stream_Activate( PStream_Rec  stream )
  {
    if ( !stream->opened )
    {
      ...
      if ( stream->size < 0 )
      {
      	Seek(stream->file,0,OFFSET_END);
        stream->size = Seek(stream->file,0,OFFSET_BEGINNING);
      }

      /* Reset cursor in file */
      if ( stream->position )
      {
        if ( Seek( stream->file, stream->position, OFFSET_BEGINNING ) == -1 )
        {
          /* error during seek */
          Close( stream->file );
          stream->opened = FALSE;
 ...
 ******************************************************************/
  ...
  static  TT_Error  Stream_Deactivate( PStream_Rec  stream )
  {
    ...
    {
      /* Save its current position within the file */
      stream->position = Seek( stream->file,0,OFFSET_CURRENT);
      Close( stream->file );
      stream->file   = 0;
      stream->opened = FALSE;
    }
...
/*		FIX: Georg Steger
		...
		strncpy of Maxon C Compiler is buggy.
		it writes one more (zero) byte if source
		stringlen and bufferlen are equal

      strncpy( stream_rec->name, filepathname, len );*/

		strcpy(stream_rec->name,filepathname);
...
#else
	if (Seek(CUR_Stream->file,position,OFFSET_BEGINNING) == -1)
	  return TT_Err_Invalid_File_Offset;
#endif
...
#else
	return TT_Seek_File( STREAM_VARS Seek( CUR_Stream->file,0,OFFSET_CURRENT) -
									 CUR_Stream->base + distance );
#endif
...
#else
	if (Read(CUR_Stream->file,buffer,count) != (ULONG) count )
	  return TT_Err_Invalid_File_Read;
#endif
...
#else
	return Seek(CUR_Stream->file,0,OFFSET_CURRENT) - CUR_Stream->base;
#endif
