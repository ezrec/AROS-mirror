...
/* Copy current glyph into original one.                */
  ...
  static void  cur_to_org( UShort  n, PGlyph_Zone  zone )
  {
    UShort  k;

    for ( k = 0; k < n; k++ )
      zone->org[k] = zone->cur[k];
  }
...
/* copy original glyph into current one                 */
  ...
  static void  org_to_cur( UShort  n, PGlyph_Zone  zone )
  {
    UShort  k;

    for ( k = 0; k < n; k++ )
      zone->cur[k] = zone->org[k];
  }
 ...
 ******************************************************************/
                                      ...
                                      PSubglyph_Record    subg )
  {
    ...
    {
      PTRACE4(( "%d ", n_points ));
      n_points = exec->pts.contours[k] = GET_UShort();
      n_points++;
    }
...
/*  TT_Outline                cached_outline; */
    ...
    glyph->outline.n_points    = num_points-2;
    glyph->outline.n_contours  = num_contours;
    glyph->outline.second_pass = TRUE;

    /* translate array so that (0,0) is the glyph's origin */
    translate_array( num_points, glyph->outline.points, -subglyph->pp1.x, 0 );
