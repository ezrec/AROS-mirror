...
inittextures(void)
{
   ...
   if (!LoadRGBMipmaps("PROGDIR:images/s128.rgb", GL_RGB)) {
   ...
   if (1)
   {
      ...
      GLenum format;
      int x, y;
      GLubyte *image = LoadRGBImage("PROGDIR:images/tree3.rgb", &w, &h, &format);
	 ...
	 exit(-1);
      ...
      if (!LoadRGBMipmaps("PROGDIR:images/tree2.rgba", GL_RGBA)) {
	 fprintf(stderr, "Error reading a texture.\n");
	 exit(-1);
      }
   }
