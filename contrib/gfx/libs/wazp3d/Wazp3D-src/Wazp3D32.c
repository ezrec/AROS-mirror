...
BOOL UseFlushFrame;    /* then CopyImageToDrawRegion inside bitmap*/
BOOL UseSetDrawRegion;
BOOL UseClearZBuffer;
...
struct SOFT3D_context{
    ...
    WORD OldPxmin,OldPxmax,OldPymin,OldPymax;    /* really updated region previous frame*/
};
/*=================================================================*/
...
void FillPixels_Tex32aOld(struct SOFT3D_context *SC);
...
void AntiAliasImage(struct SOFT3D_context *SC);
...
Libprintf("MYfree() not allocated pt: %ld  !!!!\n",pt);
}
/*==================================================================================*/
void DumpMem(UBYTE *pt,LONG nb)
{
...
/*==========================================================================*/
ULONG PrintAllT(W3D_Context *context)
{
...
SFUNCTION(SOFT3D_Start)
    ...
    if(Zbuffer16==NULL)
        SC->Zbuffer16=MYmalloc(large*high*16/8,"SOFT3D_ZBuffer auto");
    else
    ...
    YLOOP(high)
        ...
        SC->side1[y].L.Zbuffer16Y =SC->side2[y].L.Zbuffer16Y=( WORD *) &SC->Zbuffer16[y*large];
...
SFUNCTION(SOFT3D_ClearImage)
    ...
    if(RGBA32==0)
        memset(Image32,0,(SC->large*high));
    else
...
/*=============================================================*/
SOFT3D_ClearZBuffer(struct SOFT3D_context *SC)
{
...
SFUNCTION(SOFT3D_ClearZBuffer)
    ...
    NLOOP(size)
    {
    Zbuffer16[0]=z;    Zbuffer16[1]=z;    Zbuffer16[2]=z; Zbuffer16[3]=z;
    Zbuffer16[4]=z;    Zbuffer16[5]=z;    Zbuffer16[6]=z; Zbuffer16[7]=z;
    Zbuffer16+=8;
    }
}
/*=============================================================*/
...
register WORD large;
    ...
    while(0<high--)
    {
    Image8   =BPT (& Pix->L.Image32Y[Pix->W.x]);
    ...
    if(Zbuffer16[0] > Pix->W.z)
        {
        Zbuffer16[0]=Pix->W.z;
        Tex8=BPT (TindexXY[Pix->W.u ]+TindexXY[Pix->W.v+256]);
...
REM(FillPixels_Zoff_Color32)
    ...
    while(0<high--)
    {
    RGBA  =BPT (& Pix->L.Image32Y[Pix->W.x]);
    large    =Pix->W.large;
...
register union pixel3D *Pix=SC->Pix;
register union pixel3D *Xdelta=SC->Xdelta;
register ULONG *Tex32=(ULONG *)SC->ST->pt;
...
REM(FillPixels_Tex32a)
    ...
    Tex8=(UBYTE *)&(Tex32[Blend.L.Index]);
...
register WORD n,xmin,xmax,ymin,ymax,y;
register WORD R;
...
REM(Do line)
    SC->Pix=side=Pmin;
    LineHigh--;                /* ne pas recalculer les points extrémités */
    NLOOP(LineHigh)
    {
...
register union pixel3D *Pmin;
register union pixel3D *Pmax;
register union pixel3D *temp;
...
if (size > 1 )
    ...
    size--;                /* ne pas recalculer les points extrémités */
    side1=Pmin;
    DrawLine:
...
#define TRUECOLORFORMATS W3D_FMT_R5G5B5+W3D_FMT_B5G5R5+W3D_FMT_R5G5B5PC+W3D_FMT_B5G5R5PC+W3D_FMT_R5G6B5+W3D_FMT_B5G6R5+W3D_FMT_R5G6B5PC+W3D_FMT_B5G6R5PC+W3D_FMT_R8G8B8+W3D_FMT_B8G8R8+W3D_FMT_A8R8G8B8+W3D_FMT_A8B8G8R8+W3D_FMT_R8G8B8A8+W3D_FMT_B8G8R8A8
    ...
    Libstrcpy(Wazp3D.UseClearDR.name,"Use ClearDrawR");
    Libstrcpy(Wazp3D.UseMinDR.name,"Use Minimal DR");

    Libstrcpy(Wazp3D.UseAnyTexFmt.name,"Use any texFmt");
    Libstrcpy(Wazp3D.HackRGBA1.name ,"Hack1 tex RGBA");
    Libstrcpy(Wazp3D.HackRGBA2.name,"Hack2 tex RGBA");
    ...
    Wazp3D.UseClearDR.ON=TRUE;
    Wazp3D.UseClipper.ON=TRUE;
    Wazp3D.UseMinDR.ON=TRUE;
    ...
    Libstrcpy(Wazp3D.DriverName,"Wazp3D soft renderer - Alain Thellier - Paris France 2007 - Beta 32");
...
SFUNCTION(GetVertex)
    if(Wazp3D.DebugSC.ON)
        DumpMem((void *)V,sizeof(W3D_Vertex));
    P=&(WC->P[WC->Pnb]);
    WC->Pnb++;
    P->x=V->x;
    P->y=V->y;
...
REM(CopyImageToDrawRegion-------------------------)
    ...
    SC->OldPxmin=SC->Pxmin;    /* should be used to clear the previous drawing */
    SC->OldPxmax=SC->Pxmax;
    SC->OldPymin=SC->Pymin;
    SC->OldPymax=SC->Pymax;
    ...
    {
    VAR(context->yoffset)
    STACKWritePixelArray(WC->Image8,x,y,WC->large*(32/8),&WC->rastport,x,y+context->yoffset,large,high,RECTFMT_RGBA);
    SOFT3D_ClearImage(WC->SC,x,y,large,high);
    }
...
REM( Warp3Ds default context state values )
    ...
    SetState(context,W3D_ZBUFFERUPDATE ,TRUE);
    ...
    WC->firstWT=NULL;
    WC->UseFlushFrame=WC->UseSetDrawRegion=WC->UseClearZBuffer=FALSE;

    WC->SC=SOFT3D_Start(WC->large,WC->high,(ULONG *)WC->Image8,NULL,NULL);
...
struct WAZP3D_context *WC=context->driver;
    ...
    SOFT3D_End(WC->SC);
    VAR(WC->UseFlushFrame)
    VAR(WC->UseSetDrawRegion)
    VAR(WC->UseClearZBuffer)
...
{
    WAZP3D_Function(12);
    W3D_QueryDriver(context->gfxdriver,query,destfmt);
}
/*==========================================================================*/
ULONG W3D_GetTexFmtInfo(W3D_Context *context, ULONG texfmt, ULONG destfmt)
{
...
/*
    ...
    g=RGBA[1];    g=g<<24;
    b=RGBA[2];    b=b<<24;
    SetRGB32(&WC->screen->ViewPort,0,r,g,b);
 */

    SOFT3D_SetBackground(WC->SC,RGBA);

/* Now background RGBA is known so no need to pick it from screen */
/*    Wazp3D.UseColorHack.ON=FALSE;        */
    ...
    x=SC->OldPxmin;
    y=SC->OldPymin;
    large=SC->OldPxmax-x+1;
    high =SC->OldPymax-y+1;
     }
    else
    {
    x=0;
    ...
    VAR(context->yoffset)
    if(Wazp3D.UseClearDR.ON)
        {
        SetAPen (&WC->rastport,0);
        RectFill(&WC->rastport,x,y+context->yoffset,large,high+context->yoffset);
        }

/*    STACKReadPixelArray(WC->Image8,x,y,WC->large*(32/8),&WC->rastport,x,y+context->yoffset,large,high,RECTFMT_RGBA);    */

    if(!WC->UseClearZBuffer) SOFT3D_ClearZBuffer(WC->SC);
    WRETURN(W3D_SUCCESS);
}
/*==========================================================================*/
ULONG W3D_SetAlphaMode(W3D_Context *context, ULONG mode, W3D_Float *refval)
...
UBYTE RGBA[4];
    ...
    WAZP3D_Function(53);
/*    GetRGB32(WC->screen->ViewPort.ColorMap,pen,1,(ULONG *)RGBA32); */

/* horrible hack to recover pen RGBA values */
    SetAPen (&WC->rastport,pen);
    RectFill(&WC->rastport,0,0,1,1);
    STACKReadPixelArray(RGBA,0,0,1*(32/8),&WC->rastport,0,0,1,1,RECTFMT_RGBA);

/*    RGBA[0]=100;RGBA[1]=100;RGBA[2]=200; RGBA[3]=255; */
...
struct WAZP3D_context *WC=context->driver;
    ...
    WAZP3D_Function(56);
    if(context->zbufferalloc==TRUE)     WRETURN(W3D_SUCCESS);
    context->zbuffer=MYmalloc(WC->large*WC->high*2,"Zbuffer");
    if(context->zbuffer==NULL) WRETURN(W3D_NOGFXMEM);
    memset(context->zbuffer,0,WC->large*WC->high*2);
    context->zbufferalloc=TRUE;
    context->zbufferlost =TRUE;         /* Is it TRUE if just allocated ?!? */
...
{
struct WAZP3D_context *WC=context->driver;
register WORD *Zbuffer=context->zbuffer;
register ULONG n;
register UWORD z=*clearvalue;

    WAZP3D_Function(58);
    VARF(*clearvalue);
    if(context->zbuffer==NULL) WRETURN(W3D_NOZBUFFER);
    NLOOP(WC->large*WC->high)
        *Zbuffer++=z;

    WC->UseClearZBuffer=TRUE;
    CopyImageToDrawRegion(context);    /*draw the image if any */

    SOFT3D_ClearZBuffer(WC->SC);
    WRETURN(W3D_SUCCESS);
}
/*==========================================================================*/
ULONG W3D_ReadZPixel(W3D_Context *context, ULONG x, ULONG y,W3D_Double *z)
...
struct WAZP3D_context *WC=context->driver;
    ...
    context->stencilbuffer=MYmalloc(WC->large*WC->high*16/8,"Stencil");
    if(context->stencilbuffer==NULL) WRETURN(W3D_NOGFXMEM);
    memset(context->stencilbuffer,0,WC->large*WC->high*4);
...
{
struct WAZP3D_context *WC=context->driver;
register ULONG *Stencil=context->stencilbuffer;
register ULONG n;
register ULONG c=*clearvalue;
...
{
struct WAZP3D_context *WC=context->driver;
register UWORD *Stencil=context->stencilbuffer;
register ULONG offset;
UBYTE *data8 =data;
UWORD *data16=data;
ULONG *data32=data;
...
ULONG tag,data;
ULONG driver,ModeID;
ULONG format,large,high,bits,bytesperpixel;
UWORD n;
...
REM(TEXTURE_NOT_NULL)
        WT=texture->driver;
        WC->WT=WT;
        WC->uresize=(float)(WT->high -1);
        WC->vresize=(float)(WT->large-1);
        PrintWT(WT);
        }
