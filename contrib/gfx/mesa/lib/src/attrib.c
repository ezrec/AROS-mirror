...
void gl_PushAttrib( GLcontext* ctx, GLbitfield mask )
{
   ...
   if (mask & GL_ENABLE_BIT) {
      ...
      for (i=0; i<MAX_TEX_SETS; i++) {
         attr->TexGen[i] = ctx->Texture.Set[i].TexGenEnabled;
      }
   ...
   if (mask & GL_TEXTURE_BIT) {
      ...
      for (dim = 0; dim < 3; dim++) {
         ...
         ctx->Texture.BorderColor[dim][0] = obj->BorderColor[0];
         ctx->Texture.BorderColor[dim][1] = obj->BorderColor[1];
         ctx->Texture.BorderColor[dim][2] = obj->BorderColor[2];
         ctx->Texture.BorderColor[dim][3] = obj->BorderColor[3];
   ...
   while (attr) {
      switch (attr->kind) {
	    ...
	    ctx->Texture.Enabled = enable->Texture;
            for (i=0; i<MAX_TEX_SETS; i++) {
               ctx->Texture.Set[i].TexGenEnabled = enable->TexGen[i];
            }
            break;
         case GL_EVAL_BIT:
            ...
            /* Restore special texture state */
            {
               ...
               for (dim = 0; dim < 3; dim++) {
                  ...
                  if (dim == 0)
                     ...
                     obj = ctx->Texture.Set[texSet].Current3D;
                  obj->Priority = ctx->Texture.Priority[dim];
                  obj->BorderColor[0] = ctx->Texture.BorderColor[dim][0];
                  obj->BorderColor[1] = ctx->Texture.BorderColor[dim][1];
                  obj->BorderColor[2] = ctx->Texture.BorderColor[dim][2];
                  obj->BorderColor[3] = ctx->Texture.BorderColor[dim][3];
                  obj->WrapS = ctx->Texture.WrapS[dim];
                  obj->WrapT = ctx->Texture.WrapT[dim];
                  obj->WrapR = ctx->Texture.WrapR[dim];
                  obj->MinFilter = ctx->Texture.MinFilter[dim];
                  obj->MagFilter = ctx->Texture.MagFilter[dim];
               }
            }
            break;
