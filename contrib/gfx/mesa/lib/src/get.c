 ...
 *
 * Revision 3.14  1998/07/18 03:32:54  brianp
 * added GL_SELECTION_BUFFER_SIZE GL_SELECTION_BUFFER_POINTER cases
 *
 * Revision 3.13  1998/06/19 03:10:50  brianp
 * glGet(MODELVIEW/PROJECTION/MATRIX_STACK_DEPTH) was off by one
 *
 * Revision 3.12  1998/06/07 22:18:52  brianp
 * implemented GL_EXT_multitexture extension
 *
 * Revision 3.11  1998/05/31 23:50:36  brianp
 * cleaned up a few Solaris compiler warnings
 *
 * Revision 3.10  1998/04/01 02:59:42  brianp
 * updated for v0.24 of 3Dfx/Glide driver
 *
 * Revision 3.9  1998/03/27 03:37:40  brianp
 * fixed G++ warnings
 *
 * Revision 3.8  1998/02/21 01:02:03  brianp
 * query driver for max number of textures
 *
 * Revision 3.7  1998/02/20 04:50:44  brianp
 * implemented GL_SGIS_multitexture
 *
 * Revision 3.6  1998/02/13 03:23:04  brianp
 * AlphaRef is now a GLubyte
 *
 * Revision 3.5  1998/02/08 20:19:41  brianp
 * ColorMask is now GLubyte[4] instead of GLuint
 *
 * Revision 3.4  1998/02/07 14:43:46  brianp
 * get GL_MAX_TEXTURE_SIZE now queries device driver
 *
 ...
 */
static GLint max_textures(const GLcontext *ctx)
{
   if (ctx->Driver.GetParameteri) {
      GLint maxSets = (*ctx->Driver.GetParameteri)(ctx, DD_MAX_TEXTURES);
      assert(maxSets >= 1 && maxSets <= MAX_TEX_SETS);
      return maxSets;
   }
   else {
      return MAX_TEX_SETS;
 ...
 */
static GLint max_texture_size(const GLcontext *ctx)
{
   if (ctx->Driver.GetParameteri)
      return (ctx->Driver.GetParameteri)(ctx, DD_MAX_TEXTURE_SIZE);
   else
...
void gl_GetBooleanv( GLcontext *ctx, GLenum pname, GLboolean *params )
{
   ...
   GLuint texTransformSet = ctx->Texture.CurrentTransformSet;
   ...
   switch (pname) {
	 ...
	 *params = FLOAT_TO_BOOL(ctx->Current.RasterIndex);
	 ...
	 break;
      case GL_CURRENT_RASTER_TEXTURE_COORDS:
         params[0] = FLOAT_TO_BOOL(ctx->Current.RasterMultiTexCoord[texTransformSet][0]);
         params[1] = FLOAT_TO_BOOL(ctx->Current.RasterMultiTexCoord[texTransformSet][1]);
         params[2] = FLOAT_TO_BOOL(ctx->Current.RasterMultiTexCoord[texTransformSet][2]);
         params[3] = FLOAT_TO_BOOL(ctx->Current.RasterMultiTexCoord[texTransformSet][3]);
	 break;
      case GL_CURRENT_RASTER_POSITION_VALID:
         *params = ctx->Current.RasterPosValid;
	 break;
      case GL_CURRENT_TEXTURE_COORDS:
         params[0] = FLOAT_TO_BOOL(ctx->Current.MultiTexCoord[texTransformSet][0]);
         params[1] = FLOAT_TO_BOOL(ctx->Current.MultiTexCoord[texTransformSet][1]);
         params[2] = FLOAT_TO_BOOL(ctx->Current.MultiTexCoord[texTransformSet][2]);
         params[3] = FLOAT_TO_BOOL(ctx->Current.MultiTexCoord[texTransformSet][3]);
	 break;
      case GL_DEPTH_BIAS:
	 ...
	 *params = INT_TO_BOOL(0);  /* TODO */
	 break;
      case GL_TEXTURE_1D:
         {
            GLuint bit = TEXTURE0_1D << (texSet * 4);
            *params = (ctx->Texture.Enabled & bit) ? GL_TRUE : GL_FALSE;
         }
	 break;
      case GL_TEXTURE_2D:
         {
            GLuint bit = TEXTURE0_2D << (texSet * 4);
            *params = (ctx->Texture.Enabled & bit) ? GL_TRUE : GL_FALSE;
         }
	 break;
      case GL_TEXTURE_3D:
         {
            GLuint bit = TEXTURE0_3D << (texSet * 4);
            *params = (ctx->Texture.Enabled & bit) ? GL_TRUE : GL_FALSE;
         }
	 break;
      case GL_TEXTURE_BINDING_1D:
         *params = INT_TO_BOOL(textureSet->Current1D->Name);
          break;
      case GL_TEXTURE_BINDING_2D:
         *params = INT_TO_BOOL(textureSet->Current2D->Name);
          break;
	 ...
	 break;
      case GL_TEXTURE_ENV_COLOR:
         {
            params[0] = FLOAT_TO_BOOL(textureSet->EnvColor[0]);
            params[1] = FLOAT_TO_BOOL(textureSet->EnvColor[1]);
            params[2] = FLOAT_TO_BOOL(textureSet->EnvColor[2]);
            params[3] = FLOAT_TO_BOOL(textureSet->EnvColor[3]);
         }
	 break;
      case GL_TEXTURE_ENV_MODE:
	 *params = ENUM_TO_BOOL(textureSet->EnvMode);
	 break;
      case GL_TEXTURE_GEN_S:
	 *params = (textureSet->TexGenEnabled & S_BIT) ? GL_TRUE : GL_FALSE;
	 break;
      case GL_TEXTURE_GEN_T:
	 *params = (textureSet->TexGenEnabled & T_BIT) ? GL_TRUE : GL_FALSE;
	 break;
      case GL_TEXTURE_GEN_R:
	 *params = (textureSet->TexGenEnabled & R_BIT) ? GL_TRUE : GL_FALSE;
	 break;
      case GL_TEXTURE_GEN_Q:
	 *params = (textureSet->TexGenEnabled & Q_BIT) ? GL_TRUE : GL_FALSE;
	 break;
      case GL_TEXTURE_MATRIX:
	 for (i=0;i<16;i++) {
	    params[i] = FLOAT_TO_BOOL(ctx->TextureMatrix[texTransformSet][i]);
	 }
	 break;
      case GL_TEXTURE_STACK_DEPTH:
	 *params = INT_TO_BOOL(ctx->TextureStackDepth[texTransformSet] + 1);
	 break;
	 ...
	 *params = FLOAT_TO_BOOL(ctx->Pixel.ZoomY);
	 break;
      ...
      case GL_TEXTURE_COORD_ARRAY_SIZE:
         *params = INT_TO_BOOL(ctx->Array.TexCoordSize[texSet]);
         break;
      case GL_TEXTURE_COORD_ARRAY_TYPE:
         *params = ENUM_TO_BOOL(ctx->Array.TexCoordType[texSet]);
         break;
      case GL_TEXTURE_COORD_ARRAY_STRIDE:
         *params = INT_TO_BOOL(ctx->Array.TexCoordStride[texSet]);
         break;
      ...
      case GL_SELECTED_TEXTURE_SGIS:
         *params = INT_TO_BOOL(ctx->Texture.CurrentSet);
         break;
      case GL_SELECTED_TEXTURE_COORD_SET_SGIS:
         *params = INT_TO_BOOL(ctx->TexCoordSet);
         break;
      case GL_MAX_TEXTURES_SGIS:
         *params = INT_TO_BOOL(max_textures(ctx));
         break;

      /* GL_EXT_multitexture */
      case GL_SELECTED_TEXTURE_EXT:
         *params = INT_TO_BOOL(ctx->Texture.CurrentSet);
         break;
      case GL_SELECTED_TEXTURE_COORD_SET_EXT:
         *params = INT_TO_BOOL(ctx->TexCoordSet);
         break;
      case GL_SELECTED_TEXTURE_TRANSFORM_EXT:
         *params = INT_TO_BOOL(ctx->Texture.CurrentTransformSet);
          break;
      case GL_MAX_TEXTURES_EXT:
         *params = INT_TO_BOOL(max_textures(ctx));
          break;
      case GL_MAX_TEXTURE_COORD_SETS_EXT:
         *params = INT_TO_BOOL(MAX_TEX_COORD_SETS);
          break;
...
void gl_GetDoublev( GLcontext *ctx, GLenum pname, GLdouble *params )
{
   ...
   switch (pname) {
	 ...
	 break;
      ...
      case GL_CULL_FACE_MODE:
         *params = ENUM_TO_DOUBLE(ctx->Polygon.CullFaceMode);
         break;
      case GL_CURRENT_COLOR:
         params[0] = (GLfloat) ctx->Current.ByteColor[0] / 255.0F;
         params[1] = (GLfloat) ctx->Current.ByteColor[1] / 255.0F;
         params[2] = (GLfloat) ctx->Current.ByteColor[2] / 255.0F;
         params[3] = (GLfloat) ctx->Current.ByteColor[3] / 255.0F;
         break;
      case GL_CURRENT_INDEX:
	 ...
	 *params = (GLdouble) ctx->Current.RasterIndex;
	 ...
	 break;
      case GL_CURRENT_RASTER_TEXTURE_COORDS:
	 params[0] = (GLdouble) ctx->Current.RasterMultiTexCoord[texTransformSet][0];
	 params[1] = (GLdouble) ctx->Current.RasterMultiTexCoord[texTransformSet][1];
	 params[2] = (GLdouble) ctx->Current.RasterMultiTexCoord[texTransformSet][2];
	 params[3] = (GLdouble) ctx->Current.RasterMultiTexCoord[texTransformSet][3];
	 break;
      case GL_CURRENT_RASTER_POSITION_VALID:
	 *params = (GLdouble) ctx->Current.RasterPosValid;
	 break;
      case GL_CURRENT_TEXTURE_COORDS:
	 params[0] = (GLdouble) ctx->Current.MultiTexCoord[texTransformSet][0];
	 params[1] = (GLdouble) ctx->Current.MultiTexCoord[texTransformSet][1];
	 params[2] = (GLdouble) ctx->Current.MultiTexCoord[texTransformSet][2];
	 params[3] = (GLdouble) ctx->Current.MultiTexCoord[texTransformSet][3];
	 break;
      case GL_DEPTH_BIAS:
	 ...
	 *params = 0.0;   /* TODO */
	 break;
      case GL_TEXTURE_1D:
         {
            GLuint bit = TEXTURE0_1D << (texSet * 4);
            *params = (ctx->Texture.Enabled & bit) ? 1.0 : 0.0;
         }
	 break;
      case GL_TEXTURE_2D:
         {
	 ...
	 break;
      case GL_TEXTURE_BINDING_1D:
         *params = (GLdouble) textureSet->Current1D->Name;
          break;
      case GL_TEXTURE_BINDING_2D:
         *params = (GLdouble) textureSet->Current2D->Name;
	 break;
      case GL_TEXTURE_BINDING_3D:
         *params = (GLdouble) textureSet->Current3D->Name;
	 break;
      case GL_TEXTURE_ENV_COLOR:
	 params[0] = (GLdouble) textureSet->EnvColor[0];
	 params[1] = (GLdouble) textureSet->EnvColor[1];
	 params[2] = (GLdouble) textureSet->EnvColor[2];
	 params[3] = (GLdouble) textureSet->EnvColor[3];
	 break;
      case GL_TEXTURE_ENV_MODE:
	 *params = ENUM_TO_DOUBLE(textureSet->EnvMode);
	 break;
      case GL_TEXTURE_GEN_S:
	 *params = (textureSet->TexGenEnabled & S_BIT) ? 1.0 : 0.0;
	 break;
      case GL_TEXTURE_GEN_T:
	 *params = (textureSet->TexGenEnabled & T_BIT) ? 1.0 : 0.0;
	 break;
      case GL_TEXTURE_GEN_R:
	 *params = (textureSet->TexGenEnabled & R_BIT) ? 1.0 : 0.0;
	 break;
      case GL_TEXTURE_GEN_Q:
	 *params = (textureSet->TexGenEnabled & Q_BIT) ? 1.0 : 0.0;
	 break;
      case GL_TEXTURE_MATRIX:
         for (i=0;i<16;i++) {
	    params[i] = (GLdouble) ctx->TextureMatrix[texTransformSet][i];
	 }
	 break;
      case GL_TEXTURE_STACK_DEPTH:
	 *params = (GLdouble) (ctx->TextureStackDepth[texTransformSet] + 1);
	 break;
      case GL_UNPACK_ALIGNMENT:
	 ...
	 break;
      ...
      case GL_INDEX_ARRAY_COUNT_EXT:
         *params = 0.0;
         break;
      case GL_TEXTURE_COORD_ARRAY_SIZE:
         *params = (GLdouble) ctx->Array.TexCoordSize[texSet];
         break;
      case GL_TEXTURE_COORD_ARRAY_TYPE:
         *params = ENUM_TO_DOUBLE(ctx->Array.TexCoordType[texSet]);
         break;
      case GL_TEXTURE_COORD_ARRAY_STRIDE:
         *params = (GLdouble) ctx->Array.TexCoordStride[texSet];
         break;
      case GL_TEXTURE_COORD_ARRAY_COUNT_EXT:
         *params = 0.0;
         break;
      ...
      /* GL_SIGS_multitexture */
      case GL_SELECTED_TEXTURE_SGIS:
         *params = (GLdouble) ctx->Texture.CurrentSet;
         break;
      case GL_SELECTED_TEXTURE_COORD_SET_SGIS:
         *params = (GLdouble) ctx->TexCoordSet;
         break;
      case GL_MAX_TEXTURES_SGIS:
         *params = (GLdouble) max_textures(ctx);
         break;
      ...
      case GL_SELECTED_TEXTURE_COORD_SET_EXT:
         *params = (GLdouble) ctx->TexCoordSet;
         break;
      case GL_SELECTED_TEXTURE_TRANSFORM_EXT:
         *params = (GLdouble) ctx->Texture.CurrentTransformSet;
          break;
      case GL_MAX_TEXTURES_EXT:
         *params = (GLdouble) max_textures(ctx);
          break;
      case GL_MAX_TEXTURE_COORD_SETS_EXT:
         *params = (GLdouble) MAX_TEX_COORD_SETS;
          break;
...
void gl_GetFloatv( GLcontext *ctx, GLenum pname, GLfloat *params )
{
   ...
   switch (pname) {
	 ...
	 *params = (GLfloat) ctx->Current.RasterIndex;
	 ...
	 break;
      case GL_CURRENT_RASTER_TEXTURE_COORDS:
	 params[0] = ctx->Current.RasterMultiTexCoord[texTransformSet][0];
	 params[1] = ctx->Current.RasterMultiTexCoord[texTransformSet][1];
	 params[2] = ctx->Current.RasterMultiTexCoord[texTransformSet][2];
	 params[3] = ctx->Current.RasterMultiTexCoord[texTransformSet][3];
	 break;
      case GL_CURRENT_RASTER_POSITION_VALID:
	 *params = (GLfloat) ctx->Current.RasterPosValid;
	 break;
      case GL_CURRENT_TEXTURE_COORDS:
	 params[0] = (GLfloat) ctx->Current.MultiTexCoord[texTransformSet][0];
	 params[1] = (GLfloat) ctx->Current.MultiTexCoord[texTransformSet][1];
	 params[2] = (GLfloat) ctx->Current.MultiTexCoord[texTransformSet][2];
	 params[3] = (GLfloat) ctx->Current.MultiTexCoord[texTransformSet][3];
	 break;
      case GL_DEPTH_BIAS:
	 ...
	 *params = 0.0F;  /* TODO */
	 ...
	 break;
      case GL_TEXTURE_BINDING_1D:
         *params = (GLfloat) textureSet->Current1D->Name;
          break;
      case GL_TEXTURE_BINDING_2D:
         *params = (GLfloat) textureSet->Current2D->Name;
          break;
      case GL_TEXTURE_BINDING_3D:
         *params = (GLfloat) textureSet->Current2D->Name;
	 break;
      case GL_TEXTURE_ENV_COLOR:
	 params[0] = textureSet->EnvColor[0];
	 params[1] = textureSet->EnvColor[1];
	 params[2] = textureSet->EnvColor[2];
	 params[3] = textureSet->EnvColor[3];
	 break;
      case GL_TEXTURE_ENV_MODE:
	 *params = ENUM_TO_FLOAT(textureSet->EnvMode);
	 break;
      case GL_TEXTURE_GEN_S:
	 *params = (textureSet->TexGenEnabled & S_BIT) ? 1.0 : 0.0;
	 break;
	 ...
	 *params = (textureSet->TexGenEnabled & Q_BIT) ? 1.0 : 0.0;
	 break;
      case GL_TEXTURE_MATRIX:
         for (i=0;i<16;i++) {
	    params[i] = ctx->TextureMatrix[texTransformSet][i];
	 }
	 break;
      case GL_TEXTURE_STACK_DEPTH:
	 *params = (GLfloat) (ctx->TextureStackDepth[texTransformSet] + 1);
	 break;
      case GL_UNPACK_ALIGNMENT:
	 ...
	 break;
      ...
      case GL_INDEX_ARRAY_COUNT_EXT:
         *params = 0.0;
         break;
      case GL_TEXTURE_COORD_ARRAY_SIZE:
         *params = (GLfloat) ctx->Array.TexCoordSize[texSet];
         break;
      case GL_TEXTURE_COORD_ARRAY_TYPE:
         *params = ENUM_TO_FLOAT(ctx->Array.TexCoordType[texSet]);
         break;
      case GL_TEXTURE_COORD_ARRAY_STRIDE:
         *params = (GLfloat) ctx->Array.TexCoordStride[texSet];
         break;
      case GL_TEXTURE_COORD_ARRAY_COUNT_EXT:
         *params = 0.0;
         break;
      ...
      /* GL_SGIS_multitexture */
      case GL_SELECTED_TEXTURE_SGIS:
         *params = (GLfloat) ctx->Texture.CurrentSet;
         break;
      case GL_SELECTED_TEXTURE_COORD_SET_SGIS:
         *params = (GLfloat) ctx->TexCoordSet;
         break;
      case GL_MAX_TEXTURES_SGIS:
         *params = (GLfloat) max_textures(ctx);
         break;
      ...
      case GL_SELECTED_TEXTURE_COORD_SET_EXT:
         *params = (GLfloat) ctx->TexCoordSet;
         break;
      case GL_SELECTED_TEXTURE_TRANSFORM_EXT:
         *params = (GLfloat) ctx->Texture.CurrentTransformSet;
          break;
      case GL_MAX_TEXTURES_EXT:
         *params = (GLfloat) max_textures(ctx);
          break;
      case GL_MAX_TEXTURE_COORD_SETS_EXT:
         *params = (GLfloat) MAX_TEX_COORD_SETS;
          break;
...
void gl_GetIntegerv( GLcontext *ctx, GLenum pname, GLint *params )
{
   ...
   switch (pname) {
	 ...
	 break;
      ...
      case GL_CULL_FACE_MODE:
         *params = (GLint) ctx->Polygon.CullFaceMode;
         break;
      case GL_CURRENT_COLOR:
         params[0] = FLOAT_TO_INT( (GLfloat) ctx->Current.ByteColor[0] / 255.0F );
         params[1] = FLOAT_TO_INT( (GLfloat) ctx->Current.ByteColor[1] / 255.0F );
         params[2] = FLOAT_TO_INT( (GLfloat) ctx->Current.ByteColor[2] / 255.0F );
         params[3] = FLOAT_TO_INT( (GLfloat) ctx->Current.ByteColor[3] / 255.0F );
         break;
      case GL_CURRENT_INDEX:
	 ...
	 *params = (GLint) ctx->Current.RasterIndex;
	 ...
	 break;
      case GL_CURRENT_RASTER_TEXTURE_COORDS:
	 params[0] = (GLint) ctx->Current.RasterMultiTexCoord[texTransformSet][0];
	 params[1] = (GLint) ctx->Current.RasterMultiTexCoord[texTransformSet][1];
	 params[2] = (GLint) ctx->Current.RasterMultiTexCoord[texTransformSet][2];
	 params[3] = (GLint) ctx->Current.RasterMultiTexCoord[texTransformSet][3];
	 break;
      case GL_CURRENT_RASTER_POSITION_VALID:
	 *params = (GLint) ctx->Current.RasterPosValid;
	 break;
      case GL_CURRENT_TEXTURE_COORDS:
         params[0] = (GLint) ctx->Current.MultiTexCoord[texTransformSet][0];
         params[1] = (GLint) ctx->Current.MultiTexCoord[texTransformSet][1];
         params[2] = (GLint) ctx->Current.MultiTexCoord[texTransformSet][2];
         params[3] = (GLint) ctx->Current.MultiTexCoord[texTransformSet][3];
	 break;
      case GL_DEPTH_BIAS:
	 ...
	 *params = 0;  /* TODO */
	 break;
      case GL_TEXTURE_1D:
         {
            GLuint bit = TEXTURE0_3D << (texSet * 4);
            *params = (ctx->Texture.Enabled & bit) ? 1 : 0;
         }
	 break;
      case GL_TEXTURE_2D:
         {
	 ...
	 break;
      case GL_TEXTURE_BINDING_1D:
         *params = textureSet->Current1D->Name;
          break;
      case GL_TEXTURE_BINDING_2D:
         *params = textureSet->Current2D->Name;
          break;
      case GL_TEXTURE_BINDING_3D:
         *params = textureSet->Current3D->Name;
	 break;
      case GL_TEXTURE_ENV_COLOR:
	 params[0] = FLOAT_TO_INT( textureSet->EnvColor[0] );
	 params[1] = FLOAT_TO_INT( textureSet->EnvColor[1] );
	 params[2] = FLOAT_TO_INT( textureSet->EnvColor[2] );
	 params[3] = FLOAT_TO_INT( textureSet->EnvColor[3] );
	 break;
      case GL_TEXTURE_ENV_MODE:
	 *params = (GLint) textureSet->EnvMode;
	 break;
      case GL_TEXTURE_GEN_S:
	 *params = (textureSet->TexGenEnabled & S_BIT) ? 1 : 0;
	 break;
      case GL_TEXTURE_GEN_T:
	 *params = (textureSet->TexGenEnabled & T_BIT) ? 1 : 0;
	 break;
      case GL_TEXTURE_GEN_R:
	 *params = (textureSet->TexGenEnabled & R_BIT) ? 1 : 0;
	 break;
      case GL_TEXTURE_GEN_Q:
	 *params = (textureSet->TexGenEnabled & Q_BIT) ? 1 : 0;
	 break;
      case GL_TEXTURE_MATRIX:
         for (i=0;i<16;i++) {
	    params[i] = (GLint) ctx->TextureMatrix[texTransformSet][i];
	 }
	 break;
      case GL_TEXTURE_STACK_DEPTH:
	 *params = (GLint) (ctx->TextureStackDepth[texTransformSet] + 1);
	 break;
      case GL_UNPACK_ALIGNMENT:
	 ...
	 break;
      ...
      case GL_INDEX_ARRAY_COUNT_EXT:
         *params = 0;
         break;
      case GL_TEXTURE_COORD_ARRAY_SIZE:
         *params = ctx->Array.TexCoordSize[texSet];
         break;
      case GL_TEXTURE_COORD_ARRAY_TYPE:
         *params = ctx->Array.TexCoordType[texSet];
         break;
      case GL_TEXTURE_COORD_ARRAY_STRIDE:
         *params = ctx->Array.TexCoordStride[texSet];
         break;
      case GL_TEXTURE_COORD_ARRAY_COUNT_EXT:
         *params = 0;
         break;
      ...
      /* GL_SGIS_multitexture */
      case GL_SELECTED_TEXTURE_SGIS:
         *params = ctx->Texture.CurrentSet;
         break;
      case GL_SELECTED_TEXTURE_COORD_SET_SGIS:
         *params = ctx->TexCoordSet;
         break;
      case GL_MAX_TEXTURES_SGIS:
         *params = max_textures(ctx);
         break;
      ...
      case GL_SELECTED_TEXTURE_COORD_SET_EXT:
         *params = ctx->TexCoordSet;
         break;
      case GL_SELECTED_TEXTURE_TRANSFORM_EXT:
         *params = ctx->Texture.CurrentTransformSet;
          break;
      case GL_MAX_TEXTURES_EXT:
         *params = max_textures(ctx);
          break;
      case GL_MAX_TEXTURE_COORD_SETS_EXT:
         *params = MAX_TEX_COORD_SETS;
          break;
...
void gl_GetPointerv( GLcontext *ctx, GLenum pname, GLvoid **params )
{
   ...
   switch (pname) {
      ...
      case GL_TEXTURE_COORD_ARRAY_POINTER:
         *params = ctx->Array.TexCoordPtr[texSet];
         break;
