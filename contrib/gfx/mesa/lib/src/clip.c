 ...
 *
 * Revision 3.5  1998/06/21 02:00:55  brianp
 ...
 */
void gl_clip_interp_all( GLcontext *ctx, GLuint space,
                             GLuint dst, GLfloat t, GLuint in, GLuint out )
{
   struct vertex_buffer *VB = ctx->VB;

   if (ctx->ClipMask & CLIP_FCOLOR_BIT) {
      VB->Fcolor[dst][0] = (GLint) LINTERP( t, VB->Fcolor[in][0], VB->Fcolor[out][0] );
      VB->Fcolor[dst][1] = (GLint) LINTERP( t, VB->Fcolor[in][1], VB->Fcolor[out][1] );
      VB->Fcolor[dst][2] = (GLint) LINTERP( t, VB->Fcolor[in][2], VB->Fcolor[out][2] );
      VB->Fcolor[dst][3] = (GLint) LINTERP( t, VB->Fcolor[in][3], VB->Fcolor[out][3] );
   }
   ...
   if (ctx->ClipMask & CLIP_BCOLOR_BIT) {
      VB->Bcolor[dst][0] = (GLubyte) LINTERP( t, VB->Bcolor[in][0], VB->Bcolor[out][0] );
      VB->Bcolor[dst][1] = (GLubyte) LINTERP( t, VB->Bcolor[in][1], VB->Bcolor[out][1] );
      VB->Bcolor[dst][2] = (GLubyte) LINTERP( t, VB->Bcolor[in][2], VB->Bcolor[out][2] );
      VB->Bcolor[dst][3] = (GLubyte) LINTERP( t, VB->Bcolor[in][3], VB->Bcolor[out][3] );
   }
   ...
   if (ctx->ClipMask & CLIP_TEXTURE_BIT) {
	 ...
	 VB->Eye[dst][2] = LINTERP( t, VB->Eye[in][2], VB->Eye[out][2] );
      ...
      for (i=0;i<MAX_TEX_SETS;i++) {
         VB->MultiTexCoord[i][dst][0] = LINTERP(t,VB->MultiTexCoord[i][in][0],VB->MultiTexCoord[i][out][0]);
         VB->MultiTexCoord[i][dst][1] = LINTERP(t,VB->MultiTexCoord[i][in][1],VB->MultiTexCoord[i][out][1]);
         VB->MultiTexCoord[i][dst][2] = LINTERP(t,VB->MultiTexCoord[i][in][2],VB->MultiTexCoord[i][out][2]);
         VB->MultiTexCoord[i][dst][3] = LINTERP(t,VB->MultiTexCoord[i][in][3],VB->MultiTexCoord[i][out][3]);
      }
   }
...
/*
 * Specialized version gl_clip_interp_all:  just do color and texcoords.
 */
void gl_clip_interp_color_tex( GLcontext *ctx, GLuint space,
				 GLuint dst, GLfloat t, GLuint in, GLuint out )
{
   struct vertex_buffer *VB = ctx->VB;
   VB->Fcolor[dst][0] = (GLint) LINTERP( t, VB->Fcolor[in][0], VB->Fcolor[out][0] );
...
/*
 * Specialized version gl_clip_interp_all:  just do texcoords.
 */
void gl_clip_interp_tex( GLcontext *ctx, GLuint space,
			   GLuint dst, GLfloat t, GLuint in, GLuint out )
{
   struct vertex_buffer *VB = ctx->VB;
   VB->Eye[dst][2] = LINTERP( t, VB->Eye[in][2], VB->Eye[out][2] );
...
/*
 * Specialized version gl_clip_interp_all:  just do color.
 */
void gl_clip_interp_color( GLcontext *ctx, GLuint space,
			    GLuint dst, GLfloat t, GLuint in, GLuint out )
{
   struct vertex_buffer *VB = ctx->VB;
   VB->Fcolor[dst][0] = (GLint) LINTERP( t, VB->Fcolor[in][0], VB->Fcolor[out][0] );
...
#define GENERAL_CLIP							\
   if (OUTSIDE(ii)) {           	                        	\
	 ...
	 if (ctx->ClipMask)						\
            (*ctx->ClipInterpFunc)( ctx, CLIP_SPACE, VB->Free, t, jj, ii );\
	 ii = VB->Free;							\
	 VB->Free++;							\
	 ...
	 if (ctx->ClipMask)						\
	    (*ctx->ClipInterpFunc)( ctx, CLIP_SPACE, VB->Free, t, ii, jj );\
	 jj = VB->Free;							\
	 VB->Free++;							\
	 if (VB->Free==VB_SIZE)  VB->Free = 1;				\
      }                                                 		\
   ...
   for (curri=0;curri<INCOUNT;curri++) {				\
      ...
      if (INSIDE(currj)) {						\
	    ...
	    if (t>0.0) {						\
	       ...
	       if (ctx->ClipMask)					\
		  (*ctx->ClipInterpFunc)( ctx, CLIP_SPACE, VB->Free, t, currj, prevj ); \
	    ...
	    if (t>0.0) {						\
	       /* interpolate aux info using the value of t */		\
	       if (ctx->ClipMask)					\
		  (*ctx->ClipInterpFunc)( ctx, CLIP_SPACE, VB->Free, t, prevj, currj ); \
	       VB->Edgeflag[VB->Free] = VB->Edgeflag[prevj];		\
	       /* output new point */					\
	       OUTLIST[OUTCOUNT] = VB->Free;				\
	       VB->Free++;						\
	 ...
	 d = ctx->Transform.ClipEquation[p][3];
         ...
         if (OUTSIDE( ii, a,b,c,d  )) {
		  ...
		  t = -(VB->Eye[jj][0]*a+VB->Eye[jj][1]*b
	       ...
	       if (ctx->ClipMask)
		  (*ctx->ClipInterpFunc)( ctx, EYE_SPACE, VB->Free, t, jj, ii );
		  ...
		  t = -(VB->Eye[ii][0]*a+VB->Eye[ii][1]*b
	       ...
	       /* Interpolate colors, indexes, and/or texture coords */
	       if (ctx->ClipMask)
		  (*ctx->ClipInterpFunc)( ctx, EYE_SPACE, VB->Free, t, ii, jj );
   ...
   for (p=0;p<MAX_CLIP_PLANES;p++) {
	 ...
	 prevj = inlist[previ];
	    ...
	    currj = inlist[curri];
		  ...
		  if (ctx->ClipMask) {
		     (*ctx->ClipInterpFunc)( ctx, EYE_SPACE, VB->Free, t, currj, prevj);
		  }
		  VB->Edgeflag[VB->Free] = VB->Edgeflag[prevj];
		  ...
		  /* interpolate color, index, and/or texture coord */
		  if (ctx->ClipMask) {
		     (*ctx->ClipInterpFunc)( ctx, EYE_SPACE, VB->Free, t, prevj, currj);
		  }
		  VB->Edgeflag[VB->Free] = VB->Edgeflag[prevj];
