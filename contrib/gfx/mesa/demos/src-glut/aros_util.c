...
{
  /* the transparent pixel on aros is always index number 0.  So if
...
__glutAdjustCoords(Window parent, int* x, int* y, int* width, int* height)
{
  ...
  /* adjust the window rectangle because aros thinks that the x, y,
...
LONG WINAPI __glutWindowProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
  ...
  case WM_PAINT:
    ...
    if (window) {
      BeginPaint(hwnd, &ps);		/* Must have this for some aros reason. */
#if TAKE_THIS_OUT
      if (window->colormap) {
  ...
  case WM_CHAR:
    ...
    /* aros is dumb and sends these messages only to the parent
  ...
  case WM_KEYDOWN:
	...
	case VK_PRIOR:
	  /* VK_PRIOR is aros's Page Up */
	  key = GLUT_KEY_PAGE_UP;
	  break;
	case VK_NEXT:
	  /* VK_NEXT is aros's Page Down */
  ...
  case WM_RBUTTONDOWN:
    ...
    /* aros doesn't return the same numbers as X does when the mouse
       goes beyond the upper or left side of the window.  roll the
       aros's 0..2^16 pointer co-ord range to 0 +/- 2^15. */
    x = LOWORD(lParam);
    y = HIWORD(lParam);
    if(x & 1 << 15) x -= (1 << 16);
    if(y & 1 << 15) y -= (1 << 16);
  ...
  case WM_RBUTTONUP:
    ...
    if (window && window->mouse) {
      /* aros doesn't return the same numbers as X does when the
	 mouse goes beyond the upper or left side of the window.  roll
	 the aros's 0..2^16 pointer co-ord range to 0 +/- 2^15. */
      x = LOWORD(lParam);
      y = HIWORD(lParam);
      if(x & 1 << 15) x -= (1 << 16);
      if(y & 1 << 15) y -= (1 << 16);
  ...
  case WM_MOUSEMOVE:
	...
	/* aros doesn't return the same numbers as X does when the
	   ...
	   roll the aros's 0..2^16 pointer co-ord range to 0..+/-2^15. */
	if(x & 1 << 15) x -= (1 << 16);
	if(y & 1 << 15) y -= (1 << 16);
  ...
  case WM_GETMINMAXINFO:
    /* this voodoo is brought to you by aros (again).  It allows the
  ...
  case WM_SETCURSOR:
    /* If the cursor is not in the client area, then we want to send
       ...
       cursor.  otherwise, set our cursor.  aros makes us set the
       cursor every time the mouse moves (DUMB!). */
    if(LOWORD(lParam) != HTCLIENT)
      goto defproc;
    window = __glutGetWindow(hwnd);
    if (window) {
      /* Since aros allows the parent to control a child windows
  ...
  case WM_ACTIVATE:
	...
	/* Since aros only sends an activate for the toplevel window,
