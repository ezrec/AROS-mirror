#ifndef __aros_x11_h__
#define __aros_x11_h__
...
#define True						TRUE				/* AROS equivalents of X11 booleans */
#define False						FALSE

#define None						NULL				/* universal null resource or null atom */

/* Input Event Masks. Used as event-mask window attribute and as arguments
   to Grab requests.  Not to be confused with event names.  */

#define NoEventMask					None
...
/* There really isn't a way to set the icon name separate from the
   windows name in aros, so, just set the windows name. */
...
/* Anything that needs to be freed was allocated with malloc in our
   fake X windows library for aros, so free it with plain old
   free(). */
#define XFree(data) \
  free(data)
