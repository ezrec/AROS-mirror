...
#define OUTLINE_SCALE 0.023
#define FILLED_SCALE 0.023

/******************************************************************************/

static GLint strokeFont[] = {
	1,
	FONT_BEGIN, 0, 4,
	FONT_NEXT, 2, 2,
	FONT_END, 4, 4,
...
GLenum tkCreateStrokeFont(GLuint fontBase)
{
    ...
    for (ip = strokeFont; *ip != END_OF_LIST;) {
	glNewList(fontBase+(unsigned int)*ip++, GL_COMPILE);
	do {
	    mode = ip[0];
	    if (mode == FONT_BEGIN) {
		glBegin(GL_LINE_STRIP);
		glVertex2f((float)ip[1]*STROKE_SCALE,
			   (float)ip[2]*STROKE_SCALE);
		...
		glTranslatef((float)ip[1]*STROKE_SCALE,
			     (float)ip[2]*STROKE_SCALE, 0.0);
	    }
	    ip += 3;
	} while( mode != FONT_ADVANCE);
	glEndList();
    }
    return GL_TRUE;
}
...
GLenum tkCreateOutlineFont(GLuint fontBase)
{
    ...
    for (ip = outlineFont; *ip != END_OF_LIST;) {
	glNewList(fontBase+(unsigned int)*ip++, GL_COMPILE);
	do {
	    mode = ip[0];
	    if (mode == FONT_BEGIN) {
		glBegin(GL_LINE_LOOP);
		glVertex2f((float)ip[1]*OUTLINE_SCALE,
			   (float)ip[2]*OUTLINE_SCALE);
	    } else if (mode == FONT_NEXT) {
		glVertex2f((float)ip[1]*OUTLINE_SCALE,
			   (float)ip[2]*OUTLINE_SCALE);
	    } else if (mode == FONT_END) {
		...
		glTranslatef((float)ip[1]*OUTLINE_SCALE,
			     (float)ip[2]*OUTLINE_SCALE, 0.0);
	    }
	    ip += 3;
	} while( mode != FONT_ADVANCE);
	glEndList();
...
GLenum tkCreateFilledFont(GLuint fontBase)
{
    ...
    for (ip = filledFont; *ip != END_OF_LIST;) {
	glNewList(fontBase+(unsigned int)*ip++, GL_COMPILE);
	do {
	    mode = ip[0];
	    if (mode == FONT_BEGIN) {
		glBegin(GL_TRIANGLE_STRIP);
		glVertex2f((float)ip[1]*FILLED_SCALE,
			   (float)ip[2]*FILLED_SCALE);
	    } else if (mode == FONT_NEXT) {
		glVertex2f((float)ip[1]*FILLED_SCALE,
			   (float)ip[2]*FILLED_SCALE);
	    } else if (mode == FONT_END) {
		...
		glTranslatef((float)ip[1]*FILLED_SCALE,
			     (float)ip[2]*FILLED_SCALE, 0.0);
	    }
	    ip += 3;
	} while( mode != FONT_ADVANCE);
	glEndList();
