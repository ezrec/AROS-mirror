...
static XVisualInfo *FindOverlayVisual(void)
{
    ...
    list[2] = (int)None;
/*
    list[2] = GLX_TRANSPARENT_TYPE;
    list[3] = GLX_TRANSPARENT_INDEX;
    list[4] = None;
*/
    return glXChooseVisual(xDisplay, xScreen, list);
}
...
GLenum tkInitWindow(char *title)
{
	...
	if (!xDisplay) {
	    fprintf(stderr, "gltk: Can't connect to xDisplay!\n");
	    return GL_FALSE;
	}
	if (!glXQueryExtension(xDisplay, &erb, &evb)) {
	    fprintf(stderr, "gltk: No glx extension!\n");
	    return GL_FALSE;
	}
    ...
    if (!w.vInfoMain) {
       if (TK_IS_RGB(w.type)) {
	  fprintf(stderr, "gltk: Couldn't find visual for RGB mode!\n");
       }
       else {
	  fprintf(stderr, "gltk: Couldn't find visual for Color Index mode!\n");
       }
       xDisplay = 0;  /* to prevent crash in tkCloseWindow */
       return GL_FALSE;
    }

    /* make sure we get a reasonable visual type */
    if (TK_IS_RGB(w.type)) {
       /* any visual is OK */
    }
    else {
       ...
       xclass = w.vInfoMain->c_class;
#else
       xclass = w.vInfoMain->class;
#endif
       if (xclass==DirectColor || xclass==TrueColor) {
          fprintf(stderr,
            "gltk: Can't use DirectColor or TrueColor visual in CI mode!\n");
          return GL_FALSE;
       }
    }
    ...
    if (!w.cMain) {
	fprintf(stderr, "gltk: Can't create a context!\n");
	return GL_FALSE;
    }
	...
	if (MaxCmapsOfScreen(scr)==1
	   ...
	   char *private_cmap = getenv("MESA_PRIVATE_CMAP");
	   if (private_cmap) {
...
/****  ad@lms.be:  Added initialization for HP color recovery  *******/
	...
	if (w.vInfoOverlay) {
	    ...
	    fprintf(stderr, "gltk: Can't create a overlay plane!\n");
	}
    }

    if (!glXMakeCurrent(xDisplay, w.wMain, w.cMain)) {
	fprintf(stderr, "gltk: Can't make window current drawable!\n");
	return GL_FALSE;
    }
    XFlush(xDisplay);
...
/* This is here to avoid ANSI C "empty source file" warnings */
static void no_op () {}
