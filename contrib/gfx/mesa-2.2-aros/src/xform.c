...
/*
 * Apply a transformation matrix to an array of [S T R Q] texture coordinates:
 *   for i in 0 to n-1 do   tc[i] = m * tc[i]
 */
void gl_xform_texcoords_4fv( GLuint n, GLfloat tc[][4], const GLfloat m[16] )
{
   GLfloat m0 = m[0], m4 = m[4],  m8 = m[ 8], m12 = m[12];
   GLfloat m1 = m[1], m5 = m[5],  m9 = m[ 9], m13 = m[13];
   GLfloat m2 = m[2], m6 = m[6], m10 = m[10], m14 = m[14];
   ...
   for (i=0;i<n;i++) {
      GLfloat t0 = tc[i][0], t1 = tc[i][1], t2 = tc[i][2], t3 = tc[i][3];
      tc[i][0] = m0 * t0 + m4 * t1 +  m8 * t2 + m12 * t3;
      tc[i][1] = m1 * t0 + m5 * t1 +  m9 * t2 + m13 * t3;
      tc[i][2] = m2 * t0 + m6 * t1 + m10 * t2 + m14 * t3;
      tc[i][3] = m3 * t0 + m7 * t1 + m11 * t2 + m15 * t3;
   }
}
