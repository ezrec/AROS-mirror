...
/*
 * $Log$
 * Revision 1.1  2003/08/09 00:23:22  chodorowski
 * Initial revision
 *
			    ...
			    GLenum primitive )
{
   ...
   if (ctx->Texture.Enabled & TEXTURE_3D) {
      gl_texture_pixels_3d( ctx, n, s, t, u, lambda, red, green, blue, alpha );
   }
   else if (ctx->Texture.Enabled & TEXTURE_2D) {
      gl_texture_pixels_2d( ctx, n, s, t, lambda, red, green, blue, alpha );
   }
   else if (ctx->Texture.Enabled & TEXTURE_1D) {
      gl_texture_pixels_1d( ctx, n, s, lambda, red, green, blue, alpha );
   }
