...
find_variable (catalog_t * context, const char *name, const char *newvalue)
{
  ...
  struct varnames *current, *last, *new;
  ...
  if (newvalue == NULL)
    return (NULL);
  new = (struct varnames *) malloc (sizeof (struct varnames));
...
alloc_catalog (void)
{
  int i;
  catalog_t *c;
  c = (catalog_t *) malloc (sizeof (catalog_t));
  if (c == NULL)
    return NULL;
  for (i = 0; i < CHASHMAX; i++)
    c->root[i] = NULL;
...
catalog_t *
load_catalog (xio_file f, char **error)
{
  int i;
  int line = 1;
  int size;
...
char *
find_text (catalog_t * catalog, char *name)
{
