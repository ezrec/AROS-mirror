		    ...
		    struct palette *palette, void (*flip) (struct image * img), int flags, float pixelwidth, float pixelheight)
{
  ...
  if (lines1 != NULL && (nimages != 2 || lines2 != NULL))
    {
      img->scanline = lines1[1] - lines1[0];
      if (img->scanline < 0)
	img->scanline = -1;
      else
	{
...
create_image_cont (int width, int height, int scanlinesize,
		   ...
		   struct palette *palette, void (*flip) (struct image *img), int flags, float pixelwidth, float pixelheight)
{
  struct image *img = create_image_lines (width, height, nimages, NULL, NULL, palette, flip, flags, pixelwidth, pixelheight);
  int i;
  if (img == NULL)
  {
    return NULL;
  }
  if ((img->currlines = malloc (sizeof (*img->currlines) * height)) == NULL)
    {
      free (img);
      return NULL;
    }
  if (nimages == 2)
    {
      if ((img->oldlines = malloc (sizeof (*img->oldlines) * height)) == NULL)
	{
	  free (img->currlines);
	  free (img);
	  return NULL;
...
{
  unsigned char *data = calloc (((width + 3) & ~3) * height, bytesperpixel (palette->type));
  unsigned char *data1 = (nimages == 2 ? calloc (((width + 3) & ~3) * height, bytesperpixel (palette->type)) : NULL);
  struct image *img;
  if (data == NULL)
    return (NULL);
...
clear_image (struct image *img)
{
  ...
  if (!width)
    width = (img->width + 7) / 8;
  for (i = 0; i < img->height; i++)
    memset (img->currlines[i], img->palette->pixels[0], width);
}
