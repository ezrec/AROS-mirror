		   ...
		   x, register int y)
{
  ...
  register char *calc;
  int peri = 0;
  unsigned int c = calculatepixel (x, y, 1);
  unsigned int w = 0;
...
#define ethreads 1
static INLINE void
tracepoint (int xp, int yp, int dir, int color, int xstart, int xend, int ystart, int yend)
  REGISTERS (3);
     static INLINE void
       tracepoint (int xp, int yp, int dir, int color, int xstart, int xend, int ystart, int yend)
{
  ...
  if (!(*calc & (CALCULATED | CALCULATING)))
    {
      *calc |= CALCULATING;
      mycolor = calculatepixel (xp, yp, periodicity);
...
#endif
static void
dosymetries (int x1, int x2, int y1, int y2, int xsym, int cx1, int cx2, int cy1, int cy2)
{
  if (cx1 != x1)
    {
      register int y;
