...
create_rgb_table (unsigned char table[32][32][32], struct palette *palette)
{
  ...
  next=malloc(sizeof(short)*32*32*32);
  if (col_diff[0][1] == 0)
    bestfit_init ();
  ...
  if (i->image->bytesperpixel <= 1)
	...
	case C256:
	  ...
	  for (r = 0; r < MSIZE; r++)
	    for (g = 0; g < MSIZE; g++)
	      {
		s->rmat[r][g] = (matrix[r][g] - 128) * 256 / red / 256;
		s->gmat[r][g] = (matrix[(r + 3) % MSIZE][(g + 6) % MSIZE] - 128) * 256 / green / 256;
		s->bmat[r][g] = (matrix[(r + 6) % MSIZE][(g + 3) % MSIZE] - 128) * 256 / blue / 256;
	      }
	  break;
#ifdef SFIXEDCOLOR
	case FIXEDCOLOR:
	  ...
	  for (r = 0; r < MSIZE; r++)
	    for (g = 0; g < MSIZE; g++)
	      {
		s->rmat[r][g] = (matrix[r][g] - 128) * red / 256;
		s->gmat[r][g] = (matrix[(r + 3) % MSIZE][(g + 6) % MSIZE] - 128) * green / 256;
		s->bmat[r][g] = (matrix[(r + 6) % MSIZE][(g + 3) % MSIZE] - 128) * blue / 256;
	      }
	  break;
#endif
#ifdef BITMAPS
...
getinstance (struct filteraction *a)
{
  ...
  struct ditherdata *i = calloc (sizeof (*i), 1);
  i->palette = createpalette (0, 65536, TRUECOLOR, 0, 65536, NULL, NULL, NULL, NULL);
  f->data = i;
...
converttbitmap (void *data, struct taskinfo *task, int r1, int r2)
{
  ...
  for (i = r1; i < r2; i++)
	...
	case MBITMAP:
	  for (; src < srcend; dest++)
	    {
	      if (inten (src[0]) + matrix[x][0] >= 0)
		val = 128;
	      else
		val = 0;
	      if (inten (src[1]) + matrix[x][1] >= 0)
		val |= 64;
	      if (inten (src[2]) + matrix[x][2] >= 0)
		val |= 32;
	      if (inten (src[3]) + matrix[x][3] >= 0)
		val |= 16;
	      if (inten (src[4]) + matrix[x][4] >= 0)
		val |= 8;
	      if (inten (src[5]) + matrix[x][5] >= 0)
		val |= 4;
	      if (inten (src[6]) + matrix[x][6] >= 0)
		val |= 2;
	      if (inten (src[7]) + matrix[x][7] >= 0)
		val |= 1;
	      src += 8;
	      *dest = val;
	    }
...
struct filteraction truecolor_filter =
{
  "Truecolor emulator",
  "truecolor",
  0,
...
static struct filter *
getinstancefixed (struct filteraction *a)
{
  struct filter *f = createfilter (a);
  struct fixeddata *i = calloc (sizeof (*i), 1);
...
{
  struct fixeddata *i = f->data;
  destroypalette (i->palette);
  free (f->data);
  destroyinheredimage (f);
  free (f);
}
struct filteraction fixedcolor_filter =
{
  "Palette emulator",
  "fixedcolor",
  0,
...
static struct filter *
getinstancebitmap (struct filteraction *a)
{
  struct filter *f = createfilter (a);
  struct bitmapdata *i = calloc (sizeof (*i), 1);
  i->palette = createpalette (0, 256, C256, 0, 256, NULL, mysetcolor, NULL, NULL);
  i->active = -1;
  f->data = i;
  f->name = "Palete emulator";
...
convertbitmap (void *data, struct taskinfo *task, int r1, int r2)
{
  ...
  for (i = r1; i < r2; i++)
	...
	case MBITMAP:
	  for (; src < srcend; dest++)
	    {
	      if (intensity[src[0]] + matrix[x][0] >= 0)
		val = 128;
	      else
		val = 0;
	      if (intensity[src[1]] + matrix[x][1] >= 0)
		val |= 64;
	      if (intensity[src[2]] + matrix[x][2] >= 0)
		val |= 32;
	      if (intensity[src[3]] + matrix[x][3] >= 0)
		val |= 16;
	      if (intensity[src[4]] + matrix[x][4] >= 0)
		val |= 8;
	      if (intensity[src[5]] + matrix[x][5] >= 0)
		val |= 4;
	      if (intensity[src[6]] + matrix[x][6] >= 0)
		val |= 2;
	      if (intensity[src[7]] + matrix[x][7] >= 0)
		val |= 1;
	      src += 8;
	      *dest = val;
	    }
...
{
  struct bitmapdata *i = f->data;
  destroypalette (i->palette);
  free (f->data);
  destroyinheredimage (f);
  free (f);
}
struct filteraction bitmap_filter =
{
  "Palette emulator",
  "bitmap",
  0,
