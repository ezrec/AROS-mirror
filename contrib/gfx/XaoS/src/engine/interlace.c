...
{
  int x;
  struct intdata *d = f->data;
  pixel_t **lines1 = malloc (sizeof (*lines1) * i->image->height / 2),
  **lines2 = malloc (sizeof (*lines2) * i->image->height / 2);
  if (lines1 == NULL)
    return 0;
  inhermisc (f, i);
  d->first = 1;
  ...
  f->childimage = i->image = create_image_lines (i->image->width, (i->image->height) / 2, 2, lines1, lines2, i->image->palette, NULL, FREELINES, f->image->pixelwidth, f->image->pixelheight * 2);
  if (i->image == NULL)
    {
      free (lines1);
      free (lines2);
...
struct filteraction interlace_filter =
{
  "Interlace filter",
  "interlace",
  0,
