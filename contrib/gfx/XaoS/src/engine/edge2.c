...
initialize (struct filter *f, struct initdata *i)
{
  ...
  /*in/out coloring modes looks better in iter modes. This also saves some
     memory in truecolor. */
  if (f->data != NULL)
    destroypalette (f->data);
  f->data = createpalette (0, 65536, i->image->bytesperpixel <= 1 ? SMALLITER : LARGEITER, 0, 65536, NULL, NULL, NULL, NULL);
  if (!inherimage (f, i, TOUCHIMAGE | NEWIMAGE, 0, 0, f->data, 0, 0))
    return 0;
...
struct filteraction edge2_filter =
{
  "Edge detection2",
  "edge2",
  0,
