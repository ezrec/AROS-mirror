...
requirement (struct filter *f, struct requirements *r)
{
  ...
  r->nimages = 1;
  r->flags &= ~(IMAGEDATA);
  r->supportedmask = GRAYSCALE | C256 | TRUECOLOR24 | TRUECOLORS | HICOLOR | REALCOLOR;
...
#define SSTEP2 (32*8/256)
  if (datalost (f, i) || i->image->version != f->imageversion)
	...
	s->savedpalette = clonepalette (i->image->palette);
      ...
      if (i->image->palette->type & (C256 | HICOLOR | REALCOLOR))
	{
	  for (x = 0; x < 256; x++)
	    {
	  ...
	  for (x = 0; x < 256; x++)
	    {
	      ...
	      if (dist > 255)
		dist = 255;
	      if(i->image->palette->type==TRUECOLORMI)
	      s->table[x] = (dist + (dist << 8) + (dist << 16))<<8; else
	      s->table[x] = dist + (dist << 8) + (dist << 16);
	    }
	  for (x = 256; x < 512; x++)
	    {
  ...
  s->xdist = 0.1 / i->image->pixelwidth;
  s->ydist = 0.1 / i->image->pixelwidth;
  if (s->xdist < 1)
    s->xdist = 1;
  if (s->ydist < 1)
    s->ydist = 1;
  if (!inherimage (f, i, TOUCHIMAGE, i->image->width + s->xdist, i->image->height + s->ydist, s->palette, 0, 0))
    return 0;
  clear_image (f->image);
  ...
  for (i = r1; i < r2; i++)
    {
      ...
      while (src < srcend)
	{
	  *dest = *(dest + 1) = *(dest + 2) = table[((int) *src2 - (int) *src) & 511];
	  src++;
	  src2++;
	  dest += 3;
	}
...
struct filteraction emboss_filter =
{
  "Emboss",
  "emboss",
  0,
