...
initialize (struct filter *f, struct initdata *i)
{
  ...
  if (!inherimage (f, i, TOUCHIMAGE | NEWIMAGE, size / pixelsize + 1, size / pixelsize + 1, NULL, pixelsize, pixelsize))
    return 0;
...
convertup (struct filter *f, int *x, int *y)
{
  ...
  *x = f->image->width / 2 + xd / f->image->pixelwidth;
  *y = f->image->height / 2 + yd / f->image->pixelheight;
  if (f->next != NULL)
...
convertdown (struct filter *f, int *x, int *y)
{
  ...
  *x = f->childimage->width / 2 + xd / f->childimage->pixelwidth;
  *y = f->childimage->height / 2 + yd / f->childimage->pixelheight;
  if (f->previous != NULL)
...
struct filteraction rotate_filter =
{
  "Image rotation",
  "rotate",
  0,
