...
allocgenerictruecolor (struct palette *palette, int init, int r, int g, int b)
{
  ...
  if (init)
    palette->size = 0;
  else if (palette->size >= palette->end - palette->start)
    return 0;
...
allocgeneric (struct palette *palette, int init, int r, int g, int b)
{
  if (init)
    ...
    return -1;
  palette->pixels[palette->size] = palette->size + palette->start;
  palette->rgb[palette->size + palette->start][0] = r;
  palette->rgb[palette->size + palette->start][1] = g;
  palette->rgb[palette->size + palette->start][2] = b;
  palette->size++;
...
{
  palette->setpalette (palette, palette->start, palette->size + palette->start, palette->rgb + palette->start);
}
static void
...
cyclecolorsgeneric (struct palette *pal, int direction)
{
  ...
  pal->setpalette (pal, pal->start, pal->size + pal->start, pal->rgb + pal->start);
}
	       ...
	       void (*cyclecolors) (struct palette * pal, int direction))
{
  ...
  struct palette *palette = calloc (sizeof (struct palette), 1);
  ...
  switch (type)
    ...
    case MIBITMAP:
      ...
      start = 0;
      maxentries = 2;
      palette->rgb = calloc (sizeof (*palette->rgb), end);
      if (palette->rgb == NULL)
	{
	  free (palette);
	  return NULL;
  ...
  if (end < 256)
    palette->pixels = calloc (sizeof (*palette->pixels), 256);
  else
    palette->pixels = calloc (sizeof (*palette->pixels), end);
...
{
  struct palette *pal = createpalette (palette->start, palette->end, palette->type, palette->flags, palette->maxentries, palette->alloccolor, palette->setpalette, palette->allocfinished, palette->cyclecolors);
...
restorepalette (struct palette *dest, struct palette *src)
{
  ...
  for (i = 0; i < src->size; i++)
	...
	case LIBITMAP:
	  ...
	  break;
	case HICOLOR:
	  r = ((src->pixels[i] >> 11) & 31) << 3;
	  g = ((src->pixels[i] >> 5) & 63) << 2;
	  b = (src->pixels[i] & 31) << 3;
	  break;
	case REALCOLOR:
	  r = ((src->pixels[i] >> 10) & 31) << 3;
	  g = ((src->pixels[i] >> 5) & 31) << 3;
	  b = (src->pixels[i] & 31) << 3;
	  break;
	case TRUECOLORMI:
	  r = (src->pixels[i] >> 8) & 255;
	  g = (src->pixels[i] >> 16) & 255;
	  b = (src->pixels[i] >> 24) & 255;
	  break;
	case TRUECOLOR:
	case TRUECOLOR24:
	  r = (src->pixels[i] >> 16) & 255;
	  g = (src->pixels[i] >> 8) & 255;
	  b = src->pixels[i] & 255;
	}
      if(dest->size>=dest->maxentries) break;
      if (dest->alloccolor (dest, i == 0, r, g, b) == -1)
	break;
    }
...
static int allocate (int r, int g, int b, int init)
{
  ...
  if (!init && context->size == maxentries)
    return 0;
  if ((context->alloccolor (context, init, (int) r, (int) g, (int) b)) == -1)
    {
      return 0;
    }
...
mksmooth (int nsegments, int setsegments)
{
  ...
  for (i = 0; i < setsegments; i++)
	...
	{
	  segmentsize1 = context->maxentries - context->size - 2;
	}
...
randomize_segments2 (int whitemode, int nsegments)
{
  ...
  for (i = 0; i < nsegments; i++)
	...
	colors[i][0] = XaoS_random () % 256,
	  colors[i][1] = XaoS_random () % 256,
	  colors[i][2] = XaoS_random () % 256;
    }
  colors[i - 1][0] = colors[0][0];
  colors[i - 1][1] = colors[0][1];
  colors[i - 1][2] = colors[0][2];
...
randomize_segments (int whitemode, int nsegments)
{
  ...
  if (whitemode)
      ...
      for (i = 0; i < nsegments; i += 2)
	{
	  ...
	  if (i + 1 < nsegments)
	    colors[i + 1][0] = XaoS_random () % 35,
	      colors[i + 1][1] = XaoS_random () % 35,
	      colors[i + 1][2] = XaoS_random () % 35;
	}
    }
  else
    {
      for (i = 0; i < nsegments; i += 2)
	{
	  colors[i][0] = XaoS_random () % 35,
	    colors[i][1] = XaoS_random () % 35,
	    colors[i][2] = XaoS_random () % 35;
	  if (i + 1 < nsegments)
	    colors[i + 1][0] = XaoS_random () % 256,
	      colors[i + 1][1] = XaoS_random () % 256,
	      colors[i + 1][2] = XaoS_random () % 256;
	}
    }
  colors[i - 1][0] = colors[0][0];
  colors[i - 1][1] = colors[0][1];
...
#define rrandom(i) ((int)((XaoS_random()/(double)MYLONG_MAX)*(i)))
...
shiftpalette (struct palette *c, int shift)
{
  ...
  if (c->type & (TRUECOLORS | TRUECOLOR24 | HICOLOR | REALCOLOR))
    {
      int i;
      int i3;
      int *co;
...
mkrgb (struct palette *c)
{
  ...
  if (c->flags & UNKNOWNENTRIES)
    ...
    {
      number_t n = pow (c->maxentries / (0.5 * 0.2 * 0.3), 1.0 / 3);
      green = n * 0.5;
      blue = n * 0.2;
      red = n * 0.3;
      while ((blue + 1) * red * green < c->maxentries)
	blue++;
      while ((red + 1) * blue * green < c->maxentries)
	red++;
      while ((green + 1) * blue * red < c->maxentries)
	green++;
