...
requirement (struct filter *f, struct requirements *r)
{
  ...
  r->nimages = 1;
  r->flags &= ~IMAGEDATA;
  r->supportedmask = MASK1BPP | MASK2BPP | MASK4BPP;
  return (f->next->action->requirement (f->next, r));
}
static int
initialize (struct filter *f, struct initdata *i)
{
  struct siterdata *s = f->data;
  inhermisc (f, i);
...
getinstance (struct filteraction *a)
{
  ...
  struct siterdata *i = calloc (sizeof (*i), 1);
...
struct filteraction smalliter_filter =
{
