...
requirement (struct filter *f, struct requirements *r)
{
  ...
  r->nimages = 1;
  r->flags &= ~IMAGEDATA;
  r->supportedmask = C256 | TRUECOLORS | TRUECOLOR24 | HICOLOR | REALCOLOR | GRAYSCALE;
...
getinstance (struct filteraction *a)
{
  ...
  i->minc = 0;
  i->savedpalette = NULL;
  i->palette = createpalette (0, 65536, IMAGETYPE, 0, 65536, NULL, NULL, NULL, NULL);
  f->data = i;
...
{
  struct stereogramdata *i = f->data;
...
doit (struct filter *f, int flags, int time)
{
...
#ifdef HAVE_ALLOCA
  table = alloca (sizeof (int) * NCOLORS);
#else
  table = malloc (sizeof (int) * NCOLORS);
#endif
  ...
  for (i = 0; i < y; i++)
	...
	dist = y - 1;
      ...
      table[i] = EYE_DIST * dist / (dist + USER_DIST) / PIXELWIDTH;
    }
  drivercall (*f->image,
...
struct filteraction stereogram_filter =
{
