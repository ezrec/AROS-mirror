...
#define addstack(sx,sy,d,c,periodicity) { \
  ...
  if(page==npages[nstack]) starts[nstack][npages[nstack]]=malloc(sizeof(struct stack)*PAGESIZE),npages[nstack]++;\
...
boundarytrace (int x1, int y1, int x2, int y2, number_t * xpos, number_t * ypos)
{
  ...
  int ydiv;
#ifdef HAVE_ALLOCA
  calculated = alloca (cimage.width * (y2 + 1));
#else
  calculated = malloc (cimage.width * (y2 + 1));
#endif
  if (calculated == NULL)
    {
      return 0;
  ...
  if (cursymetry.xsym < cfractalc.rs.nc || cursymetry.xsym > cfractalc.rs.mc)
    ...
    xsym = 0.5 + ((cursymetry.xsym - cfractalc.rs.nc) * cimage.width / (cfractalc.rs.mc - cfractalc.rs.nc));
  if (cursymetry.ysym < cfractalc.rs.ni || cursymetry.ysym > cfractalc.rs.mi)
    ysym = -10;
  else
    ysym = 0.5 + ((cursymetry.ysym - cfractalc.rs.ni) * cimage.height / (cfractalc.rs.mi - cfractalc.rs.ni));
  ydiv = 0.5 + ((-cfractalc.rs.ni) * cimage.height / (cfractalc.rs.mi - cfractalc.rs.ni));
  if (xsym > x1 && xsym < x2)
    {
      if (xsym - x1 > x2 - xsym)
	cx1 = x1, cx2 = xsym;
...
#endif
  drivercall (cimage,
	      dosymetries8 (x1, x2, y1, y2, xsym, cx1, cx2, cy1, cy2),
	      dosymetries16 (x1, x2, y1, y2, xsym, cx1, cx2, cy1, cy2),
	      dosymetries24 (x1, x2, y1, y2, xsym, cx1, cx2, cy1, cy2),
	      dosymetries32 (x1, x2, y1, y2, xsym, cx1, cx2, cy1, cy2))
    if (cy1 != y1)
    {
      ...
      while (yy1 < yy2)
	{
	  memcpy (cimage.currlines[yy1] + xstart, cimage.currlines[yy2] + xstart, xsize);
	  yy1++;
	  yy2--;
	}
    }
