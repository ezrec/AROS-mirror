		      ...
		      struct SYMBOL *s)
{
	...
	case B_SINGLE:
		PUT3("%.1f %.1f %.1f bar", h, x, y); break;
	case B_DOUBLE:
		PUT3("%.1f %.1f %.1f dbar", h, x, y); break;
	case B_LREP:
	case B_THICK_THIN:
		PUT3("%.1f %.1f %.1f fbar1", h, x, y); break;
	case B_RREP:
	case B_THIN_THICK:
		PUT3("%.1f %.1f %.1f fbar2", h, x, y); break;
	case B_DREP:
		PUT3("%.1f %.1f %.1f fbar1 ", h, x - 1, y);
		PUT3("%.1f %.1f %.1f fbar2", h, x + 1, y); break;
	case B_INVIS: break;
	case B_DASH:
...
static char *acc2_tb[] = { "", "gsh0", "gnt0", "gft0", "gds0", "gdf0" };
	...
	for (g = s->grace; ; g = g->next) {
		...
		if (s->grace->next != 0) {
			...
			} else if (s->as.u.grace_start.sappo)
				c = 'a';
			else	c = ' ';
			PUT4("%.1f %.1f %.1f gn1%c ",
			     lg, g->x, g->y + staffb, c);
	...
	if (s->grace->next != 0) {
		...
		px0 = s->grace->x + GSTEM_XOFF;
		pxn = last->x + GSTEM_XOFF;
		PUT5("%.1f %.1f %.1f %.1f gbm%d ",
		     px0, a * px0 + b + staffb,
		     pxn, a * pxn + b + staffb,
		     voice_tb[s->voice].bagpipe ? 3 : 2);
	}
...
/*fixme: have an other symbol flag */
	...
	/* draw the head */
	PUT2("%.1f %.1f ", x + s->shhd[m], (float) y + staffb);
	switch (s->head) {
	case H_SQUARE:
...
/*	}	*/
	...
	if (k2->type == NOTE) {
		...
		/* special case when 2 notes and different stem directions */
		if (k1->type == NOTE
		    && k1->stem != k2->stem) {
...
#endif
		y1 += staff_tb[k1->staff].y;
		y2 += staff_tb[k2->staff].y;
		...
		case FMTCHG:
			if (s->u == STBRK) {
				...
				PUT4("currentgray 1.0 setgray"
				     ...
				     " 0 exch neg rlineto neg 0 rlineto fill"
				     " setgray\n",
				     dx, dy, x, y);
...
#if 1
/*fixme: should remove the %%left/right margins */
			break;
#else
			/* fall thru */
#endif
