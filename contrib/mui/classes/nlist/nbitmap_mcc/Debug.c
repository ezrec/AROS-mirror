...
void SetupDebug(void)
{
  ...
  kprintf("** NBitmap.mcc v" LIB_REV_STRING " startup ***********************\n");
    ...
    while((tok = strtok(debug, ", ;")) != NULL)
    {
      ...
      if(tok[0] == '@')
      {
        ...
        if(tok[1] == '!')
        {
          ...
          for(i = 0; dbclasses[i].token != NULL; i++)
          {
      ...
      else
      {
        ...
        if(tok[0] == '!')
        {
          for(i = 0; dbflags[i].token != NULL; i++)
          {
  ...
  kprintf("set debug classes/flags (env:nbitmap.mcc.debug): %08lx/%08lx\n", debug_classes, debug_flags);
