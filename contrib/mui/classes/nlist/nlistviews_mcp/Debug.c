...
void SetupDebug(void)
{
  ...
  kprintf("** NListviews.mcp v" LIB_REV_STRING " startup ***********************\n");
  kprintf("Initializing runtime debugging:\n");

  if(GetVar("nlistviews.mcp.debug", var, sizeof(var), 0) > 0)
  {
    char *tok;
    char *debug = var;
  ...
  kprintf("set debug classes/flags (env:nlistviews.mcp.debug): %08lx/%08lx\n", debug_classes, debug_flags);
