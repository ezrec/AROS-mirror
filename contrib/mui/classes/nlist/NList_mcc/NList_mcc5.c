...
/*
 *      if (data->DRAW && ((_window(obj)->Flags & WFLG_REFRESHBITS) == WFLG_SIMPLE_REFRESH))
 *        data->do_draw_all = TRUE;
 */
...
void NL_Stack_Alert(Object *obj,struct NLData *data,LONG why)
{
  ...
  if (!data->NList_SPLower)
    return;
  if (data->NList_Process->pr_Task.tc_Node.ln_Name)
    taskname = data->NList_Process->pr_Task.tc_Node.ln_Name;
  else
