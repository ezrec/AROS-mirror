...
void SetupDebug(void)
{
  ...
  kprintf("** NListtree.mcc v" LIB_REV_STRING " startup ***********************\n");
  kprintf("Initializing runtime debugging:\n");

  if(GetVar("nlisttree.mcc.debug", var, sizeof(var), 0) > 0)
  {
    char *tok;
    char *debug = var;
  ...
  kprintf("set debug classes/flags (env:nlisttree.mcc.debug): %08lx/%08lx\n", debug_classes, debug_flags);
