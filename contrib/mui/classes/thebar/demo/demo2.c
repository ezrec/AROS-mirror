...
struct MUIS_TheBar_Button buttons[] =
{
    ...
    {2, 1,  "_Stop", "Stop the connection."},
    {MUIV_TheBar_ButtonSpacer, -1},
    {1, 2,  "Sa_ve", "Save the disc."},
    {MUIV_TheBar_BarSpacer, -1},
    {3,  3, "_Disc", "Disc page.", 0, (1<<4)|(1<<5)},
    {4,  4, "_Matches", "Matches page.", MUIV_TheBar_ButtonFlag_Selected, (1<<3)|(1<<5)},
    {5,  5, "_Edit", "Edit page.", 0, (1<<3)|(1<<4)},
    {MUIV_TheBar_End, -1},
};
...
char *barPoss[]     = {"Left","Center","Right",NULL};
            ...
            End)
        {
            ...
            while ((id = DoMethod(app,MUIM_Application_NewInput,&sigs))!=MUIV_Application_ReturnID_Quit)
            {
                if (id==TAG_USER)
                {
                    ...
                    rem = 0;
                    if (rbV) rem |= MUIV_TheBar_Remove_BarSpacers;
                    if (rsV) rem |= MUIV_TheBar_Remove_ButtonSpacers;
                    if (riV) rem |= MUIV_TheBar_Remove_ImageSpacers;
                    if (raV) rem |= MUIV_TheBar_Remove_All;
