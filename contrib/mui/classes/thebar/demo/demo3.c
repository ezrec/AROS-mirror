...
{
    {0,     0, "_Get", "Get the disc."},
    {1,     1, "Sa_ve", "Save the disc."},
    {2,     2, "_Stop", "Stop the connection."},
    {MUIV_TheBar_BarSpacer,-1},
    {3,     3, "_Disc", "Disc page."},
    {MUIV_TheBar_ButtonSpacer,-1},
    {0,     4, "_Stop2", "Stop2 the connection."},
    {4,     5, "_Matches", "Matches page.", MUIV_TheBar_ButtonFlag_Immediate|MUIV_TheBar_ButtonFlag_Selected, 1<<6},
    {MUIV_TheBar_ImageSpacer,-1},
    {5,     6, "_Edit", "Edit page.", MUIV_TheBar_ButtonFlag_Immediate, 1<<5},
    {MUIV_TheBar_End},
};

char *rowss[]       = {"0","1","2","3","6",NULL};
char *colss[]       = {"0","1","2","3","4","5","6",NULL};
char *viewModes[]   = {"Images and text","Images","Text",NULL};
char *buttonsPoss[] = {"Left","Center","Right",NULL};
    ...
    if (MUIMasterBase = OpenLibrary("muimaster.library",19))
    {
            ...
            End)
        {
            ...
            while ((id = DoMethod(app,MUIM_Application_NewInput,&sigs))!=MUIV_Application_ReturnID_Quit)
            {
                if (id==TAG_USER)
                {
                    ULONG c, r, vm, bp, lp, br, su, ra, sc, k, bs, fi, fr, hl0, sl4, rbl, rsl, ril, rem;
                    ...
                    rem = 0;
                    if (rbl) rem |= MUIV_TheBar_Remove_BarSpacers;
                    if (rsl) rem |= MUIV_TheBar_Remove_ButtonSpacers;
                    if (ril) rem |= MUIV_TheBar_Remove_ImageSpacers;
                    ...
                    SetAttrs(sb,MUIA_TheBar_Columns,      c,
                                ...
                                MUIA_TheBar_Remove,       rem,
                                MUIA_TheBar_LabelPos,     lp,
                                TAG_DONE);

                    DoMethod(sb,MUIM_TheBar_SetAttr,0,MUIV_TheBar_Attr_Hide,hl0);
                    DoMethod(sb,MUIM_TheBar_SetAttr,4,MUIV_TheBar_Attr_Sleep,sl4);

                    DoMethod(sb,MUIM_TheBar_SetAttr,6,MUIV_TheBar_Attr_Disabled,fr);
