...
#undef DebugPrintF
#define kprintf ((struct ExecIFace *)((*(struct ExecBase **)4)->MainInterface))->DebugPrintF

/*
 *              These four pointers are imported from PATCHCODE.S, which is
 *              also where the real code size is determined.
 */
#define CODESIZE                128      /* Must equal PatchCode_End - PatchCode_Start   */
...
#define MarkCallAddr            ULONG volatile callmarker[1],linkreg
//#define CallAddr                        (callmarker[6])

/*
 * We are using a little hack here. The asm stub is moving the original
 * lr to ctr and that register is not touched by the variable initialization.
 * So when we want our caller, we can simply fetch the saved value
 */

#define MarkCallAddr2 ULONG linkreg; __asm__ volatile ( "mfctr %0" : "=r" (linkreg));
#define CallAddr                        linkreg
...
typedef struct Patch {
        ULONG   code[CODESIZE/4];               /* Assembly code to handle patch        */
        ...
        ULONG   usecount;                               /* No. of tasks currently in code       */
        void    *sysbase;                               /* Points to ExecBase                           */
        ULONG   stackneeded;                    /* #bytes free stack required           */
        ULONG  library;                                /* Library to patch                                     */
        WORD    offset;                                 /* Offset into library                          */
        WORD    pad;                                    /* Keep structure longword-aligned      */
} Patch;

#define PATCH_DEF(lib,iface ,func)     { lib, offsetof(struct iface , func), (FuncPtr)New_##func }
#define PATCH_END                       { 0, 0, 0 }

#define OBSOLETENewLoadSeg Private6
...
struct PatchInitTable {
        ...
        PATCH_DEF(      EXEC_LIB,               ExecIFace,       FindPort                ),      /* GID_FINDPORT                 */
        PATCH_DEF(      EXEC_LIB&0,               ExecIFace,       FindResident    ),      /* GID_FINDRESIDENT             */
        PATCH_DEF(      EXEC_LIB,               ExecIFace,       FindSemaphore   ),      /* GID_FINDSEMAPHORE    */
        PATCH_DEF(      EXEC_LIB,               ExecIFace,       FindTask                ),      /* GID_FINDTASK                 */
        PATCH_DEF(      INTUITION_LIB,          IntuitionIFace,  LockPubScreen   ),      /* GID_LOCKSCREEN               */
        PATCH_DEF(      EXEC_LIB,               ExecIFace,       OpenDevice              ),      /* GID_OPENDEVICE               */
        PATCH_DEF(      GRAPHICS_LIB,           GraphicsIFace,   OpenFont                ),      /* GID_OPENFONT                 */
        PATCH_DEF(      EXEC_LIB,               ExecIFace,       OpenLibrary             ),      /* GID_OPENLIBRARY              */
        PATCH_DEF(      EXEC_LIB,               ExecIFace,       OpenResource    ),      /* GID_OPENRESOURCE             */
        PATCH_DEF(      ICON_LIB,               IconIFace,       FindToolType    ),      /* GID_READTOOLTYPES    */
        PATCH_DEF(      EXEC_LIB&0,               ExecIFace,       PutMsg          ),      /* GID_SENDREXX         */

        PATCH_DEF(      DOS_LIB,                DOSIFace,        CurrentDir              ),      /* GID_CHANGEDIR                */
        PATCH_DEF(      DOS_LIB,                DOSIFace,        DeleteFile              ),      /* GID_DELETE                   */
        PATCH_DEF(      DOS_LIB,                DOSIFace,        Execute                 ),      /* GID_EXECUTE                  */
        PATCH_DEF(      DOS_LIB,                DOSIFace,        GetVar                  ),      /* GID_GETVAR                   */
        PATCH_DEF(      DOS_LIB&0,                DOSIFace,        LoadSeg                 ),      /* GID_LOADSEG                  */
        PATCH_DEF(      DOS_LIB,                DOSIFace,        Lock                    ),      /* GID_LOCKFILE                 */
        PATCH_DEF(      DOS_LIB,                DOSIFace,        CreateDir               ),      /* GID_MAKEDIR                  */
        PATCH_DEF(      DOS_LIB,                DOSIFace,        MakeLink                ),      /* GID_MAKELINK                 */
        PATCH_DEF(      DOS_LIB,                DOSIFace,        Open                    ),      /* GID_OPENFILE                 */
        PATCH_DEF(      DOS_LIB,                DOSIFace,        Rename                  ),      /* GID_RENAME                   */
        PATCH_DEF(      DOS_LIB,                DOSIFace,        RunCommand              ),      /* GID_RUNCOMMAND               */
        PATCH_DEF(      DOS_LIB,                DOSIFace,        SetVar                  ),      /* GID_SETVAR                   */
        PATCH_DEF(      DOS_LIB&0,                DOSIFace,        SystemTagList   ),      /* GID_SYSTEMTAGLIST            */

        /*
         *              Now the paired functions that track their partner's state
         */
        PATCH_DEF(      ICON_LIB,               IconIFace,       MatchToolValue  ),      /* GID_READTOOLTYPES2   */
        PATCH_DEF(      DOS_LIB&0,                DOSIFace,        OBSOLETENewLoadSeg              ),      /* GID_LOADSEG2                 */
        PATCH_DEF(      DOS_LIB,                DOSIFace,        FindVar                 ),      /* GID_GETVAR2                  */
        PATCH_DEF(      DOS_LIB,                DOSIFace,        DeleteVar               ),      /* GID_SETVAR2                  */
        PATCH_DEF(      DOS_LIB,                DOSIFace,        AddDosEntry         ),  /* GID_ADDDOSENTRY              */
...
extern void AtomicDecrease(ULONG *counter);
...
int InitPatches(void)
{
        ...
        if (CODESIZE != PatchCode_Size/* || STACKADJUST != PatchCode_StackAdjust*/) {
                Printf("In patches.c, adjust CODESIZE by %ld and STACKADJUST by %ld\n",
                                PatchCode_Size-CODESIZE,0 /* PatchCode_StackAdjust-STACKADJUST*/ );
                return (FALSE);
        }
        LibList[0] = NULL; // To be able to disable patches easily
        LibList[DOS_LIB]                = IDOS;
        LibList[EXEC_LIB]               = IExec;
        LibList[INTUITION_LIB]  = IIntuition;
        LibList[GRAPHICS_LIB]   = IGraphics;
        LibList[ICON_LIB]               = IIcon;
        ...
        if (!PatchAnchor) {
                ...
                PatchAnchor = AllocMem(sizeof(PatchAnchorData) + PatchInitSize,
                                                           MEMF_ANY | MEMF_CLEAR | MEMF_EXECUTABLE);
                if (!PatchAnchor) {
                        Permit();
                        return (FALSE);
                }
         ...
         */
        SnoopTask       = FindTask(NULL);
        InputTask       = FindTask("input.device");
        /*
         *              TR: moved here mainly to please PatchWork.
         */
        Permit();
...
void UpdatePatches(void)
{
        ...
        for (p = PatchList+1; p->offset; p++) {
                ...
                if (p->origfunc == NULL && func_enabled != PATCH_DISABLED) {
                         ...
                         */
                        p->origfunc = (FuncPtr)SetMethod(lib, p->offset, (FuncPtr)p);
                         ...
                         *              calls this function.
                         */
                        FuncPtr curfunc = (FuncPtr)SetMethod(lib, p->offset, p->origfunc);

                        if (curfunc != (FuncPtr)p) {
                                /*
                                 *              Uhoh -- someone else patched us!
                                 */
                                SetMethod(lib, p->offset, (FuncPtr)curfunc);
                        } else {
                                /*
...
void CleanupPatches(void)
{
        ...
        if (BackgroundProc) {
                 ...
                 *              for it to respond
                 */
#warning TODO: Messages is placed on the stack which is forbidden
                PatternMsg pmsg;

                pmsg.type                = QUIT_MSG;
                pmsg.sigmask                     = SIGF_SINGLE;
                pmsg.task                                = FindTask(NULL);
...
void SetPattern(char *pattern, int ignorewb)
{
        ...
        if (ignorewb) {
                if (*pattern == '\0')
                        ...
                        mysprintf(PatternString, PAT_COMPLEX_NOT_EXCLUDE, &pattern[1]);
                else
        ...
        /*
         *              Now, due to a bug in AmigaDOS's ParsePatternNoCase (before V39),
         *              we have to convert the string to upper case first. (The bug
...
#define DBP(s)
         ...
         */
        BackgroundPort.mp_SigTask = FindTask(NULL);
...
char *MyNameFromLock(BPTR lock, char *filename, char *buf, int maxlen)
{
        Process *myproc = (Process *)FindTask(NULL);
        int pos = maxlen - 1;
...
{
        Process *pr = (Process *)FindTask(NULL);
        char *upper;
        char *lower;
        ULONG total;
        ULONG avail;
                          ...
                          char *filename, char *options, int expandname, ...)
{
        ...
        if (!filename)  filename = MSG(MSG_NULLSTR);
 ...
 */
ULONG ASM New_FindPort(APTR iface, char *name)
{
        ULONG (*origfunc)(APTR,char *) = (ULONG (*)())PatchList[GID_FINDPORT].origfunc;
...
ULONG ASM New_FindResident(reg_a1 char *name, reg_a6 void *libbase)
{
        ...
        if (FindTask(NULL) == RealRamLibTask)
                JumpOrigFunc(0);
 ...
 */
ULONG ASM New_FindSemaphore(APTR iface, char *name)
{
        ULONG (*origfunc)(APTR,char *) = (ULONG (*)())PatchList[GID_FINDSEMAPHORE].origfunc;
        int onlyfails = OnlyShowFails;
        ULONG result;
        Event *ev;
        LONG  seqnum;
 ...
 */
ULONG ASM New_FindTask(APTR iface, char *name)
{
        ULONG (*origfunc)(APTR,char *) = (ULONG (*)())PatchList[GID_FINDTASK].origfunc;
        int onlyfails = OnlyShowFails;
        ULONG result;
        Event *ev;
        LONG  seqnum;
 ...
 */
ULONG ASM New_LockPubScreen(APTR iface,char *name)
{
        ULONG (*origfunc)(APTR,char *) = (ULONG (*)())PatchList[GID_LOCKSCREEN].origfunc;
        int onlyfails = OnlyShowFails;
        ULONG result;
        Event *ev;
        LONG  seqnum;
 ...
 *              names as an error.
 */
ULONG ASM New_OpenDevice(APTR iface, char *name, long unit,
                         struct IORequest *ioreq, long flags)
{
        ULONG (*origfunc)(APTR,char *, long, struct IORequest *, long) = (ULONG (*)())PatchList[GID_OPENDEVICE].origfunc;
        int onlyfails = OnlyShowFails;
        ULONG result;
        Event *ev;
        char unitstr[20];
 ...
 */
         ...
         *              ROM space).
         * 
         * 		sba: Seems to be fixed within OS4
         */
#if 0
        if ((CallAddr >= RomStart && CallAddr <= RomEnd) ||
                                        strcmp(FindTask(NULL)->tc_Node.ln_Name, "CON") == 0) {
                if (onlyfails)
                        return (result);
                JumpOrigFunc(unit);
        }
        ...
        if (!ev) {
                if (onlyfails)
                        return (result);
/* sba: JumpOrigFunc not yet supported */
/*                JumpOrigFunc(unit);*/
            return origfunc(iface, name, unit, ioreq, flags);
        }
 ...
 */
ULONG ASM New_OpenFont(APTR iface, struct TextAttr *textattr)
{
        ULONG (*origfunc)(APTR,struct TextAttr *) = (ULONG (*)())PatchList[GID_OPENFONT].origfunc;
        int onlyfails = OnlyShowFails;
        ULONG result;
        Event *ev;
        char *name;
        ...
        if (onlyfails) {
                result = origfunc(iface, textattr);
                if (result)
                        return (result);
        }
        if (textattr) {
        ...
        /*
         * THOR: Do not monitor input.device OpenFont().
         * This is only annoying...
         */

        if (FindTask(NULL) == InputTask) {
                ev = NULL;
        } else {
        ...
        if (!ev) {
                if (onlyfails)
                        return (result);
                return origfunc(iface, textattr);
        }
 ...
 */
ULONG ASM New_OpenLibrary(APTR iface, char *name, long version)
{
        ULONG (*origfunc)(APTR, char *, long) = PatchList[GID_OPENLIBRARY].origfunc;
        int onlyfails = OnlyShowFails;
        ULONG result;
        Event *ev;
        char verstr[20];
...
/* sba: Actually the DateStamp() call seems to be removed from the 
 * CreateEvent() call, furthermore OS4 DateStamp() doesn't call OpenLibrary()
 */
#if 0
        /*
         ...
         */
        if (BufSem.ss_Owner == FindTask(NULL) ||
        ...
        if (onlyfails) {
                result = origfunc(iface, name, version);
                if (result)
                        return (result);
        }
        ...
        if (!ev) {
                if (onlyfails)
                        return (result);
/* sba: JumpOrigFunc() not yet supported by the OS4 port */
/*                JumpOrigFunc(version);*/
                return origfunc(iface, name, version);
        }
 ...
 */
ULONG ASM New_OpenResource(APTR iface, char *name)
{
        ULONG (*origfunc)(APTR,char *) = (ULONG (*)())PatchList[GID_OPENRESOURCE].origfunc;
        int onlyfails = OnlyShowFails;
        ULONG result;
        Event *ev;
        LONG  seqnum;
 ...
 */
ULONG ASM New_FindToolType(APTR iface, char **array, char *tooltype)
{
        ULONG (*origfunc)(APTR,char **,char *) = (ULONG (*)())PatchList[GID_READTOOLTYPES].origfunc;
        int onlyfails = OnlyShowFails;
        ULONG result;
        Event *ev;
        ...
        if (onlyfails) {
                result = origfunc(iface, array, tooltype);
                if (result)
                        return (result);
        }
        if (toolname == NULL)
        ...
        if (!ev) {
                if (onlyfails)
                        return (result);
                return origfunc(iface, array, tooltype);
        }
 ...
 */
ULONG ASM New_MatchToolValue(APTR iface, char *tooltype, char *value)
{
        ULONG (*origfunc)(APTR,char *,char *) = (ULONG (*)())PatchList[GID_READTOOLTYPES2].origfunc;
        int onlyfails = OnlyShowFails;
        ULONG result;
        Event *ev;
        char *toolname = tooltype;
        ...
        if (onlyfails) {
                result = origfunc(iface, tooltype, value);
                if (result)
                        return (result);
        }
        if (toolname == NULL)
        ...
        if (!ev) {
                if (onlyfails)
                        return (result);
                return origfunc(iface, tooltype, value);
        }
 ...
 */
ULONG ASM New_CurrentDir(APTR iface, BPTR lock)
{
        ULONG (*origfunc)(APTR,BPTR) = (ULONG (*)())PatchList[GID_CHANGEDIR].origfunc;
        int onlyfails = OnlyShowFails;
        Event *ev;
        ...
        if (onlyfails)
                return origfunc(iface, lock);         /* CurrentDir never fails */
 ...
 */
ULONG ASM New_DeleteFile(APTR iface, char *name)
{
        ULONG (*origfunc)(APTR,char *) = (ULONG (*)())PatchList[GID_DELETE].origfunc;
        int onlyfails = OnlyShowFails;
        Event *ev;
        ULONG result;
        LONG  seqnum;
 ...
 */
ULONG ASM New_Execute(APTR iface,char *cmdline, BPTR fin, BPTR fout)
{
        ULONG (*origfunc)(APTR,char *,BPTR,BPTR) = (ULONG (*)())PatchList[GID_EXECUTE].origfunc;
        int onlyfails = OnlyShowFails;
        Event *ev;
        ULONG result;
        char *optstr;
        ...
        if (!MonROMCalls && CallAddr >= RomStart && CallAddr <= RomEnd)
                return origfunc(iface, cmdline, fin, fout);

        if (onlyfails) {
                result = origfunc(iface, cmdline, fin, fout);
                if (result)
                        return (result);
        }
        if (fin == ZERO)
 ...
 */
ULONG ASM New_GetVar(APTR iface, char *name, char *buffer,
                                         long size, long flags)
{
        ULONG (*origfunc)(APTR,char *,char *,long,long) = (ULONG (*)())PatchList[GID_GETVAR].origfunc;
        int onlyfails = OnlyShowFails;
        Event *ev;
        ULONG result;
        char optstr[20];
        ...
        if (onlyfails) {
                result = origfunc(iface, name, buffer, size, flags);
                if (result != -1)
                        return (result);
        }
        /*
        ...
        if (!ev) {
                if (onlyfails)
                        return (result);
                return origfunc(iface, name, buffer, size, flags);
        }
 ...
 */
ULONG ASM New_FindVar(APTR iface,char *name, ULONG type)
{
        ULONG (*origfunc)(APTR,char *,ULONG) = (ULONG (*)())PatchList[GID_GETVAR2].origfunc;
        int onlyfails = OnlyShowFails;
        Event *ev;
        ULONG result;
        ULONG optid;
        ...
        if (onlyfails) {
                result = origfunc(iface, name, type);
                if (result)
                        return (result);
        }
        ...
        if (!ev) {
                if (onlyfails)
                        return (result);
                return origfunc(iface, name, type);
        }
 ...
 */
ULONG ASM New_SetVar(APTR iface, char *name, char *buffer,
                                         long size, long flags)
{
        ULONG (*origfunc)(APTR,char *,char *,long,long) = (ULONG (*)())PatchList[GID_SETVAR].origfunc;
        int onlyfails = OnlyShowFails;
        Event *ev;
        ULONG result;
 ...
 */
ULONG ASM New_DeleteVar(APTR iface, char *name, ULONG flags)
{
        ULONG (*origfunc)(APTR,char *,ULONG) = (ULONG (*)())PatchList[GID_SETVAR2].origfunc;
        int onlyfails = OnlyShowFails;
        Event *ev;
        ULONG result;
        ULONG optid;
        ...
        if (onlyfails) {
                result = origfunc(iface, name, flags);
                if (result)
                        return (result);
        }
        /*
        ...
        if (!ev) {
                if (onlyfails)
                        return (result);
                return origfunc(iface, name, flags);
        }
 ...
 */
ULONG ASM New_LoadSeg(reg_d1 char *name, reg_d2 BPTR hunk, reg_d3 BPTR file,reg_a6 void *libbase)
{
        MarkCallAddr;
        ...
        /*
         * THOR: Fix overlay loading here.
         */
        if (name == NULL) {
                return origfunc(NULL,hunk,file,libbase);
        }

        if (onlyfails) {
                result = origfunc(name, ZERO, ZERO, libbase);
                if (result)
                        return (result);
        }
...
#if MONITOR_SEMAPHORE
                LoadTask = FindTask(NULL);
                result = origfunc(name, NULL, NULL, libbase);
                LoadTask = 0;
#else
                result = origfunc(name, ZERO, ZERO, libbase);
#endif
...
/*
 *              New_OBSOLETENewLoadSeg()
 *
 *              Tries to load in a module from disk
 */
ULONG ASM New_OBSOLETENewLoadSeg(reg_d1 char *name, reg_d2 TAGPTR tags,
                                                 reg_a6 void *libbase)
{
        MarkCallAddr;
 ...
 */
ULONG ASM New_Lock(APTR iface,char *name, LONG mode)
{
        ULONG (*origfunc)(APTR,char *,LONG) = (ULONG (*)())PatchList[GID_LOCKFILE].origfunc;
        int onlyfails = OnlyShowFails;
        Event *ev;
        ULONG result;
        LONG optid;
        ...
        if (onlyfails) {
                result = origfunc(iface, name, mode);
                if (result)
                        return (result);
        }
        ...
        if (!ev) {
                if (onlyfails)
                        return (result);
                return origfunc(iface, name, mode);
        }
 ...
 */
ULONG ASM New_CreateDir(struct Interface *iface,char *name)
{
        ULONG (*origfunc)(struct Interface *iface,char *name) = (ULONG (*)(struct Interface *iface,char *name))PatchList[GID_MAKEDIR].origfunc;
        int onlyfails = OnlyShowFails;
        Event *ev;
        ULONG result;
        LONG  seqnum;
 ...
 */
ULONG ASM New_MakeLink(APTR iface, char *name, LONG dest, LONG soft)
{
        struct Process *myproc = (struct Process *)FindTask(NULL);
        ULONG (*origfunc)(APTR,char *,LONG,LONG)   = (ULONG (*)())PatchList[GID_MAKELINK].origfunc;
        int onlyfails          = OnlyShowFails;
        Event *ev;
        ULONG result;
        LONG optid;
        ...
        if (onlyfails) {
                result = origfunc(iface, name, dest, soft);
                if (result)
                        return (result);
        }
        ...
        if (!ev) {
                if (onlyfails)
                        return (result);
                return origfunc(iface, name, dest, soft);
        }
 ...
 */
ULONG ASM New_Open(APTR iface,char *name, LONG accessMode)
{
        ULONG (*origfunc)(APTR,char *,LONG) = (ULONG (*)())PatchList[GID_OPENFILE].origfunc;
        int onlyfails = OnlyShowFails;
        Event *ev;
        ULONG result;
        char *optstr;
        ...
        if (onlyfails) {
                result = origfunc(iface, name, accessMode);
                if (result)
                        return (result);
        }
        ...
        if (!ev) {
                if (onlyfails)
                        return (result);
                return origfunc(iface, name, accessMode);
        }
 ...
 */
ULONG ASM New_Rename(APTR iface,char *oldname, char *newname)
{
        ULONG (*origfunc)(APTR, char *, char *) = (ULONG (*)())PatchList[GID_RENAME].origfunc;
        int onlyfails = OnlyShowFails;
        Event *ev;
        ULONG result;
        LONG  seqnum;
        ...
        if (onlyfails) {
                result = origfunc(iface, oldname, newname);
                if (result)
                        return (result);
        }
        ...
        if (!ev) {
                if (onlyfails)
                        return (result);
                return origfunc(iface, oldname, newname);
        }
        ev->status = ES_READY;
 ...
 */
ULONG ASM New_RunCommand(APTR iface, BPTR seglist,  ULONG stack,
                                                 char *cmdline, long cmdlen)
{
        ULONG (*origfunc)(APTR,BPTR,ULONG,char *,long) = (ULONG (*)())PatchList[GID_RUNCOMMAND].origfunc;
        int onlyfails = OnlyShowFails;
        Event *ev;
        ULONG result;
         ...
         */
        if (!MonROMCalls && CallAddr >= RomStart && CallAddr <= RomEnd)
                return origfunc(iface, seglist, stack, cmdline, cmdlen);

        if (onlyfails) {
                result = origfunc(iface, seglist, stack, cmdline, cmdlen);
                if (result != -1)
                        return (result);
        }
                                                ...
                                                reg_a6 void *libbase)
{
        ...
        FP_SystemTagList origfunc = (FP_SystemTagList)
                                                                 PatchList[GID_SYSTEMTAGLIST].origfunc;
        int onlyfails = OnlyShowFails;
        Event *ev;
        ULONG result;
 ...
 *              list.
 */
ULONG ASM New_AddDosEntry(APTR iface, struct DosList *dlist)
{
        ULONG (*origfunc)(APTR,struct DosList *) = (ULONG (*)())
...
/*
 * AttemptSemaphoreHeavely
 *
 * Try to get a semaphore, try by waiting a certain amount of time,
 * but give up if the semaphore is really not free.
 */
...
BYTE pri;
        ...
        /*
         * Now initialize the port and the timer for waiting...
         */

        myport.mp_Node.ln_Type  = NT_MSGPORT;
        myport.mp_Node.ln_Pri   = 0;
        myport.mp_Flags         = PA_SIGNAL;
        myport.mp_SigBit        = SIGB_SINGLE;
        myport.mp_SigTask       = FindTask(NULL);
        NewList(&myport.mp_MsgList);
        treq                    = *TimerRequest;
        treq.tr_node.io_Message.mn_ReplyPort    =       &myport;


        rnd >>= 27;             /* at most about two seconds... */
        if (pri > 0) {
                if (pri < 5) {
                        rnd >>= pri;
                } else return FALSE;
        ...
        do {
                ...
                treq.tr_time.tv_micro   = 100000;       /* one tenth of a second */
                DoIO(&treq.tr_node);
