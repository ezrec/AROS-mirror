...
#else
#undef RawPutChar
#define RawPutChar(c) DebugPrintF("%lc",c)
#endif
...
void SingleStep(void)
{
         ...
         */
        oldpri = SetTaskPri(FindTask(NULL), 25);
        ReleaseSemaphore(&PauseSem);
        ObtainSemaphore(&PauseSem);
...
#define SetMenuState(menunum, val) \
        ...
        if (MainWindow && MainWinMenu) {
                int curpri = FindTask(NULL)->tc_Node.ln_Pri;
                int i;
...
BOOL OpenMainWindow(void)
{
        ...
        if (DraggingColumn || DraggingRow) {
                ...
                DraggingColumn = 0;
                DraggingRow    = 0;
                Signal(FindTask(NULL), NewEventMask);
        }
        RedrawMainWindow();
        ...
        *TaskWindowPtr = SaveWindowPtr;         /* Restore previous screen address */
        if (MainWindow) {
                ...
                {                               /* To avoid race conditions, we
                                                 * NULL the pointer here immediately so
                                                 * the patches will no longer test
                                                 * an invalid pointer.
                                                 * THOR, 4.8.2000
                                                 */
...
void ScrollHorizontal(int amount)
{
        ...
        if (!AttemptSemaphoreHeavely(&BufSem)) /* Try non-blocking here, important! */
                return;
...
void ShowBuffer(LONG seqnum, int displaytype)
{
        ...
        */
        ...
        if (seqnum == oldtopseq) {
                ...
                /*
                 * THOR: ScrollRasterBF is buggy. Do not use it.
                 *
                if (GfxBase->LibNode.lib_Version >= 39) {
                        struct Hook *oldhook;
