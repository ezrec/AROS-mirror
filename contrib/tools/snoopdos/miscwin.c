 ...
 */
#ifdef __amigaos4__
void VARARGS68K ShowError(char *errormsg, ...)
#else
...
#endif
{
	 ...
	 */
#ifdef __amigaos4__
	EasyRequestArgs(MainWindow, &es, 0, va_getlinearva(args,APTR));
#else
 ...
 */
#ifdef __amigaos4__
int VARARGS68K GetResponse(char *prompts, char *reqmsg, ...)
#else
...
#endif
{
	...
	DisableAllWindows();
#ifdef __amigaos4__
	choice = EasyRequestArgs(MainWindow, &es, 0, va_getlinearva(args,APTR));
#else
...
int CheckForASL(void)
{
	...
	if (!AslBase) {
#ifdef __amigaos4__
		AslBase = OpenLib("asl.library", 0,(struct Interface **)&IAsl);
#else
...
int SelectFile(char *newname, char *defname, struct Window *win, int type)
{
	...
	n = (char *)PathPart(defname) - defname;
	if (n)
...
void InitFonts(void)
{
	 ...
	 */
	strcpy(SystemFontName, ((struct GfxBase *)GfxBase)->DefaultFont->tf_Message.mn_Node.ln_Name);
	SystemFontAttr.ta_YSize = ((struct GfxBase *)GfxBase)->DefaultFont->tf_YSize;
...
int ShowAGuide(char *cmdstring)
{
	if (!AmigaGuideBase) {
#ifdef __amigaos4__
		AmigaGuideBase = OpenLib("amigaguide.library", 34,(struct Interface **)&IAmigaGuide);
#else
...
#ifdef __amigaos4__
	if (IAmigaGuide)	DropInterface((struct Interface*)IAmigaGuide),		IAmigaGuide = NULL;
#endif
