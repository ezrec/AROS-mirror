...
FileStream::FileStream(myFILE *fA, Guint startA, GBool limitedA,
...
void FileStream::setPos(Guint pos, int dir) {
  ...
  if (dir >= 0) {
    ...
    rewind(f);
#endif
    myfseek(f, -(int)pos, SEEK_END);
    bufPos = (Guint)myftell(f);
  }
  bufPtr = bufEnd = buf;
}
...
void FileStream::preRead(int pos, int length) {
  f->preread(pos, length);
}

void FileStream::removeParts() {
  f->remove_parts();
}
...
StrStream::StrStream(GString *str, Object *dict):
    BaseStream(dict) {
  this->str = str;
  buf = str->getCString();
  bufPtr = buf;
  bufEnd = buf + str->getLength();
}
