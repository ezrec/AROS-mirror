...
GfxFont *GfxFont::makeFont(XRef *xref, char *tagA, Ref idA, Dict *fontDict) {
  ...
  if (obj1.isName()) {
    ...
    char buf[32];
    sprintf(buf,"Unnamed-%d-%d", idA.gen, idA.num);
    nameA = new GString(buf);
  }
  obj1.free();
...
GfxFontDict::GfxFontDict(XRef *xref, Ref *fontDictRef, Dict *fontDict) {
  ...
  fonts = (Entry *)gmallocn(numFonts, sizeof(Entry));
  for (i = 0; i < numFonts; ++i) {
    ...
    fonts[i].gfxFont = gfxFontCache->getGfxFont(xref, fontDict->getKey(i), r);
    obj1.free();
  }
}
