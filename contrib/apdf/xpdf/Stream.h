...
public:
  ...
  virtual void preRead(int pos, int length) {}
...
public:
  ...
  FileStream(myFILE *fA, Guint startA, GBool limitedA,
...
class StrStream: public BaseStream {
public:

  StrStream(GString *str, Object *dict1);
  virtual ~StrStream();
  virtual Stream *makeSubStream(Guint start, GBool limited, Guint length, Object *dict)
    { return NULL; }
  virtual StreamKind getKind() { return strString; }
  virtual void reset() { bufPtr = buf; }
  virtual int getChar()
    { return bufPtr >= bufEnd ? EOF : (*bufPtr++ & 0xff); }
  virtual int lookChar()
    { return bufPtr >= bufEnd ? EOF : (*bufPtr & 0xff); }
  virtual int getPos() { return bufPtr - buf; }
  virtual void setPos(Guint pos1, int dir) { bufPtr = (dir < 0 ? bufEnd - pos1 : buf + pos1); }
  virtual GBool isBinary(GBool last = gTrue) { return last; }
