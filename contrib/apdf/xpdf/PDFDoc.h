...
//             - Added document specific fontMap/fontCache.
//
#ifndef PDFDOC_H
#define PDFDOC_H
...
public:
  ...
  Destination *findDest(GString *name)
  ...
  GBool isEncrypted() { return xref && xref->isEncrypted(); }
  ...
  GBool isLinearized() { return xref->isLinearized(); }
  ...
  // Get the document's FontMap.
  FontMap *getFontMap() { return &fontMap; }

  // Make this document the active one.
  void activate();

  // Get the document's GfxFontCache.
  GfxFontCache *getGfxFontCache() { return &gfxFontCache; }

  // Get the page annotations.
  Annots *getAnnots(int page);
