...
void PSOutputDev::setupExternalType1Font(GString *fileName, GString *psName) {
  ...
  if (c == 0x80) {
    ...
    do {
      // get block type
      int type = fgetc(fontFile);

      // get block length
      int len;

      len = fgetc(fontFile);
      len |= fgetc(fontFile) << 8;
      len |= fgetc(fontFile) << 16;
      len |= fgetc(fontFile) << 24;

      switch (type) {
      case 1:
       // ASCII block
       while (--len >= 0 && (c = fgetc(fontFile)) != EOF)
         writePSChar(c);
       break;
      ...
      default:
       error(-1, "Unknown font format.");
       return;
      }
    } while ((c = fgetc(fontFile)) == 0x80);
  out:;
