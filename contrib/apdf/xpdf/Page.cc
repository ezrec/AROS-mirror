...
void Page::displaySlice(OutputDev *out, double hDPI, double vDPI,
  ...
  if (annots) {
    gfx->saveState();
    for (i = 0; i < annots->getNumAnnots() && !*abortFlagPtr; ++i) {
	...
	if (annot->getAppearance(xref, &obj)->isStream())
	  gfx->doAnnot(&obj, x1, y1, x2, y2);
	else if (!obj.isNull() && !obj.isNone())
	  error(-1, "Bad annotation appearance");
	obj.free();

	if (annot->getBorder(&obj)->isStream())
	  gfx->doAnnot(&obj, x1, y1, x2, y2);
	obj.free();
      }
    }
...
Annots *Page::getAnnots(Catalog *catalog) {
  DEBUG_INFO
  if (!annots && !annotsObj.isNull()) {
    Object obj, *p = &annotsObj;
    if (annotsObj.isRef()) {
      annotsObj.fetch(xref, &obj);
      p = &obj;
    }
    annots = new Annots(xref, p, catalog->getAcroForm());
    obj.free();
    annotsObj.free();
    annotsObj.initNull();
  }
