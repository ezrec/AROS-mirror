...
GlobalParams::GlobalParams(char *cfgFileName) {
  ...
  if (cfgFileName && cfgFileName[0]) {
    ...
    if (!(f = myfopen(fileName->getCString(), "r"))) {
      delete fileName;
    }
  }
  if (!f) {
...
void GlobalParams::parseFile(GString *fileName, myFILE *f) {
  int line;
  GList *tokens;
  GString *cmd, *incFile;
  char *p1, *p2;
  ...
  while (getLine(buf, sizeof(buf) - 1, f)) {
	...
	if (tokens->getLength() == 2) {
	  ...
	  if ((f2 = myfopen(incFile->getCString(), "r"))) {
...
void GlobalParams::parseNameToUnicode(GList *tokens, GString *fileName,
  ...
  if (!(f = myfopen(name->getCString(), "r"))) {
...
void GlobalParams::setupBaseFonts(char *dir) {
  ...
  for (i = 0; displayFontTab[i].name; ++i) {
    ...
    if (dir) {
      ...
      kind = displayFontT1;
      if ((f = myfopen(fileName->getCString(), "rb"))) {
	myfclose(f);
      } else {
	delete fileName;
	fileName = NULL;
...
myFILE *GlobalParams::getUnicodeMapFile(GString *encodingName) {
  GString *fileName;
  myFILE *f;

  lockGlobalParams;
  if ((fileName = (GString *)unicodeMaps->lookup(encodingName))) {
    f = myfopen(fileName->getCString(), "r");
  } else {
    f = NULL;
  }
  unlockGlobalParams;
...
myFILE *GlobalParams::findCMapFile(GString *collection, GString *cMapName) {
  GList *list;
  GString *dir;
  GString *fileName;
  myFILE *f;
...
myFILE *GlobalParams::findToUnicodeFile(GString *name) {
...
void GlobalParams::addFontDir(GString *dir) {
  fontDirs->append(dir);
}

void GlobalParams::clearFontDirs() {
  deleteGList(fontDirs, GString);
  fontDirs = NULL;
  fontDirs = new GList;
}
