...
//========================================================================
//
// EL (12/99): - Modified the handling of path painting operators
//               to avoid multiple conversions of the same path in
//               the output device.
//
#include <aconf.h>
...
Gfx::Gfx(XRef *xrefA, OutputDev *outA, int pageNum, Dict *resDict,
  ...
  if (cropBox) {
    ...
    out->pathOp(state, popClose | popClip);
    state->clearPath();
  }
}
...
//  while (state != curState && state && state->hasSaves()) {
//    state = state->restore();
...
void Gfx::opCloseFillStroke(Object args[], int numArgs) {
  ...
  pathOp |= popClose | popFill | popStroke;
  doEndPath();
}
...
void Gfx::opCloseEOFillStroke(Object args[], int numArgs) {
  ...
  pathOp |= popClose | popEOFill | popStroke;
  doEndPath();
}
...
void Gfx::doTilingPatternFill(GfxTilingPattern *tPat, GBool eoFill) {
  ...
  state->clip();
  if (eoFill) {
    out->pathOp(state, popEOClip);
  } else {
    out->pathOp(state, popClip);
  }
  state->clearPath();
...
void Gfx::doEndPath() {
  if (state->isPath()) {
    ...
    if (pathOp & (popFill | popEOFill) &&
	state->getFillColorSpace()->getMode() == csPattern) {
      doPatternFill((pathOp & popEOFill) != 0);
      pathOp &= ~(popFill | popEOFill);
    }
    if (pathOp != popNone)
      out->pathOp(state, pathOp);
  }
  pathOp = popNone;
  state->clearPath();
}
...
void Gfx::doShowText(GString *s) {
  ...
  if (font->getType() == fontType3 && out->interpretType3Chars()) {
	...
	dx += state->getWordSpace();
      ...
      //~ out->updateCTM(???)
//out->updateCTM(state, newCTM[0], newCTM[1], newCTM[2], newCTM[3], x, y);
//state->setTextMat(1, 0, 0, 1, 0, 0);
//out->updateTextMat(state);
//state->textMoveTo(0, 0);
//out->updateTextPos(state);
...
void Gfx::doImage(Object *ref, Stream *str, GBool inlineImg) {
  ...
  if (mask) {
    ...
    // draw it
    out->drawImage(state, ref, str, width, height, NULL, NULL, inlineImg, invert);
    ...
    if (haveSoftMask) {
      ...
      out->drawImage(state, ref, str, width, height, colorMap,
		     haveColorKeyMask ? maskColors : (int *)NULL, inlineImg, gFalse);
    }
    delete colorMap;
    //str->close();
