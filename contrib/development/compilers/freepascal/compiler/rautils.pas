...
Begin
  ...
  if assigned(procinfo^.returntype.def) and
   ...
   begin
     ...
     if (opr.ref.offset=0) and
        ((m_tp in aktmodeswitches) or (m_delphi in aktmodeswitches)) then
       Message(asmr_e_cannot_use_RESULT_here)
     else
       SetupResult:=true;
   end
...
{
  $Log$
  Revision 1.1  2002/02/19 08:23:45  sasu
  Initial revision

  Revision 1.1.2.4  2000/11/06 16:57:38  pierre
   * issue error in TP and Delphi modes if result is a register

  Revision 1.1.2.3  2000/10/31 07:30:12  pierre
   * fix for bug 1157 second part

  Revision 1.1.2.2  2000/10/07 23:41:06  pierre
   * fix for bug 1157

  Revision 1.1.2.1  2000/08/06 10:29:03  peter
    * fixed expression evaluator (bug 1090)
