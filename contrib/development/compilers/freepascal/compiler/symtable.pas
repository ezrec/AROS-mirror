...
{$endif GDB}
          function getnewtypecount : longint; virtual;
       end;
...
****************************************************************************}
    ...
    var
       { for STAB debugging }
       globaltypecount  : longint;
       pglobaltypecount : plongint;
...
***********************************************}
   ...
   function tsymtable.getnewtypecount : longint;
      begin
...
{$endif CHAINPROCSYMS}
         ...
         { in Delphi mode, this is allowed                   }
         if (sym^.typ=varsym) and
            (symtabletype=objectsymtable) and
            assigned(defowner) and
            (
             not(m_delphi in aktmodeswitches) or
             not(pobjectdef(defowner)^.is_class)
            ) then
           begin
           ...
           if (hp^.typ=typesym) and make_ref then
             begin
               if assigned(ptypesym(hp)^.restype.def) then
                 ptypesym(hp)^.restype.def^.numberstring
               else
...
{$endif GDB}
   ...
   function tunitsymtable.getnewtypecount : longint;
...
{
  $Log$
  Revision 1.1  2002/02/19 08:23:57  sasu
  Initial revision

  Revision 1.1.2.9  2000/12/10 14:10:45  florian
  ...
  Revision 1.1.2.5  2000/08/18 12:56:33  pierre
   * avoid infinite recursion in stabs generation

  Revision 1.1.2.4  2000/08/16 18:26:01  peter
    * splitted namedobjectitem.next into indexnext and listnext so it
      can be used in both lists
    * don't allow "word = word" type definitions

  Revision 1.1.2.3  2000/08/08 19:17:07  peter
    * memdebug: total mangledname size
    * setmangledname doesn't set parast,localst name to mangledname anymore
      but when only extdebug is set

  Revision 1.1.2.2  2000/08/07 14:42:44  jonas
    * reduced memory usage of symtable type by reordering fields
