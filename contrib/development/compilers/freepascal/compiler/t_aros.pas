...
interface
  ...
  type
    pimportlibaros=^timportlibaros;
    timportlibaros=object(timportlib)
    ...
    pexportlibaros=^texportlibaros;
    texportlibaros=object(texportlib)
      procedure preparelib(const s : string);virtual;
...
procedure timportlibaros.preparelib(const s : string);
begin
end;


procedure timportlibaros.importprocedure(const func,module : string;index : longint;const name : string);
begin
...
procedure timportlibaros.importvariable(const varname,module:string;const name:string);
begin
  { insert sharedlibrary }
{$IFDEF NEWST}
  current_module^.linkothersharedlibs.
...
procedure texportlibaros.preparelib(const s:string);
begin
end;


procedure texportlibaros.exportprocedure(hp : pexported_item);
var
  hp2 : pexported_item;
begin
  ...
  if (hp^.options and eo_index)<>0 then
   begin
     Message1(parser_e_no_export_with_index_for_target,'aros');
     exit;
   end;
...
procedure texportlibaros.exportvar(hp : pexported_item);
begin
  hp^.is_var:=true;
  exportprocedure(hp);
end;
...
procedure texportlibaros.generatelib;
var
  hp2 : pexported_item;
begin
  hp2:=pexported_item(current_module^._exports^.first);
...
{$endif i386}
      ...
      Message1(parser_e_no_export_of_variables_for_target,'aros');
     hp2:=pexported_item(hp2^.next);
   end;
end;
...
begin
  ...
  with Info do
   begin
     ...
     { first try glibc2 }
     DynamicLinker:='/lib/ld-aros.so.2';
     if FileExists(DynamicLinker) then
      ...
      DynamicLinker:='/lib/ld-aros.so.1';
     {$ifdef BSD}
      DynamicLinker:='';
     {$endif}
   end;
     ...
     While not SharedLibFiles.Empty do
      begin
        ...
        if s<>'c' then
         ...
         begin
           linklibc:=true;
           linkdynamic:=false; { libc will include the ld-aros for us }
         end;
      end;
...
Function TLinkeraros.MakeSharedLibrary:boolean;
var
  binstr,
  cmdstr  : string;
  success : boolean;
...
{
  $Log$
  Revision 1.1  2002/02/19 08:24:10  sasu
  Initial revision
