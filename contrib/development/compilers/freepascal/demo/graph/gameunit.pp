...
{$IFDEF Win32Graph}
  WinMouse,             {Mouse for Win32 GUI programs via win32 API}
{$ELSE}
 {$IFDEF MouseAPI}
   Mouse,                {Mouse via API (non BP/Go32V2 platforms}
 {$ELSE}
   MsMouse,
 {$endif}
{$ENDIF}
{$IFDEF Win32Graph}
  WinCrt,               {Win32 + Graph -> GUI -> special CRT}
{$else}
  Crt,
{$endif}
{$IFDEF UseGraphics}
...
BEGIN
  {$IFDEF MouseAPI}
   ...
   {$else}
     Mouse.GetMouseEvent(MouseEvent);
...
BEGIN
 {$IFDEF MouseAPI}
  {$ifdef Win32Graph}
 //   WinMouse.SetMousePos(X,Y);  {N/I yet}
  {$else}
   Mouse.SetMouseXY(x,y);
  {$endif}
 {$ELSE}
  SetMousePos(X,Y);
 {$ENDIF}
...
{
  $Log$
  Revision 1.1  2002/02/19 08:24:19  sasu
  Initial revision

  Revision 1.1.2.1  2000/12/03 19:19:53  marco
   * Win32/Graph Mouse support via winmouse implemented I hope
