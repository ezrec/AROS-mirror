...
long vcformat(void *data, long (*outc)(long, void *), const char *format, va_list args)
{
...
LONG VSPrintf(UBYTE *str, const UBYTE *format, va_list args)
{
	...
	size = vcformat(&str, (APTR)vsprintf_uc, (const char *)format, args);
	*str = 0;
...
LONG VSNPrintf(UBYTE *str, ULONG n, const UBYTE *format, va_list args)
{
	...
	vcformat(&data, (APTR)vsnprintf_uc, (const char *)format, args);
