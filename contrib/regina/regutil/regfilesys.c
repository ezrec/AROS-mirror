...
rxfunc(sysfilesystemtype)
{
   ...
   if (argc == 1)
      chDriveLetter[0]=argv[0].strptr[0];   /* letter passed in  */
   else {
      char *cwd;
   ...
   if (rc == NO_ERROR) {
      sprintf(result->strptr, "%s", (pfsqBuffer->szName + pfsqBuffer->cbName + 1));
      result->strlength = strlen(result->strptr);
   }
   else result->strlength = 0;        /* return a null string       */
...
#ifndef _WIN32
# ifdef __EMX__
#  define ST_MODE st.st_attr
# else
#  define ST_MODE st.st_mode
# endif
