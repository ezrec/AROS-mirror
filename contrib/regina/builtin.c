...
#if defined(_AMIGA) || defined(__AROS__)
   for (start=0;(start<Str_len(input))&&strchr(padstr,input->value[start])&&(leading);start++) ;
   for (stop=Str_len(input)-1;(stop >=start)&&strchr(padstr,input->value[stop])&&(trailing);stop--) ;
#else
