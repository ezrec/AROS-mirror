...
static struct UnLZX *unlzx_init(void)
{
...
static void unlzx_free(struct UnLZX *unlzx)
{
  if (unlzx)
  {
    free(unlzx);
...
static int pmatch(const char *mask, const char *name)
{
  int           calls = 0,
                wild  = 0,
                q     = 0;
...
static unsigned long argopt(unsigned char * ao_strg, unsigned long ao_argc, unsigned char **ao_argv)
{
  while(ao_argc > 1)
  {
...
static void crc_calc(unsigned char *memory, unsigned long length, struct UnLZX *unlzx)
{
  unsigned long temp;
...
static signed long make_decode_table(signed long number_symbols, signed long table_size, unsigned char *length, unsigned short *table)
{
...
static signed long read_literal_table(struct UnLZX *unlzx)
{
...
static void decrunch(struct UnLZX *unlzx)
{
...
static FILE *open_output(unsigned char * filename)
{
  unsigned long temp;
  FILE *file;
...
static signed long extract_normal(FILE *in_file, struct UnLZX *unlzx)
{
  ...
  for (node = unlzx->filename_list; (!abort) && node; node = node->next)
  {
    ...
    if (unlzx->use_outdir)
    {
      strcpy(unlzx->work_buffer, unlzx->output_dir);
      strcat(unlzx->work_buffer, node->filename);
    }
    else
    {
      strcpy(unlzx->work_buffer, node->filename);
    }
    fflush(stdout);
    if (!pmatch(unlzx->match_pattern, node->filename))
    {
      printf("Extracting \"%s\"...", (char *)node->filename);
      out_file = open_output(unlzx->work_buffer);
    }
    else
    {
      out_file = NULL;
...
static signed long extract_store(FILE *in_file, struct UnLZX *unlzx)
{
  struct filename_node *node;
  FILE *out_file;
  unsigned long count;
  ...
  for (node = unlzx->filename_list; (!abort) && (node); node = node->next)
  {
    ...
    while (unlzx->unpack_size > 0)
    {
      ...
      if (fread(unlzx->read_buffer, 1, count, in_file) != count)
      {
        printf("\n");
        #ifdef UNLZX_DEBUG
        if (ferror(in_file))
      ...
      if (out_file)
      {
        if (fwrite(unlzx->read_buffer, 1, count, out_file) != count)
        {
          #ifdef UNLZX_DEBUG
          perror("FWrite");
          #endif
...
static signed long extract_unknown(FILE *in_file, struct UnLZX *unlzx)
{
  struct filename_node *node;
...
static signed long extract_archive(FILE *in_file, struct UnLZX *unlzx)
{
  unsigned long temp;
  ...
  {
    abort = 1;
    actual = fread(unlzx->archive_header, 1, 31, in_file);
    if (!ferror(in_file))
    {
      if (actual)
      {
        if (actual == 31)
        {
          ...
          actual = fread(unlzx->header_filename, 1, temp, in_file);
          if (!ferror(in_file))
          {
            if (actual == temp)
            {
              ...
              actual = fread(unlzx->header_comment, 1, temp, in_file);
              if (!ferror(in_file))
              {
                if (actual == temp)
                {
                    ...
                    if ((node = (struct filename_node *)malloc(sizeof(struct filename_node))))
                    {
                      *filename_next = node;
                      ...
                      if (unlzx->pack_size)
                      {
                        ...
                        {
                          case 0:
                            abort = extract_store(in_file, unlzx);
                          break;
                          case 2:
                            abort = extract_normal(in_file, unlzx);
                          break;
                          default:
                            abort = extract_unknown(in_file, unlzx);
                          break;
                        }
                        if (abort) break;
                        ...
                        unlzx->filename_list = 0;
                        filename_next = &unlzx->filename_list;
                        if (fseek(in_file, unlzx->pack_size, SEEK_CUR))
                        {
                          #ifdef UNLZX_DEBUG
                          perror("FSeek(Data)");
                          #endif
...
static signed long view_archive(FILE *in_file, struct UnLZX *unlzx)
{
...
static signed long process_archive(unsigned char * filename, struct UnLZX *unlzx)
{
    ...
    if (!ferror(in_file))
    {
      if (actual == 10)
      {
        if ((unlzx->info_header[0] == 76) && (unlzx->info_header[1] == 90) && (unlzx->info_header[2] == 88))
        {
          ...
          {
            case 1:
              result = extract_archive(in_file, unlzx);
            break;
            case 2:
...
int main(int argc, char **argv)
{
  ...
  if ((unlzx = unlzx_init()))
  {
    ...
    if ((option = argopt("-o", (unsigned long)argc, (unsigned char **)argv)))
    {
      if (++option < argc)
      {
        strcpy(unlzx->output_dir, argv[option]);
        unlzx->use_outdir = 1;
  
        tmp = strlen(unlzx->output_dir) - 1;
        #ifdef AMIGA
    ...
    #endif
    result = process_archive(argv[option], unlzx);
    #ifdef UNLZX_TIME
