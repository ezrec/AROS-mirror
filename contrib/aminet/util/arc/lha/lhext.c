...
static boolean make_parent_path(char *name)
{
...
static FILE *open_with_make_path(char *name)
{
  FILE *fp;
...
static void adjust_info(char *name, LzHeader *hdr)
{
#ifdef AROS
#else
  time_t utimebuf[2];
...
static void extract_one(FILE *afp, LzHeader *hdr)
{
  FILE *fp;					/* output file */
  char name[257];
  int crc = 0;
