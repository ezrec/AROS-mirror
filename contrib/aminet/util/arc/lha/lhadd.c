...
static void add_one(FILE *fp, FILE *nafp, LzHeader *hdr)
{
...
static void find_update_files(FILE *oafp)
{
...
static void delete(FILE *oafp, FILE *nafp)
{
  LzHeader ahdr;
  long old_header_pos;
...
/*									*/
/*----------------------------------------------------------------------*/
static FILE *build_temporary_file(void)
{
#ifndef AROS
  int old_umask;
#endif
...
static void build_backup_file(void)
{
...
static void report_archive_name_if_different(void)
{
...
static void set_archive_file_mode(void)
{
#ifndef AROS
  int umask_value;
  struct stat stbuf;
...
static void remove_one(char *name)
{
  struct stat stbuf;
  int filec;
  char **filev;
