/* UNARJ.C, UNARJ, R JUNG, 02/17/93 (Amiga Port 03/18/95)
 * Main Extractor routine
 * Copyright (c) 1991-93 by Robert K Jung.  All rights reserved.
 * Amiga Port done by Andreas R. Kleinert in 1995-97, powerUP (TM) in 1998
 *
 ...
 *   source code.
 *
 *   I am holding the copyright on the source code, so please do not
 ...
 * 03/18/95  A. Kleinert Ported V2.41 to Amiga. Did some necessary changes.
 * 10/27/97  A. Kleinert recompiled with SAS/C 6.58
 * 02/07/98  A. Kleinert ported to powerUP (TM), using SAS/C for PPC
 * 02/17/98  A. Kleinert updated
 * 04/29/98  R. Schulz   Added extraction of patternmatched files
 *                       Added a destination-path for the extracted files
 *                       Added destination dirs will be created now
 *
 */
...
static char *M_USAGE  [] =
{
"Usage:  UNARJ <command> <archive> [<file>...] [<destdir>]\n",
"\n",
"<command>:\n",
...
"USA\n\n",
"Original ported by Andreas R. Kleinert\n",
"EMail: Andreas_Kleinert@t-online.de\n\n",
"Some changes, done by Ronny Schulz\n",
"E-Mail: Cannon@BigPlanet.tobcv.de\n\n",
NULL
};

char M_VERSION [] = "UNARJ (Demo version) 2.41 Copyright (c) 1991-93 Robert K Jung\n"
                    "Amiga Port V1.04 of UnARJ 2.41, done by Ronny Schulz in 1998\n\n";
...
static char   *arc_f[XFILES_MAX+1];
static char   *arc_d;
...
extract()
{
    ...
    {
        strcpy(name, arc_d);
        strcat(name, &filename[entry_pos]);
    }
    else
    {
        strcpy(name, arc_d);
...
{
    char *res;
    char cpath[FNAME_MAX];

    strcpy(cpath, path);
    res = strtok(cpath, PATH_SEPARATORS);
    if (res != NULL) strcpy(buffer, res);
    res = strtok(NULL, PATH_SEPARATORS);

    while ((res != NULL) && (strlen(buffer)+strlen(res) < BUF_SIZE))
    {
        MKDIR(buffer, 0777);
        buffer[strlen(buffer)+1] = NULL_CHAR;
        buffer[strlen(buffer)] = PATH_CHAR;
        strcat(buffer, res);
        res = strtok(NULL, PATH_SEPARATORS);
    }
}
...
char *argv[];
{
    ...
    else if (argc > 3)
    {
        ...
        if ((argv[argc-1][strlen(argv[argc-1])-1] == ':') ||
            (argv[argc-1][strlen(argv[argc-1])-1] == PATH_CHAR))
        {
            arc_d = argv[argc-1];
            i--;
        }
	if (i >= XFILES_MAX) i = XFILES_MAX-1;
        arc_f[i+1] = NULL;
        while (i > 0)
        {
            i--;
