 ...
 * 02/07/98  A. Kleinert added some typecasts for SAS/C PPC
 *                       uncommented no-op case path and ftime_mode functions
 *
 */
...
FILE *stream;
{
    return (int) fread(buf, (int) size, (int) nitems, stream);
}
...
FILE *stream;
{
    return (int) fwrite(buf, (int) size, (int) nitems, stream);
}
