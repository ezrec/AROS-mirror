...
static char *ProcessNewIcon(struct IconRepresentation *ico)
{
  ...
  if((txt = ProcessNewIconImage((unsigned char **)ico->ToolTypes, ico->NewIconImage+0, '1')))
    return txt;
  if((txt = ProcessNewIconImage((unsigned char **)ico->ToolTypes, ico->NewIconImage+1, '2')))
    return txt;

  a = (unsigned char **)ico->ToolTypes; d = 0;
  while(*a)
  {
