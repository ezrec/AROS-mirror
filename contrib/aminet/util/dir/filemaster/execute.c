...
void executeexe (struct FMList *, struct FMList *, UBYTE *,
		 struct CMenuConfig *, struct ExecuteFlags *);
...
void
executefile (struct FMHandle *h, struct CMenuConfig *cmc)
{
  struct GUIBase *gb;
  WORD c;
  UBYTE params[128];
...
//priority(cmc);
  ec = getconfig (cmc);
  if (h->filename[0])
    sformatmsg (h->owner->fmmessage1, MSG_FILECLICK_EXECUTABLE, h->filename);
  fmmessage (h->owner);
...
void
executeexe (struct FMList *list, struct FMList *dlist, UBYTE * command,
	    struct CMenuConfig *cmc, struct ExecuteFlags *ef2)
{
  ...
  shellwindow =
    ((struct ExecuteConfig *) (getconfig (getconfignumber (EXECUTECONFIG))))->
    shellwindow;
  ...
  for (c = 0; c < 10; c++)
    {
      ...
      if (!(lock = Lock (exename, SHARED_LOCK)))
	break;
      UnLock (lock);
    }
  if (c >= 10)
...
WORD
txtwrite (BPTR handle, UBYTE * ptr, WORD eof)
{
...
void
executeown (struct FMList *slist, struct FMList *dlist, struct FMNode *cnode,
	  ...
	  *ptr2 = 0;
	  switch (*ptr1)
		...
		goto serror;
	      ...
	      if (!askstring (0, cmc->label, vara, varapath, 80))
		goto endi;
	      strcpy (ptr2, varapath);
	      ptr2 += strlen (ptr2);
	      break;
...
void *
ownexecuteconfigdefault (struct CMenuConfig *cmc)
{
  WORD ret;
  struct OwnCommand *oc;
...
void *
executeconfigdefault (struct CMenuConfig *cmc)
{
...
WORD
executeconfig (struct GUIBase *gb, struct CMenuConfig *cmc)
{
  WORD c;
  struct ExecuteConfig *ec;
  LONG timeout;
