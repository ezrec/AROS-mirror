...
WORD format (struct FMList *, struct FMNode *, UBYTE *,
...
static WORD
createoperatewindow (UBYTE * title, struct FMNode *node, UBYTE * string1,
		     UBYTE * string2, UBYTE * string3, UWORD cdir,
		     BOOL bprot[])
{
  struct GUIBase *gb;
  ...
  if (!dev)
    {
      if (cdir & 1)
	reqbuttonmsg (gb, MSG_OPERATE_CREATEICON, 2, guiUC);
      if (cdir & 2)
	reqbuttonmsg (gb, MSG_ALL, 3, guiUC);
    }
  else
    {
      ...
      if (fssm)
	{
	  ...
	  if (de->de_Surfaces * de->de_BlocksPerTrack * (de->de_HighCyl + 1) *
	      de->de_SizeBlock < 786432)
	    {
...
static WORD
operoi (struct FMList *list, struct FMNode *node, UBYTE * name, UBYTE * comm,
	ULONG date, BOOL bprot[], WORD pmode)
{
  WORD cnt3, cnt2;
  ULONG prot;

  node->flags &= ~NSELECTED;
  if (name && !(fmrename (list, node, name)))
    return (0);
  if (comm && !(fmcomment (list, node, comm)))
    return (0);
  if (!fmfiledate (list, node, date))
    return (0);
  if (!pmode)
    return (1);
  if (pmode == 1)
...
operate (void)
{
  ...
  for (;;)
      ...
      if (node->flags & NASSIGN)
	{
      ...
      cnt2 =
	createoperatewindow (list->workname, node, string1, string2, string3,
			     2, bprot);
      if (!cnt2)
	break;
      ...
      if (cnt2 == 3)
	{
	  ...
	  reqcyclemsg (gb, 100, &cflags, MSG_NON, MSG_OPERATE_CHANGE,
		       MSG_OPERATE_ADD, MSG_OPERATE_SUB, 0);
	  ...
	  if (ret == 1)
	      ...
	      while (node = findselnodeall (list))
		{
		  node->flags &= ~NSELECTED;
		  if (!operoi
		      (list, node, 0, ccom ? string2 : 0,
		       cdate ? date : 0xffffffff, bprot, cflags))
		    goto opend;
		  outputlistline (list, node);
		}
	      endofdirtxt (list, node);
      ...
      if (cnt2 == 1)
	{
	  if (list->flags & LDIRLIST)
	    {
	      if (!operoi (list, node, string1, string2, date, bprot, 1))
		goto opend;
	    }
		...
		{
		  dosrequestmsg (list, 0, MSG_FMDOS_RELABELERR,
				 NDCOMM (node));
		  break;
		}
	      else
...
//priority(pm->cmc);
  ...
  string3[0] = 0;
  cnt2 =
    createoperatewindow (list->workname, node, string1, string2, string3, 1,
			 bprot);
  date = datestringtolong (string3);
  freevecpooled (node);
  node = 0;
      ...
      if (node = fmcreatedir (list, string1))
	{
	  if (cnt2 == 2)
	      ...
	      if (openicon (list))
		{
		  ...
		  if (!dob)
		    {
		      requestmsg (list->workname, 0, MSG_OK,
				  MSG_FMDOS_OPENERR,
				  getstring (MSG_OPERATE_DEFAULTDRAWER));
		    }
		  else
		    {
			  ...
			  if (node2)
			    {
			      ...
			      strncpy (NDFILE (node2), vstring,
				       fmmain.filelen);
			    }
			}
		      else
			{
			  dosrequestmsg (list, 0, MSG_FMDOS_WRITEERR,
					 getstring
					 (MSG_OPERATE_DEFAULTDRAWER));
			}
		      FreeDiskObject (dob);
		    }
		}
...
WORD
format (struct FMList *list, struct FMNode *node, UBYTE * name,
	struct CMenuConfig *cmc)
{
  struct GUIBase *gb;
  ...
  if (*name == 0)
    strcpymsg (name, MSG_OPERATE_DEFAULTNAME);
...
#else
  fssm =
    (struct FileSysStartupMsg *) (doslist->dol_misc.dol_handler.dol_Startup);
  de = (struct DosEnvec *) (fssm->fssm_Environ);
#endif
  ...
  UnLockDosList (LDF_DEVICES | LDF_READ);
  buttonbarmsg (gb, MSG_OPERATE_DOFORMAT, 1, MSG_OPERATE_DOQFORMAT, 2,
		MSG_OPERATE_DOINSTALL, 3, MSG_CANCEL, 0, 0);
  ...
  if (ret == 1)
...
#ifndef AROS
      ioreq =
	(struct IOStdReq *) opendevice (list,
					(UBYTE *) (fssm->fssm_Device << 2) +
					1, fssm->fssm_Unit, fssm->fssm_Flags,
	...
	goto fend;
      *((ULONG *) formatdata) = 'FM30';
      surfacecnt = de->de_Surfaces;
      trackcnt = de->de_HighCyl;
      cylcnt = (surfacecnt * trackcnt + 1) * formatlen;
      surfacecnt--;
      while (cylcnt >= 0)
	{
	  sformat (list->fmmessage1, getstring (MSG_OPERATE_FORMATTING),
		   trackcnt, surfacecnt);
	  fmmessage (list);
	  ...
	  if (operateconfig->fver)
	    {
	      sformat (list->fmmessage1, getstring (MSG_OPERATE_VERIFYING),
		       trackcnt, surfacecnt);
	      fmmessage (list);
	      ioreq->io_Offset = cylcnt;
  ...
  if (ret == 1 || ret == 2)
	...
	{
	  dt =
	    operateconfig->fffs | (operateconfig->
				   fint << 1) | (operateconfig->fdir << 2);
	  if (dt >= 6)
	    dt -= 2;
  ...
  if (ret == 3 || jatko)
      ...
      if (diskio (list, ioreq))
	{
	  ...
	  if (formatdata[0] == 'D' && formatdata[1] == 'O'
	      && formatdata[2] == 'S')
	    {
...
WORD
diskio (struct FMList * list, struct IOStdReq * req)
{
  UBYTE ret;
  WORD cnt;
  WORD ocommand;
  ...
  while (requestmsg
	 (list->workname, MSG_RETRY, MSG_CANCEL, MSG_DEVICE_ERROR, errtxt,
	  ret));
  return (0);
}

void *
operateconfigdefault (struct CMenuConfig *cmc)
{
