...
UBYTE *
siirrabstr (UBYTE * ptr1, UBYTE * ptr2)
{
...
struct FMNode *
adddevice (struct FMList *list, struct DosList *doslist, WORD type)
{
  struct FMNode *node;
  UBYTE empty[80];
  UBYTE *ptr;
...
drives (struct FMList *list)
{
  ...
  while (doslist = NextDosEntry (doslist, LDF_DEVICES))
    adddevice (list, doslist, DLT_DEVICE);
		  ...
		  if (Examine (lock, fib))
		    {
		      ...
		      *ptr++ = ':';
		      *ptr = 0;
		      infodata (NDFILE (node),
				NDCOMM (node) + fmmain.commlen / 2, 0);
		    }
		  UnLock (lock);
		}
	      node->flags = NDEVICE;
  ...
  freemem (fib);
  shellsortlist (list, list->head->succ, UtilityBase, fmmain.fileoffset, 0,
		 0);

  doslist = LockDosList (LDF_ASSIGNS | LDF_READ);
  while (doslist = NextDosEntry (doslist, LDF_ASSIGNS))
    adddevice (list, doslist, DLT_DIRECTORY);
  UnLockDosList (LDF_ASSIGNS | LDF_READ);
  node = list->head->succ;
  while (node->flags & NDEVICE)
    node = node->succ;
