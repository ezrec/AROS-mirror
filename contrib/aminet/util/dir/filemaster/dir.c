...
readdir (struct FMList *list)
{
  ...
  if (lock = fmlock (list, list->path))
	...
	RefreshGList (&list->li->string, fmmain.ikkuna, 0, 1);
      list->flags &= ~LDEVLIST;
      list->flags |= LDIRLIST | LUPDATEMSG;
      updadirmeter (list);
...
void
parentpath (struct FMList *list, BPTR origlock)
{
  BPTR lock = 0, lock2 = 0;
...
void
endofdirtxt (struct FMList *list, struct FMNode *node)
{
  UWORD selfils = 0, seldirs = 0, totfils = 0, totdirs = 0, mfils = 0, mdirs =
    0;
      ...
      for (node2 = list->head; node2->succ; node2 = node2->succ)
	{
	  if (node2->flags & NMATCHED)
	      ...
	      if (node2->flags & NSELECTED)
		{
		  ...
		  if (node2->flags & NFILE)
		    selfils++;
		  if (node2->flags & NDIRECTORY)
		    seldirs++;
		}
	      if (node2->flags & NFILE)
		totfils++;
      ...
      sformatmsg (list->fmmessage1, MSG_DIRREDLINE1, selfils, totfils, mfils,
		  seldirs, totdirs, mdirs, selbytes, totbytes, mtotbytes);
      list->fmmessage2[0] = 0;
      if (node)
	{
	  sformatmsg (list->fmmessage2, MSG_DIRREDLINE2, NDFILE (node),
		      NDPROT (node), NDDATE (node), NDCOMM (node));
	}
    }
  else
    {
...
dirmeters (void)
{
  ...
  for (cnt = 0; cnt < LISTS; cnt++)
    updadirmeter (&fmlist[cnt]);
}

WORD
infodata (UBYTE * file, UBYTE * dst, WORD mode)
{
	  ...
	  if (Info (lock, idata))
	    {
	      ...
	      apu1 = totalbytes >> 10;
	      if (apu1)
		percent = (usedbytes >> 10) * 100 / apu1;
	      if (idata->id_DiskState == ID_VALIDATED)
		state = getstring (MSG_DIRMETER_RW)[1];
	      ret = 1;
	    }
	  UnLock (lock);
	}
  ...
  if (ret)
	...
	{
	case 1:
	  sformat (dst, getstring (MSG_DIRMETER), usedbytes, used, totalbytes,
		   total, percent, state);
	  break;
	case 0:
...
void
updadirmeter (struct FMList *list)
{
  UBYTE varatxt[40];
  ...
  if (list->flags & LDIRLIST)
	...
	{
	  len =
	    TextFit (fmmain.rp, varatxt, strlen (varatxt), &txtext, 0, 1,
		     list->li->dirwidth, 32767);
	  WaitTOF ();
	  RectFill (fmmain.rp, xx - 1, yy - 1, xx + li->dirwidth,
		    yy + fmmain.txtysize - 1);
	  SetAPen (fmmain.rp, fmconfig->txtpen);
	  Move (fmmain.rp, xx + (list->li->dirwidth - txtext.te_Width) / 2,
		yy + fmmain.txtbaseline);
...
setparent (struct FMList *list)
{
  ...
  if (list->flags & LDIRLIST && fmmain.numpathlist)
      ...
      for (cnt = 0; cnt < fmmain.numpathlist; cnt++)
	{
      ...
      if (duplist (list, (struct FMList *) pathlist))
	strcpy (pathlist->path, list->path);
...
initpathlist (void)
{
  ...
  while (pc->parentbuffers > 0)
    {
      pathlist =
	allocvec (0, sizeof (struct FMPathlist) * pc->parentbuffers,
  ...
  if (pathlist)
    {
      for (cnt1 = 0; cnt1 < pc->parentbuffers; cnt1++)
	NewList ((struct List *) (pathlist + cnt1));
    }
...
void *
parentconfigdefault (struct CMenuConfig *cmc)
{
...
WORD
parentconfig (struct GUIBase * gb, struct CMenuConfig * cmc)
{
  WORD c;
  struct ParentConfig *pc;
  LONG bufs;
