...
static void
outputline (struct FMList *list, struct FMNode *node, WORD maxlen,
	    WORD offset, WORD xx, WORD yy)
{
  UBYTE varatxt[512];
  UBYTE *ptr1, *ptr2;
  ...
  if (node && node->succ)
	...
	SetAPen (fmmain.rp, fmconfig->devicepen);
	  ...
	  if (node->flags & NDEVICE)
	    {
	      ...
	      CopyMem (NDCOMM (node) + fmmain.commlen / 2, ptr2,
		       strlen (NDCOMM (node) + fmmain.commlen / 2));
	    }
	}
	...
	rlen = maxlen;
      if (rlen > 0)
	Text (fmmain.rp, &varatxt[offset], rlen);
      else
	rlen = 0;
    }
  else
  ...
  if (rlen < maxlen)
    Text (fmmain.rp, space, maxlen - rlen);
}
...
//nlines=lines to draw
  ...
  if (!rscroll->flags)
      ...
      if (rscroll->LineSpacing == 1)
	{
      ...
      ScrollRaster (fmmain.rp, 0, samount, xx, yy, xx + li->dirwidth - 1,
		    yy + li->dirheight - 1);
      yy += liney;
	...
	{
	  nlines--;
	  outputline (list, node, li->visiblelinesw, list->firstlinew, xx,
		      yy);
	  iteratenode (&node);
	  yy += fmmain.listysize;
	}
      ...
      if (rscroll->LineSpacing == 1)
	{
      ...
      ScrollRaster (fmmain.rp, samount, 0, xx, yy, xx + li->dirwidth - 1,
		    yy + li->dirheight - 1);
      aa = list->firstline;
      while (aa > 0)
	{
...
void
outputlistline (struct FMList *list, struct FMNode *node)
{
  struct FMNode *node2;
  struct ListInfo *li;
  WORD cnt = 0, yy;
  ...
  if (cnt < li->visiblelines)
    {
      ...
      outputline (list, node, li->visiblelinesw, list->firstlinew, li->dirx,
		  yy);
      SetDrMd (fmmain.rp, JAM2);
      SetBPen (fmmain.rp, fmconfig->backpen);
      SetFont (fmmain.rp, fmmain.txtfont);
...
void
drawall (struct ReqScrollStruct *rscroll)
{
  struct FMList *list;
  struct ListInfo *li;
  struct FMNode *node;
...
updatelist (struct FMList *list)
{
  ...
  if (totlines - firstline < visiblelines)
    firstline = totlines - visiblelines;
  if (firstline < 0)
    firstline = 0;
...
static void
createsliderimage (struct Image *im, WORD type)
{
  struct RastPort rp;
  struct BitMap bm;
...
static void
setpot (struct Gadget *gadg, UWORD overlap, UWORD totlines, UWORD vislines,
	UWORD topline, UWORD type)
{
  ...
  if (hidden > 0)
    body =
      (UWORD) (((ULONG) (vislines - overlap) * MAXBODY) /
	       (totlines - overlap));
  else
    body = MAXBODY;
  if (hidden > 0)
  ...
  if (fmmain.ikkuna)
      ...
      if (type)
	{
	  if (pi->HorizPot != pot || pi->HorizBody != body)
	    {
	      NewModifyProp (gadg, fmmain.ikkuna, 0,
			     FREEHORIZ | PROPBORDERLESS, pot, MAXPOT, body,
			     MAXBODY, 1);
	      ...
	      if (size < 8)
		{
	      ...
	      ((struct Image *) gadg->GadgetRender)->Width =
		((struct Image *) gadg->SelectRender)->Width = size;
      ...
      else
	{
	  if (pi->VertPot != pot || pi->VertBody != body)
	    {
	      NewModifyProp (gadg, fmmain.ikkuna, 0,
			     FREEVERT | PROPBORDERLESS, MAXPOT, pot, MAXBODY,
			     body, 1);
	      oldpos = RemoveGadget (fmmain.ikkuna, gadg);
	      size = ((ULONG) body * (ULONG) (gadg->Height)) / (ULONG) 65535;
	      if (size < 8)
		size = 8;
	      ((struct Image *) gadg->GadgetRender)->Height =
		((struct Image *) gadg->SelectRender)->Height = size;
	      createsliderimage ((struct Image *) gadg->GadgetRender, 0);
	      createsliderimage ((struct Image *) gadg->SelectRender, 1);
	      AddGadget (fmmain.ikkuna, gadg, oldpos);
	      RefreshGList (gadg, fmmain.ikkuna, 0, 1);
...
void
setsliders (struct FMList *list)
{
  if (list->li && fmmain.ikkuna)
    {
      setpot (&list->li->slider1, 1, list->totlines, list->li->visiblelines,
	      list->firstline, 0);
      setpot (&list->li->slider2, 1, list->totlinesw, list->li->visiblelinesw,
	      list->firstlinew, 1);
    }
}
