...
WORD selectfont (struct FMHandle *, struct FontContentsHeader *,
		 struct TextAttr *);
...
WORD
showicon (struct FMHandle *h, struct CMenuConfig *cmc)
{
  return (showiconfont (h, 0, cmc));
}

WORD
showfont (struct FMHandle * h, struct CMenuConfig * cmc)
{
  return (showiconfont (h, 1, cmc));
}

WORD
showiconfont (struct FMHandle * h, WORD type, struct CMenuConfig * cmc)
{
  WORD ret, quitti, ignorebutton, started;
  WORD x, y, width, height, pagew, pageh, depth, retry =
    0, apu1, apu2, apu3, txtcolor;
  ...
  if (type)
    {
      ...
      if (!opendisk (list))
	goto sierr;
      if (!(fch = allocvec (list, h->size, 0)))
	goto sierr;
      if (readbufferfile (h, fch, h->size) == 0)
	goto sierr;
    }
  else
    {
      sformat (list->fmmessage1, getstring (MSG_FILECLICK_WBICON),
	       h->filename);
      fmmessage (list);
      if (!openicon (list))
	goto sierr;
...
newicon:
  ...
  if (type == 0)
    {
      sformat (text, getstring (MSG_SHOWICON_TITLE), h->filename, depth,
	       (WORD) power2 (depth));
    }
  else
    {
	...
	{
	  requestmsg (list->workname, 0, MSG_OK, MSG_FMDOS_OPENERR,
		      tattr.ta_Name);
	  goto sierr;
	}
  ...
  scr = OpenScreenTags (0,
			...
			SA_Pens, penlist, SA_Interleaved, 1, TAG_DONE);
  if (!scr)
    {
  ...
  if (ctfont)
      ...
      for (apu1 = 0; apu1 < ctfont->ctf_ColorFontColors->cfc_Count; apu1++)
	{
	  ...
	  SetRGB4 (&scr->ViewPort, apu1 + ctfont->ctf_Low,
		   ((*cfptr) >> 8) & 0x0f, ((*cfptr) >> 4) & 0x0f,
		   ((*cfptr)) & 0x0f);
	}
    }
  else
      ...
      if (coltable)
	{
	  ...
	  *coltable = (ULONG) numcols << 16;
	  ...
	  if (apu1 > 0)
	    {
	      coltable2 = (UBYTE *) coltable;
	      for (apu2 = 0; apu2 < numcols; apu2++)
		SetRGB4 (&scr->ViewPort, apu2 + apu1, *coltable2++,
			 *coltable2++, *coltable2++);
	    }
	  coltable2 = (UBYTE *) coltable;
	  for (apu2 = 0; apu2 < numcols; apu2++)
	    SetRGB4 (&scr->ViewPort, numcols, *coltable2++, *coltable2++,
		     *coltable2++);
#endif
	}
    }
  ...
  win = OpenWindowTags (0,
			...
			IDCMP_ACTIVEWINDOW, WA_Flags,
			WFLG_NOCAREREFRESH | WFLG_RMBTRAP | WFLG_ACTIVATE,
			WA_CustomScreen, scr, TAG_DONE);
  if (!win)
    {
  ...
  if (type)
    {
      SetDrMd (rp, JAM1);
      SetBPen (rp, 0);
      sformat (text, getstring (MSG_SHOWFONT_TITLE), h->filename,
	       tattr.ta_YSize, depth, power2 (depth));
      SetWindowTitles (win, 0, text);
      for (apu1 = 0; apu1 < 256; apu1++)
	chars[apu1] = apu1;
      SetAPen (rp, 0);
      RectFill (rp, win->BorderLeft, win->BorderTop, pagew - win->BorderRight,
		pageh - win->BorderBottom);
      SetFont (rp, tfont);
      SetAPen (rp, txtcolor);
  ...
  else
    {
      ...
      sformatmsg (list->fmmessage2, MSG_SHOWPIC_INFO2, i1->Width, i1->Height,
		  i1->Depth, power2 (i1->Depth), " ", " ", " ");
      fmmessage (list);
	  ...
	  if ((igadg->Flags & GADGHIGHBITS) == GADGHCOMP)
	    {
	      ...
	      rp->Mask = mask - 1;
	      RectFill (rp, x + i1->Width + 32, y, width + x + i1->Width + 32,
			height + y);
	    }
	}
      if (vi)
	{
	  DrawBevelBox (rp, x - 6, y - 3, i1->Width + 12, i1->Height + 6,
			GT_VisualInfo, vi);
	  DrawBevelBox (rp, x + i1->Width + 32 - 6, y - 3, width + 12,
			height + 6, GTBB_Recessed, 1, GT_VisualInfo, vi);
	}
    }
...
WORD
selectfont (struct FMHandle * h, struct FontContentsHeader * fch,
	    struct TextAttr * ta)
{
  struct GUIBase *gb;
  struct FMList *list;
  ...
  switch (fch->fch_FileID)
    ...
    case FCH_ID:
      ...
      if (!
	  (ptr2 = allocvec (list, numsizes * (MAXFONTPATH + 10), MEMF_CLEAR)))
	goto ferr;
      ptr1 = ptr2;
      apu4 = 0;
      for (apu1 = 0; apu1 < numsizes; apu1++)
	{
	  ...
	  for (apu2 = 0; apu2 < numsizes; apu2++)
	    {
	      fc2 =
		(struct FontContents *) ((UBYTE *) fch + 4 +
					 apu2 * sizeof (struct FontContents));
