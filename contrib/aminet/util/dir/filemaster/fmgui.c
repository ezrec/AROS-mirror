...
struct GUISlot *
getguislotonly (struct GUIBase *gb)
{
  struct GUIGroup *gg;
  struct GUISlot *gs;
...
struct GUISlot *
getguislot (struct GUIBase *gb, Object * obj, WORD id, WORD type, void *data)
{
  struct GUISlot *gs;
...
static void
guimulticonvert2 (void **text, LONG * msgs)
{
  void **txtptr;
  txtptr = text;
  while (*msgs)
...
static void
guimulticonvert (UBYTE ** text, LONG * msgs)
{
  UBYTE **txtptr;
  txtptr = text;
  while (*msgs)
...
static struct GUISlot *
reqinfo2 (struct GUIBase *gb, UBYTE * txt, WORD id, UWORD flags)
{
  WORD numlines;
  UBYTE *ptr1, *mem;
  struct GUISlot *gs;
  ...
  *ptr1 = 0;
  ...
  gs = getguislot (gb, BGUI_NewObject (BGUI_INFO_GADGET,
				       INFO_HorizOffset, 4, INFO_VertOffset,
				       4, INFO_TextFormat, mem, INFO_MinLines,
				       numlines, INFO_FixTextWidth, TRUE,
				       FRM_Type, FRTYPE_BUTTON, FRM_Flags,
				       FRF_RECESSED, TAG_DONE), id, 0, mem);
  if (gs)
    gs->shortcut[0] = sh;
  return (gs);
}
...
struct GUISlot *
reqinfo (struct GUIBase *gb, UBYTE * txt, WORD id, UWORD flags)
{
  return (reqinfo2 (gb, txt, id, flags));
}

struct GUISlot *
reqinfomsg (struct GUIBase *gb, WORD msg, WORD id, UWORD flags)
{
...
static struct GUISlot *
reqcyclei (struct GUIBase *gb, WORD id, WORD * selected, UBYTE ** cycles)
{
  struct CycleData *cd;
  WORD num, len;
  UBYTE **cycs;
...
struct GUISlot *
reqcycle2msg (struct GUIBase *gb, WORD id, WORD * selected)
{
  UBYTE *cycletab[3];
...
struct GUISlot *
reqcyclemsg (struct GUIBase *gb, WORD id, WORD * selected, LONG cycles, ...)
{
  UBYTE *cycletab[64];
...
struct GUISlot *
reqcycle (struct GUIBase *gb, WORD id, WORD * selected, UBYTE * cycles, ...)
{
...
struct GUISlot *
reqtoggle (struct GUIBase *gb, UBYTE * txt, WORD id, UWORD flags,
	   BOOL * selected)
{
#ifdef GUIDEBUG
  printf ("TOGGLE:\n");
...
struct GUISlot *
reqtogglemsg (struct GUIBase *gb, WORD txt, WORD id, UWORD flags,
	      BOOL * selected)
{
...
struct GUISlot *
reqbutton (struct GUIBase *gb, UBYTE * txt, WORD id, UWORD flags)
{
#ifdef GUIDEBUG
  printf ("BUTTON:\n");
#endif
  return (getguislot (gb, BGUI_NewObject (BGUI_BUTTON_GADGET,
					  ...
					  LAB_Underscore, '_',
					  GA_ID, id,
					  TAG_DONE), id, BUTTON_KIND, 0));
}

struct GUISlot *
reqbuttonmsg (struct GUIBase *gb, WORD msg, WORD id, UWORD flags)
{
...
struct GUISlot *
reqstring (struct GUIBase *gb, WORD id, UBYTE * str, WORD max)
{
  WORD vis;
  ...
  return (getguislot (gb, BGUI_NewObject (BGUI_STRING_GADGET,
					  ...
					  GACT_STRINGCENTER, STRINGA_TextVal,
					  str, STRINGA_MinCharsVisible, vis,
					  STRINGA_MaxChars, max, GA_TabCycle,
					  TRUE, GA_ID, id, TAG_DONE), id,
		      STRING_KIND, str));
}
...
struct GUISlot *
reqinteger (struct GUIBase *gb, WORD id, LONG * value, LONG min, LONG max)
{
#ifdef GUIDEBUG
  printf ("INTEGER:\n");
#endif
  return (getguislot (gb, BGUI_NewObject (BGUI_STRING_GADGET,
					  ...
					  STRINGA_Justification,
					  GACT_STRINGCENTER, STRINGA_LongVal,
					  *value, STRINGA_MaxChars, 10,
					  STRINGA_MinCharsVisible, 10,
					  GA_TabCycle, TRUE, GA_ID, id,
					  TAG_DONE), id, INTEGER_KIND,
		      value));
}
...
struct GUISlot *
reqpalette (struct GUIBase *gb, WORD id, UBYTE * col)
{
#ifdef GUIDEBUG
  printf ("PALETTE:\n");
#endif
  return (getguislot (gb, BGUI_NewObject (BGUI_PALETTE_GADGET,
					  ...
					  PALETTE_CurrentColor, *col, GA_ID,
					  id, TAG_DONE), id, PALETTE_KIND,
		      col));
}
...
static void
activatestrings (struct GUIBase *gb)
{
  WORD cnt1, cnt2;
  struct GUIGroup *gg;
  struct GUISlot *gs;
...
settabcycle (struct GUIBase *gb)
{
  ...
  ss[cnt3++] = 0;
  if (cnt3 > 2)
    DoMethodA (gb->obj, (Msg) ss);
}

void
setconnectgroup (struct GUIBase *gb, WORD group, WORD alignment, WORD dest)
{
...
void
connectgroups (struct GUIBase *gb, WORD destgroup, WORD srcgroup)
{
  struct GUISlot *gs;
  WORD apu1;
...
void
setguigroup (struct GUIBase *gb, WORD group, WORD alignment)
{
...
freereq (struct GUIBase *gb)
{
  ...
  for (cnt1 = 0; cnt1 < GUIGROUPS; cnt1++)
      ...
      if (gg)
	{
	  for (cnt2 = 0; cnt2 < gg->used; cnt2++)
	    {
	      if (dispo && gg->slots[cnt2].obj)
		DisposeObject (gg->slots[cnt2].obj);
...
Object *
recurseslot (struct GUIBase *gb, WORD group)
{
  Object *obj = 0;
  struct GUIGroup *gg;
  struct GUISlot *gs;
...
initrequ (struct GUIBase * gb)
{
  ...
  if (gb->screen)
    tptr[3] = (ULONG) gb->screen;
  else
    tptr[3] = (ULONG) fmmain.naytto;
...
openreq (struct GUIBase * gb)
{
  ...
  if (gb && gb->obj)
    {
      if ((gb->win = (struct Window *) DoMethod (gb->obj, WM_OPEN)))
	return (1);
    }
  return (0);
}
...
BOOL
initreq (struct GUIBase * gb)
{
  return (initrequ (gb));
}

WORD
quickreq (struct GUIBase * gb)
{
  WORD ret;
...
void
startbuttonbar (struct GUIBase *gb)
{
...
void
buttonbari (struct GUIBase *gb, void **buttons)
{
  startbuttonbar (gb);
  while (*buttons)
    {
...
void
buttonbar (struct GUIBase *gb, void *button, ...)
{
  buttonbari (gb, &button);
}

void
buttonbarmsg (struct GUIBase *gb, LONG button, ...)
{
  void *text[32];
...
WORD
reqinfowindow (UBYTE * title, UBYTE * body, UWORD flags, LONG button, ...)
{
  struct GUIBase *gb;
  void *text[32];
  WORD ret;
...
static void
checkgui (struct GUIBase *gb, WORD id)
{
  struct GUISlot *gs;
  struct GUIGroup *gg;
  struct CycleData *cd;
...
LONG
getobject (struct GUIBase *gb, WORD id)
{
  WORD cnt1, cnt2;
  struct GUISlot *gs;
  struct GUIGroup *gg;
	      ...
	      if (gs->id == id)
		{
		  switch (gs->type)
		    ...
		    case 10:
		      ...
		      break;
		    case PALETTE_KIND:
		      GetAttr (PALETTE_CurrentColor, gs->obj,
			       (ULONG *) & data);
		      break;
		    }
		}
...
void
setobject (struct GUIBase *gb, WORD id, LONG data)
{
  WORD cnt1, cnt2;
  struct GUISlot *gs;
  struct GUIGroup *gg;
	  ...
	  for (cnt2 = 0; cnt2 < gg->used; cnt2++)
	      ...
	      if (gs->id == id)
		{
		  switch (gs->type)
		    {
		    case STRING_KIND:
		      SetGadgetAttrs ((struct Gadget *) gs->obj, gb->win, 0,
				      STRINGA_TextVal, data, TAG_DONE);
		      break;
		    case INTEGER_KIND:
		      SetGadgetAttrs ((struct Gadget *) gs->obj, gb->win, 0,
				      STRINGA_LongVal, data, TAG_DONE);
		      *((LONG *) gs->data) = data;
		      break;
		    case CYCLE_KIND:
		      cd = (struct CycleData *) gs->data;
		      SetGadgetAttrs ((struct Gadget *) gs->obj, gb->win, 0,
				      CYC_Active, data, TAG_DONE);
		      *cd->selected = data;
		      break;
		    case 10:
		      SetGadgetAttrs ((struct Gadget *) gs->obj, gb->win, 0,
				      GA_SELECTED, data, TAG_DONE);
		      *((BOOL *) gs->data) = data;
		      break;
		    case PALETTE_KIND:
		      SetGadgetAttrs ((struct Gadget *) gs->obj, gb->win, 0,
				      PALETTE_CurrentColor, data, TAG_DONE);
		      break;
		    }
		}
	    }
...
void
denaobject (struct GUIBase *gb, WORD id, BOOL flag)
{
  WORD cnt1, cnt2;
  struct GUISlot *gs;
  struct GUIGroup *gg;
...
WORD
reqmsghandler (struct GUIBase *gb)
{
  Object *obj;
  ULONG signal, sigs;
  ULONG rc;
