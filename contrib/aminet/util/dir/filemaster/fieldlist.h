...
SetFLAttr (register __a0 FLD * fld, register __a1 struct TagItem *attr)
{
...
DispatchFL (register __a0 Class * cl, register __a2 Object * obj,
	    register __a1 Msg msg)
{
  ...
  switch (msg->MethodID)
	...
	{
	  if (acceptapu && acceptapu == fld->fld_Accept[cnt1])
	    break;
	}
      if (cnt1 < 6)
	{
      ...
      while (entry = (APTR) FirstSelected (DROP (msg)->bmd_Source))
	{
	  ...
	  if (fld->fld_SortDrops == FALSE)
	    ...
	    DoMethod (obj, LVM_ADDSINGLE, NULL, entry, LVAP_SORTED,
		      LVASF_SELECT);
      ...
      BGUI_DoGadgetMethod (DROP (msg)->bmd_Source,
			   ...
			   DROP (msg)->bmd_SourceReq, LVM_REFRESH, NULL);
      rc = 1;
      break;
