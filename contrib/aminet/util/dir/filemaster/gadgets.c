...
gretry:
  ...
  *ptr1 = 0;
  ...
  newwindow.Height =
    lines * (tfh + 1) + fmmain.naytto->WBorTop +
    fmmain.naytto->Font->ta_YSize + 1 + 2 * fmconfig->spaceh;
  newwindow.Width = rows + fmconfig->spacew * 4;
  newwindow.Flags =
    WFLG_DRAGBAR | WFLG_DEPTHGADGET | WFLG_CLOSEGADGET | WFLG_SMART_REFRESH |
    WFLG_RMBTRAP | WFLG_ACTIVATE, newwindow.IDCMPFlags =
    IDCMP_CLOSEWINDOW | IDCMP_VANILLAKEY | IDCMP_MOUSEBUTTONS,
...
//newwindow.CheckMark=0;
  ...
  if (newwindow.Height > fmmain.naytto->Height
      || newwindow.Width > fmmain.naytto->Width)
    {
      trycnt++;
      if (trycnt == 1)
	goto gretry;
    }
  getwinposition (fmmain.naytto, &newwindow, 0, newwindow.Width / 2,
		  newwindow.Height / 2);
  ...
  if (window = OpenWindow (&newwindow))
    {
      ...
      if (trycnt == 0)
	SetFont (window->RPort, fmmain.listfont);
      else
	SetFont (window->RPort, fmmain.smallfont);
      SetAPen (window->RPort, fmconfig->txtpen);
      SetDrMd (window->RPort, JAM1);
      apu1 =
	fmmain.naytto->WBorTop + fmmain.naytto->Font->ta_YSize + 1 +
	fmconfig->spaceh + tfb;
      while (lines--)
	{
      ...
      while (quit == -2)
	{
	  if (smallevent (window, &class, &code))
	    quit = 0;
	  if (class == IDCMP_CLOSEWINDOW || class == IDCMP_VANILLAKEY
...
void
getwinposition (struct Screen *scr, struct NewWindow *win, WORD flag,
		WORD apu1, WORD apu2)
{
  WORD mousex, mousey;
  ...
  if (mousex + win->Width > dwidth + dxoffset)
    mousex = dwidth + dxoffset - win->Width;
  if (mousey + win->Height > dheight + dyoffset)
    mousey = dheight + dyoffset - win->Height;
  if (mousex < dxoffset)
    mousex = dxoffset;
  if (mousey < dyoffset)
    mousey = dyoffset;
  if (mousex + win->Width > scr->Width)
    mousex = scr->Width - win->Width;
  if (mousey + win->Height > scr->Height)
    mousey = scr->Height - win->Height;
  if (mousex < 0)
    mousex = 0;
  if (mousey < 0)
    mousey = 0;
...
gretry:
  ...
  while (*tauluptr != 101)
      ...
      while (*tauluptr != 100)
	{
	  switch (*tauluptr)
		...
		textextent (&rp, (UBYTE *) (*(tauluptr + 2)), &apu1,
			    &txoffset);
	      else
		textextentuc (&rp, (UBYTE *) (*(tauluptr + 2)), &apu1,
			      &txoffset);
	      break;
	    case BUTTON_KIND:
	      textextentuc (&rp, (UBYTE *) (*(tauluptr + 2)), &apu1,
		...
		{
		  textextentuc (&rp, (UBYTE *) (*(tauluptr + 2 + apu2)),
				&apu3, &txoffset);
		  apu3 += 8 * spacew;
		  if (apu3 > apu1)
		    apu1 = apu3;
  ...
  while (*tauluptr != 101)
      ...
      while (*tauluptr != 100)
	{
	  ...
	  if (apu1)
	    {
	      if (txtlentab[kplcnt1] > txtmaxlentab[apu1])
		txtmaxlentab[apu1] = txtlentab[kplcnt1];
	    }
	  kplcnt1++;
	  tauluptr += ((*(tauluptr + 1)) & 0xff) + 2;
	}
  ...
  newwindow.Height =
    fmmain.naytto->WBorTop + fmmain.naytto->Font->ta_YSize + 1 +
    gadhnum * (tfh + valikoko) + 2 * spaceh;
  newwindow.Width = maxgadw + 3 * spacew;
  newwindow.Flags =
    WFLG_DRAGBAR | WFLG_DEPTHGADGET | WFLG_CLOSEGADGET | WFLG_SMART_REFRESH |
    WFLG_ACTIVATE | WFLG_RMBTRAP, newwindow.IDCMPFlags =
    IDCMP_REFRESHWINDOW | IDCMP_GADGETDOWN | IDCMP_GADGETUP |
    IDCMP_CLOSEWINDOW | IDCMP_ACTIVEWINDOW | IDCMP_RAWKEY,
    newwindow.DetailPen = fmconfig->txtpen;
  newwindow.BlockPen = fmconfig->hilipen;
  newwindow.FirstGadget = 0;
//newwindow.CheckMark=0;
  ...
  if (!(gtgad = CreateContext (&glist)))
    goto endreq;

  arrayptr = 0;
  gadhnum =
    fmmain.naytto->WBorTop + fmmain.naytto->Font->ta_YSize + 1 + spaceh;
  kplcnt1 = 0;
  kplcnt2 = 0;
  gadgetnum = 0;
  tauluptr = realtaulu + 1;
      ...
      while (txtlentab[kplcnt1])
	{
	  ...
	  switch (gadgettype)
	      ...
	      if ((*(tauluptr + 1) & GADGTEXTNUC) == 0)
		{
	      ...
	      if (vptr)
		shortcuts[gadgetnum] = ToUpper (*(vptr + 1));
	      break;
	    case CHECKBOX_KIND:
	      tags[0] = GT_Underscore;
	      tags[1] = '_';
	  ...
	  if (gadgettype != TEXT_KIND)
	    {
	      ...
	      if ((*(tauluptr + 1)) >> 24)
		ngad.ng_GadgetID = ((*(tauluptr + 1)) >> 24) | 0x100;
	    }
	  else
	    {
	      ngad.ng_GadgetID = 0;
		...
		{
		  textextentuc (&rp, (UBYTE *) (*(tauluptr + 2)),
				&ngad.ng_Width, &txoffset);
		  if ((*(tauluptr + 1)) & GADGTEXTRIGHT)
		    ngad.ng_LeftEdge += apu4 - ngad.ng_Width;
		}
	    }
	  if (trycnt <= 1)
	    ngad.ng_TextAttr = &fmconfig->reqfontattr;
	  else
	    ngad.ng_TextAttr = &fmconfig->smallfontattr;
	  ...
	  ngad.ng_UserData = 0;
	  if (gadgettype == TEXT_KIND || gadgettype == BUTTON_KIND
	      || gadgettype == TOGGLE_KIND)
	    {
	  ...
	  if (gadgettype == TOGGLE_KIND)
	    {
	      if (!
		  (gtgad =
		   CreateGadgetA (BUTTON_KIND, gtgad, &ngad,
				  (struct TagItem *) tags)))
		goto endreq;
	      gtgad->Activation |= TOGGLESELECT;
	      if (*(tauluptr + 3))
		gtgad->Flags |= SELECTED;
	    }
	  else
	    {
	      if (!
		  (gtgad =
		   CreateGadgetA (gadgettype, gtgad, &ngad,
				  (struct TagItem *) tags)))
		goto endreq;
	    }
	  ...
	  if (gadgettype != TEXT_KIND)
	    gadgetnum++;
	  if ((gadgettype == STRING_KIND || gadgettype == INTEGER_KIND)
	      && !fstringgad)
	    fstringgad = gtgad;
	  tauluptr += ((*(tauluptr + 1)) & 0xff) + 2;
  ...
  getwinposition (screen, &newwindow, center, apu2 + apu4 / 2,
		  apu1 + apu3 / 2);
  if (!(window = OpenWindowTagList (&newwindow, 0)))
    goto endreq;
...
//fmconfig->dosbuffersize|=(hcsum&0xff000000);
  ...
  while (quit == -2)
	...
	{
	  sigs =
	    Wait ((1L << window->UserPort->mp_SigBit) | SIGBREAKF_CTRL_C);
	...
	case IDCMP_ACTIVEWINDOW:
	  if (fstringgad && !(fstringgad->Flags & GADGDISABLED))
		  ...
		  || (asciicode == 13 && retgadget == shortcuts[apu1]))
		{
		  ...
		  switch (*tauluptr)
			...
			*(tauluptr + 3) = 0;
			...
			apu1++;
		      if (apu1 < 0)
			apu1 = ((*(tauluptr + 1)) & 0xff) - 2;
		      if (apu1 >= ((*(tauluptr + 1)) & 0xff) - 1)
			apu1 = 0;
		      GT_SetGadgetAttrs (vgad, window, 0, GTCY_Active, apu1,
					 TAG_DONE);
			...
			apu1++;
		      ...
		      GT_SetGadgetAttrs (vgad, window, 0, GTPA_Color, apu1,
					 TAG_DONE);
		      *(tauluptr + 2) = apu1;
		      break;
		    }
	      ...
	      if (addressi[apu1 * 2 + 1] == (ULONG) vgad)
		{
		  ...
		  switch (*tauluptr)
		    ...
		    case INTEGER_KIND:
		      ...
		      is->value =
			((struct StringInfo *) vgad->SpecialInfo)->LongInt;
		      if (is->value < is->min)
			is->value = is->min;
		      if (is->value > is->max)
			is->value = is->max;
		      ((struct StringInfo *) vgad->SpecialInfo)->LongInt =
			is->value;
		      sformat (((struct StringInfo *) vgad->SpecialInfo)->
			       Buffer, "%ld", is->value);
		      RefreshGList (vgad, window, 0, 1);
		    case STRING_KIND:
		      if (code != 0x09)
...
*/
  ...
  for (apu1 = 0; apu1 < gadgetnum; apu1++)
	...
	case STRING_KIND:
	  ...
	  strcpy ((UBYTE *) * (tauluptr + 2),
		  (UBYTE *) (((struct StringInfo *) vgad->SpecialInfo)->
			     Buffer));
	  break;
	case INTEGER_KIND:
	  is = ((struct IntString *) (*(tauluptr + 2)));
...
WORD
pressgadget (struct Window * win, struct Gadget * gad)
{
  WORD pos;

  if (gad < (struct Gadget *) 4)
    return (1);
...
void
unpressgadget (struct Window *win, struct Gadget *gad)
{
  WORD pos;
...
void
dotext (WORD * c, ULONG * t, WORD msg, ULONG type)
{
  t += *c;
  *t++ = TEXT_KIND;
  *t++ = 1 | (type & 0xffffff00);
...
void
dostring (WORD * c, ULONG * t, UBYTE * msg, ULONG type)
{
  UBYTE *ptr;
  ULONG *tt;
...
void
dobutton (WORD * c, ULONG * t, WORD msg, ULONG type)
{
  dobuttonstring (c, t, getstring (msg), type);
}

void
dobuttonstring (WORD * c, ULONG * t, UBYTE * msg, ULONG type)
{
  t += *c;
  *t++ = BUTTON_KIND;
  *t++ = 1 | (type & 0xffffff00);
...
void
doletterbutton (WORD * c, ULONG * t, WORD msg, WORD letter, ULONG type)
{
  donumbuttonstring (c, t, getstring (msg), letter + 'A' - '0', type);
}

void
doletterbuttonstring (WORD * c, ULONG * t, UBYTE * msg, WORD letter,
		      ULONG type)
{
...
void
donumbutton (WORD * c, ULONG * t, WORD msg, WORD num, ULONG type)
{
  donumbuttonstring (c, t, getstring (msg), num, type);
}

void
donumbuttonstring (WORD * c, ULONG * t, UBYTE * msg, WORD num, ULONG type)
{
  UBYTE *txt;
  ...
  *t++ =
    (LONG) (num +
	    '0') << 24 | 14L << 16 | 1 | GADGTEXTNUC | (type & 0xffffff00);
  *t++ = (ULONG) msg;
...
ULONG *
docyclenumber (WORD * c, ULONG * t, ULONG type, WORD sel, WORD s, WORD e)
{
  WORD cnt, num;
  ULONG *dat;
...
ULONG *
docyclestring (WORD * c, ULONG * t, WORD num, ULONG type, WORD sel,
	       UBYTE * txt1, ...)
{
  WORD cnt;
  ULONG *dat;
...
ULONG *
docycle (WORD * c, ULONG * t, WORD num, ULONG type, WORD sel, WORD msg1, ...)
{
  WORD cnt;
  ULONG *dat;
...
ULONG *
doswitch (WORD * c, ULONG * t, WORD msg, WORD num, ULONG type)
{
  return (doswitchstring (c, t, getstring (msg), num, type));
}

ULONG *
doswitchstring (WORD * c, ULONG * t, UBYTE * msg, WORD num, ULONG type)
{
  ULONG *dat;
...
ULONG *
dotoggle (WORD * c, ULONG * t, WORD msg, WORD num, ULONG type)
{
  return (dotogglestring (c, t, getstring (msg), num, type));
}

ULONG *
dotogglestring (WORD * c, ULONG * t, UBYTE * msg, WORD num, ULONG type)
{
  ULONG *dat;
  WORD numi = num ? 1 : 0;
...
ULONG *
dointegergad (WORD * c, ULONG * t, struct IntString * is, ULONG type)
{
  ULONG *dat;
...
void
dostringgad (WORD * c, ULONG * t, UBYTE * string, WORD maxlen, ULONG type)
{
  t += *c;
  *t++ = STRING_KIND;
  *t++ = 2 | (type & 0xffffff00);
...
void
supercheck (struct Window *win, ULONG * super, ULONG * taulu,
	    ULONG * addressi)
{
  struct Gadget *vgad;
  ULONG *tauluptr;
  ...
  while (super && super[cnt])
	...
	{
	case TOGGLE_KIND:
	  if ((vgad->Flags & SELECTED && gadvalue)
	      || (!(vgad->Flags & SELECTED) && !gadvalue))
	    selected = 1;
	  break;
	case CYCLE_KIND:
	  if (*(tauluptr + ((*(tauluptr + 1)) & 0xff) + 1) == gadvalue)
	    selected = 1;
	  break;
	case PALETTE_KIND:
	  if (*(tauluptr + 2) == gadvalue)
	    selected = 1;
	  break;
	}
      if ((selected && !sel2) || (!selected && sel2))
	{
	  for (apu1 = 0; apu1 < listnum; apu1++)
	      ...
	      else if (!(lapu & SDISABLE) && (vgad->Flags & GADGDISABLED))
		{
		  ...
		  AddGadget (win, vgad, -1);
		  RefreshGList (vgad, win, 0, 1);
		  if ((vgad->GadgetType & 7) == STRGADGET)
		    ActivateGadget (vgad, win, 0);
		}
	      if (lapu & SCHANGE)
		{
		  switch (*tauluptr)
		      ...
		      if (*(tauluptr + 3) != apu2)
			{
			  ...
			  *(tauluptr + 3) = apu2;
		      ...
		      break;
		    case CYCLE_KIND:
		      if (*(tauluptr + ((*(tauluptr + 1)) & 0xff) + 1) !=
			  apu2)
			{
			  *(tauluptr + ((*(tauluptr + 1)) & 0xff) + 1) = apu2;
			  GT_SetGadgetAttrs (vgad, win, 0, GTCY_Active, apu2,
					     TAG_DONE);
			}
		      break;
		    case PALETTE_KIND:
			...
			{
			  *(tauluptr + 2) = apu2;
			  GT_SetGadgetAttrs (vgad, win, 0, GTPA_Color, apu2,
					     TAG_DONE);
			}
		      break;
		    }
