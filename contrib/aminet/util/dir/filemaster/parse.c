...
WORD
checkmatch (UBYTE * token1, UBYTE * token2, UBYTE * token3,
	    struct FMNode *node, struct Match *match, WORD matchtype)
{
  ...
  if (match->mcase)
    {
      if (match->mcase == 2)
	{
	  if (MatchPattern (token2, NDFILE (node)))
	    newnode = 1;
	}
      else
	{
	  if (MatchPatternNoCase (token1, NDFILE (node)))
	    newnode = 1;
	}
    }
  if (match->mflags)
    {
      ...
      if (MatchPatternNoCase (token3, apuflags))
	newnode = 1;
    }
  if (match->msize && (node->flags & NFILE))
	...
	case 5:		//between
	  if (node->numlen >= match->patsize
	      && node->numlen <= match->patsize2)
	    newnode = 1;
	  break;
	case 6:		//notbetween
	  if (node->numlen < match->patsize || node->numlen > match->patsize2)
	    newnode = 1;
	  break;
	}
    }
  ...
  if (node->flags & NFILE
      && (match->mfiles == 1 || (match->mfiles == 0 && newnode)))
    matched = 1;
  if (node->flags & NDIRECTORY
      && (match->mdirs == 1 || (match->mdirs == 0 && newnode)))
    matched = 1;
...
void
parselist (struct FMList *list, WORD matchtype)
{
  struct FMNode *node;
  ...
  if (matchtype >= 10 && matchtype < 19)
    match = getconfignumber (INCLUDECONFIG)->moreconfig;
  if (matchtype >= 20)
    match = getconfignumber (EXCLUDECONFIG)->moreconfig;
...
WORD
parsematch (struct FMList *list, struct CMenuConfig *cmc)
{
  struct GUIBase *gb;
  WORD c, pc;
  WORD ccase, cfiles, cdirs, csize, cdate, cflags, ccomm, csortby, csortud,
    cmix, cdfirst;
  LONG sizeis, sizeis2;
  UBYTE patdate[32];
  UBYTE patdate2[32];
  ...
  match = getconfig (cmc);
  if (cmc->cmenucount == PARSECONFIG)
    pc = 1;
  else
    pc = 0;
  sformat (titlestr, "%s (%ld)", findbuildstring (cmc),
	   (WORD) list->li->linumber);
  ...
  if (pc)
    reqinfomsg (gb, MSG_PARSE_SORTBY, 105, guiUC | guiLEFT);
  ...
  if (pc)
    {
      ...
      reqcyclemsg (gb, 106, &csortby, MSG_PARSE_NAMEMATCH,
		   MSG_PARSE_SIZEMATCH, MSG_PARSE_DATEMATCH,
		   MSG_PARSE_COMMENTMATCH, 0);
    }

  setconnectgroup (gb, 5, 1, 14);
  ccase = match->mcase;
  reqcyclemsg (gb, 100, &ccase, MSG_CONFIG_OFF, MSG_PARSE_CASEINSENSITIVE,
	       MSG_PARSE_CASESENSITIVE, 0);
  csize = match->msize;
  reqcyclemsg (gb, 101, &csize, MSG_CONFIG_OFF, MSG_PARSE_SIZEEQUAL,
	       MSG_PARSE_SIZENOEQUAL, MSG_PARSE_SIZESMALLER,
	       MSG_PARSE_SIZELARGER, MSG_PARSE_BETWEEN, MSG_PARSE_NOTBETWEEN,
	       0);
  cdate = match->mdate;
  reqcyclemsg (gb, 102, &cdate, MSG_CONFIG_OFF, MSG_PARSE_DATEOLDER,
	       MSG_PARSE_DATENEWER, MSG_PARSE_BETWEEN, MSG_PARSE_NOTBETWEEN,
	       0);
  cflags = match->mflags;
  ...
  if (pc)
    {
      ...
      reqcyclemsg (gb, 105, &csortud, MSG_PARSE_SORTUP, MSG_PARSE_SORTDOWN,
		   0);
    }
  ...
  if (pc)
    reqinfomsg (gb, MSG_PARSE_MIX, 110, guiUC | guiLEFT);
  ...
  cfiles = match->mfiles;
  reqcyclemsg (gb, 111, &cfiles, MSG_PARSE_TYPE1, MSG_PARSE_TYPE2,
	       MSG_PARSE_TYPE3, 0);
  setconnectgroup (gb, 8, 1, 13);
  if (pc)
    reqinfomsg (gb, MSG_PARSE_DRAWERS, 120, guiUC | guiLEFT);
  ...
  cdirs = match->mdirs;
  reqcyclemsg (gb, 121, &cfiles, MSG_PARSE_TYPE1, MSG_PARSE_TYPE2,
	       MSG_PARSE_TYPE3, 0);
...
void *
parseconfigdefault (struct CMenuConfig *cmc)
{
  struct Match *config;
  WORD ret;
  struct DateStamp ds;
