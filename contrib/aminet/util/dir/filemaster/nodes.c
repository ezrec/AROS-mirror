...
#include "fmdos.h"
#include "fmlocale.h"
//extern struct LocaleBase *XLocaleBase;
...
void shellsortlist (struct FMList *, struct FMNode *, struct Library *, WORD,
		    WORD, WORD);
...
void
shellsortlist (struct FMList *list, struct FMNode *node, struct Library *ub,
	       WORD fo, WORD stype, WORD sud)
{
  struct FMNode *node2;
  ...
  i = nodes;
  while (i--)
    *--ptr1 = (struct FMNode *) RemTail ((struct List *) list);
  ...
  for (range = nodes / 2; range > 0; range /= 2)
      ...
      for (i = range; i < nodes; i++)
	{
	  ...
	  while (j >= 0)
		...
		case 0:
		  if (Stricmp
		      (NDFILE (sortbuf[j]), NDFILE (sortbuf[j + range])) > 0)
		    apu = 1;
		  break;
		case 1:
		  if (sortbuf[j]->numlen > sortbuf[j + range]->numlen)
		    apu = 1;
		  break;
		case 2:
		  if (sortbuf[j]->numdate > sortbuf[j + range]->numdate)
		    apu = 1;
		  break;
		case 3:
		  if (Stricmp
		      (NDCOMM (sortbuf[j]), NDCOMM (sortbuf[j + range])) > 0)
		    apu = 1;
		  break;
		}
	      if (apu ^ sud)
  ...
  i = nodes;
  while (i--)
    AddTail ((struct List *) list, (struct Node *) *ptr1++);

  freemem (sortbuf);
}

void
csortlist (struct FMList *list)
{
  struct FMList list2;
  ...
  if ((list->flags & LDEVLIST) || (!list->li))
    return;
  ...
  if (match->msortby || match->msortud)
    shellsortlist (list, list->head, UtilityBase, fmmain.fileoffset,
		   match->msortby, match->msortud);
  if (match->mdrawersfirst)
    ftest = NFILE;
  else
    ftest = NDIRECTORY;
  if (!match->mmix)
      ...
      while (node->succ)
	{
      ...
      if (match->msortby == 1 && ftest == NDIRECTORY)
	shellsortlist (&list2, list2.head, UtilityBase, fmmain.fileoffset, 0,
		       0);
...
struct FMNode *
disknode (struct FMList *list, UBYTE * newname)
{
  ...
  while (node2->succ)
    {
      node3 = node2->succ;
      if (!Stricmp (NDFILE (node2), newname))
...
WORD
duplist (struct FMList * list1, struct FMList * list2)
{
  struct FMNode *node1;
  struct FMNode *node2;
...
void
removenode (struct FMList *list, struct FMNode *node)
{
  Remove ((struct Node *) node);
  freevecpooled (node);
  list->totlines--;
  if (list->totlines - list->firstline < list->li->visiblelines)
    list->firstline = list->totlines - list->li->visiblelines;
  if (list->firstline < 0)
    list->firstline = 0;
  setsliders (list);
}
...
struct FMNode *
scrollifneeded (struct FMList *l, UWORD * keyline, UWORD * keylinew)
{
  struct FMNode *node;
  ...
  if (*keyline > 65000)
    *keyline = 0;
  if (*keyline >= l->totlines)
    *keyline = l->totlines - 1;
  if (*keylinew > 65000)
    *keylinew = 0;
  if (*keylinew >= l->totlinesw)
    *keylinew = l->totlinesw - 1;
  kline = *keyline;
  klinew = *keylinew;
...
struct FMNode *
findselnodeall (struct FMList *list)
{
  struct FMNode *node;
  UWORD line = 0, linew;
...
struct FMNode *
findselnode (struct FMList *list)
{
  struct FMNode *node;
  UWORD line = 0, linew;
...
void __asm rawdo (register __a0 UBYTE *, register __a1 void *,
		  register __a3 UBYTE *);
#else

AROS_UFH3 (VOID, formathook,
	   AROS_UFCA (struct Hook *, putCharFunc, A0),
	   AROS_UFCA (struct Locale *, locale, A2), AROS_UFCA (char, c, A1))
{
...
void
sformatmsg (UBYTE * ptr, LONG form, ...)
{
  va_list args;
  va_start (args, form);
...
void
sformatti (UBYTE * ptr, UBYTE * form, UBYTE * args)
{
  struct Hook hook;
...
allocnode (void)
{
  ...
  if (node)
    node->nodelen = fmmain.totnodelen;
  return (node);
}

void
strcpymsg (UBYTE * ptr, WORD msg)
{
  strcpy (ptr, getstring (msg));
}

struct FMNode *
iteratenode (struct FMNode **node)
{
  struct FMNode *node2;
  ...
  if (node2 && (node2->succ))
    node2 = node2->succ;
  while (node2 && node2->succ && !(node2->flags & NMATCHED))
    node2 = node2->succ;
  if (node2 == 0 || node2->succ == 0)
    node2 = 0;
  *node = node2;
  return (node2);
}
...
WORD
sselected (struct FMList * list, WORD mode)
{
  struct FMNode *node;
  ...
  if (!(list->flags & LDIRLIST))
      ...
      for (;;)
	{
	  ...
	  if (node->flags & NSELECTED)
	    {
	      if (mode > 0 && node->flags & NFILE)
		return (1);
	      if (mode < 0 && node->flags & (NFILE | NDIRECTORY))
		return (1);
	    }
	}
...
ULONG
dstolong (struct DateStamp * ds)
{
  return (((ULONG) ds->ds_Days * 24 * 60 + (ULONG) ds->ds_Minute) * 60 +
	  (ULONG) ds->ds_Tick / TICKS_PER_SECOND);
}

void
longtods (struct DateStamp *ds, ULONG date)
{
...
datestringtolong (UBYTE * date)
{
  ...
  if (StrToDate (&dt))
    return (dstolong (&dt.dat_Stamp));
  return (0xffffffff);
}

void
longtodatestring (UBYTE * sdate, ULONG date)
{
  struct DateTime dt;
  UBYTE pattime[18];
  UBYTE patdate[34];
...
void
siirran (UBYTE * dst, UBYTE * src, WORD len)
{
  strncpy (dst, src, len);
  *(dst + len) = 0;
}
...
void
fittext (struct RastPort *rp, UBYTE * txt, WORD txtlen, WORD x, WORD y,
	 WORD width, WORD mode)
{
  WORD len;
...
strtohex (UBYTE * ptr)
{
  ...
  bapu1 = ToUpper (ptr[0]);
  bapu2 = ToUpper (ptr[1]);
  if (((bapu2 < '0' || bapu2 > '9') && (bapu2 < 'A' || bapu2 > 'F'))
      || ((bapu1 < '0' || bapu1 > '9') && (bapu1 < 'A' || bapu1 > 'F')))
    return (-1);
  if (bapu1 >= 'A')
    bapu1 -= 7;
  if (bapu2 >= 'A')
...
void
memseti (void *dst, UBYTE ch, LONG len)
{
