...
WORD showdatatypepicture (struct FMHandle *, struct DataType *,
			  struct CMenuConfig *);
...
WORD playdatatypesample (struct FMHandle *, struct DataType *,
			 struct CMenuConfig *);
...
void executeown (struct FMList *, struct FMList *, struct FMNode *,
		 struct CMenuConfig *);

WORD
isdatatypes (struct FMHandle *h, LONG dtype)
{
  ...
  if (dtn)
	...
	{
	  if (dtype == GID_PICTURE)
	    ret =
	      showdatatypepicture (h, dtn, getconfignumber (SHOWPICCONFIG));
	  if (dtype == GID_SOUND)
	    ret = playdatatypesample (h, dtn, getconfignumber (HEARCONFIG));
	}
      ReleaseDataType (dtn);
    }
  if (lock)
...
static WORD
testclick (struct FMHandle *h, struct FMNode *node, UBYTE * m, WORD usedt)
{
  UWORD apu1, apu2;
  ULONG *apu;
  UBYTE *ptr1;
  ...
  if ((fmconfig->usepicturedt | usedt) && *apu == 'FORM'
      && *(apu + 2) == 'ILBM')
    {
      showpicture (h, getconfignumber (SHOWPICCONFIG));
      return (1);
    }
  if ((fmconfig->usesampledt | usedt) && *apu == 'FORM'
      && *(apu + 2) == '8SVX')
    {
      playsample (h, getconfignumber (HEARCONFIG));
      return (1);
    }
  if ((fmconfig->useexecutedt | usedt) && *apu == 0x3f3
      && !(node->numprot & 2))
    {
      executefile (h, getconfignumber (EXECUTECONFIG));
      return (1);
    }
  if ((fmconfig->useicondt | usedt) && *m == 0xe3 && *(m + 1) == 0x10
      && !Strnicmp (h->filename + strlen (h->filename) - 5, ".info", 5))
    {
      showicon (h, getconfignumber (SHOWPICCONFIG));
      return (1);
    }
  if ((fmconfig->usefontdt | usedt) && *m == 0x0f
      && (*(m + 1) == 0x00 || *(m + 1) == 0x03)
      && !Strnicmp (h->filename + strlen (h->filename) - 5, ".font", 5))
    {
  ...
  if (*apu == 'M.K.' || *apu == 'FLT4')
    mod = 1;
  if (!Strnicmp (ptr1 + strlen (ptr1) - 4, ".mod", 4))
    mod = 1;
  if (!Strnicmp (ptr1, "mod.", 4))
    mod = 1;
  if (mod)
    {
...
void
checkfile (struct FMHandle *h, UBYTE * m, struct FMNode *node)
{
  ...
  for (cnt = 0; cnt < TOTALCOMMANDS; cnt++)
    {
      ...
      if (cmc->cmenucount != 100 || !cmc->moreconfig)
	continue;
      oc = cmc->moreconfig;
      if (oc->namematch[0])
	{
      ...
      while (*ptr1)
	{
	  ...
	  if (*ptr1 == '$')
	      ...
	      while (ptr2 < ptr1)
		{
		  apu5 = strtohex (ptr2);
		  ptr2 += 2;
		  if (longi + apu3 < MEMSIZE
		      && m[apu3 + longi] == (UBYTE) apu5)
		    apu4++;
		  if (apu5 < 0)
		    break;
		  apu3++;
		...
		{
		  if (longi + apu3 < MEMSIZE && m[apu3 + longi] == ptr2[apu3])
		    apu4++;
		}
	      if (apu5 == apu4)
		{
  ...
  if (testclick (h, node, m, 0))
    return;

  if (isdatatypes (h, 0) != -99)
    return;
