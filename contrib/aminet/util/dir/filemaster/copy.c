...
void copyflags (struct CopyConfig *, struct FMList *, struct FMList *,
		UBYTE *, UBYTE *);
...
void
copyroutine (struct CMenuConfig *cmc)
{
  struct FMList *slist;
  struct FMList *dlist;
  struct FMNode *node;
...
//priority(cmc);
  ...
  if (cmc->cmenucount == COPYASCONFIG)
    delstr->method = 1;
  ...
  slist->flags |= LUPDATEMSG;
  dlist->flags |= LUPDATEMSG;
  ...
  for (node = slist->head; node->succ; node = node->succ)
      ...
      if (node->flags & NFILE && node->flags & NSELECTED)
	{
	  ...
	  if (delstr->cconfig->update == 0)
	    outputlist (dlist);
	  if (!ret)
	    goto delfailure1;
	  sformatmsg (slist->fmmessage1, MSG_COPYING, delstr->filecnt,
		      delstr->dircnt, delstr->bytecnt);
	  strcpy (dlist->fmmessage1, slist->fmmessage1);
	}
    }
  for (node = slist->head; node->succ; node = node->succ)
      ...
      if (node->flags & NDIRECTORY && node->flags & NSELECTED)
	{
	  ...
	  outputlistline (slist, node);
	  disknode (dlist, varaname);
	  if (delstr->cconfig->update <= 1)
	    outputlist (dlist);
	  if (!ret)
	    goto delfailure1;
	}
    }
  sformatmsg (slist->fmmessage1, MSG_COPIED, delstr->filecnt, delstr->dircnt,
	      delstr->bytecnt);
...
WORD
copydir (struct FMList *slist, struct FMList *dlist, UBYTE * dirname,
	 struct DelStr *ds, WORD flag)
{
  ...
  sformatmsg (slist->fmmessage1, MSG_COPYING, ds->filecnt, ds->dircnt,
	      ds->bytecnt);
  strcpy (dlist->fmmessage1, slist->fmmessage1);
...
//*(PathPart(dlist->path))=0;
  ...
  if (!(lock = fminitdirread (slist, fib, slist->path)))
    goto dderr;
  while (fmexnext (slist, lock, fib) > 0)
    {
      if (dflag)
	{
	  if (!deletefile (slist, name, ds->dconfig, 1))
	    goto dderr;
	  dflag = 0;
	}
      if (dmatch (fib, ds))
	{
	  if (fib->fib_DirEntryType > 0)
	  ...
	  else
	    {
	      ...
	      if (!dret)
		goto dderr;
	      sformatmsg (slist->fmmessage1, MSG_COPYING, ds->filecnt,
			  ds->dircnt, ds->bytecnt);
	      strcpy (dlist->fmmessage1, slist->fmmessage1);
	    }
	}
      if (flag && dret > 0)
...
WORD
copyfile (struct FMList * slist, struct FMList * dlist, UBYTE * name,
	  struct DelStr * ds)
{
  struct GUIBase *gb;
...
//struct DeleteConfig dc;
  ...
  if (hs =
      openfile (slist, varaname,
		OFBUFFER | (ds->cconfig->bufsize ? ds->cconfig->
			    bufsize : 60000)))
    {
      size = hs->size;

      sformatmsg (slist->fmmessage2, MSG_COPYING2, hs->filename, 0L,
		  hs->size);
      strcpy (dlist->fmmessage2, slist->fmmessage2);
      ...
      if (askcopyname (dlist, name, ds))
	{
	  ...
	  if (ds->cconfig->freespace)
	      ...
	      if (id && lock && Info (lock, id))
		{
		  ...
		  if (id->id_DiskType == ID_DOS_DISK
		      || id->id_DiskType == ID_INTER_DOS_DISK)
		    bytesperblock = 488;
		  if (id->id_DiskType & 0xffffff00 ==
		      ID_DOS_DISK & 0xffffff00)
		    size +=
		      ((((size + bytesperblock) / bytesperblock) / 72) * 512);
		  if (freeblocks)
		    freeblocks += alreadyblocks;
		  if (freeblocks && (freeblocks - 1) * bytesperblock <= size)
		    {
		      ...
		      buttonbarmsg (gb, MSG_IGNORE, 1, MSG_SKIP, 2,
				    MSG_CANCEL, 0, 0);
		      c = quickreq (gb);
		      if (!c)
			{
	  ...
	  if (!ds->owflag && ds->cconfig->overwrite && currentdir (dlist))
	      ...
	      if (olock)
		{
		  ...
		  sformatmsg (text, MSG_COPY_OVERWRITE, name, date1, hs->size,
			      date2, dstsize);
		  reqinfo (gb, text, -1, guiCENTER);
		  buttonbarmsg (gb, MSG_YES, 1, MSG_ALL, 2, MSG_NO, 3,
				MSG_CANCEL, 0, 0);
		  c = quickreq (gb);
		  if (!c)
		    {
	  ...
	  if (hd = openfile (dlist, name, OFWRITE))
	    {
	      ...
	      for (;;)
		{
		  sformatmsg (slist->fmmessage2, MSG_COPYING2, hs->filename,
			      cnt2, hs->size);
		  strcpy (dlist->fmmessage2, slist->fmmessage2);
		  cnt = readfile (hs);
		  if (cnt < 0)
		  ...
		  if (cnt == 0)
		    break;
		  sformatmsg (slist->fmmessage2, MSG_COPYING2, hd->filename,
			      cnt2, hs->size);
		  strcpy (dlist->fmmessage2, slist->fmmessage2);
		  if (!doubleabort (slist, dlist))
		    {
	      ...
	      closefile (hd);
	      if (ret > 0)
		copyflags (ds->cconfig, slist, dlist, varaname, name);
	    }
	}
    nocopy:
      closefile (hs);
...
WORD
askcopyname (struct FMList * list, UBYTE * txt, struct DelStr * ds)
{
  struct GUIBase *gb;
  WORD ret = 0, c;

  if (!ds->method || ds->asflag)
    return (1);
...
void
copyflags (struct CopyConfig *cconfig, struct FMList *slist,
	   struct FMList *dlist, UBYTE * sname, UBYTE * dname)
{
  struct FileInfoBlock *fib;
  ...
  if (cconfig->comment | cconfig->flags | cconfig->date)
	...
	{
	  if (cconfig->comment && fib->fib_Comment[0])
	    fmsetcomment (dlist, dname, fib->fib_Comment);
	  if (cconfig->flags && fib->fib_Protection != 0x0f)
	    fmsetprotection (dlist, dname, fib->fib_Protection);
	  if (cconfig->date)
	    fmsetfiledate (dlist, dname, &fib->fib_Date);
	}
    }
  freemem (fib);
}
...
//priority(pm->cmc);
  ...
  for (node = list->head; node->succ; node = node->succ)
      ...
      if (node->flags & NFILE && node->flags & NSELECTED)
	{
	  if (first)
	    {
	      if (!deletefirst (list, node, delstr))
		goto delfailure1;
	      first = 0;
	    }
	  nxtnode = node->pred;
	  ...
	  if (apu1 > 0)
	    {
	      ...
	      sformatmsg (list->fmmessage1, MSG_DELETING, delstr->filecnt,
			  delstr->dircnt, delstr->bytecnt);
	      sformat (list->fmmessage2, "'%s'", NDFILE (node));
	      removenode (list, node);
	      node = nxtnode;
	    }
	  if (delstr->dconfig->update == 0)
	    outputlist (list);
	}
    }
  for (node = list->head; node->succ; node = node->succ)
      ...
      if (node->flags & NDIRECTORY && node->flags & NSELECTED)
	{
	  ...
	  nxtnode = node->pred;
	  apu1 =
	    deletedir (list, NDFILE (node), delstr, delstr->dconfig->askdir);
	  node->flags &= ~NSELECTED;
	  if (!apu1)
	    goto delfailure1;
	  if (apu1 > 0)
	    {
	      if (!
		  (apu1 =
		   deletefile (list, NDFILE (node), delstr->dconfig, 0)))
		goto delfailure1;
	      if (apu1 > 0)
		{
		  delstr->dircnt++;
	  ...
	  if (delstr->dconfig->update <= 1)
	    outputlist (list);
	}
    }
  sformatmsg (list->fmmessage1, MSG_DELETED, delstr->filecnt, delstr->dircnt,
	      delstr->bytecnt);
  list->fmmessage2[0] = 0;
delfailure1:
  freemem (delstr);
...
WORD
deletedir (struct FMList *list, UBYTE * dirname, struct DelStr *ds,
	   WORD subdir)
{
  struct GUIBase *gb;
  ...
  sformatmsg (list->fmmessage1, MSG_DELETING, ds->filecnt, ds->dircnt,
	      ds->bytecnt);
  ...
  while (fmexnext (list, lock, fib) > 0)
      ...
      if (!empty && subdir && !ds->allsubdelete)
	{
	  ...
	  reqinfo (gb, text, -1, guiCENTER);
	  buttonbarmsg (gb, MSG_YES, 1, MSG_ALL, 2, MSG_NO, 3, MSG_CANCEL, 0,
			0);
	  c = quickreq (gb);
	  if (!c)
	    {
      ...
      if (dmatch (fib, ds))
	{
	  if (fib->fib_DirEntryType > 0)
	    {
	      if (!(apu1 = deletedir (list, fib->fib_FileName, ds, 0)))
		goto dderr;
	      if (apu1 == 1)
		{
		  strcpy (name, fib->fib_FileName);
	  ...
	  else
	    {
	      if (!
		  (apu1 =
		   deletefile (list, fib->fib_FileName, ds->dconfig, 0)))
		goto dderr;
	      if (apu1 < 0)
		{
		  ret = -1;
...
WORD
deletefile (struct FMList * list, UBYTE * name, struct DeleteConfig * dc,
	    WORD req)
{
  WORD error;
  WORD ret = 0;
  ...
  if (!DeleteFile (name))
      ...
      if (error == 222)
	{
	  if (dc->protected)
	    {
	      if (!requestmsg
		  (getstring (MSG_MAIN_AREYOUSURE), MSG_YES, MSG_NO,
		   MSG_DELETE_DELPROTECTED, name))
		{
		  ret = -1;
		  goto dferr;
		}
...
WORD
deletefirst (struct FMList * list, struct FMNode * node, struct DelStr * ds)
{
...
//priority(pm->cmc);
  ...
  for (node = slist->head; node->succ; node = node->succ)
      ...
      if (node->flags & NFILE && node->flags & NSELECTED)
	{
	  ...
	  if (rename)
	    {
	      if (!(apu1 = renamefile (slist, dlist, varaname)))
		goto delfailure1;
	      node->flags &= ~NSELECTED;
	      disknode (dlist, varaname);
	      if (moveconfig->update == 0)
		outputlist (dlist);
	      if (apu1 > 0)
		{
		  delstr->filecnt++;
		...
		goto delfailure1;
	      if (apu1 > 0)
		if (deletefile (slist, varaname, delstr->dconfig, 1) <= 0)
		  goto delfailure1;
	    }
	  sformatmsg (slist->fmmessage1, MSG_MOVING, delstr->filecnt, string,
		      delstr->dircnt, string, delstr->bytecnt, string);
	  strcpy (dlist->fmmessage1, slist->fmmessage1);
	  ...
	  if (rename)
	    {
	      ...
	      outputlistline (slist, node);
	      disknode (dlist, varaname);
	      if (moveconfig->update <= 1)
		outputlist (dlist);
	      if (!apu1)
		goto delfailure1;
	      if (node->flags & (NDIRECTORY | NDEVICE | NASSIGN))
		string = "+";
	      if (apu1 > 0)
		delstr->dircnt++;
	    }
  ...
  sformatmsg (slist->fmmessage1, MSG_MOVED, delstr->filecnt, string,
	      delstr->dircnt, string, delstr->bytecnt, string);
  slist->fmmessage2[0] = 0;
  strcpy (dlist->fmmessage1, slist->fmmessage1);
  strcpy (dlist->fmmessage2, slist->fmmessage2);
...
WORD
renamefile (struct FMList *slist, struct FMList *dlist, UBYTE * name)
{
  WORD ret = 0;
...
WORD
doubleabort (struct FMList * slist, struct FMList * dlist)
{
  if (testabort (slist))
    {
      if (askabort (slist))
...
//priority(pm->cmc);
  ...
  for (node = list->head; node->succ; node = node->succ)
      ...
      if (node->flags & NFILE && node->flags & NSELECTED)
	{
	  ...
	  delstr->bytecnt += node->numlen;
	  node->flags &= ~NSELECTED;
	  sformatmsg (list->fmmessage1, MSG_DISKSIZING, delstr->filecnt,
		      delstr->dircnt, delstr->bytecnt);
  ...
  for (node = list->head; node->succ; node = node->succ)
    {
      if (node->flags & (NDIRECTORY | NASSIGN | NDEVICE)
	  && node->flags & NSELECTED)
	{
	  dsize = 0;
  ...
  sformatmsg (list->fmmessage1, MSG_DISKSIZED, delstr->filecnt,
	      delstr->dircnt, delstr->bytecnt);
  list->fmmessage2[0] = 0;
sizfailure1:
  freemem (delstr);
...
WORD
sizedir (struct FMList *list, UBYTE * name, struct DelStr *ds, LONG * dsize)
{
  struct FileInfoBlock *fib;
  BPTR lock = 0;
  WORD ret = 0;

  sformatmsg (list->fmmessage1, MSG_DISKSIZING, ds->filecnt, ds->dircnt,
	      ds->bytecnt);
  if (testabort (list))
    {
      list->fmmessage2[0] = 0;
  ...
  if (!(fib = allocmem (sizeof (struct FileInfoBlock))))
    return (0);
      ...
      if (lock = fminitdirread (list, fib, list->path))
	{
	  while (fmexnext (list, lock, fib) > 0)
	      ...
	      if (dmatch (fib, ds))
		{
	      ...
	      sformatmsg (list->fmmessage1, MSG_DISKSIZING, ds->filecnt,
			  ds->dircnt, ds->bytecnt);
	      sformat (list->fmmessage2, "'%s'", list->path);
	    }
	  UnLock (lock);
...
WORD
dmatch (struct FileInfoBlock * fib, struct DelStr * ds)
{
  struct FMNode *node;
  WORD ret = 0;
  ...
  if (node)
    {
      ...
      ret =
	checkmatch (ds->token1, ds->token2, ds->token3, node, &ds->match, 0);
    }
  freevecpooled (node);
  return (ret);
}
...
struct DelStr *
createtoken (struct FMList *list)
{
  struct DelStr *ds;
...
WORD
askcontinue (struct FMList * list)
{
  WORD ret;
...
void *
copyconfigdefault (struct CMenuConfig *cmc)
{
...
WORD
copyconfig (struct GUIBase * gb, struct CMenuConfig * cmc)
{
  WORD c;
  ...
  setguigroup (gb, 2, 0);
  reqcyclemsg (gb, 200, &upd, MSG_COPYMOVE_CONFIG_REFRESH1,
	       MSG_COPYMOVE_CONFIG_REFRESH2, MSG_COPYMOVE_CONFIG_REFRESH3, 0);
  ov = config->overwrite;
  reqcyclemsg (gb, 201, &ov, MSG_COPYMOVE_CONFIG_OVERWRITE1,
	       MSG_COPYMOVE_CONFIG_OVERWRITE2, 0);
  co = config->comment;
...
void *
deleteconfigdefault (struct CMenuConfig *cmc)
{
...
WORD
deleteconfig (struct GUIBase * gb, struct CMenuConfig * cmc)
{
  WORD c;
