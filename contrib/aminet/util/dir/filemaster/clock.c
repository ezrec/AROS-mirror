...
nostring (void)
{
  ...
  for (cnt1 = 0; cnt1 < LISTS; cnt1++)
    {
      if (fmlist[cnt1].li
	  && fmlist[cnt1].li->string.Flags & GACT_ACTIVEGADGET)
	return (0);
    }
  return (1);
}
...
remappicon (void)
{
  ...
  if (fmmain.appiconport)
    {
      while (appmsg = (struct AppMessage *) GetMsg (fmmain.appiconport))
  ...
  if (fmmain.appiconport)
    {
      ...
      fmmain.appicon =
	AddAppIconA (0, 0, "FM", fmmain.appiconport, 0, fmmain.dobj, 0);
      if (fmmain.appicon)
	return (fmmain.appicon);
    }
  remappicon ();
...
void
clockformat (UBYTE * src)
{
  LONG afast, achip;
  struct DateStamp stamp;
  UBYTE sfast[60], schip[60], stotal[60], sdate[60];

  afast = AvailMem (MEMF_FAST);
  achip = AvailMem (MEMF_CHIP);
  ...
  if ((getappicon () & 1) || !fmmain.appiconport)
    {
      ...
      swin = OpenWindowTags (0,
			     ...
			     WFLG_NOCAREREFRESH, TAG_DONE);
    }
  *pswin = swin;
  UnlockPubScreen (0, wbscr);
  if (!fmmain.appicon && !swin)
    return (0);
  return (1);
}

struct timerequest *
gettimerequest (struct MsgPort **amp)
{
  ...
  if (mp = CreateMsgPort ())
    {
      if (tr =
	  (struct timerequest *) CreateIORequest (mp,
						  sizeof (struct
							  timerequest)))
	{
	  *amp = mp;
	  if (!OpenDevice
	      ("timer.device", UNIT_VBLANK, (struct IORequest *) tr, 0))
	    return (tr);
	  DeleteIORequest (tr);
	}
      DeleteMsgPort (mp);
  ...
  while (fmmain.clock)
	...
	case STARTSLEEP:
	  if (!setsleep (&swin))
	  ...
	  else
	    {
	      ...
	      saimask = 0;
	      if (swin)
		swinmask = 1L << swin->UserPort->mp_SigBit;
	      if (fmmain.appiconport)
		saimask = 1L << fmmain.appiconport->mp_SigBit;
	      sleepsigmask = swinmask | saimask;
	    }
	  break;
	case SLEEPING:
	  if (saimask && (sigs & saimask))
	    sleepmode = QUITSLEEP;
      ...
      if (fmmain.cxport)
	{
	  while (cxmsg = (CxMsg *) GetMsg (fmmain.cxport))
		...
		{
		case CXM_IEVENT:
		  if (!sleepmode && fmmain.fmactive == 3 && !fmmain.passhotkey
		      && nostring ())
		    {
		      fmmain.passhotkey = msgtype;
		      Signal ((struct Task *) fmmain.myproc,
			      SIGBREAKF_CTRL_F);
		    }
		  break;
		case CXM_COMMAND:
      ...
      if (CheckIO ((struct IORequest *) tr))
	{
	  ...
	  fmmain.timer++;
	  timeri = 0;
	  if (fmmain.sourcedir->flags & LUPDATEMSG)
	  ...
	  if (fmmain.ikkuna && AttemptSemaphore (&fmmain.gfxsema))
	    {
	      len =
		TextFit (fmmain.rp, clockstr, strlen (clockstr), &txtext, 0,
			 1, fmmain.bottomlinewidth - 2, 32767);
	      ...
	      if (txtext.te_Width < fmmain.bottomlinewidth)
		RectFill (fmmain.rp, txtext.te_Width + xx, fmmain.bottomliney,
			  fmmain.bottomlinewidth + xx - 3,
			  fmmain.bottomliney + fmmain.txtysize - 1);
