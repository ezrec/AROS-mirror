...
#ifndef AROS
WORD
xfddecrunch (struct FMHandle *h, struct xfdBufferInfo *xfdbi)
{
  UBYTE *obuf;
  ...
  if (xfdbi->xfdbi_SourceBuffer = allocvec (h->owner, h->size, 0))
    {
      ...
      freemem (obuf);
      if (readbufferfile
	  (h, (UBYTE *) xfdbi->xfdbi_SourceBuffer + xfdbi->xfdbi_SourceBufLen,
	   h->size - xfdbi->xfdbi_SourceBufLen) ==
	  h->size - xfdbi->xfdbi_SourceBufLen)
	{
	  xfdbi->xfdbi_SourceBufLen = h->size;
	  xfdbi->xfdbi_TargetBufMemType = MEMF_PUBLIC;
	  sformat (h->owner->fmmessage1, getstring (MSG_FMDOS_DECRUNCHING),
		   xfdbi->xfdbi_PackerName, h->filename);
...
LONG
fmread (struct FMHandle * h, void *buffer, LONG size)
{
  return (Read (h->handle, buffer, size));
}

LONG
seek (struct FMHandle * h, LONG p, LONG mode)
{
  LONG ret, ret2;
  ...
  if (h->flags & OFDECRUNCH)
	...
	case OFFSET_END:
      ...
      if (h->position < 0 || h->position > h->size)
	ret = -1;
    }
  else
    {
      ...
      if (ret >= 0)
	ret2 = Seek (h->handle, 0, OFFSET_CURRENT);
      if (ret < 0 || ret2 < 0)
	{
...
LONG
readbufferfile (struct FMHandle * handle, void *buffer, LONG len)
{
  LONG ret = -1;
  LONG len2;
  LONG oldseek;
...
rretry:
  if (handle->size - handle->position >= len)
      ...
      else
	{
	  ...
	  if (ret == -1)
	    {
	      if (dosrequestmsg
		  (handle->owner, 1, MSG_FMDOS_READERR, handle->filename))
		{
		  if (seek (handle, oldseek, OFFSET_BEGINNING) >= 0)
		    goto rretry;
		}
	      return (ret);
	    }
  ...
  if (ret != len)
    {
      requestmsg (handle->owner->workname, 0, MSG_OK, MSG_FMDOS_UEOF,
		  handle->filename);
      ret = 0;
    }
  return (ret);
...
LONG
readfile (struct FMHandle * handle)
{
  LONG ret;
  LONG oldseek;
...
LONG
writefile (struct FMHandle * handle, void *buffer, LONG size)
{
  LONG ret;
  LONG oldseek;
...
wretry:
  if (handle->flags & OFDECRUNCH)
	...
	{
	  if (dosrequestmsg
	      (handle->owner, 1, MSG_FMDOS_WRITEERR, handle->filename))
	    {
	      if (seek (handle, oldseek, OFFSET_BEGINNING) >= 0)
		goto wretry;
	    }
...
struct FMHandle *
openfile (struct FMList *list, UBYTE * name, ULONG flags)
{
...
oretry:
  ...
  if (!
      (handle =
       (struct FMHandle *) AllocVec (sizeof (struct FMHandle),
  ...
  if (!(flags & OFFAKE))
	...
	goto ofile1;
      if (!fmexaminefile (list, fib, name))
	goto ofile1;
  ...
  if (flags & OFNORMAL)
  ...
  else if (flags & OFBUFFER)
    {
      for (;;)
	{
	  if (bufsize < handle->size && bufsize < 4000)
	    break;
	  handle->bufsize = bufsize;
	  if ((handle->buffer = AllocVec (bufsize, MEMF_CLEAR | MEMF_PUBLIC)))
	    break;
	  bufsize /= 2;
	}
    }
  if (!handle->buffer)
    {
      retry =
	requestmsg (list->workname, MSG_RETRY, MSG_CANCEL,
		    MSG_FMDOS_NOBUFMEM);
      goto ofile1;
    }
  if (!(flags & OFFAKE))
	...
	{
	  requestmsg (list->workname, 0, MSG_OK, MSG_FMDOS_ZEROSIZE,
		      handle->filename);
	  goto ofile1;
	}
#ifndef AROS
...
void
closefile (struct FMHandle *handle)
{
  if (!handle)
    return;
  if (handle->flags & OFDECRUNCH && !(handle->flags & OFFAKE))
    FreeMem (handle->decbuffer, handle->decbufsize);
...
WORD
fmexaminefile (struct FMList *list, struct FileInfoBlock *fib, UBYTE * name)
{
  BPTR lock;
  WORD ret;
...
WORD
fmexamine (struct FMList * list, BPTR lock, struct FileInfoBlock * fib,
	   UBYTE * name)
{
  do
    {
...
WORD
fmexnext (struct FMList * list, BPTR lock, struct FileInfoBlock * fib)
{
...
BPTR
fmlock (struct FMList * list, UBYTE * name)
{
  BPTR lock;
...
WORD
currentdir (struct FMList * list)
{
  BPTR lock;
  BPTR curri;

  if (!(lock = Lock (list->path, SHARED_LOCK)))
    return (0);
  ...
  if (list->pair)
	...
	{
	  if (list->pair->oldcur == 1)
	    {
	      list->pair->oldcur = curri;
	      curri = 0;
	    }
	  goto curend;
	}
...
WORD
fmsetrename (struct FMList * list, UBYTE * oldname, UBYTE * newname)
{
  if (!currentdir (list))
    return (0);
  do
...
WORD
fmrename (struct FMList * list, struct FMNode * node, UBYTE * name)
{
...
WORD
fmsetfiledate (struct FMList * list, UBYTE * name, struct DateStamp * ds)
{
  if (!currentdir (list))
    return (0);
  do
...
WORD
fmfiledate (struct FMList * list, struct FMNode * node, ULONG date)
{
  struct DateStamp ds;
  UBYTE dstring[32];
...
WORD
fmsetcomment (struct FMList * list, UBYTE * name, UBYTE * comm)
{
  if (!currentdir (list))
    return (0);
  do
    {
      if (SetComment (name, comm))
	return (1);
    }
...
WORD
fmcomment (struct FMList * list, struct FMNode * node, UBYTE * comm)
{
...
WORD
fmsetprotection (struct FMList * list, UBYTE * name, ULONG prot)
{
  if (!currentdir (list))
    return (0);
  do
    {
      if (SetProtection (name, prot))
	return (1);
    }
...
WORD
fmprotect (struct FMList * list, struct FMNode * node, ULONG prot)
{
  WORD cnt, cnt2;
  ...
  if (node->numprot != prot)
      ...
      if (fmsetprotection (list, NDFILE (node), prot))
	{
	  ...
	  for (cnt = 0; cnt < 8; cnt++)
	    {
	      if (prot & cnt2)
		*(NDPROT (node) + cnt) = hsparwed[cnt * 3 + 1];
	      else
		*(NDPROT (node) + cnt) = '-';
	      cnt2 = cnt2 >> 1;
	    }
...
struct FMNode *
fmcreatedir (struct FMList *list, UBYTE * name)
{
  BPTR lock;
  struct FMNode *node;
...
WORD
examinefile (struct FMList * list, struct FMNode * node)
{
  BPTR lock;
	  ...
	  if (fmexamine (list, lock, fib, NDFILE (node)))
	    {
	      ...
	      protect = fib->fib_Protection ^ 0x0f;
	      cnt2 = 128;
	      for (cnt = 0; cnt < 8; cnt++)
		{
		  if (protect & cnt2)
...
BPTR
fminitdirread (struct FMList * list, struct FileInfoBlock * fib, UBYTE * name)
{
  BPTR lock;
  ...
  if (fmmain.pool)
    {
      ...
      a += 4;
      if (ret = (ULONG *) AllocPooled (fmmain.pool, a))
	*ret++ = a;
...
void *
allocvec (struct FMList *l, LONG a, LONG b)
{
  void *ret;
  UBYTE *ptr;
  ...
  do
	...
	return (0);
      if (b & MEMF_CHIP)
	ptr = getstring (MSG_FMDOS_CHIPMEM);
      else
	ptr = getstring (MSG_FMDOS_PUBLICMEM);
    }
  while (requestmsg
	 (l->workname, MSG_RETRY, MSG_CANCEL, MSG_FMDOS_ALLOCATEERR, a, ptr));
  return (0);
}

struct IORequest *
opendevice (struct FMList *l, UBYTE * name, WORD unit, ULONG flags,
	    WORD iolen)
{
  struct MsgPort *mp;
  struct IORequest *io;
