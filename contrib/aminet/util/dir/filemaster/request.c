...
WORD
errorreq (WORD title, UBYTE * txt, WORD pos, WORD neg)
{
  UBYTE text[100];
  UBYTE *ptr1;
...
void
outputfmmessage (struct FMList *list, UBYTE * ptr, WORD yy)
{
  WORD width;
  UBYTE *ptr1;
  ...
  if (fmmain.ikkuna)
	...
	fmmain.ikkuna->BorderRight - 2 * fmconfig->spacew;
      fittext (fmmain.rp, ptr, -1, fmconfig->spacew, yy, width, 1);
    }
}

WORD
request2 (UBYTE * title, WORD pos, WORD neg, UBYTE * reqtxt)
{
  struct GUIBase *gb;
  WORD ret;
...
WORD
requestmsg (UBYTE * title, WORD pos, WORD neg, LONG msg, ...)
{
  UBYTE reqtxt[200];
...
WORD
request (UBYTE * title, WORD pos, WORD neg, UBYTE * txt, ...)
{
  UBYTE reqtxt[200];
...
WORD
dosrequesti (struct FMList * list, WORD flag, UBYTE * dostxt1)
{
  struct GUIBase *gb;
  ULONG ioerr;
  UBYTE dostxt2[100];
  ...
  startbuttonbar (gb);
  if (flag & 1)
    reqbuttonmsg (gb, MSG_RETRY, 2, guiUC);
  if (flag & 2)
    reqbuttonmsg (gb, MSG_IGNORE, 3, guiUC);
  if (!flag)
...
WORD
dosrequestmsg (struct FMList * list, WORD flag, LONG msg, ...)
{
  UBYTE dostxt1[200];
...
WORD
dosrequest (struct FMList * list, WORD flag, UBYTE * txt, ...)
{
  UBYTE dostxt1[200];
...
WORD
asklong (struct Screen * scr, UBYTE * title, UBYTE * text, LONG * value,
	 LONG min, LONG max)
{
  struct GUIBase *gb;
  WORD c;
...
WORD
askstring (struct Screen * scr, UBYTE * title, UBYTE * text, UBYTE * string,
	   WORD len)
{
  struct GUIBase *gb;
  WORD c;
