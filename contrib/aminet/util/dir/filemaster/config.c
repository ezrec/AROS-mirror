...
extern struct ReqToolsBase *ReqToolsBase = 0;
extern UBYTE reqlib[];

#ifndef AROS
void *__asm rleunpack (register __a0 void *, register __a1 void *,
		       register __d0 LONG);
LONG __asm rlepack (register __a0 void *, register __a1 void *,
		    register __d0 LONG);
#endif
...
void
freeconfig (struct FMConfig *sc, struct FMConfig *dc)
{
...
WORD
askscreen (struct ScreenConfig * sc, WORD title, WORD scroll)
{
 ...
 D (bug ("config.c 170...........\n"));  
  ...
  req =
    (struct rtScreenModeRequester *) rtAllocRequestA (RT_SCREENMODEREQ, 0);
  if (req)
    {
      rtChangeReqAttr (req,
		       ...
		       RTSC_OverscanType, sc->overscan, TAG_DONE);
...
WORD
askfont (struct TextAttr * ta, WORD title, WORD type)
{
  ...
  if (req)
    {
      rtChangeReqAttr (req,
		       ...
		       RTFO_FontStyle, ta->ta_Style, TAG_DONE);
      ret = rtFontRequest (req, getstring (title),
			   RTFO_Flags,
			   (type ? 0 : FREQF_FIXEDWIDTH) | FREQF_SCALE,
			   TAG_DONE);
      if (ret)
	{
...
WORD
datatypesconfig (struct FMConfig * ofmc)
{
  struct GUIBase *gb;
  WORD retcode, c;
  struct FMConfig *fmc;
...
WORD
generalconfig (struct FMConfig * ofmc)
{
  struct GUIBase *gb;
  WORD retcode = 0;
  ...
  reqcyclemsg (gb, 106, &rbut, MSG_CONFIG_RIGHTMOUSE_DOUBLECLICK,
	       MSG_CONFIG_RIGHTMOUSE_PARENT, 0);
  buttonbarmsg (gb, MSG_OK, 1, MSG_CANCEL, 0, 0);
...
static WORD
savecommands (struct FMConfig *fmc, void *moreconfigs[])
{
...
static void
restorecommands (struct FMConfig *fmc, void *moreconfigs[])
{
  struct CMenuConfig *cmc;
  WORD cnt;
...
static WORD
commandconfig (struct FMConfig *ofmc, struct CMenuConfig *cmc)
{
  struct GUIBase *gb;
...
D (bug ("config.c 515...........\n")); 
  ...
  if (!(fmc = newconfig (ofmc)))
    return (0);
  if (!savecommands (fmc, moreconfigs))
    goto error;

  sformat (vara1, "%s (%s)", getstring (MSG_CONFIG_NAME),
	   findbuildstring (cmc));
  getconfig (cmc);
...
//pri=cmc->priority;
  reqcyclemsg (gb, 104, &pri, MSG_CONFIG_COMLOWPRI, MSG_CONFIG_COMMEDPRI,
	       MSG_CONFIG_COMHIGHPRI, 0);
  ...
  if (cmc->cmenucount == 100)
    {
      ...
      cd = oc->ef.cd;
      reqcyclemsg (gb, 111, &cd, MSG_EXE_CONFIG_NOCHANGE,
		   MSG_EXE_CONFIG_SOURCE, MSG_EXE_CONFIG_DESTINATION, 0);
      res = oc->ef.rescan;
      reqcyclemsg (gb, 112, &res, MSG_EXE_CONFIG_NOCHANGE,
		   MSG_EXE_CONFIG_SOURCE, MSG_EXE_CONFIG_DESTINATION,
		   MSG_EXE_CONFIG_SOURCEDEST, 0);
      frs = oc->ef.startfront;
      reqcyclemsg (gb, 113, &frs, MSG_EXE_CONFIG_NOCHANGE,
		   MSG_EXE_CONFIG_FMFRONT, MSG_EXE_CONFIG_WBFRONT, 0);
      fre = oc->ef.endfront;
      reqcyclemsg (gb, 114, &fre, MSG_EXE_CONFIG_NOCHANGE,
		   MSG_EXE_CONFIG_FMFRONT, MSG_EXE_CONFIG_WBFRONT, 0);
  ...
  if (apu1 == 1)
    {
      ...
      cmc->backpen = back;
      //cmc->priority=pri;
      CopyMem (cmc, &fmc->cmenuconfig[cmc - ofmc->cmenuconfig],
	       sizeof (struct CMenuConfig));
      if (oc)
	{
...
readconfig (void)
{
  ...
  if (!handle)
    handle = Open ("ENVARC:fm.prefs", 1005);
  if (!handle)
    handle = Open ("S:fm.prefs", 1005);
  if (!handle)
    goto error;
  D (bug ("config.c 655...........\n"));
  ...
  if (apu2 != OLDCONFIGVERSION && apu2 != FMCONFIGVERSION)
    {
      errorreq (MSG_MAIN_WARNING, getstring (MSG_CONFIG_OLDCONFIG), 0,
		MSG_OK);
      goto error;
    }
  Read (handle, &apu1, 2);
  if (Read (handle, rletemp, len) != len)
    goto error;
  if (apu2 == OLDCONFIGVERSION)
      ...
      for (cnt1 = 0; cnt1 < TOTALCOMMANDS; cnt1++)
	{
	  ...
	  if (cmc->moreconfiglen)
	    {
	      cmc->moreconfig =
		allocvec (0, cmc->moreconfiglen, MEMF_CLEAR | MEMF_PUBLIC);
	      if (!cmc->moreconfig)
		goto error;
...
void
writeconfig (struct FMConfig *fmc)
{
  BPTR handle;
  LONG size;
  UBYTE *rletemp, *ptr1;
  ...
  rletemp =
    allocmem (sizeof (struct FMConfig) + TOTALCOMMANDS * HELPCONFIGSIZE);
  if (!rletemp)
    goto error;
...
void
commandanswer (struct GUIBase *gb)
{
  buttonbarmsg (gb, MSG_OK, 1, MSG_CANCEL, 0, 0);
}

WORD
allocconfig (struct CMenuConfig *cmc, WORD len)
{
  if (cmc->moreconfig)
    return (-1);
...
void *
getconfig (struct CMenuConfig *cmc)
{
  if (!cmc->moreconfig)
    {
      switch (cmc->cmenucount)
...
APTR
dlldisplayhookfunc (struct Hook * hook, Object * obj, struct lvRender * lvr)
{
  UBYTE *str;
...
APTR
dllresourcehookfunc (struct Hook * hook, Object * obj, struct lvResource * lvr)
{
...
setdllgads (struct GUIBase *gb, struct GUISlot *gs)
{
  ULONG data;
...
WORD
dirlayoutconfig (struct FMConfig * ofmc)
{
  struct FMConfig *fmc;
  struct GUIBase *gb;
  struct GUISlot *gs1;
  ...
  gs1 = getguislot (gb, NewObject (ddlistclass, 0,
				   GA_ID, 20,
				   LAB_Label,
				   getstring (MSG_CONFIG_LAYOUT_DIRLIST),
				   LAB_Place, PLACE_ABOVE, LISTV_ShowDropSpot,
				   TRUE, LISTV_MinEntriesShown, 8,
  ...
  for (cnt1 = 0; cnt1 < 5; cnt1++)
    {
      dll = &fmc->dl[cnt1];
      BGUI_DoGadgetMethod (gs1->obj, 0, 0, LVM_ADDSINGLE, 0, dll, LVAP_TAIL,
			   0);
    }
	  ...
	  if (ret == 1)
	    {
	      closereq (gb);
	      dll =
		(struct DirListLayout *) DoMethod (gs1->obj, LVM_FIRSTENTRY,
						   0, 0, TAG_DONE);
	      cnt1 = 0;
	      do
		{
		  CopyMem (dll, &fmc->dl[cnt1],
			   sizeof (struct DirListLayout));
		  dll =
		    (struct DirListLayout *) DoMethod (gs1->obj,
						       LVM_NEXTENTRY, dll, 0,
						       TAG_DONE);
		  cnt1++;
		}
...
WORD
screenconfig (struct FMConfig * ofmc)
{
  struct GUIBase *gb;
  WORD c;
  WORD apu1, apu2;
  ...
  for (;;)
    {
      ...
      sformatmsg (ptr1, MSG_CONFIG_SCRFORMAT,
		  getstring (MSG_CONFIG_MAINSCREEN), fmc->mainscreen.width,
		  fmc->mainscreen.height, fmc->mainscreen.depth,
		  fmc->mainscreen.screenmode);
      reqbutton (gb, ptr1, 10, guiUC | guiCENTER);
      ptr1 += strlen (ptr1) + 1;
      sformatmsg (ptr1, MSG_CONFIG_SCRFORMAT,
		  getstring (MSG_CONFIG_TEXTSCREEN), fmc->textscreen.width,
		  fmc->textscreen.height, fmc->textscreen.depth,
		  fmc->textscreen.screenmode);
      reqbutton (gb, ptr1, 11, guiUC | guiCENTER);
      ptr1 += strlen (ptr1) + 1;

      sformatmsg (ptr1, MSG_CONFIG_FONTFORMAT,
		  getstring (MSG_CONFIG_TEXTFONT), fmc->txtfontattr.ta_Name,
		  fmc->txtfontattr.ta_YSize);
      reqbutton (gb, ptr1, 12, guiUC | guiCENTER);
      ptr1 += strlen (ptr1) + 1;
      sformatmsg (ptr1, MSG_CONFIG_FONTFORMAT,
		  getstring (MSG_CONFIG_LISTFONT), fmc->listfontattr.ta_Name,
		  fmc->listfontattr.ta_YSize);
      reqbutton (gb, ptr1, 13, guiUC | guiCENTER);
      ptr1 += strlen (ptr1) + 1;
      sformatmsg (ptr1, MSG_CONFIG_FONTFORMAT, getstring (MSG_CONFIG_REQFONT),
		  fmc->reqfontattr.ta_Name, fmc->reqfontattr.ta_YSize);
      reqbutton (gb, ptr1, 14, guiUC | guiCENTER);
      ptr1 += strlen (ptr1) + 1;
      sformatmsg (ptr1, MSG_CONFIG_FONTFORMAT,
		  getstring (MSG_CONFIG_SHOWFONT),
		  fmc->txtshowfontattr.ta_Name,
		  fmc->txtshowfontattr.ta_YSize);
	...
	strcat (ptr1, getstring (MSG_CONFIG_LAYOUT));
	D (bug ("config.c 1210...........\n"));     
      *ptr1++ = 32;
      apu1 = 0;
	...
	D (bug ("config.c 1222...........\n"));     
      *ptr1++ = 0;
      ...
      reqcyclemsg (gb, 90, &stype, MSG_CONFIG_SCREENTYPE1,
		   MSG_CONFIG_SCREENTYPE2, MSG_CONFIG_SCREENTYPE3, 0);
      setconnectgroup (gb, 2, 1, 9);
	...
	case 11:
	  refresh =
	    askscreen (&fmc->textscreen, MSG_CONFIG_TEXTSCREEN, FALSE);
	  break;
	case 12:
	...
	case 15:
	  refresh =
	    askfont (&fmc->txtshowfontattr, MSG_CONFIG_SHOWFONT, FALSE);
	  break;
	case 16:
...
static void
rescanbuttonconfig (struct FMConfig *fmc, struct GUISlot *gs[])
{
  ...
  while (cnt1 < TOTALCOMMANDS)
    {
	  ...
	  D (bug ("config.c 1409...........\n"));    
  CopyMem (cmcbuf, fmc->cmenuconfig,
	   TOTALCOMMANDS * sizeof (struct CMenuConfig));
	  D (bug ("config.c 1412...........\n"));    
  freemem (cmcbuf);
	  D (bug ("config.c 1414...........\n"));    
}

static struct CMenuConfig *
findnewcmc (struct FMConfig *fmc)
{
  struct CMenuConfig *cmc;
  WORD cnt1;
  ...
  for (cnt1 = 0; cnt1 < TOTALCOMMANDS; cnt1++)
    {
      cmc = &fmc->cmenuconfig[cnt1];
      if (cmc->position == 0xff)
	break;
    }
  if (cnt1 == TOTALCOMMANDS)
    {
      cmc = 0;
      request (getstring (MSG_CONFIG_NAME), MSG_OK, 0,
	       getstring (MSG_CONFIG_COMSLOTSFULL));
    }
  if (cmc)
    {
      //cmc->priority=1;
...
static void
newbuildin (struct FMConfig *fmc, struct CMenuConfig *cmc)
{
...
WORD
findbuild (struct CMenuConfig *cmc)
{
  WORD cnt1;
  UBYTE *ptr1;
...
UBYTE *
findbuildstring (struct CMenuConfig * cmc)
{
  UBYTE *ptr1;
  WORD cnt1;
  ...
  cnt1 = cmc->cmenucount;
  if (cnt1 >= 100)
    return (getstring (MSG_CONFIG_COMUSERCOMMAND));
  cnt1--;
  while (cnt1--)
    ptr1 += strlen (ptr1) + 1;
  return (ptr1);
...
newuser (struct FMConfig *fmc)
{
  ...
  if (!newcmc)
    return;
  if (!ownexecuteconfigdefault (newcmc))
    return;
...
static void
gsdoubleclick (struct FMConfig *fmc, struct GUISlot *gs[],
	       struct GUISlot *obj, void *data)
{
  ...
  if (!buildin)
    reqbuttonmsg (gb, MSG_CONFIG_COMEDITCONFIG, 4, 0);
  if (flags & 1)
    reqbuttonmsg (gb, MSG_CONFIG_COMMOVEGADGETBAR, 5, 0);
  if (flags & 2)
    reqbuttonmsg (gb, MSG_CONFIG_COMMOVECORNERMENU, 6, 0);
  if (flags & 4)
    reqbuttonmsg (gb, MSG_CONFIG_COMMOVEHIDDEN, 7, 0);
  if (flags & 8)
    reqbuttonmsg (gb, MSG_CONFIG_COMCOPYGADGETBAR, 5, 0);
  if (flags & 16)
    reqbuttonmsg (gb, MSG_CONFIG_COMCOPYCORNERMENU, 6, 0);
  if (flags & 32)
    reqbuttonmsg (gb, MSG_CONFIG_COMDUPLICATECOMMAND, 3, 0);
  if (flags & 128)
    reqbuttonmsg (gb, MSG_CONFIG_COMREMOVECOMMAND, 8, 0);
	  ...
	  if (cmc->moreconfig)
	    {
	      ...
	      if (cmc3->moreconfig)
		CopyMem (cmc->moreconfig, cmc3->moreconfig,
			 cmc->moreconfiglen);
	    }
	}
      break;
...
static void
refreshcommands (struct FMConfig *fmc, struct GUIBase *gb,
		 struct GUISlot *gs[])
{
  struct CMenuConfig *cmc;
  WORD cnt1;
  ...
  for (cnt1 = 0; cnt1 < BUILDCOMMANDS; cnt1++)
    {
      BGUI_DoGadgetMethod (gs[0]->obj, 0, 0, LVM_ADDSINGLE, 0, ptr1,
			   LVAP_SORTED, 0);
      ptr1 += strlen (ptr1) + 1;
    }
  for (cnt1 = 0; cnt1 < TOTALCOMMANDS; cnt1++)
    {
      ...
      if (bap1 == 1)
	{
	  BGUI_DoGadgetMethod (gs[1]->obj, 0, 0, LVM_ADDSINGLE, 0, cmc,
			       LVAP_TAIL, 0);
	}
      else if (bap1 == 2)
	{
	  BGUI_DoGadgetMethod (gs[2]->obj, 0, 0, LVM_ADDSINGLE, 0, cmc,
			       LVAP_TAIL, 0);
	}
      else if (bap1 == 9)
	{
	  BGUI_DoGadgetMethod (gs[3]->obj, 0, 0, LVM_ADDSINGLE, 0, cmc,
			       LVAP_TAIL, 0);
	}
    }
...
APTR
comdisplayhookfunc (struct Hook *hook, Object * obj, struct lvRender *lvr)
{
...
APTR
comresourcehookfunc (struct Hook * hook, Object * obj, struct lvResource * lvr)
{
  struct CMenuConfig *cmc;
  struct cmenudata *cmd;
  UBYTE *vara;
...
struct Hook comresourcehook =
  { NULL, NULL, (HOOKFUNC) comresourcehookfunc, NULL, NULL };
struct Hook comdisplayhook =
  { NULL, NULL, (HOOKFUNC) comdisplayhookfunc, NULL, NULL };

WORD
commands (struct FMConfig *ofmc)
{
  struct FMConfig *fmc;
  Object *obj;
  ...
  gs[0] = getguislot (gb, NewObject (ddlistclass, 0,
				     ...
				     LAB_Label,
				     getstring
				     (MSG_CONFIG_COMBUILDINCOMMANDS),
				     LAB_Place, PLACE_ABOVE,
				     LISTV_MinEntriesShown, 20,
				     LISTV_SortEntryArray, TRUE,
				     LISTV_ListFont, &fmc->listfontattr,
				     BT_DragObject, TRUE, BT_DropObject,
				     FALSE, TAG_DONE), 0, LISTVIEW_KIND, 0);
	D (bug ("config.c 1740...........\n"));  
  gs[1] =
    getguislot (gb,
		NewObject (ddlistclass, 0, GA_ID, 21, LAB_Label,
			   getstring (MSG_CONFIG_COMBARCOMMANDS), LAB_Place,
			   PLACE_ABOVE, LISTV_ShowDropSpot, TRUE,
			   LISTV_MinEntriesShown, 20, LISTV_SortEntryArray,
			   FALSE, LISTV_ListFont, &fmc->listfontattr,
  ...
  gs[2] = getguislot (gb, NewObject (ddlistclass, 0,
				     GA_ID, 22,
				     LAB_Label,
				     getstring (MSG_CONFIG_COMCORNERCOMMANDS),
				     LAB_Place, PLACE_ABOVE,
  ...
  gs[3] = getguislot (gb, NewObject (ddlistclass, 0,
				     GA_ID, 23,
				     LAB_Label,
				     getstring (MSG_CONFIG_COMHIDDENCOMMANDS),
				     LAB_Place, PLACE_ABOVE,
				     LISTV_ShowDropSpot, TRUE,
				     LISTV_MinEntriesShown, 20,
				     LISTV_SortEntryArray, FALSE,
  ...
  reqcyclemsg (gb, 110, &cmenutype, MSG_CONFIG_LAYOUTVER,
	       MSG_CONFIG_LAYOUTHOR, 0);
	D (bug ("config.c 1796...........\n"));  
  ...
  reqcycle (gb, 111, &cmenucolumns, &msg[0], &msg[2], &msg[4], &msg[6],
	    &msg[8], &msg[10], &msg[12], &msg[14], &msg[16], 0);
  D (bug ("config.c 1806...........\n"));  
  buttonbarmsg (gb, MSG_OK, 1, MSG_CANCEL, 0, 0);
  last = 0xffffffff;
  D (bug ("config.c 1809...........\n"));  
  ...
  if (openinitreq (gb))
      ...
      else
	{
	  ...
	  fmmain.maxdirlistwidth =
	    fmconfig->dl[0].width + fmconfig->dl[1].width +
	    fmconfig->dl[2].width + fmconfig->dl[3].width +
	    fmconfig->dl[4].width + 4;
	}
    }
cerr:
  freereq (gb);
