...
#ifndef AROS
WORD __asm hexconvert (register __a0 UBYTE *, register __a1 UBYTE *,
		       register __d0 WORD, register __d1 WORD);
#endif
...
fileedit (void)
{
  ...
  if (sselected (edit->list, 1))
      ...
      if (edit->node = findselnode (edit->list))
	{
	  ...
	  if (edit->handle =
	      openfile (edit->list, NDFILE (edit->node), OFNORMAL))
	    {
...
#ifndef AROS
	  edit->ioreq =
	    (struct IOStdReq *) opendevice (edit->list,
					    (UBYTE *) (fssm->
						       fssm_Device << 2) + 1,
					    fssm->fssm_Unit, fssm->fssm_Flags,
					    sizeof (struct IOStdReq));
#else
	  edit->ioreq =
	    (struct IOStdReq *) opendevice (edit->list,
					    (UBYTE *) (fssm->fssm_Device) + 1,
					    fssm->fssm_Unit, fssm->fssm_Flags,
					    sizeof (struct IOStdReq));
#endif
	  apu1 = de->de_Surfaces * de->de_BlocksPerTrack * 512L;
	  if (!edit->dec->lowcyl)
	    edit->lowoffset = de->de_LowCyl * apu1;
...
//de=edit->dosenv;
  ...
  if (fmmain.appiconport)
    signalmask |= (1L << fmmain.appiconport->mp_SigBit);
  if (fmmain.appwinport)
    signalmask |= (1L << fmmain.appwinport->mp_SigBit);
  ...
  while (!quit)
	...
	case IDCMP_RAWKEY:
	  ...
	  if (edit->mode >= 0)
		...
		case 0x2f:
	      ...
	      if (mode != edit->mode || curx != edit->cursorx
		  || cury != edit->cursory)
		{
		  if (edit->mode != mode)
		    edit->mode = mode;
		...
		case 0x2e:	//root
		  if (edit->type)
		    offset = edit->rootoffset;
		  break;
		}
	      if (offset != edit->offset)
		{
		  if (!readoffset (edit, offset))
		    quit = 1;
		  break;
		}
	    }
	  ...
	  if (MapRawKey (&ie, chartab, 2, 0) == 1)
		...
		case -1:	//none
		  ...
		  if (!(code & 0x80))
		    {
		      if (asciicode == 0x0d && !edit->type)
			askline (edit);
		      apu1 = (message->Qualifier & 3) ? -1 : 1;
		      apu2 = -1;
		      if (asciicode == edit->btsh[0])
			apu2 = 0;
		      if (asciicode == edit->btsh[3])
			apu2 = 3;
		      if (asciicode == edit->btsh[2])
			apu2 = 2;
		      if (asciicode == edit->btsh[1])
			apu2 = 1;
		  ...
		  if (asciicode)
		      ...
		      for (apu1 = 0; apu1 < 4; apu1++)
			{
			  ...
			  if (asciicode == cmgad->GadgetID)
			    {
			      if (editprocessgadget
				  (edit, message, cmgad->GadgetID))
				quit = 1;
			      break;
			    }
			  fbg++;
	...
	case IDCMP_MOUSEBUTTONS:
	  if (mousey <= edit->editheight + edit->yoffset)
	  ...
	  else
	    {
		  ...
		  && mousey >=
		  edit->editheight + edit->yoffset + fmconfig->spaceh
		  && mousey <=
		  edit->editheight + edit->yoffset + fmconfig->spaceh +
		  fmmain.txtysize)
		{
		  if (edit->type)
		    {
		      if (mousex >= edit->xoffsethex
			  && mousex <=
			  edit->xoffsethex + 64 - fmconfig->spacew)
			readoffset (edit, edit->rootoffset);
		      for (apu1 = 0; apu1 < 4; apu1++)
			{
			  if (mousex >= edit->btshxw[apu1 * 2]
			      && mousex <= edit->btshxw[apu1 * 2 + 1])
			    limitblocks (edit, apu1,
					 code == SELECTDOWN ? 1 : -1);
			}
		    }
		  else
		    {
		      if (code == SELECTDOWN && mousex >= edit->xoffsetasc
			  && mousex <= edit->xoffsetasc + edit->ascwidth)
			askline (edit);
		    }
		}
	    }
	...
	case IDCMP_GADGETDOWN:
	  ...
	  if (apu1 >= 32)
	    {
	      if (editprocessgadget (edit, message, apu1))
		quit = 1;
	    }
	  break;
	}
...
WORD
readoffset (struct Edit *edit, LONG offset)
{
  edit->oldsearch = -1;
  switch (edit->type)
    {
    case 0:
      if (offset > edit->handle->size - edit->bufsize)
	offset = edit->handle->size - edit->bufsize;
      break;
    case 1:
      if (offset >= edit->maxoffset)
	offset = edit->maxoffset - edit->bufsize;
      break;
    }
  if (offset < 0)
    offset = 0;
  edit->offset = offset;
  if (!readfileedit (edit))
    return (0);
  editoutput (edit);
  return (1);
}
...
void
convertmouse (struct Edit *edit, WORD * amousex, WORD * amousey)
{
  WORD mousex, mousey;
  ...
  if (mousex >= edit->xoffsetasc)
  ...
  else
    {
      ...
      if (mousex >= 0)
	mousex /= (fmmain.listxsize * 2);
      else
	mousex = 0;
    }
  if (mousey >= edit->rows)
    mousey = edit->rows - 1;
  if (mousex >= edit->columns)
    mousex = edit->columns - 1;
  *amousex = mousex;
  *amousey = mousey;
}

void
editoutputline (struct Edit *edit)
{
  ObtainSemaphore (&fmmain.gfxsema);
  SetAPen (fmmain.rp, fmconfig->txtpen);
  SetDrMd (fmmain.rp, JAM2);
  SetFont (fmmain.rp, fmmain.listfont);
  editoutputlistline (edit,
		      edit->buffer + edit->cursory * edit->columns +
		      edit->screenoffset,
		      edit->bufsize - edit->cursory * edit->columns -
		      edit->screenoffset, edit->cursory);
  SetFont (fmmain.rp, fmmain.txtfont);
  ReleaseSemaphore (&fmmain.gfxsema);
}

void
editoutput (struct Edit *edit)
{
  WORD yy;
  UBYTE *memory;
  LONG bsize;
  ...
  if (edit->type)
    {
      if (edit->screenoffset > edit->bufsize - edit->screenbytes)
	edit->screenoffset = edit->bufsize - edit->screenbytes;
      if (edit->screenoffset < 0)
	edit->screenoffset = 0;
    }
...
void
editoutputlistline (struct Edit *edit, UBYTE * memory, LONG bsize, WORD yy)
{
  UBYTE vara[1000];
  WORD yyy, yyyb;
  UBYTE *ptr;
  ...
  if (bsize >= edit->columns)
  ...
  else if (bsize > 0)
    {
      ...
      RectFill (fmmain.rp, edit->xoffsethex + bsize * fmmain.listxsize * 2,
		yyyb,
		edit->xoffsethex + (edit->columns * 2) * fmmain.listxsize - 1,
		yyyb + fmmain.listysize - 1);
      RectFill (fmmain.rp, edit->xoffsetasc + bsize * fmmain.listxsize, yyyb,
		edit->xoffsetasc + (edit->columns) * fmmain.listxsize - 1,
		yyyb + fmmain.listysize - 1);
      SetAPen (fmmain.rp, fmconfig->txtpen);
    }
  else
    {
      SetAPen (fmmain.rp, fmconfig->backpen);
      RectFill (fmmain.rp, edit->xoffsethex, yyyb,
		edit->xoffsethex + edit->columns * fmmain.listxsize * 2 - 1,
		yyyb + fmmain.listysize - 1);
      RectFill (fmmain.rp, edit->xoffsetasc, yyyb,
		edit->xoffsetasc + edit->columns * fmmain.listxsize - 1,
		yyyb + fmmain.listysize - 1);
      SetAPen (fmmain.rp, fmconfig->txtpen);
    }
  ...
  for (yyy = 0; yyy < edit->columns; yyy++)
	...
	fmmain.rp->Mask = 3;
      if (ptr[yyy])
	{
	  RectFill (fmmain.rp, edit->xoffsethex + yyy * fmmain.listxsize * 2,
		    yyyb,
		    edit->xoffsethex + ((yyy + 1) * fmmain.listxsize) * 2 - 1,
		    yyyb + fmmain.listysize - 1);
	  RectFill (fmmain.rp, edit->xoffsetasc + yyy * fmmain.listxsize,
		    yyyb,
		    edit->xoffsetasc + ((yyy + 1) * fmmain.listxsize) - 1,
		    yyyb + fmmain.listysize - 1);
	}
...
void
editcursor (struct Edit *edit, WORD x, WORD y)
{
  WORD yyyb, xxxb;
  UBYTE vara[40];
  LONG offset;
  ...
  if (edit->cursorx >= 0)
    {
      ...
      RectFill (fmmain.rp, edit->xoffsethex + xxxb * 2, yyyb,
		edit->xoffsethex + (xxxb + fmmain.listxsize) * 2 - 1,
		yyyb + fmmain.listysize - 1);
      RectFill (fmmain.rp, edit->xoffsetasc + xxxb, yyyb,
		edit->xoffsetasc + xxxb + fmmain.listxsize - 1,
		yyyb + fmmain.listysize - 1);
    }
  if (x >= 0 && y >= 0)
    {
  ...
  switch (edit->type)
    ...
    case 0:
      offset = edit->offset;
      if (edit->cursorx >= 0)
	offset += edit->cursory * edit->columns + edit->cursorx;
      sformatmsg (vara, MSG_FDEDIT_FILEINFO1, offset, edit->handle->size,
		  aschex);
      fittext (fmmain.rp, vara, -1, edit->xoffsetasc - 1, edit->infoliney + 1,
	       edit->ascwidth, 0);
      break;
    case 1:
      blockinfo (edit);
...
WORD
editwindownewsize (struct Edit *edit)
{
  ...
  edit->mode = -1;
  edit->columns =
    ((width - 3 * fmconfig->spacew) / (fmmain.listxsize * 3)) & 0xfffe;
  edit->rows =
    (height - 5 * tfh - 3 * fmconfig->spaceh + 1) / fmmain.listysize;
  ...
  apu1 =
    (((width - 3 * fmconfig->spacew - edit->ascwidth - edit->hexwidth) +
      2) / 3);
  edit->xoffsethex = fmconfig->spacew + 1 + apu1;
  edit->xoffsetasc =
    edit->xoffsethex + edit->hexwidth + apu1 + fmconfig->spacew;
  ...
  draw3dbox (fmmain.rp, edit->xoffsethex - 1, edit->yoffset - 1,
	     edit->hexwidth, edit->editheight, 0);
  draw3dbox (fmmain.rp, edit->xoffsetasc - 1, edit->yoffset - 1,
	     edit->ascwidth, edit->editheight, 0);
  edit->infoliney = edit->yoffset + edit->editheight + fmconfig->spaceh - 1;

  switch (edit->type)
    {
    case 0:			//file editor
      draw3dbox (fmmain.rp, edit->xoffsethex - 1, edit->infoliney,
		 edit->hexwidth, tfh, 0);
      draw3dbox (fmmain.rp, edit->xoffsetasc - 1, edit->infoliney,
		 edit->ascwidth, tfh, 0);
      fittext (fmmain.rp, edit->handle->filename, -1, edit->xoffsethex - 1,
	       edit->infoliney + 1, edit->hexwidth, 0);
      edit->bufsize = edit->screenbytes;
      if (edit->handle->size < edit->bufsize)
	edit->bufsize = edit->handle->size;
      break;
    case 1:			//disk editor
      draw3dbox (fmmain.rp, edit->xoffsethex - 1, edit->infoliney, 64, tfh,
		 0);
      fittext (fmmain.rp, edit->devname, -1, edit->xoffsethex - 1,
	       edit->infoliney + 1, 64, 0);
      edit->xoffsetbtsh = edit->xoffsethex + 64 + fmconfig->spacew - 1;
      edit->xwidthbtsh = edit->hexwidth - 64 - fmconfig->spacew;
      draw3dbox (fmmain.rp, edit->xoffsetbtsh, edit->infoliney,
		 edit->xwidthbtsh, tfh, 0);
      draw3dbox (fmmain.rp, edit->xoffsetasc - 1, edit->infoliney,
		 edit->ascwidth, tfh, 0);
  ...
  for (cnt = 0; cnt < 4; cnt++)
      ...
      while (*ptr1)
	{
      ...
      doborder (fbg->xy1, &fbg->border1, &fbg->border2, fbg->xy2,
		&fbg->border3, &fbg->border4, gad->Height, gad->Width);
  ...
  fmmain.bottomlinewidth =
    width - bottomwidth - fmconfig->spacew - 2 * fmconfig->spacew / 3 * 2;
  draw3dbox (fmmain.rp, fmconfig->spacew, fmmain.bottomliney - 1,
	     fmmain.bottomlinewidth, tfh, 0);
  ...
  if (edit->screenbytes > edit->usebufsize)
    edit->usebufsize = edit->screenbytes;
...
freeedit (struct Edit *edit)
{
  ...
  edit->usebuffer = 0;
  if (edit->gads)
    RemoveGList (fmmain.ikkuna, &edit->exitgadget.gadget, -1);
  edit->gads = 0;
}

WORD
readfileedit (struct Edit *edit)
{
  WORD ret;
  ...
  switch (edit->type)
    {
    case 0:
      if (seek (edit->handle, edit->offset, OFFSET_BEGINNING) < 0)
	return (0);
      if (readbufferfile (edit->handle, edit->buffer, edit->bufsize) < 0)
	return (0);
      return (1);
      break;
    case 1:
      ...
      if (!ret)
	memseti (edit->buffer, 0, edit->blocklen);
...
WORD
writefileedit (struct Edit *edit)
{
  if (!requestmsg
      (edit->list->workname, MSG_YES, MSG_NO, MSG_MAIN_AREYOUSURE))
    return (1);
  memseti (edit->usebuffer, 0, edit->usebufsize);
  switch (edit->type)
    {
    case 0:
      if (seek (edit->handle, edit->offset, OFFSET_BEGINNING) < 0)
	return (0);
      if (writefile (edit->handle, edit->buffer, edit->bufsize) < 0)
	return (0);
	...
	{
	  if (requestmsg
	      (edit->list->workname, MSG_YES, MSG_NO,
	       MSG_FDEDIT_CALCCHECKSUM))
	    {
	      *((ULONG *) & edit->buffer[4 * 5]) = 0;
	      *((ULONG *) & edit->buffer[4 * 5]) =
		-checksum (edit->buffer, edit->blocklen);
	      editoutput (edit);
	    }
	}
#else
      if (CalcChecksum (edit->buffer, edit->blocklen))
	{
	  if (requestmsg
	       ...
	       MSG_FDEDIT_CALCCHECKSUM))
	    {
	      *((ULONG *) & edit->buffer[4 * 5]) = 0;
	      *((ULONG *) & edit->buffer[4 * 5]) =
		-CalcChecksum (edit->buffer, edit->blocklen);
	      editoutput (edit);
	    }
	}
#endif
...
blockinfo (struct Edit *edit)
{
  ...
  if (type1 == 2 && type2 == 1)
      ...
      if (edit->changed)
	{
	  ...
	  longtodatestring (date1,
			    dstolong ((struct DateStamp *) (edit->buffer +
							    0x1a4)));
	  longtodatestring (date2,
			    dstolong ((struct DateStamp *) (edit->buffer +
							    0x1e4)));
	  sformatmsg (list->fmmessage1, MSG_FDEDIT_DISKROOT1, name, date2,
		      date1);
  ...
  if (type1 == 2 && type2 == 0xfffffffd)
      ...
      if (edit->changed)
	{
	  ...
	  longtodatestring (date1,
			    dstolong ((struct DateStamp *) (edit->buffer +
							    0x1a4)));
	  sformatmsg (list->fmmessage1, MSG_FDEDIT_DISKFILE1, name, ptr[81],
		      date1);
	  sformatmsg (list->fmmessage2, MSG_FDEDIT_DISKFILE2, ptr[2], ptr[4],
		      ptr[124], ptr[125], ptr[126]);
	}
    }
  if (type1 == 2 && type2 == 2)
    {
      ...
      if (edit->changed)
	{
	  ...
	  sformatmsg (list->fmmessage2, MSG_FDEDIT_DISKDIR2, ptr[124],
		      ptr[125]);
	}
    }
  if (type1 == 16 && type2 == 0xfffffffd)
    {
      ...
      if (edit->changed)
	{
	  sformatmsg (list->fmmessage1, MSG_FDEDIT_DISKLIST1, ptr[2],
		      ptr[125], ptr[126]);
	}
    }
  if (type1 == 8 && edit->dosenv->de_DosType == 0x444f5300)
    {
      ...
      if (edit->changed)
	{
	  sformatmsg (list->fmmessage1, MSG_FDEDIT_DISKDATA1, ptr[1], ptr[2],
		      ptr[3], ptr[4]);
	}
    }
  cnt[0] = 0;
  ...
  if (edit->mode >= 0)
    sformat (cnt, "%03.3d %s", edit->cursoroffset + edit->screenoffset,
	     aschex);

#ifndef AROS
  sformatmsg (vara, MSG_FDEDIT_DISKINFO1,
	      checksum (edit->buffer, edit->blocklen) ? '-' : '+',
	      getstring (dtype), edit->screenoffset, cnt);
  fittext (fmmain.rp, vara, -1, edit->xoffsetasc - 1, edit->infoliney + 1,
	   edit->ascwidth, 0);
#else
  sformatmsg (vara, MSG_FDEDIT_DISKINFO1,
...
void
checkblocks (struct Edit *edit, WORD mode)
{
  struct DosEnvec *de;
  ...
  if (!mode)
    {
      if (edit->tracks < 0)
	edit->tracks = 0;
      if (edit->tracks > de->de_HighCyl)
	edit->tracks = de->de_HighCyl;
      if (edit->sectors < 0)
	edit->sectors = 0;
	...
	edit->heads = de->de_Surfaces - 1;
      edit->blocks =
	edit->tracks * edit->dosenv->de_Surfaces *
	edit->dosenv->de_BlocksPerTrack;
...
void
limitblocks (struct Edit *edit, WORD mode, WORD dir)
{
  LONG *ptr;
  struct DosEnvec *de;
  ...
  if (edit->blocks < 0)
    edit->blocks = 0;
  if (edit->blocks >= edit->maxoffset / edit->blocklen)
    edit->blocks = edit->maxoffset / edit->blocklen - 1;
...
void
search (struct Edit *edit, WORD mode)
{
  UBYTE str[64];
  WORD len, len2;
  WORD cnt;
  ...
  if (!mode || edit->searchstring[0] == 0)
    {
      if (!askstring
	  (0, edit->list->workname, getstring (MSG_FDEDIT_SEARCH),
	   edit->searchstring, 32))
	return;
    }
  ...
  if (edit->searchstring[0] == '$')
	...
	str[cnt] = ToUpper (edit->searchstring[cnt]);
      str[cnt] = 0;
      if (cnt <= 0)
	goto fail;
    }
  if (edit->type)
    {
      bufsize =
	((fmconfig->dosbuffersize & 0x00ffffff) / edit->blocklen) *
	edit->blocklen;
      ...
      if (sbuf)
	{
	  ...
	  if (edit->oldsearch != -1)
	    ...
	    offset = edit->offset;
	  edit->oldsearch = -1;
	  sformatmsg (edit->list->fmmessage1, MSG_FDEDIT_SEARCHING,
		      offset / edit->blocklen,
		      edit->maxoffset / edit->blocklen);
	  fmmessage (edit->list);
	  while (offset < edit->maxoffset)
		...
		break;
	      if (!hexdec)
		for (apu3 = 0; apu3 < size; apu3++)
		  sbuf[apu3] = ToUpper (sbuf[apu3]);
		  ...
		  if (!memcmp (str, &sbuf[apu3], len))
		    {
		      ...
		      for (cnt = 0; cnt < len; cnt++)
			{
			  if (apu4 + cnt >= 0 && apu4 + cnt <= edit->blocklen)
			    edit->usebuffer[apu4 + cnt] = 2;
			}
		      editoutput (edit);
		      edit->oldsearch = apu3;
		      goto sok;
      ...
      if (sbuf)
	{
	  ...
	  if (edit->oldsearch != -1)
	    ...
	    offset = edit->offset;
	  edit->oldsearch = -1;
	  sformatmsg (edit->list->fmmessage1, MSG_FDEDIT_SEARCHING, offset,
		      edit->handle->size);
	  fmmessage (edit->list);
	  while (offset < edit->handle->size)
	    {
	      ...
	      if (testabort (edit->list))
		break;
	      if (seek (edit->handle, offset, OFFSET_BEGINNING) < 0)
		break;
	      size = bufsize;
	      ...
	      else
		{
	      ...
	      if (readbufferfile (edit->handle, sbuf, size) < 0)
		break;
	      if (!hexdec)
		for (apu3 = 0; apu3 < size; apu3++)
		  sbuf[apu3] = ToUpper (sbuf[apu3]);
...
void
askline (struct Edit *edit)
{
  LONG offset;

  offset = edit->offset;
  if (!asklong
      (0, edit->list->workname, getstring (MSG_FDEDIT_BYTENUMBER), &offset, 0,
       edit->handle->size))
    return;
  readoffset (edit, offset);
}
...
WORD
editprocessgadget (struct Edit * edit, struct IntuiMessage * msg, WORD id)
{
  if (edit->keyshorts[3] == id)
    {
      search (edit, msg->Qualifier & 0x80);
    }
  if (edit->keyshorts[0] == id)
    return (1);
...
void *
diskeditconfigdefault (struct CMenuConfig *cmc)
{
...
WORD
diskeditconfig (struct GUIBase * gb, struct CMenuConfig * cmc)
{
  struct DiskEditConfig *config;
  WORD c, lowcyl;
