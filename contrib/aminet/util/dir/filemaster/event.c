...
sleep (WORD sleepv)
{
  ...
  while (fmmain.sleepon == fmmain.quitti)
    waitsig (SIGBREAKF_CTRL_F);
  if (fmmain.sleepon)
    {
      for (;;)
	{
...
void
setsource (struct FMList *list1)
{
  struct FMList *vlist;
  if (list1 == fmmain.sourcedir)
    return;
...
static struct FMNode *
mousenode (struct FMList *list, WORD xx, WORD yy)
{
  struct ListInfo *li;
  struct FMNode *node;
  WORD cnt, apu1;
...
static WORD
processclick (struct IntuiMessage *msg)
{
...
//freeese
  ...
  if (msg->Code == MENUDOWN && xx >= fmmain.cmenux
      && xx <= fmmain.cmenux + fmmain.totalcmenuwidth && yy >= fmmain.cmenuy
      && yy <= fmmain.cmenuy + fmmain.totalcmenuheight)
    {
  ...
  for (apu1 = 0; apu1 < LISTS; apu1++)
    {
      ...
      if (li && xx >= li->x && xx <= li->x + li->width && yy >= li->y
	  && yy <= li->y + li->height)
	{
  ...
  if (yy >= fmmain.messageliney1 && msg->Code == MENUDOWN)
    askmodabort ();
  ...
  if (msg->Code == MENUDOWN && fmmain.sourcedir->flags & LACTIVE)
    fmmain.sourcedir->flags |= LABORTREQ;
  D (bug ("event.c 249 processclick func \n"));
  setsource (list1);
  D (bug ("event.c 251 processclick func \n"));
  if (list1->flags & LALLOCATED)
    return (dragtype);
  ...
  if (msg->Code == MENUDOWN)
    {
      ...
      if (xx >= li->slider1.LeftEdge
	  && xx <= li->slider1.LeftEdge + li->slider1.Width
	  && yy >= li->slider1.TopEdge
	  && yy <= li->slider1.TopEdge + li->slider1.Height)
	{
	  D (bug ("event.c 260 processclick func \n"));
	  launch (launchtable[PARENTCONFIG], getconfignumber (PARENTCONFIG),
		  0, 0);
  ...
  if (msg->Code == MENUDOWN && fmconfig->rightmouse == 0)
  ...
  if (msg->Code == SELECTDOWN)
	...
	dragtype = 0;
      if (list1->flags & LDIRLIST && node->flags & NSELECTED)
	endofdirtxt (list1, node);
      if (list1->flags & LDIRLIST && !(node->flags & NSELECTED))
	endofdirtxt (list1, 0);
      outputlistline (list1, node);
      D (bug ("event.c 292 processclick func \n"));
      tv1.tv_secs = fmmain.secs;
      tv1.tv_micro = fmmain.mics;
      tv2.tv_secs = msg->Seconds;
      tv2.tv_micro = msg->Micros;
      SubTime (&tv2, &tv1);
      D (bug ("event.c 296 processclick func \n"));
      if (list1->lastclicked == node && !tv2.tv_secs
	  && tv2.tv_micro / 10000 <= fmconfig->doubleclick)
	{
	  dirfileclick (list1, node, xx);
	  list1->lastclicked = 0;
	}
      else
	{
      ...
      fmmain.secs = msg->Seconds;
      fmmain.mics = msg->Micros;
    }
...
static void
processgadget (WORD id, ULONG class)
{
  if (id >= 301 && id < (301 + TOTALCOMMANDS))
    {
      if (id - 301 < TOTALCOMMANDS)
	runcommand (&fmmain.gadkeytab[id - 301 + LISTGADGETS * WINDOWLISTS]);
    }
  // slider1s 101->
  // slider2s 111->
  // string gadgets 201->
  ...
  if (id >= 101 && id <= 110)
    {
      ...
      fmmain.sourcedir->firstline =
	fmmain.sourcedir->li->rscroll1.TopEntryNumber;
      return;
    }
  if (id >= 111 && id <= 120)
    {
      ...
      fmmain.sourcedir->firstlinew =
	fmmain.sourcedir->li->rscroll2.TopEntryNumber;
      return;
    }
}
...
runhotkey (WORD hotkey)
{
  ...
  if ((gkt->onelist && gkt->listnum == fmmain.sourcedir->listnumber)
      || (!gkt->onelist))
    {
      D (bug ("event.c 330 \n"));
      if (gkt->cmc)
...
WORD
smallevent (struct Window * win, ULONG * class, UWORD * code)
{
...
void
getappdir (struct AppMessage *appmsg)
{
  BPTR lock;
...
eventti:
  ...
  while (!fmmain.quitti)
      ...
      if (waiti)
	{
	  ...
	  if (sigs & SIGBREAKF_CTRL_F)
		...
		{
		  D (bug ("event.c 448 \n"));
		  launch (launchtable[SLEEPCONFIG],
			  getconfignumber (SLEEPCONFIG), 0, 0);
		}
	    }
	}
      if (fmmain.appwinport)
      ...
      if (!message)
	{
	  ...
	  && !(fmmain.sourcedir->flags & LALLOCATED))
	{
	  D (bug ("event.c 494 \n"));
	  switch (code)
	    {
	    case 0x4c:		//cursor up
	      ...
	      if (message->Qualifier & 0x0003)
		fmmain.sourcedir->keyline -=
		  fmmain.sourcedir->li->visiblelines - 2;
	      if (message->Qualifier & 0x0008)
		fmmain.sourcedir->keyline = 1;
	      fmmain.sourcedir->keyline--;
	      frameoutput (fmmain.sourcedir);
	      break;
		...
		fmmain.sourcedir->keyline +=
		  fmmain.sourcedir->li->visiblelines - 2;
	      if (message->Qualifier & 0x0008)
		fmmain.sourcedir->keyline = fmmain.sourcedir->totlines - 2;
	      fmmain.sourcedir->keyline++;
	      frameoutput (fmmain.sourcedir);
	      break;
		...
		fmmain.sourcedir->keylinew +=
		  fmmain.sourcedir->li->visiblelinesw;
	      frameoutput (fmmain.sourcedir);
	      break;
      ...
      if (class == IDCMP_RAWKEY)
	{
	  ...
	  switch (code)
		...
		*((UBYTE *) (fmmain.modptr)) = 1;
	      apu1 = 1;
	      break;
	    case 0x80:		//~released
	      D (bug ("event.c 533 if IDCMP_RAWKEY \n"));
	      if (fmmain.modflag == MODPLAY)
		*((UBYTE *) (fmmain.modptr)) = 0;
      ...
      if (asciicode)
	{
	  ...
	  if (asciicode == 9)
		...
		case 13:	//return
		  ...
		  fakeintu.MouseX =
		    fmmain.sourcedir->li->dirx + fmmain.ikkuna->BorderLeft;
		  fakeintu.MouseY =
		    (fmmain.sourcedir->keyline -
		     fmmain.sourcedir->firstline) * fmmain.listysize +
		    fmmain.sourcedir->li->diry + fmmain.ikkuna->BorderTop;
		  if (message->Qualifier & 0x03)
		    fakeintu.Code = MENUDOWN;
		  else
		    fakeintu.Code = SELECTDOWN;
	...
	case IDCMP_CLOSEWINDOW:
	  ...
	  framewas = 0;
	  launch (launchtable[QUITCONFIG], getconfignumber (QUITCONFIG), 0,
		  0);
	  break;
	...
	case IDCMP_DISKREMOVED:
	  ...
	  for (code = 0; code < WINDOWLISTS; code++)
	    {
	      if (fmmain.li[code]
		  && !(fmmain.li[code]->list->flags & LALLOCATED)
		  && fmmain.li[code]->list->flags & LDEVLIST)
		drives (fmmain.li[code]->list);
	    }
	  break;
	...
	case IDCMP_MOUSEMOVE:
	  ...
	  do
	      ...
	      if (!apunode1 && apu1 >= 0)
		{
		  ...
		  else if (apu1 > apulist->li->ediry
			   && apulist->firstline <
			   apulist->totlines + apulist->li->visiblelines)
		    {
		      apulist->firstline++;
		      apu4 =
			apulist->firstline + apulist->li->visiblelines - 1;
		    }
		  if (apu4 >= 0)
		    {
	  ...
	  while (apu1
		 && !fmmain.ikkuna->UserPort->mp_MsgList.lh_Head->ln_Succ);
	  apu1 = 0;
	  apunode2 = (struct FMNode *) apulist;
	  if (!
	      (!apunode1 || apunode1 == apulist->lastclicked
	       || !apulist->lastclicked || apunode1 == apunode2
	       || draggi < 0))
	    {
	      while (iteratenode (&apunode2))
		{
		  apu3 = -1;
		  if (apunode2 == apunode1
		      || apunode2 == apulist->lastclicked)
		    apu3 = apu1 ^ 1;
		  if (apu3 == 1)
		    apu1 = apu3;
	...
	case IDCMP_GADGETDOWN:
	  ...
	  framewas = 0;
	  processgadget (((struct Gadget *) message->IAddress)->GadgetID,
			 class);
	  break;
	...
	case IDCMP_MOUSEBUTTONS:
	  ...
	  framewas = 0;
	  if (code == SELECTUP || code == SELECTDOWN || code == MENUDOWN
	      || code == MENUUP)
	    {
	      D (bug ("event.c 694"));
  ...
  if (fmmain.ikkuna && fmmain.quitti == 2
      && !requestmsg (getstring (MSG_MAIN_QUIT), MSG_YES, MSG_NO,
		      MSG_MAIN_AREYOUSURE))
    goto eventti;
  aboutti (1);
qquit:
...
void
dirfileclick (struct FMList *list1, struct FMNode *node, WORD xx)
{
  setparent (list1);
  if (node->flags & (NDEVICE | NASSIGN | NDIRECTORY))
      ...
      else
	{
	  ...
	  if (xx >= list1->li->dirx + 8 * fmmain.listxsize
	      && xx < list1->li->edirx && NDCOMM (node)[0])
  ...
  if (((fmmain.sourcedir->flags | fmmain.destdir->flags) & LALLOCATED) == 0)
    {
      ...
      fmmain.destdir->flags |=
	fmmain.sourcedir->flags & (LDIRLIST | LDEVLIST);
      if (fmmain.destdir->li)
	RefreshGList (&fmmain.destdir->li->string, fmmain.ikkuna, 0, 1);
...
gparent (void)
{
  ...
  if (!(fmmain.sourcedir->flags & LALLOCATED))
	...
	{
	  apup1 = PathPart (fmmain.sourcedir->path);
	  *apup1 = 0;
...
void *
sleepconfigdefault (struct CMenuConfig *cmc)
{
  struct SleepConfig *config;
  WORD ret;
...
WORD
sleepconfig (struct GUIBase * gb, struct CMenuConfig * cmc)
{
  WORD c;
  struct SleepConfig *sc;
...
void *
quitconfigdefault (struct CMenuConfig *cmc)
{
  struct QuitConfig *config;
  WORD ret;
...
WORD
quitconfig (struct GUIBase * gb, struct CMenuConfig * cmc)
{
  WORD c;
