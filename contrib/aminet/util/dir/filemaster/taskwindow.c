...
taskwindow (WORD listnum)
{
...
#ifdef V39
  SetWindowPointer (fmmain.ikkuna, WA_BusyPointer, TRUE, WA_PointerDelay,
		    TRUE, TAG_DONE);
#endif
  Forbid ();
  txtptr = text;
  for (cnt = 0; cnt < LISTS; cnt++)
    {
      ...
      if (fmlist[cnt].flags)
	{
	  if (fmlist[cnt].flags & LDEVLIST)
	    ptr1 = getstring (MSG_TASKWINDOW_DEVLIST);
	  if (fmlist[cnt].workname[0])
	    ...
	    {
	      if (fmlist[cnt].path[0])
		ptr1 = fmlist[cnt].path;
	    }
	}
  ...
  txtptr = text;
  c = 0;
  sformat (title, getstring (MSG_TASKWINDOW_LISTS),
	   (WORD) fmmain.li[listnum]->list->listnumber);
  gb = getguibase (title);
  for (cnt = 0; cnt < LISTS; cnt++)
    {
...
void
nextlist (struct ListInfo *li, WORD cnt)
{
  WORD apu1;
  ...
  for (;;)
	...
	apu1 = 0;
      if (apu1 < 0)
	apu1 = LISTS - 1;
      if (setlist (li, &fmlist[apu1]))
	break;
    }
}

WORD
setlist (struct ListInfo *li, struct FMList *list)
{
  WORD apu1;

  for (apu1 = 0; apu1 < LISTS; apu1++)
    {
      if (fmmain.li[apu1] && fmmain.li[apu1]->list == list
	  && fmmain.li[apu1] != li)
	return (0);
    }
  if (li->list)
  ...
  if (list->path[0] == 0 || list->flags & LDEVLIST)
    bdrives (list);
...
void
updatetasknumbers (struct ListInfo *li)
{
  WORD cnt;
  ...
  if (!li)
      ...
      for (cnt = 0; cnt < LISTS; cnt++)
	{
	  if (fmmain.li[cnt])
	    {
	      fmmain.li[cnt]->taskgadget.UserData =
		(APTR) (fmmain.li[cnt]->list->listnumber + '0');
