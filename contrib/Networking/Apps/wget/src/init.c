...
static struct {
  ...
  { "dotfiles",		&opt.dot_files,		cmd_boolean },
  ...
  { "maxfilenamelength",&opt.max_filename_length, cmd_number },
  { "mirror",		NULL,			cmd_spec_mirror },
  { "netrc",		&opt.netrc,		cmd_boolean },
  { "noclobber",	&opt.noclobber,		cmd_boolean },
#if _AMIGASF
  { "nofilecomments",	&opt.no_file_comments,	cmd_boolean },
#endif
...
defaults (void)
{
  ...
  opt.max_filename_length = 8192;	/* no default limit, I mean */
#if _AMIGASF
  opt.no_file_comments = 0;
#endif
}

...
#else /* AmigaOS */
  /* On AmigaOS all the configs are into 'env:'
   */
   file = "env:wgetcfg";
...
#define VAL_IS(string_literal) BOUNDED_EQUAL (val, end, string_literal)
  ...
  if (VAL_IS ("unix"))
  ...
  else if (VAL_IS ("amiga"))
    restrict_os = restrict_amiga;
  else if (VAL_IS ("amiga-strict"))
    restrict_os = restrict_amiga_strict;
    ...
    err:
      fprintf (stderr,
	       _("%s: %s: Invalid restriction `%s', use `unix', 'amiga', 'amiga-strict' or `windows'.\n"),
	       exec_name, com, val);
      return 0;
    }
