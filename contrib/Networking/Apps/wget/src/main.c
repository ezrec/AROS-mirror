...
struct cmdline_option option_data[] =
  {
    ...
    { "dot-files", '.', OPT_BOOLEAN, NULL, no_argument },
    ...
    { "max-filename-length", 0, OPT_VALUE, NULL, required_argument },
    { "mirror", 'm', OPT_BOOLEAN, "mirror", -1 },
    { "no", 'n', OPT__NO, NULL, required_argument },
    { "no-clobber", 0, OPT_BOOLEAN, "noclobber", -1 },
#if _AMIGASF
    { "no-file-comments", 0, OPT_BOOLEAN, NULL, no_argument },
#endif
...
Download:\n"),
  ...
  -Q,  --quota=NUMBER            set retrieval quota to NUMBER.\n"),
    ...
    N_("\
       --max-filename-length=#   maximum length of local file names.\n"),
    N_("\
       --no-file-comments        do not set the url as comment to the file.\n"),
#ifdef ENABLE_IPV6
    N_("\
...

#ifdef _AMIGASF
static void __close_bsdsocket( void )
{
	if(SocketBase) {
...
Can't timestamp and not clobber old files at the same time.\n"));
  ...
  if (opt.background)
    ...
    fork_to_background ();
#else
    fprintf( stderr, "forking to the background isn't implemented on this version.\n\a");
#endif
...
# warning using latest know build version
#endif

#ifndef __BUILDER
# error define __BUILDER with your name...
#endif

static const unsigned char __amiga_version_id[] __attribute__((unused)) =
"$VER:Wget v" VERSION ", Compiled for " OS_TYPE " by " __BUILDER " [" __DATE__ "]\0";
