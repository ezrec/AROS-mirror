...
/* those socket macros was there until wget 1.9 but no longer needed, anyhow
   I keep it here for now... also 'typedef ULONG LARGE_INT' isnt needed 
   either */
# undef READ
# undef CLOSE
# undef WRITE
# define READ(fd, buf, cnt)	recv((LONG)(fd),(UBYTE *)(buf),(LONG)(cnt),0)
# define WRITE(fd, buf, cnt)	send((LONG)(fd),(UBYTE *)(buf),(LONG)(cnt),0)
# define REALCLOSE(x)		CloseSocket((LONG)x)
# define _AMIGASF		1
#endif
