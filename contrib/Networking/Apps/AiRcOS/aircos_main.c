...
int aircos_sendline(struct IRC_Connection_Private *forConnection)
{
	...
	if ((count = send(forConnection->connection_socket, forConnection->connection_buff_send, strlen(forConnection->connection_buff_send),0)) < 0)
	{
		return -1;
	}
	return count;
...
int aircos_processServerData
(struct IRC_Connection_Private	*process_thisConnection, char *serverin_buffer)
{
   int pos, found = 0;
...
D(bug("[AiRcOS](processserverdata) Quick Reply to PING ..\n"));
   ...
   if (*(process_thisConnection->connection_serv_ARGS[pos]) == ':') process_thisConnection->connection_serv_ARGS[pos]++;
...
D(bug("\n"));
   ...
   if (process_thisConnection->connection_serv_ARGS[pos] != NULL && *(process_thisConnection->connection_serv_ARGS[pos]) == ':') process_thisConnection->connection_serv_ARGS[pos]++;
...
D(bug("[AiRcOS](processserverdata) Text Command recieved '%s'\n",process_thisConnection->connection_serv_ARGS[1]));
		for (pos = 0; commandList_array[pos].command!=NULL && !found; pos++) found = (strcasecmp(commandList_array[pos].command, process_thisConnection->connection_serv_ARGS[1]) == 0);
...
/**/
    ...
    AiRcOS_Base->aircos_app = ApplicationObject,
        ...
        SubWindow, (IPTR) AiRcOS_Base->aircos_quickconnectwin = WindowObject,
            MUIA_Window_Title, (IPTR) "Connect to ..",
            MUIA_Window_Activate, TRUE,
            ...
            WindowContents, (IPTR) VGroup,
                    ...
                    Child, (IPTR) ColGroup(2),
	   				     Child, (IPTR) LLabel("IRC Server Address"),
	    				     Child, (IPTR) (AiRcOS_Base->quickcon_servadd = StringObject,
	   						...
	   						End ),
	    				     Child, (IPTR) LLabel(".. connect on Port"),
                ...
                Child, (IPTR) VGroup, 
                    GroupFrame,
                    Child, (IPTR) ColGroup(2),
	    				     Child, (IPTR) LLabel("Nickname ..."),
	    				     Child, (IPTR) (AiRcOS_Base->quickcon_nick = StringObject,
	    						   StringFrame,
	    						   MUIA_CycleChain, 1,
	    							MUIA_String_Format, MUIV_String_Format_Left,
	    						End ),
	    				     Child, (IPTR) LLabel("Join Channel"),
                    ...
                    End,
                End,
                Child, (IPTR) (AiRcOS_Base->butt_connectServer = SimpleButton("Connect")),
            End,
        End,
        End;
   ...
   set( AiRcOS_Base->quickcon_servuser, MUIA_String_Contents, "aircosuser");
   set( AiRcOS_Base->quickcon_servpass, MUIA_String_Contents,"");
   set( AiRcOS_Base->quickcon_nick, MUIA_String_Contents, "aircos_guest");
   set( AiRcOS_Base->quickcon_channel, MUIA_String_Contents, "#AROS");
