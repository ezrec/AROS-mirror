...
D(bug("%s: S2CmdWrite()\n", unit->sis900u_name));
    ...
    if(error == 0) {
        request->ios2_Req.io_Flags &= ~IOF_QUICK;
        PutMsg(unit->sis900u_request_ports[WRITE_QUEUE], (APTR)request);
    }
    else
    {
...
D(bug("%s: S2CmdOnEvent()\n", unit->sis900u_name));
    ...
    {
        request->ios2_Req.io_Flags &= ~IOF_QUICK;
        PutMsg(unit->sis900u_request_ports[EVENT_QUEUE], (APTR)request);
    }
...
D(bug("%s: S2CmdReadOrphan()\n", unit->sis900u_name));
    ...
    {
        request->ios2_Req.io_Flags &= ~IOF_QUICK;
        PutMsg(unit->sis900u_request_ports[ADOPT_QUEUE], (struct Message *)request);
    }
    else
    {
        request->ios2_Req.io_Error = error;
...
static BOOL CmdOnline(LIBBASETYPEPTR LIBBASE, struct IOSana2Req *request)
{
    struct SiS900Unit *unit = (struct SiS900Unit *)request->ios2_Req.io_Unit;
    BYTE error = 0;
    ULONG wire_error = 0;
    UWORD i;
...
D(bug("%s: S2CmdOnline()\n", unit->sis900u_name));
    ...
    if((error == 0) && ((unit->sis900u_ifflags & IFF_UP) == 0))
    {
        unit->sis900u_stats.PacketsReceived = 0;
        unit->sis900u_stats.PacketsSent = 0;
        unit->sis900u_stats.BadData = 0;
        unit->sis900u_stats.Overruns = 0;
