 ...
 * filter function).
 */
AROS_UFH3(void, SiS900_RX_IntF,
    AROS_UFHA(struct SiS900Unit *,  unit, A1),
...
D(bug("[%s]: CreateUnit: Rx Ring Buffer Descriptors @ %p [DMA @ %p]\n", unit->sis900u_name, unit->rx_ring, unit->rx_ring_dma));
            ...
            if (unit->sis900u_irqhandler && unit->sis900u_touthandler)
            {
                struct Message *msg;

                unit->sis900u_irqhandler->h_Node.ln_Pri = 100;
                unit->sis900u_irqhandler->h_Node.ln_Name = LIBBASE->sis900b_Device.dd_Library.lib_Node.ln_Name;
                unit->sis900u_irqhandler->h_Code = SiS900_IntHandlerF;
                unit->sis900u_irqhandler->h_Data = unit;

                unit->sis900u_touthandler->h_Node.ln_Pri = 100;
                unit->sis900u_touthandler->h_Node.ln_Name = LIBBASE->sis900b_Device.dd_Library.lib_Node.ln_Name;
                unit->sis900u_touthandler->h_Code = SiS900_TimeoutHandlerF;
                unit->sis900u_touthandler->h_Data = unit;

                unit->sis900u_rx_int.is_Node.ln_Name = unit->sis900u_name;
                unit->sis900u_rx_int.is_Code = SiS900_RX_IntF;
                unit->sis900u_rx_int.is_Data = unit;

                unit->sis900u_tx_int.is_Node.ln_Name = unit->sis900u_name;
                unit->sis900u_tx_int.is_Code = SiS900_TX_IntF;
