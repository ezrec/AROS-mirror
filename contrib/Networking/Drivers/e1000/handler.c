...
void handle_request(LIBBASETYPEPTR LIBBASE, struct IOSana2Req *request)
{
    ...
    ReleaseSemaphore(&((struct e1000Unit *)request->ios2_Req.io_Unit)->e1ku_unit_lock);
}
...
D(bug("[%s]: CmdWrite()\n", unit->e1ku_name));
    ...
    if(error == 0) {
        request->ios2_Req.io_Flags &= ~IOF_QUICK;
        PutMsg(unit->e1ku_request_ports[WRITE_QUEUE], (APTR)request);
    }
    else
    {
...
D(bug("[%s]: CmdConfigInterface()\n", unit->e1ku_name));
    ...
    {
        CopyMem(request->ios2_SrcAddr, unit->e1ku_dev_addr, ETH_ADDRESSSIZE);
        e1000func_set_mac(unit);
        unit->e1ku_ifflags |= IFF_CONFIGURED;
    }
    else
    {
...
D(bug("[%s]: CmdTrackType()\n", unit->e1ku_name));
    ...
    tracker = (struct TypeTracker *)
        FindTypeStats(LIBBASE, unit, &unit->e1ku_type_trackers, packet_type);
    ...
    else
    {
        ...
        if(tracker != NULL)
        {
            ...
            Disable();
            AddTail((APTR)&unit->e1ku_type_trackers, (APTR)tracker);
            Enable();
        }
   }
...
D(bug("[%s]: CmdGetTypeStats()\n", unit->e1ku_name));
    ...
    tracker = FindTypeStats(LIBBASE, unit, &unit->e1ku_type_trackers, packet_type);
    opener = request->ios2_BufferManagement;
    initial_stats = FindTypeStats(LIBBASE, unit, &opener->initial_stats, packet_type);
...
D(bug("[%s]: CmdOnEvent()\n", unit->e1ku_name));
    ...
    {
        request->ios2_Req.io_Flags &= ~IOF_QUICK;
        PutMsg(unit->e1ku_request_ports[EVENT_QUEUE], (APTR)request);
    }
...
D(bug("[%s]: CmdReadOrphan()\n", unit->e1ku_name));
    ...
    {
        request->ios2_Req.io_Flags &= ~IOF_QUICK;
        PutMsg(unit->e1ku_request_ports[ADOPT_QUEUE], (struct Message *)request);
    }
    else
    {
        request->ios2_Req.io_Error = error;
...
D(bug("[%s]: CmdOnline()\n", unit->e1ku_name));
    ...
    if((error == 0) && ((unit->e1ku_ifflags & IFF_UP) == 0))
    {
        unit->e1ku_stats.PacketsReceived = 0;
        unit->e1ku_stats.PacketsSent = 0;
        unit->e1ku_stats.BadData = 0;
        unit->e1ku_stats.Overruns = 0;
