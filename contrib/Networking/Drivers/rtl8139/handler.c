...
D(bug("%s: S2CmdWrite()\n", unit->rtl8139u_name));
    ...
    if(error == 0) {
        request->ios2_Req.io_Flags &= ~IOF_QUICK;
        PutMsg(unit->rtl8139u_request_ports[WRITE_QUEUE], (APTR)request);
    }
    else
    {
...
D(bug("%s: S2CmdConfigInterface()\n", unit->rtl8139u_name));
    ...
    {
        CopyMem(request->ios2_SrcAddr, unit->rtl8139u_dev_addr, ETH_ADDRESSSIZE);
        unit->set_mac_address(unit);
        unit->rtl8139u_flags |= IFF_CONFIGURED;
    }
    else
    {
...
D(bug("%s: S2CmdTrackType()\n", unit->rtl8139u_name));
    ...
    else
    {
        ...
        if(tracker != NULL)
        {
            ...
            Disable();
            AddTail((APTR)&unit->rtl8139u_type_trackers, (APTR)tracker);
            Enable();
        }
   }
...
D(bug("%s: S2CmdOnEvent()\n", unit->rtl8139u_name));
    ...
    {
        request->ios2_Req.io_Flags &= ~IOF_QUICK;
        PutMsg(unit->rtl8139u_request_ports[EVENT_QUEUE], (APTR)request);
    }
...
D(bug("%s: S2CmdReadOrphan()\n", unit->rtl8139u_name));
    ...
    {
        request->ios2_Req.io_Flags &= ~IOF_QUICK;
        PutMsg(unit->rtl8139u_request_ports[ADOPT_QUEUE], (struct Message *)request);
    }
    else
    {
        request->ios2_Req.io_Error = error;
...
D(bug("%s: S2CmdOnline()\n", unit->rtl8139u_name));
    ...
    if((error == 0) && ((unit->rtl8139u_flags & IFF_UP) == 0))
    {
        unit->rtl8139u_stats.PacketsReceived = 0;
        unit->rtl8139u_stats.PacketsSent = 0;
        unit->rtl8139u_stats.BadData = 0;
        unit->rtl8139u_stats.Overruns = 0;
        unit->rtl8139u_stats.UnknownTypesReceived = 0;
        unit->rtl8139u_stats.Reconfigurations = 0;
