/*
 * $Id: unit.c 24814 2006-09-16 22:08:33Z neil $
 */
 ...
 * filter function).
 */
AROS_UFH3(void, VIARHINE_RX_IntF,
    AROS_UFHA(struct VIARHINEUnit *,  unit, A1),
 ...
 */
AROS_UFH3(void, VIARHINE_TX_IntF,
    AROS_UFHA(struct VIARHINEUnit *,  unit, A1),
    AROS_UFHA(APTR,             dummy, A5),
    AROS_UFHA(struct ExecBase *,SysBase, A6))
{
 ...
 */
static void VIARHINE_TimeoutHandlerF(HIDDT_IRQ_Handler *irq, HIDDT_IRQ_HwInfo *hw)
{
...
D(bug("%s CreateUnit:   NIC Private Data Area @ %x, start @ %x\n", unit->rhineu_name, unit->rhineu_fe_privbase, unit->rhineu_fe_priv));
                ...
                unit->rhineu_irqhandler = AllocMem(sizeof(HIDDT_IRQ_Handler), MEMF_PUBLIC|MEMF_CLEAR);
                unit->rhineu_touthandler = AllocMem(sizeof(HIDDT_IRQ_Handler), MEMF_PUBLIC|MEMF_CLEAR);

                if (unit->rhineu_irqhandler && unit->rhineu_touthandler)
                {
                    struct Message *msg;

                    unit->rhineu_irqhandler->h_Node.ln_Pri = 100;
                    unit->rhineu_irqhandler->h_Node.ln_Name = LIBBASE->rhineb_Device.dd_Library.lib_Node.ln_Name;
                    unit->rhineu_irqhandler->h_Code = VIARHINE_IntHandlerF;
                    unit->rhineu_irqhandler->h_Data = unit;

                    unit->rhineu_touthandler->h_Node.ln_Pri = 100;
                    unit->rhineu_touthandler->h_Node.ln_Name = LIBBASE->rhineb_Device.dd_Library.lib_Node.ln_Name;
                    unit->rhineu_touthandler->h_Code = VIARHINE_TimeoutHandlerF;
                    unit->rhineu_touthandler->h_Data = unit;
...
void DeleteUnit(struct VIARHINEBase *VIARHINEDeviceBase, struct VIARHINEUnit *Unit)
{
    ...
    if (Unit)
    {
        ...
        if (Unit->rhineu_irqhandler)
        {
            FreeMem(Unit->rhineu_irqhandler, sizeof(HIDDT_IRQ_Handler));
            LIBBASE->rhineb_irq = NULL;
        }
