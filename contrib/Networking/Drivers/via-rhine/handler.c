/*
 * $Id: handler.c 24063 2006-02-07 23:00:07Z neil $
 */
...
void handle_request(LIBBASETYPEPTR LIBBASE, struct IOSana2Req *request)
{
    ...
    ReleaseSemaphore(&((struct VIARHINEUnit *)request->ios2_Req.io_Unit)->rhineu_unit_lock);
}
...
D(bug("%s: S2CmdWrite()\n", unit->rhineu_name));
    ...
    if(error == 0) {
        request->ios2_Req.io_Flags &= ~IOF_QUICK;
        PutMsg(unit->rhineu_request_ports[WRITE_QUEUE], (APTR)request);
    }
    else
    {
...
D(bug("%s: S2CmdDeviceQuery()\n", unit->rhineu_name));
    ...
    CopyMem(&LIBBASE->rhineb_Sana2Info, info, size);
...
D(bug("%s: S2CmdGetStationAddress()\n", unit->rhineu_name));
    ...
    CopyMem(unit->rhineu_dev_addr, request->ios2_SrcAddr, ETH_ADDRESSSIZE);
    CopyMem(unit->rhineu_org_addr, request->ios2_DstAddr, ETH_ADDRESSSIZE);
...
D(bug("%s: S2CmdConfigInterface()\n", unit->rhineu_name));
    ...
    if((unit->rhineu_flags & IFF_CONFIGURED) == 0)
    {
        CopyMem(request->ios2_SrcAddr, unit->rhineu_dev_addr, ETH_ADDRESSSIZE);
        unit->set_mac_address(unit);
        unit->rhineu_flags |= IFF_CONFIGURED;
    }
    else
    {
...
D(bug("%s: S2CmdTrackType()\n", unit->rhineu_name));
    ...
    else
    {
        ...
        if(tracker != NULL)
        {
            ...
            Disable();
            AddTail((APTR)&unit->rhineu_type_trackers, (APTR)tracker);
            Enable();
        }
   }
...
D(bug("%s: S2CmdGetTypeStats()\n", unit->rhineu_name));
    ...
    tracker = FindTypeStats(LIBBASE, unit, &unit->rhineu_type_trackers, packet_type);
    opener = request->ios2_BufferManagement;
    initial_stats = FindTypeStats(LIBBASE, unit, &opener->initial_stats, packet_type);
...
D(bug("%s: S2CmdGetGlobalStats()\n", unit->rhineu_name));
    ...
    CopyMem(&unit->rhineu_stats, request->ios2_StatData,
...
D(bug("%s: S2CmdOnEvent()\n", unit->rhineu_name));
    ...
    {
        request->ios2_Req.io_Flags &= ~IOF_QUICK;
        PutMsg(unit->rhineu_request_ports[EVENT_QUEUE], (APTR)request);
    }
...
D(bug("%s: S2CmdReadOrphan()\n", unit->rhineu_name));
    ...
    {
        request->ios2_Req.io_Flags &= ~IOF_QUICK;
        PutMsg(unit->rhineu_request_ports[ADOPT_QUEUE], (struct Message *)request);
    }
    else
    {
        request->ios2_Req.io_Error = error;
...
static BOOL CmdOnline(LIBBASETYPEPTR LIBBASE, struct IOSana2Req *request)
{
    struct VIARHINEUnit *unit = (struct VIARHINEUnit *)request->ios2_Req.io_Unit;
    BYTE error = 0;
    ULONG wire_error = 0;
    UWORD i;
...
D(bug("%s: S2CmdOnline()\n", unit->rhineu_name));
    ...
    if((error == 0) && ((unit->rhineu_flags & IFF_UP) == 0))
    {
        unit->rhineu_stats.PacketsReceived = 0;
        unit->rhineu_stats.PacketsSent = 0;
        unit->rhineu_stats.BadData = 0;
        unit->rhineu_stats.Overruns = 0;
        unit->rhineu_stats.UnknownTypesReceived = 0;
        unit->rhineu_stats.Reconfigurations = 0;
