...
void handle_request(LIBBASETYPEPTR LIBBASE, struct IOSana2Req *request)
{
    ...
    ReleaseSemaphore(&((struct PCN32Unit *)request->ios2_Req.io_Unit)->pcnu_unit_lock);
}
...
static BOOL CmdS2DeviceQuery(LIBBASETYPEPTR LIBBASE, struct IOSana2Req *request)
{
    struct PCN32Unit *unit = (APTR)request->ios2_Req.io_Unit;
...
D(bug("%s: S2CmdDeviceQuery()\n", unit->pcnu_name));
    ...
    CopyMem(&LIBBASE->pcnb_Sana2Info, info, size);
...
D(bug("%s: S2CmdGetStationAddress()\n", unit->pcnu_name));
    ...
    CopyMem(unit->pcnu_dev_addr, request->ios2_SrcAddr, ETH_ADDRESSSIZE);
    CopyMem(unit->pcnu_org_addr, request->ios2_DstAddr, ETH_ADDRESSSIZE);
...
D(bug("%s: S2CmdConfigInterface()\n", unit->pcnu_name));
    ...
    if((unit->pcnu_flags & IFF_CONFIGURED) == 0)
    {
        CopyMem(request->ios2_SrcAddr, unit->pcnu_dev_addr, ETH_ADDRESSSIZE);
        unit->set_mac_address(unit);
        unit->pcnu_flags |= IFF_CONFIGURED;
    }
    else
    {
...
D(bug("%s: S2CmdTrackType()\n", unit->pcnu_name));
    ...
    else
    {
        ...
        if(tracker != NULL)
        {
            ...
            Disable();
            AddTail((APTR)&unit->pcnu_type_trackers, (APTR)tracker);
            Enable();
        }
   }
...
D(bug("%s: S2CmdGetTypeStats()\n", unit->pcnu_name));
    ...
    tracker = FindTypeStats(LIBBASE, unit, &unit->pcnu_type_trackers, packet_type);
    opener = request->ios2_BufferManagement;
    initial_stats = FindTypeStats(LIBBASE, unit, &opener->initial_stats, packet_type);
...
D(bug("%s: S2CmdGetGlobalStats()\n", unit->pcnu_name));
    ...
    CopyMem(&unit->pcnu_stats, request->ios2_StatData,
...
D(bug("%s: S2CmdOnEvent()\n", unit->pcnu_name));
    ...
    {
        request->ios2_Req.io_Flags &= ~IOF_QUICK;
        PutMsg(unit->pcnu_request_ports[EVENT_QUEUE], (APTR)request);
    }
...
D(bug("%s: S2CmdReadOrphan()\n", unit->pcnu_name));
    ...
    {
        request->ios2_Req.io_Flags &= ~IOF_QUICK;
        PutMsg(unit->pcnu_request_ports[ADOPT_QUEUE], (struct Message *)request);
    }
    else
    {
        request->ios2_Req.io_Error = error;
...
static BOOL CmdOnline(LIBBASETYPEPTR LIBBASE, struct IOSana2Req *request)
{
    struct PCN32Unit *unit = (struct PCN32Unit *)request->ios2_Req.io_Unit;
    BYTE error = 0;
    ULONG wire_error = 0;
    UWORD i;
...
D(bug("%s: S2CmdOnline()\n", unit->pcnu_name));
    ...
    if((error == 0) && ((unit->pcnu_flags & IFF_UP) == 0))
    {
        unit->pcnu_stats.PacketsReceived = 0;
        unit->pcnu_stats.PacketsSent = 0;
        unit->pcnu_stats.BadData = 0;
        unit->pcnu_stats.Overruns = 0;
        unit->pcnu_stats.UnknownTypesReceived = 0;
        unit->pcnu_stats.Reconfigurations = 0;

        for(i = 0; i < STAT_COUNT; i++)
            unit->pcnu_special_stats[i] = 0;
