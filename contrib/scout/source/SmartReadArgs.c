 ...
 *
 * $VER: SmartReadArgs.c 1.6 (7.9.98)
 *
...
LONG SmartReadArgs(struct WBStartup * wb_startup, struct SmartArgs * args)
{
   ...
   // ReadArgs() scheint bei fehlerfreier Arbeit IoErr() nicht neu zu setzen, darum machen wir das hier selbst
   SetIoErr(0);
   ...
   while ((current_word[0] != '\0') && (!found))
   {
      ...
      else
      {
         if ((name_length == current_word_length)
             && !Strnicmp(name, current_word, name_length))
         {
            found = TRUE;
         }
      }
