...
struct IntegrityCk {
	...
	STRPTR zErrMsg; /* An error message.  NULL of no errors seen. */
};
		...
		)
{
	...
	if( pCheck->zErrMsg )
	{
		STRPTR zOld = pCheck->zErrMsg;
		pCheck->zErrMsg = NULL;
		sqlite3SetString(&pCheck->zErrMsg, zOld, "\n", zMsg1, zMsg2, NULL);
		sqliteFree(zOld);
	}
	else sqlite3SetString(&pCheck->zErrMsg, zMsg1, zMsg2, NULL);
	sqliteFree(zMsg2);
}
