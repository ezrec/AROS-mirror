...
void sqlite3CreateIndex(
		...
		Token *pStart,     /* The CREATE token that begins a CREATE TABLE
							  statement */
		...
		)
{
	...
	{
		/*
		 * Use the two-part index name to determine the database to search for
		 * the table. 'Fix' the table name to this db before looking up the
		 * table.
		 * */
		...
		/*
		 * If the index name was unqualified, check if the the table is a temp
		 * table. If so, set the database to 1.
		 * */
		pTab = sqlite3SrcListLookup(pParse, pTblName);
		if( pName2 && !pName2->n && pTab && pTab->iDb == 1 )
			iDb = 1;
#endif
	...
	/*
	 * Find the name of the index. Make sure there is not already another index
	 ...
	 *
	 * If pName==0 it means that we are dealing with a primary key or UNIQUE
	 * constraint. We have to invent our own name.
	 * */
	if( pName )
	{
		zName = sqlite3NameFromToken(pName);
		...
		if( !db->init.busy )
		{
			...
			if( SQLITE_OK!=sqlite3ReadSchema(pParse) ) goto exit_create_index;
			if( (pISameName = sqlite3FindIndex(db, zName, db->aDb[iDb].zName)))
			{
	...
	else
	{
		...
		sprintf(zBuf,"_%d",n);
		zName = NULL;
		sqlite3SetString(&zName, "sqlite_autoindex_", pTab->zName, zBuf, NULL);
		if( !zName ) goto exit_create_index;
	}
	...
	/*
	 * If pList==0, it means this routine was called to make a primary key out
	 * of the last column added to the table under construction. So create
	 * a fake list to simulate this.
	 * */
	if( !pList )
	{
		...
		if( !pList ) goto exit_create_index;
	}
	...
	pIndex->zName = (STRPTR)&pIndex->aiColumn[pList->nExpr];
	...
	/*
	 * Scan the names of the columns of the table to be indexed and load the
	 * column indices into the Index structure. Report an error if any column
	 * is not found.
	 * */
	for(i=0; i<pList->nExpr; i++)
	{
		for(j=0; j<pTab->nCol; j++)
			if( sqlite3StrICmp(pList->a[i].zName, pTab->aCol[j].zName)==0 )
				break;
		if( j>=pTab->nCol )
		{
		...
		else pIndex->keyInfo.aColl[i] = pTab->aCol[j].pColl;
		...
		for(pIdx=pTab->pIndex; pIdx; pIdx=pIdx->pNext)
		{
			...
			if( k==pIdx->nColumn )
			{
				if( pIdx->onError!=pIndex->onError )
				{
					 ...
					 * constraint specified somewhere in the CREATE TABLE
					 * statement. However the ON CONFLICT clauses are
					 * different. If both this constraint and the previous
					 * equivalent constraint have explicit ON CONFLICT clauses
					 * this is an error. Otherwise, use the explicitly
					 * specified behaviour for the index.
					 * */
	...
	/*
	 * Link the new Index structure to its table and to the other in-memory
	 * database structures.
	 * */
	if( db->init.busy )
	{
	...
	/*
	 * If the db->init.busy is 0 then create the index on disk. This involves
	 * writing the index into the master table and filling in the index with
	 * the current table contents.
	 *
	 * The db->init.busy is 0 when the user first enters a CREATE INDEX
	 * command. db->init.busy is 1 when a database is opened and CREATE INDEX
	 * statements are read out of the master table. In the latter case the
	 * index already exists on disk, which is why we don't want to recreate it.
	 * 
	 * If pTblName==0 it means this index is generated as a primary key or
	 * UNIQUE constraint of a CREATE TABLE statement. Since the table has just
	 * been created, it contains no data and the index initialization step can
	 * be skipped.
	 * */
	else if( db->init.busy==0 )
	{
		...
		/*
		 * Gather the complete text of the CREATE INDEX statement into the
		 * zStmt variable.
		 * */
		if( pStart && pEnd )
		...
		{
			/* An automatic index created by a PRIMARY KEY or UNIQUE
			 * constraint */
	...
	/*
	 * When adding an index to the list of indices for a table, make sure all
	 * indices labeled OE_Replace come after all those labeled OE_Ignore. This
	 * is necessary for the correct operation of UPDATE and INSERT.
	 * */
...
/*
 * This routine will drop an existing named index. This routine implements the
