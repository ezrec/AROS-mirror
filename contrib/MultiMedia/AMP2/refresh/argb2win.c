...
void argb2win_gray(rect_type *rect, unsigned char *src[])
{
  ...
  for (j=0; j<rect->yloop; j++) {
    ...
    for (i=0; i<(rect->xloop >> 2); i++) {
      val = *argb++;
      gray = pixel[RGB2GRAY(((val>>16)&0xff), ((val>>8)&0xff), (val&0xff))] << 24;
      val = *argb++;
      gray |= pixel[RGB2GRAY(((val>>16)&0xff), ((val>>8)&0xff), (val&0xff))] << 16;
      val = *argb++;
      gray |= pixel[RGB2GRAY(((val>>16)&0xff), ((val>>8)&0xff), (val&0xff))] << 8;
      val = *argb++;
      gray |= pixel[RGB2GRAY(((val>>16)&0xff), ((val>>8)&0xff), (val&0xff))];
      *dst++ = gray;
    }
    add += row_add;
    dst += rect->dst_row_length - (rect->xloop >> 2);
