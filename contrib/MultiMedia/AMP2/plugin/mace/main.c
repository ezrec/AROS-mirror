...
#define FOURCC_MAC3 FOURCC( 'M', 'A', 'C', '3')
#define FOURCC_MAC6 FOURCC( 'M', 'A', 'C', '6')

static void (*DecodeMACE)(void *src, void *dst, unsigned long size);
static unsigned char *audio = NULL;

static unsigned short MACEtab1[] =
{ 0xfff3, 0x0008, 0x004c, 0x00de, 0x00de, 0x004c, 0x0008, 0xfff3 };

static unsigned short MACEtab3[] =
{ 0xffee, 0x008c, 0x008c, 0xffee };

static unsigned short MACEtab2[][8] = {
...
static unsigned short MACEtab4[][8] = {
...
void Exp1to3(const unsigned char *inBuffer, void *outBuffer, unsigned long cnt, unsigned long numChannels, unsigned long whichChannel)
{
   ...
   while (cnt>0) {
     pkt=inBuffer[0];
     chomp3( pkt       & 7, MACEtab1, MACEtab2, numChannels);
     chomp3((pkt >> 3) & 3, MACEtab3, MACEtab4, numChannels);
     chomp3( pkt >> 5     , MACEtab1, MACEtab2, numChannels);
     pkt=inBuffer[1];
     chomp3( pkt       & 7, MACEtab1, MACEtab2, numChannels);
     chomp3((pkt >> 3) & 3, MACEtab3, MACEtab4, numChannels);
     chomp3( pkt >> 5     , MACEtab1, MACEtab2, numChannels);
...
void chomp6(unsigned char val, unsigned short tab1[], unsigned short tab2[][8], unsigned long numChannels)
{
  short current;
  ...
  if ((previous^current)>=0) {
    ...
    if (factor-314<-32768) factor=-32767;
    else factor-=314;
  }

  if (current+level>32767) current=32767;
  else if (current+level<-32768) current=-32767;
  else current+=level;

  level=((current*factor) >> 15);
  current>>=1;

  *outPtr=(previous+prev2-((prev2-current) >> 2)) >> 8;
  outPtr += numChannels; /* FIXME */
  *outPtr=(previous+current+((prev2-current) >> 2)) >> 8;
  outPtr += numChannels; /* FIXME */
...
void Exp1to6(unsigned char *inBuffer, void *outBuffer, unsigned long cnt, unsigned long numChannels, unsigned long whichChannel)
{
   ...
   while (cnt>0) {
     ...
     chomp6( pkt >> 5     , MACEtab1, MACEtab2, numChannels);
     chomp6((pkt >> 3) & 3, MACEtab3, MACEtab4, numChannels);
     chomp6( pkt       & 7, MACEtab1, MACEtab2, numChannels);
...
void DecodeMACE3Mono(void *src, void *dst, unsigned long size)
{
  Exp1to3(src, dst, size/2, 1, 1);
}

void DecodeMACE3Stereo(void *src, void *dst, unsigned long size)
{
  Exp1to3(src, (unsigned char *)dst + 0, size/2, 2, 1); /* Left */
  Exp1to3(src, (unsigned char *)dst + 1, size/2, 2, 2); /* Right */
}

void DecodeMACE6Mono(void *src, void *dst, unsigned long size)
{
  Exp1to6(src, dst, size, 1, 1);
}

void DecodeMACE6Stereo(void *src, void *dst, unsigned long size)
{
  Exp1to6(src, (unsigned char *)dst + 0, size, 2, 1); /* Left */
  Exp1to6(src, (unsigned char *)dst + 1, size, 2, 2); /* Right */
}

static int mace_error = 0;

static double current_time = 0.0;
static int bytes_per_second = 0;
static int bufsize = 0;

static int compress_factor = 0;

int decode_mace(unsigned char *buf, unsigned long length, double sync_time)
{
  if (mace_error != 0) {
    return PLUGIN_FAIL;
  }
  ...
  if (length > 0) {
    ...
    if ((length * compress_factor) > bufsize) {
      bufsize = (length * compress_factor);
      free(audio);
    ...
    DecodeMACE(buf, audio, length);

    if (sync_time > 0.0) {
      current_time = sync_time;
    }

    amp->audio_refresh(audio, length * compress_factor, current_time);

    current_time += (double)(length * compress_factor) / (double)bytes_per_second;
  }

  return PLUGIN_OK;
}

int init_mace(unsigned long type, unsigned long fourcc)
{
  int bits, channels;
  ...
  if (fourcc == FOURCC_MAC3) {
    compress_factor = 3;
    if (channels == 2) {
      bufsize *= 2;
      DecodeMACE = DecodeMACE3Stereo;
    } else {
      DecodeMACE = DecodeMACE3Mono;
      bufsize *= 1;
    }
  } else {
    compress_factor = 6;
    if (channels == 2) {
      bufsize *= 2;
      DecodeMACE = DecodeMACE6Stereo;
    } else {
      DecodeMACE = DecodeMACE6Mono;
      bufsize *= 1;
    }
  }
...
VERSION("MACE v1.00 (020216)");

IDENT_INIT
IDENT_ADD((TYPE_AUDIO | SUBTYPE_NONE | 0x0003), FOURCC_MAC3, init_mace, decode_mace, exit_mace)
IDENT_ADD((TYPE_AUDIO | SUBTYPE_NONE | 0x0006), FOURCC_MAC6, init_mace, decode_mace, exit_mace)
IDENT_EXIT
