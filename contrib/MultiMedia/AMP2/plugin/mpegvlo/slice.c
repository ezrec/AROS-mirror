...
extern void idct_block_copy (int16_t * block, uint8_t * dest, int stride, int full);
extern void idct_block_add (int16_t * block, uint8_t * dest, int stride, int full);

#include "vlc.h"

static int non_linear_quantizer_scale [] ATTR_ALIGN(32) = {
     0,  1,  2,  3,  4,  5,   6,   7,
...
#define bit_ptr (picture->bitstream_ptr)
    ...
    idct_block_copy (picture->DCTblock, dest, stride, picture->full);
#undef bit_buf
#undef bits
#undef bit_ptr
}
                         ...
                         int stride)
{
    ...
    idct_block_add (picture->DCTblock, dest, stride, picture->full);
}

static inline void slice_intra_DCT_fake (picture_t * picture, int cc)
{
#define bit_buf (picture->bitstream_buf)
#define bits (picture->bitstream_bits)  
#define bit_ptr (picture->bitstream_ptr)
...
static inline void slice_non_intra_DCT_fake (picture_t * picture)
{
    if (picture->mpeg1)
     get_mpeg1_non_intra_block (picture);
    else
...
static inline void motion_block (picture_t * picture,
...
/* LORES */
    ...
    src1 = src[0] + src_offset + (x_pred >> 1) + (y_pred >> 1) * stride +
     second_half * (stride << 2);

    table[xy_half] (dest[0] + dest_offset + second_half * (stride << 2),
              src1, stride, height);
    ...
    if (!picture->gray) {
     ...
     stride >>= 1;
     height >>= 1;
     src_offset >>= 1;    src_offset += second_half * (stride << 1);
     dest_offset >>= 1;   dest_offset += second_half * (stride << 1);
...
#define bit_ptr (picture->bitstream_ptr)
    ...
    motion_block (picture, table, motion_x, motion_y, dest, offset,
...
/* LORES */
    motion_block (picture, table, motion_x, motion_y << 1, dest, offset,
            motion->ref[0], offset, stride, 16, 0);
...
#define bit_ptr (picture->bitstream_ptr)
    ...
    motion_block (picture, mc_functions.put, motion_x, motion_y, dest, offset,
            motion->ref[0], offset, stride * 2, 8, 0);

    m = picture->top_field_first ? 1 : 3;
    other_x = ((motion_x * m + (motion_x > 0)) >> 1) + dmv_x;
    other_y = ((motion_y * m + (motion_y > 0)) >> 1) + dmv_y - 1;
    motion_block (picture, mc_functions.avg, other_x, other_y, dest, offset,
            motion->ref[0], offset + stride, stride * 2, 8, 0);

    motion_block (picture, mc_functions.put, motion_x, motion_y, dest, offset + stride,
            motion->ref[0], offset + stride, stride * 2, 8, 0);

    m = picture->top_field_first ? 3 : 1;
    other_x = ((motion_x * m + (motion_x > 0)) >> 1) + dmv_x;
    other_y = ((motion_y * m + (motion_y > 0)) >> 1) + dmv_y + 1;
    motion_block (picture, mc_functions.avg, other_x, other_y, dest, offset + stride,
            motion->ref[0], offset, stride * 2, 8, 0);
#undef bit_buf
#undef bits
#undef bit_ptr
...
static void motion_fr_reuse (picture_t * picture, motion_t * motion,
                    ...
                    void (** table) (uint8_t *, uint8_t *, int, int))
{
    motion_block (picture, table, motion->pmv[0][0], motion->pmv[0][1], dest, offset,
            motion->ref[0], offset, stride, 16, 0);
}
...
static void motion_fr_zero (picture_t * picture, motion_t * motion,
                   ...
                   void (** table) (uint8_t *, uint8_t *, int, int))
{
    motion_block (picture, table, 0, 0, dest, offset,
            motion->ref[0], offset, stride, 16, 0);
}
...
#define bit_ptr (picture->bitstream_ptr)
    ...
    motion_block (picture, mc_functions.avg, motion_x, motion_y, dest, offset,
            motion->ref[!picture->current_field], offset, stride, 16, 0);
#undef bit_buf
#undef bits
#undef bit_ptr
...
do {                                         \
    if (offset == picture->coded_picture_width) {           \
     ...
     if (vertical_offset >= picture->coded_picture_height)       \
         return;                                \
     picture->f_motion.ref[0][0] += 8 * stride;            \
     picture->f_motion.ref[0][1] += 2 * stride;             \
     picture->f_motion.ref[0][2] += 2 * stride;             \
     picture->b_motion.ref[0][0] += 8 * stride;            \
     picture->b_motion.ref[0][1] += 2 * stride;             \
     picture->b_motion.ref[0][2] += 2 * stride;             \
     offset = 0;                                  \
    }                                             \
} while (0)
...
#define bit_ptr (picture->bitstream_ptr)
    ...
    offset = (code - 1) * stride * 2;
    vertical_offset = (code - 1) * 8;
    ...
    NEEDBITS (bit_buf, bits, bit_ptr);
    offset = get_macroblock_address_increment (picture) << 3;
...
/* LORES */
         ...
         slice_intra_DCT (picture, 0, dest[0] + offset + 4, DCT_stride);

         if (macroblock_modes & DCT_TYPE_INTERLACED) {
          slice_intra_DCT_fake (picture, 0);
          slice_intra_DCT_fake (picture, 0);
         } else {
          slice_intra_DCT (picture, 0, dest[0] + offset + DCT_offset,
                     DCT_stride);
          slice_intra_DCT (picture, 0, dest[0] + offset + DCT_offset + 4,
                     DCT_stride);
         }

         /* Decode chroma blocks */
         if (picture->gray) {
          slice_intra_DCT_fake (picture, 1);
          slice_intra_DCT_fake (picture, 2);
         } else {
...
/* LORES */
          ...
          if (coded_block_pattern & 0x10)
              slice_non_intra_DCT (picture, dest[0] + offset + 4,
                          DCT_stride);

          if (macroblock_modes & DCT_TYPE_INTERLACED) {
           if (coded_block_pattern & 0x08)
               slice_non_intra_DCT_fake (picture);
