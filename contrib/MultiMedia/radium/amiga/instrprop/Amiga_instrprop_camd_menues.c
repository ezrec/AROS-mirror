...
int CPPWindowResetAllControllers( void )
{
	...
	for(channel=0;channel<16;channel++){
		R_PutMidi3(mymidilink->midilink,0xb0|channel,121,0);
...
int CPPWindowLocalKeyboardOn( void )
{
	...
	for(channel=0;channel<16;channel++){
		R_PutMidi3(mymidilink->midilink,0xb0|channel,122,127);
	}
...
int CPPWindowLocalKeyboardOff( void )
{
	...
	for(channel=0;channel<16;channel++){
		R_PutMidi3(mymidilink->midilink,0xb0|channel,122,0);
	}
...
int CPPWindowAllNotesOff( void )
{
	...
	for(channel=0;channel<16;channel++){
		R_PutMidi3(mymidilink->midilink,0xb0|channel,123,0);
	}
...
int CPPWindowAllSoundsOff( void )
{
	...
	for(channel=0;channel<16;channel++){
		R_PutMidi3(mymidilink->midilink,0xb0|channel,120,0);
	}
