...
#ifdef __AROS__
struct TextAttr fontstruct={"ttcourier.font",17,0,0};
#else
...
int GFX_CreateVisual(struct Tracker_Windows *tvisual){
	...
	if ( ! ( window = OpenWindowTags( NULL,
				...
				WA_IDCMP,	IDCMP_NEWSIZE|IDCMP_MOUSEMOVE|IDCMP_MOUSEBUTTONS|IDCMP_REFRESHWINDOW|IDCMP_RAWKEY|IDCMP_CHANGEWINDOW,
				WA_Flags,	WFLG_REPORTMOUSE|WFLG_ACTIVATE|WFLG_DRAGBAR|WFLG_DEPTHGADGET|WFLG_SMART_REFRESH|WFLG_RMBTRAP,
				WA_Title,	"",
...
//		NULL
	...
	if(bitmap==NULL){
		fprintf(stderr,"Not enough memory cursor bitmap\n");
		return 20;
	}
...
char *GFX_GetLoadFileName(
	...
	name=talloc_atomic(strlen(requester->fr_File)+1+strlen(requester->fr_Drawer)+1);
	sprintf(
		name,
		"%s%s%s",
		requester->fr_Drawer,
		requester->fr_Drawer[strlen(requester->fr_Drawer)-1]==':'?"":"/",
		requester->fr_File
	);
	FreeAslRequest(requester);
