...
/******************************************************************
  ...
  and must be put into their respective directories. The other
  OS_*.h files are not.
******************************************************************/
...
/* Unfortunately, AmigaOS has one absolute address that is legal to
   read from; 4, which often fools MuForce to not report a faulty 
   address. By inserting SDB at all 4 offsets, we
   never read from 4 if a pointer is NULL.
*/
#ifdef SYSBASEDEBUG
