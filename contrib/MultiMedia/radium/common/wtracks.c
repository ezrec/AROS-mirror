...
void UpdateWTrackCoordinates(
	...
	wtrack->notearea.x2 = wtrack->notearea.x  + wtrack->notesonoff*((window->fontwidth*wtrack->notelength));
...
void SwapTrack_CurrPos(
	...
	temp=NULL;		//Help the GC a bit. (next five operations are quite heavy) ("stupid" optimizers eliminates this one)
