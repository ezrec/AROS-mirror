...
typedef struct SignalSemaphore CRITICAL_SECTION;
#define InitializeCriticalSection(x) InitSemaphore(x)
#define DeleteCriticalSection(x) 
#define EnterCriticalSection(x) ObtainSemaphore(x)
#define LeaveCriticalSection(x) ReleaseSemaphore(x)
