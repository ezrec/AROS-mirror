...
struct xadMasterBase {
  ...
  xadSTRPTR            xmb_DefaultName; /* name for XADFIF_NOFILENAME (V6) */
#endif
};
...
struct xadForeman {
  xadUINT32          xfm_Security;    /* should be XADFOREMAN_SECURITY */
...
struct xadClient {
  ...
  xadBOOL         (* xc_RecogData)();
  xadERROR        (* xc_GetInfo)();
  xadERROR        (* xc_UnArchive)();
  void            (* xc_Free)();
};
...
ASM(xadERROR) xc_GetInfo(REG(a0, struct xadArchiveInfo *ai),
                REG(a6, struct xadMasterBase *xadMasterBase));
ASM(xadERROR) xc_UnArchive(REG(a0, struct xadArchiveInfo *ai),
                REG(a6, struct xadMasterBase *xadMasterBase));
