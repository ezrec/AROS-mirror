...
{
  "GUI",
  "Main Lister Archive Types",
  "Misc",
  "Archive Types File Types",
  "File Types",
  NULL
};

static APTR WIN_3,WIN_4;
...
#ifdef __SASC
SAVEDS ASM LONG MultiTestFunc(REG(a0) struct Hook *hook, REG(a2) Object *obj, REG(a1) char *entry)
{
#endif
#ifdef __GNUC__
static LONG MultiTestFunc(void)
{ register Object *a2 __asm("a2");                      Object *obj = a2;
  register char *a1 __asm("a1");                        char *entry = a1;
  register struct Hook *a0 __asm("a0");                 struct Hook *hook = a0;
#endif
/*
  if (entry && (entry[0] == '\033') && (entry[1] == 'c') && (entry[2] == 'a'))
    return(FALSE);
  else
*/
    return(TRUE);
...
{ register Object *a2 __asm("a2");                      Object *obj = a2;
  ...
  { ndm->preparses[0]  = "\033c";
    if      (ndm->entry_pos % 20 == 3)
      ndm->strings[0] = "\033o[0]";
    else if (ndm->entry_pos % 20 == 13)
      ndm->strings[0] = "\033o[1]";
    else
    { sprintf(buf,"%ld",ndm->entry_pos);
      ndm->strings[0]  = buf;
    }

    sprintf(buf2,"%lx",ndm->entry);
    ndm->preparses[1]  = "\033c";
    ndm->strings[1]  = buf2;
    if (ndm->entry_pos & 8)
      ndm->preparses[2]  = "\033c";
    else
      ndm->preparses[2]  = "\033r";
    if (ndm->entry_pos & 2)
      ndm->strings[2]  = buf2;
    else
      ndm->strings[2]  = buf;
    ndm->strings[3]  = (char *) ndm->entry;
  }
  else
  {
    ndm->preparses[0]  = "\033c";
    ndm->preparses[1]  = "\033c";
    ndm->preparses[2]  = "\033c";
    ndm->preparses[3]  = "\033c";
    ndm->strings[0]  = "Num N";
    ndm->strings[1]  = "Adr A";
    ndm->strings[2]  = "Tst";
    ndm->strings[3]  = "This is the list title !\033n\033b   :-)";
  }
}
...
*/
  if (ncm->sort_type & MUIV_NList_TitleMark_TypeMask)
    return ((LONG) stricmp(ncm->entry2,ncm->entry1));
  else
    return ((LONG) stricmp(ncm->entry1,ncm->entry2));
}
...
static void EntriesFunc(void)
{
  register long *a1 __asm("a1");    long *val = a1;
  LONG entries = (LONG) (val[0]);
...
{
  register Object **a1 __asm("a1");    Object **val = a1;
  Object *list = (Object *) (val[0]);

  set(list, MUIA_NList_Quiet, TRUE);
  DoMethod(list, MUIM_NList_Clear);
  DoMethod(list, MUIM_NList_Insert,MainTextArray,1,MUIV_NList_Insert_Bottom);
...
int main(int argc,char *argv[])
{
  LONG win_opened;
  LONG result;
  static const struct Hook MultiTestHook = { { NULL,NULL },(VOID *)MultiTestFunc, NULL,NULL };
  static const struct Hook DisplayLI_TextHook = { { NULL,NULL },(VOID *)DisplayLI_TextFunc, NULL,NULL };
  static const struct Hook CompareLI_TextHook = { { NULL,NULL },(VOID *)CompareLI_TextFunc, NULL,NULL };
  static const struct Hook ColumnsHook = { { NULL,NULL },(VOID *)ColumnsFunc, NULL,NULL };
  static const struct Hook EntriesHook = { { NULL,NULL },(VOID *)EntriesFunc, NULL,NULL };
  ...
  APP_Main = ApplicationObject,
    ...
    MUIA_Application_Version    , "$VER: NList-Demo 0.6 (" __DATE__ ")",
    ...
    SubWindow, WIN_Main = WindowObject,
      MUIA_Window_Title, "NList-Demo 1997",
      ...
      WindowContents, VGroup,
        Child, HGroup,
          ...
          Child, BT_Clear = KeyButton("Clear",'c') /*SimpleButtonCycle("Clear")*/,
          Child, BT_Add = KeyButton("Add",'a') /*SimpleButtonCycle("Add")*/,
        ...
        Child, HGroup,
          ...
          Child, BT_Disable = SimpleButtonCycle("Disable"),
          Child, BT_Enable = SimpleButtonCycle("Enable"),
       End,
        Child, HGroup,
          ...
          Child, BT_Sort2 = SimpleButtonCycle("Sort2"),
          Child, BT_Sort3 = SimpleButtonCycle("Sort3"),
        End,
        Child, LV_Text = NListviewObject,
          MUIA_CycleChain, 1,
/*          MUIA_NListview_Vert_ScrollBar, MUIV_NListview_VSB_Left,*/
          MUIA_NListview_NList, LI_Text = NListObject,
            ...
            MUIA_NList_MultiTestHook, &MultiTestHook,
            MUIA_NList_Format, "BAR COL=1 W=-1,BAR COL=0 W=-1,BAR W=-1,BAR",
/*            MUIA_NList_Format, "BAR COL=1 W=-1,BAR COL=0 W=-1,BAR W=-1",*/
...
/*            MUIA_NList_TitleMark, (MUIV_NList_TitleMark_Box|2),*/
    ...
    SubWindow, WIN_2 = WindowObject,
      ...
      WindowContents, VGroup,
        Child, FL_Text = NFloattext(MainTextString2),
    ...
    SubWindow, WIN_3 = WindowObject,
      WindowContents, VGroup,
        Child, HGroup,
          Child, PoppenObject, MUIA_Pendisplay_Spec, "m6", End,
          Child, PoppenObject, MUIA_Pendisplay_Spec, "m5", End,
        End,
        Child, HGroup,
          Child, VGroup,
            Child, PoppenObject, MUIA_Pendisplay_Spec, "m5", End,
          ...
          Child, NListviewObject ,
            MUIA_NListview_NList, NListObject,
              MUIA_NList_Format, "W=-2 MIW=-1 MAW=-1",
              MUIA_NList_Active, 0,
...
/*
              MUIA_Disabled,TRUE,
*/
              MUIA_NList_Title, "Sections",
              MUIA_NList_SourceArray, SAString,
            End,
            MUIA_NListview_Vert_ScrollBar, MUIV_NListview_VSB_Always,
            MUIA_NListview_Horiz_ScrollBar, MUIV_NListview_HSB_None,
          End,
        ...
        Child, HGroup,
          ...
          Child, ListviewObject ,
            MUIA_Listview_List, ListObject,
              InputListFrame,
              MUIA_List_Format, "W=-2 MIW=-1 MAW=-1",
              MUIA_List_Active, 0,
              MUIA_List_AdjustWidth,TRUE,
              MUIA_List_AdjustHeight,TRUE,
              MUIA_List_Title, "Sections",
              MUIA_List_SourceArray, SAString,
            End,
          End,
          Child, VGroup,
            Child, PoppenObject, MUIA_Pendisplay_Spec, "m5", End,
  ...
  DoMethod(LI_Text,MUIM_Notify,MUIA_NList_Columns, MUIV_EveryTime,
    LI_Text, 3, MUIM_CallHook, &ColumnsHook,MUIV_TriggerValue);

  DoMethod(LI_Text,MUIM_Notify,MUIA_NList_Entries, MUIV_EveryTime,
    LI_Text, 3, MUIM_CallHook, &EntriesHook,MUIV_TriggerValue);

  DoMethod(BT_TitleOn, MUIM_Notify, MUIA_Pressed,FALSE,
    LI_Text, 3, MUIM_CallHook, &ButtonHook,LI_Text);
...
*/
  ...
  DoMethod(BT_Add, MUIM_Notify, MUIA_Pressed,FALSE,
    LI_Text, 4, MUIM_NList_Insert,MainTextArray,100,MUIV_NList_Insert_Bottom);
  DoMethod(BT_Add, MUIM_Notify, MUIA_Pressed,FALSE,
    LI_Text, 4, MUIM_NList_Insert,MainTextArray,100,MUIV_NList_Insert_Bottom);
  DoMethod(BT_Add, MUIM_Notify, MUIA_Pressed,FALSE,
    LI_Text, 4, MUIM_NList_Insert,MainTextArray,100,MUIV_NList_Insert_Bottom);
  ...
  DoMethod(BT_Disable, MUIM_Notify, MUIA_Pressed,FALSE,
    LI_Text, 3, MUIM_Set,MUIA_Disabled,TRUE);
  DoMethod(BT_Enable, MUIM_Notify, MUIA_Pressed,FALSE,
    LI_Text, 3, MUIM_Set,MUIA_Disabled,FALSE);

  DoMethod(BT_Sort, MUIM_Notify, MUIA_Pressed,FALSE,
    LI_Text, 1, MUIM_NList_Sort);

  DoMethod(BT_Sort2, MUIM_Notify, MUIA_Pressed,FALSE,
    FL_Text, 1, MUIM_NList_Sort);

  DoMethod(BT_Sort3, MUIM_Notify, MUIA_Pressed,FALSE,
    LI_Text2, 1, MUIM_NList_Sort);
  ...
  DoMethod(LI_Text, MUIM_Notify, MUIA_NList_TitleClick,MUIV_EveryTime,
    LI_Text, 4, MUIM_NList_Sort3, MUIV_TriggerValue, MUIV_NList_SortTypeAdd_4Values, MUIV_NList_Sort3_SortType_Both);
  DoMethod(LI_Text, MUIM_Notify, MUIA_NList_TitleClick2,MUIV_EveryTime,
    LI_Text, 4, MUIM_NList_Sort3, MUIV_TriggerValue, MUIV_NList_SortTypeAdd_4Values, MUIV_NList_Sort3_SortType_2);
  ...
  DoMethod(LI_Text, MUIM_Notify, MUIA_NList_MultiClickAlone,2,
    LI_Text, 4, MUIM_NList_Select, MUIV_TriggerValue,MUIV_NList_Select_Toggle, NULL);
  DoMethod(LI_Text, MUIM_Notify, MUIA_NList_MultiClickAlone,4,
    LI_Text, 4, MUIM_NList_Select, 0,MUIV_NList_Select_Toggle, NULL);
  ...
  DoMethod(WIN_3,MUIM_Notify,MUIA_Window_CloseRequest,TRUE,
    APP_Main, 6, MUIM_Application_PushMethod,
    WIN_3, 3, MUIM_Set,MUIA_Window_Open,FALSE);

  DoMethod(WIN_4,MUIM_Notify,MUIA_Window_CloseRequest,TRUE,
    APP_Main, 6, MUIM_Application_PushMethod,
    WIN_4, 3, MUIM_Set,MUIA_Window_Open,FALSE);
...
/*  DoMethod(APP_Main,MUIM_Application_Load,MUIV_Application_Load_ENVARC);*/
  ...
  DoMethod(LI_Text,MUIM_NList_Select,10,MUIV_NList_Select_Toggle, NULL);

  set(WIN_Main,MUIA_Window_Open,TRUE);
/*  set(WIN_3,MUIA_Window_Open,TRUE);*/
/*  set(WIN_4,MUIA_Window_Open,TRUE);*/
