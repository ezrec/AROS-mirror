...
struct FS_Render_Fill                           { uint32 x; uint32 y; uint32 w; uint32 h; uint32 RGBA; };
...
F_METHODM(void,Render_DrawGradient,FS_Render_DrawGradient)
{
   ...
   if (!(FF_Render_TrueColors & LOD -> Flags) ||
       !(CyberGfxBase) ||
       !(LOD -> RPort))
   {
      return;
   ...
   switch (Msg -> Angle)
   {
      ...
      default:
      {
         ...
         if (Msg -> Origin)
         {
            ...
            {
               clip = (APTR) F_Do(Obj,FM_Render_AddClip,Msg -> Rect);
            }
         }
         ...
         if (clip)
         {
            F_Do(Obj,FM_Render_RemClip,clip);
         }
      }
      break;
   }
...
F_METHODM(uint32, Render_Fill, FS_Render_Fill)
{
    ...
    {
        uint32 mod = Msg->w * sizeof (uint32);
        uint32 *line_buffer = F_New(mod);

        if (line_buffer != NULL)
        {
            int32 r = F_PIXEL_RED(Msg->RGBA);
            int32 g = F_PIXEL_GREEN(Msg->RGBA);
            int32 b = F_PIXEL_BLUE(Msg->RGBA);
            int32 a = 255 - F_PIXEL_ALPHA(Msg->RGBA);
            ...
            for ( ; y < y2 ; y++)
            {
                ...
                ReadPixelArray(line_buffer, 0,0,mod, LOD->RPort, x,y, Msg->w,1, RECTFMT_RGBA);
                ...
                WritePixelArray(line_buffer, 0,0,mod, LOD->RPort, x,y, Msg->w,1, RECTFMT_RGBA);
