...
#ifndef _RCUESESSION_H_
#define _RCUESESSION_H_
...
class rCUESession : public IFileReader
{
protected:
   DbgHandler                   *dbg;
   ...
   VectorT<class file*>          files;
   int32                         currtrk;

protected:
   rCUESession(const char* sFileName, EDtError &rc);
   virtual                      ~rCUESession();
   bool                          analyse(const char* sFileName, EDtError &rc);
   uint32                        parseLocation(String &s);
   DbgHandler                   *getDebug();
