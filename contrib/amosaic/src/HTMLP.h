// MJW - THIS INFO IS PRIVATE.... SHOULD ONLY BE USED IN HTMLamiga.c 
/* DON'T YOU DARE INCLUDE THIS EXCEPT FOR CODE THAT RUNS INSIDE THE 
   HTML GADGET
*/
...
#if  (defined(_AMIGA) && defined (_STRICT_ANSI))
...
typedef struct HTMLGadClDataS
{
  ...
  int				new_scroll_dir;
	...
	Object			*Virt;
	WidgetInfo		*old_widget_list;
} HTMLGadClData; /* Was HTMLRec */
#endif

typedef struct HTMLTextClDataS
{
	HTMLGadClData *HTML_Data;
	ULONG new_scroll_value;
	ULONG new_scroll_dir;
	ULONG Flags;

} HTMLTextClData;

typedef struct HTMLTextNClDataS
{
	HTMLGadClData *HTML_Data;
	ULONG new_scroll_value;
	ULONG new_scroll_dir;
...
/* HTML_img.c (was img.c) */

int ImageLoadAndCache(HTMLGadClData *, char *);
