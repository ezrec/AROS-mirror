  ...
  if (me->end_command)
    {
	  ...
	  { /*MJW 17 Oct 1993  -- can't put multiple commands on a line,
	      so made temporary script instead - updated Nov 7 1993 removed free endcommand*/
            static char AMcommandprefix[]="c:RUN >NIL: <NIL: c:EXECUTE ";
	    char *AMcommand=(char *)malloc(strlen(AMcommandprefix)+16+L_tmpnam);
	    ...
	    if ((FILE *)NULL ==(AMtmpfile = fopen(AMcomfile,"w"))){
	      ...
	      /* break the command apart at semicolons */
              static char AMdelprefix[]="c:delete >NIL: <NIL: ";
              char *semi;
              char *AMdelcommand=
      ...
      if (!strstr (pres->command, "mosaic-internal-reference"))
        {
 		...
 		(char *)malloc((strlen (command)+32+strlen(me->fnam)+strlen(scrptr))
 			       * sizeof (char));
 	      sprintf (me->end_command, "%s PUBSCREEN=\"%s\"; c:delete %s ",
 		       command, scrptr, me->fnam);
 	    } else {
 	      me->end_command =
 		(char *)malloc ((strlen (command) + 32 + strlen(me->fnam))
 				* sizeof (char));
 	      sprintf (me->end_command, "%s ; c:delete %s ",
 		       command, me->fnam);
 	    }
 	  }
