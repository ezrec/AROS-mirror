...
/* Version Tag */
#include "jpeg2000.datatype_rev.h"
#define LIBNAME "jpeg2000.datatype"

CONST UBYTE USED verstag[] = VERSTAG;

struct OpenJPEGIFace *IOpenJPEG;
struct Library *OpenJPEGBase;
...
static int openDTLibs (struct ClassBase *libBase) {
	...
	OpenJPEGBase = IExec->OpenLibrary("openjpeg.library", 52);
	IOpenJPEG = (struct OpenJPEGIFace *)IExec->GetInterface(OpenJPEGBase, "main", 1, NULL);
	if (!IOpenJPEG) return FALSE;

	return TRUE;
}

static void closeDTLibs (struct ClassBase *libBase) {
	IExec->DropInterface((struct Interface *)IOpenJPEG);
	IExec->CloseLibrary(OpenJPEGBase);
