		 ...
		 unsigned char const sfbwidth[39])
{
  ...
  bcopy(&tmp[sb], &xr[18 * sb], (576 - 18 * sb) * sizeof(mad_fixed_t));
}
...
int mad_layer_III(struct mad_stream *stream, struct mad_frame *frame)
{
  ...
  if (si.main_data_begin == 0) {
	...
	bcopy(mad_bit_nextbyte(&stream->ptr),
...
# endif
  ...
  if (frame_free >= next_md_begin) {
    bcopy(stream->next_frame - next_md_begin,
	   *stream->main_data, next_md_begin);
	...
	bcopy(*stream->main_data + stream->md_len - extra,
		*stream->main_data, extra);
	stream->md_len = extra;
      }
    }
    else
    ...
    bcopy(stream->next_frame - frame_free,
	   *stream->main_data + stream->md_len, frame_free);
