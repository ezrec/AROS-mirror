 ...
 */
static
void synth_quart(struct mad_synth *synth, struct mad_frame const *frame,
		unsigned int nch, unsigned int ns)
{
  unsigned int phase, ch, s, sb, pe, po;
  mad_fixed_t *pcm1, *pcm2, (*filter)[2][2][16][8];
...
void mad_synth_frame(struct mad_synth *synth, struct mad_frame const *frame)
{
  ...
  if (frame->options & MAD_OPTION_QUARTSAMPLERATE) {
    synth->pcm.samplerate /= 4;
    synth->pcm.length     /= 4;

    synth_frame = synth_quart;
  } else if (frame->options & MAD_OPTION_HALFSAMPLERATE) {
