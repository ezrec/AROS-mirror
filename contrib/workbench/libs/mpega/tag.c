	       ...
	       unsigned short crc)
{
  ...
  if (magic == LAME_MAGIC)
  {
    if (version[2] == '.' || (version[0] > '3' && version[1] == '.') || (version[0] == '3' && version[1] == '.' && ((version[2] == '9' && ((version[3] == '5' && version[4] == '.') || version[3] > '5')) || (version[2] >= '1' && version[4] >= '0' && version[4] <= '9'))))
    {
      if (lame->replay_gain[0].name != TAG_RGAIN_NAME_NOT_SET)
        lame->replay_gain[0].adjustment -= 60;  /* 6.0 dB */
      if (lame->replay_gain[1].name != TAG_RGAIN_NAME_NOT_SET)
        lame->replay_gain[1].adjustment -= 60;  /* 6.0 dB */
    }
  }
