...
void SetupDebug(void)
{
  ...
  kprintf("** openurl.library %s (%s) startup ****************************\n", LIB_REV_STRING, LIB_DATE);
  kprintf("Initializing runtime debugging:\n");

  if(GetVar("openurl.library.debug", var, sizeof(var), 0) > 0)
  {
    char *s = var;
  ...
  kprintf("set debug classes/flags (env:openurl.library.debug): %08lx/%08lx\n", debug_classes, debug_flags);
