...
pid_t __vfork(jmp_buf env)
{
    ...
    CopyMem(&aroscbase->acb_acud.acud_startup_jmp_buf, &udata->startup_jmp_buf, sizeof(jmp_buf));
    ...
    D(bug("__vfork: Setting jmp_buf at %p\n", &aroscbase->acb_acud.acud_startup_jmp_buf));
    if(setjmp(aroscbase->acb_acud.acud_startup_jmp_buf))
    {
	...
	if(!udata->child_executed)
	{
	    ...
	    udata->child_aroscbase->acb_acud.acud_startup_error = aroscbase->acb_acud.acud_startup_error;
            ...
            */
            ...
            if (etask)
                etask->et_Result = aroscbase->acb_acud.acud_startup_error;
	...
	CopyMem(&udata->startup_jmp_buf, &aroscbase->acb_acud.acud_startup_jmp_buf, sizeof(jmp_buf));
