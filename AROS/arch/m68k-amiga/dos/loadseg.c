...
#define ZROMBase  IDosBase(DOSBase)->zromBase
...
*****************************************************************************/
{
    ...
    /* Late opening of z-rom.library */
    if (!ZROMBase)
        ZROMBase = OpenLibrary("z-rom.library", 0L);

    /* Open the file */
    D(bug("[LoadSeg] Opening '%s'...\n", name));

    file = gzOpen (name, MODE_OLDFILE);

    if (file)
    {
        D(bug("[LoadSeg] Loading '%s'...\n", name));

        gzSetVBuf(file, NULL, BUF_FULL, 4096);
