...
OOP_Object * METHOD(NV44AMockHardware, Root, New)
{
    ...
    /* Capabilities */
    ADD_PCI_CAP(hwdata, 0x02); /* CAPABILITY: PCICAP_AGP */
//    ADD_PCI_CAP(hwdata, 0x10); /* CAPABILITY: PCICAP_PCIE */
    ADD_PCI_CAP(hwdata, 0xff); /* CAPABILITY: END */

    /* Fill in BAR 0 */
    SET_ASR_DWORD(hwdata, PCI_BAR0, 0x00000000, 0x04a100a1); /* NV03_PMC_BOOT_0 */
    SET_ASR_DWORD(hwdata, PCI_BAR0, 0x0010020c, 0x10000000); /* NV04_PFB_FIFO_DATA */

    return o;
}

VOID METHOD(NV44AMockHardware, Hidd_PCIMockHardware, MemoryReadAtAddress)
{
    GET_PCIMOCKHWDATA
