...
ULONG pci_inl_mt(ULONG addr, struct CardData *card)
{
    return LONGIN(card->mtbase + addr);
}

UWORD pci_inw_mt(ULONG addr, struct CardData *card)
{
    return WORDIN(card->mtbase + addr);
}

UBYTE pci_inb_mt(ULONG addr, struct CardData *card)
{
    return BYTEIN(card->mtbase + addr);
}

void pci_outl_mt(ULONG value, ULONG addr, struct CardData *card)
{
    LONGOUT(card->mtbase + addr, value);
}

void pci_outw_mt(UWORD value, ULONG addr, struct CardData *card)
{
    WORDOUT(card->mtbase + addr, value);
}

void pci_outb_mt(UBYTE value, ULONG addr, struct CardData *card)
{
    BYTEOUT(card->mtbase + addr, value);
}
...
BOOL ahi_pci_add_intserver(struct Interrupt *i, APTR dev)
{
    ...
    inthandler.h_Node.ln_Name = "VIA-AC97 IRQ";
