...
const char version[]={"\0$VER: SFSCheck 1.3 (3.2.2013)\r\n"};
...
struct Globals _globals, *globals = &_globals;
        ...
        if((readarg=ReadArgs(template,(IPTR *)&arglist,0))!=0) {
          ...
          if((dl=FindDosEntry(dl,arglist.device,LDF_DEVICES))!=0) {
            ...
            if(arglist.lock==0 || Inhibit(arglist.device, DOSTRUE)!=DOSFALSE) {
              ...
              if((initcachedio((UBYTE *)BADDR(fssm->fssm_Device), fssm->fssm_Unit, fssm->fssm_Flags, dosenvec))==0) {
                ...
                printf("Partition start offset : 0x%08lx:%08lx   End offset : 0x%08lx:%08lx\n", (unsigned long)globals->byte_lowh, (unsigned long)globals->byte_low, (unsigned long)globals->byte_highh, (unsigned long)globals->byte_high);
