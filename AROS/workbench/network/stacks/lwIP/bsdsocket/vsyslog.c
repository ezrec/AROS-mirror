...
/*  LOCATION */
        struct bsdsocketBase *, bsdsocketBase, 43, BSDSocket)
...
*****************************************************************************/
{
    ...
    struct Library *SocketBase = &bsdsocketBase->lib;
    int errno_val = bsd_errno(bsdsocketBase->bsd);
    ...
    if (bsdsocketBase->bsd_syslog < 0) {
        bsdsocketBase->bsd_syslog = socket(AF_INET, SOCK_DGRAM, 0);
    }
    ...
    if (_vasprintf(&msg, format, args) >= 0) {
        ...
        if (_asprintf(&rfc5424, "<%d>%d %04d-%02d-%02dT%02d:%02d:%02d.%02dZ %s %s %llu - - %s",
            ...
            sendto(bsdsocketBase->bsd_syslog, rfc5424, strlen(rfc5424), 0, (struct sockaddr *)&sa, sizeof(sa));
            FreeVec(rfc5424);
        }
