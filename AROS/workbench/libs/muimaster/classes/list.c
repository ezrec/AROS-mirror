...
static void List_HandleScrollerPos(struct IClass *cl, Object *obj)
{
    ...
    if (data->area_connected)
        DoMethod(data->group, OM_REMMEMBER, data->area);
    if (data->vert_connected)
        DoMethod(data->group, OM_REMMEMBER, data->vert);
...
IPTR List__OM_NEW(struct IClass *cl, Object *obj, struct opSet *msg)
{
    ...
    /* at this point tag still points to found tag, restore it */
    if (rectattrs[0].ti_Tag == MUIA_Frame)
        tag->ti_Tag = MUIA_Frame;
            ...
            if (msg->imsg->Code == SELECTDOWN)
            {
                if (_isinobject(data->area, msg->imsg->MouseX, msg->imsg->MouseY))
                {
                    ...
                    if (!data->read_only && pos.entry != -1)
                    {
                        ...
                        /* Look out for mouse movement and timer events while mouse button is down */
                        DoMethod(_win(obj), MUIM_Window_RemEventHandler, (IPTR) &data->ehn);
