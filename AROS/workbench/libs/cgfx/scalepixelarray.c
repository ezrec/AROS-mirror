...
static ULONG RenderHook(struct render_data *data, LONG srcx, LONG srcy,
    OOP_Object *dstbm_obj, OOP_Object *dst_gc, LONG x1, LONG y1,
    LONG x2, LONG y2, struct GfxBase *GfxBase);
...
*****************************************************************************/
{
    ...
    struct Rectangle rr;
    OOP_Object *gfx_hidd = NULL, *tempbm_obj, *tempbm2_obj, *gc;
    ...
    (bug("ScalePixelArray(%p, %d, %d, %d, %p, %d, %d, %d, %d, %d)\n",
    ...
    if (gc != NULL && tempbm_obj != NULL && tempbm2_obj != NULL)
    {
        ...
        result = DoRenderFunc(RastPort, NULL, &rr, (RENDERFUNC)RenderHook,
            &data, TRUE);
    }
