                ...
                Child, (IPTR)VGroup,
                    ...
                    Child, (IPTR)ColGroup(2),
                        ...
                        Child, (IPTR)HGroup,
                            ...
                            Child, (IPTR)HVSpace,
                        End,
                        Child, (IPTR)(data->ata_ahci =
                            MUI_MakeObject(MUIO_Checkmark, NULL)),
                        Child, (IPTR)HGroup,
                            Child, (IPTR)Label2(__(MSG_ATA_AHCI)),
                            Child, (IPTR)HVSpace,
                        End,
                        Child, (IPTR)HVSpace,
                        Child, (IPTR)HVSpace,
    ...
    if (self != NULL)
    {
        ...
        *data = temp_data;
        ...
        DoMethod(data->acpi_enable, MUIM_Notify,
            MUIA_Selected, MUIV_EveryTime,
            (IPTR)self, 3, MUIM_Set, MUIA_PrefsEditor_Changed, TRUE);
        DoMethod(data->ata_ahci, MUIM_Notify,
            MUIA_Selected, MUIV_EveryTime,
            (IPTR)self, 3, MUIM_Set, MUIA_PrefsEditor_Changed, TRUE);
...
static BOOL ReadBootArgs(CONST_STRPTR line, struct BootEditor_DATA *data)
{
    ...
    NNSET(data->ata_ahci, MUIA_Selected,
        strstr(line, "ahci") != NULL);
...
static BOOL WriteBootArgs(BPTR file, struct BootEditor_DATA *data)
{
    ...
    if(!XGET(data->acpi_enable, MUIA_Selected))
        FPrintf(file, " noacpi");
    if(XGET(data->ata_ahci, MUIA_Selected))
        FPrintf(file, " ahci");
