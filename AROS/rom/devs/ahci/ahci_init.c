...
{
    struct Library *BootLoaderBase;
    CONST_STRPTR ahci_arg = NULL;

    D(bug("[AHCI--] AHCI_Init: ahci.device Initialization\n"));

    /* Is AHCI support desired? */
    if ((BootLoaderBase = OpenResource("bootloader.resource"))) {
        struct List *args;
        if ((args = GetBootInfo(BL_Args))) {
            struct Node *tmp;
            ForeachNode(args, tmp) {
                if (strcmp(tmp->ln_Name, "ahci") == 0) {
                    ahci_arg = tmp->ln_Name;
                    break;
                }
            }
        }
