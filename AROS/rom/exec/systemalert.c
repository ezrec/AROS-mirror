...
{
    struct Task *ThisTask = THISCPU->ThisTask;

    D(bug("[SystemAlert] Code 0x%08X, type %d, data 0x%p\n", alertNum, type, data));

    if ((ThisTask == PrivExecBase(SysBase)->SAT.sat_Task) &&
    ...
    else if (PrivExecBase(SysBase)->SAT.sat_IsAvailable && !(alertNum & AT_DeadEnd))
    {
        ...
        PrivExecBase(SysBase)->SAT.sat_Params[1] = (IPTR)ThisTask;

        Signal(PrivExecBase(SysBase)->SAT.sat_Task, SIGF_SINGLE);
    }
    else
    {
        Alert_DisplayKrnAlert(ThisTask, alertNum, location, stack, type, data, SysBase);
    }
}
