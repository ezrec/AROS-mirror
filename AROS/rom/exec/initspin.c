...
/*  FUNCTION
        Init a spinlock. If the lock is already held
...
*****************************************************************************/
{
    ...
    if(spin->nest > 0) 
    {
        ...
        {
            D(bug("[INITSPIN] WARNING: spinlock %lx initialized, but was held by task %p (%s) before!\n", 
                   spin, sectask, sectask->tc_Node.ln_Name));
        }
        else 
        {
            D(bug("[INITSPIN] WARNING: spinlock %lx initialized, but was held by a NULL task!?\n", spin));
        }
        nest=spin->nest;
    }
    spin->owner=NULL;
