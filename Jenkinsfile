...
def buildStep(ext, iconset = 'default') {
	...
	stage('Configuring...') {
		sh "cd build-$ext && ../AROS/configure --target=$ext --enable-ccache --with-iconset=$iconset --enable-build-type=nightly --with-serial-debug --with-binutils-version=2.30 --with-gcc-version=6.3.0 --with-portssources=${env.WORKSPACE}/externalsources"
	}
